...
VERSION = "v18.56-INSTITUTIONAL-GATEWAY-COVERAGE-REPAIR-SPDR-IMF-FEDFUNDS-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
def _gld_real():
    """Read the official SPDR GLD historical XLSX with upstream-drift guards.

    SPDR migrated its historical archive to an XLSX API. The API can return a
    PDF unless browser-like Origin/Referer headers are supplied. We therefore
    validate HTTP content type and XLSX magic bytes before parsing.
    """
    ...
    try:
        page_r=_inst_http('https://www.spdrgoldshares.com/usa/gld/',timeout=20,
                          headers={'Referer':'https://www.spdrgoldshares.com/usa/gld/'})
        hrefs=re.findall(r'(?:href|data-href)=["\']([^"\']+)',page_r.text,flags=re.I)
        for href in hrefs:
            low=href.lower()
            if '.xlsx' in low and ('historical' in low or 'archive' in low):
                urls.insert(0,requests.compat.urljoin(page_r.url,href))
    except Exception:
        pass
    last_exc=None
    for url in dict.fromkeys(urls):
        try:
            ...
            if not r.content.startswith(b'PK\x03\x04'):
                raise RuntimeError(f'SPDR historical archive is not XLSX (content-type={ct})')
            if ct and not any(x in ct for x in ('spreadsheet','xlsx','octet-stream','zip')):
                raise RuntimeError(f'SPDR historical archive returned unexpected content-type: {ct}')
            try:
                xl=pd.ExcelFile(io.BytesIO(r.content)); sheets=[(sh,pd.read_excel(xl,sheet_name=sh,header=0)) for sh in xl.sheet_names]
            except Exception as exc:
                if 'openpyxl' not in str(exc).lower(): raise
                sheets=_read_xlsx_without_openpyxl(r.content)
            best=None
            for sh,d in sheets:
                if d is None or d.empty or str(sh).lower()=='disclaimer':
                    continue
                cols=list(d.columns)
                # Current SPDR layout is stable by position: Date, ..., Total Ounces,
                # Tonnes, Total NAV USD. Prefer semantic headers, then positional fallback.
                cols_l={str(x).strip().lower():x for x in cols}
                dc=next((cols_l[k] for k in cols_l if 'date' in k or 'as of' in k),None)
                oc=next((cols_l[k] for k in cols_l if any(t in k for t in ('total ounces','gold ounces','ounces of gold','gold oz')) and 'share' not in k),None)
                tc=next((cols_l[k] for k in cols_l if 'tonnes' in k or 'metric tonnes' in k),None)
                ...
                if len(x)>=2: best=(x,oc,tc); break
            if best is None: raise RuntimeError('SPDR GLD XLSX schema unavailable')
            d,oc,tc=best; a,b=d.iloc[-1],d.iloc[-2]
            ao=pd.to_numeric(a[oc],errors='coerce') if oc is not None else np.nan
            bo=pd.to_numeric(b[oc],errors='coerce') if oc is not None else np.nan
            at=pd.to_numeric(a[tc],errors='coerce') if tc is not None else np.nan
            bt=pd.to_numeric(b[tc],errors='coerce') if tc is not None else np.nan
            if pd.isna(ao) and pd.notna(at): ao=at*TROY_OZ_PER_TONNE
            if pd.isna(bo) and pd.notna(bt): bo=bt*TROY_OZ_PER_TONNE
            if pd.isna(ao) or ao<=0: raise RuntimeError('SPDR GLD latest holdings value is invalid')
            direction='IN' if pd.notna(bo) and ao>bo else 'OUT' if pd.notna(bo) and ao<bo else 'NEUTRAL'
            out={'source':'SPDR Gold Shares official historical archive','date':a['_date'].strftime('%Y-%m-%d'),
                 ...
                 'measurement':'physical gold holdings; not cash flow'}
...
def _imf_irfcl_monthly(indicator):
    """Fetch IMF IRFCL monthly series for a gold-reserve indicator.

    IMF IRFCL uses COUNTRY.INDICATOR.SECTOR.FREQUENCY. The returned JSON is
    parsed by dimension identifiers rather than hard-coded array positions.
    """
    url=f"{IMF_IRFCL_DATAFLOW}/*.{indicator}.*.M?dimensionAtObservation=TIME_PERIOD&attributes=dsd&measures=all"
    r=_inst_http(url,timeout=90,headers={'Accept':'application/json'})
    payload=r.json(); data=payload.get('data') or {}
    struct=(data.get('structures') or [{}])[0]; ds=(data.get('dataSets') or [{}])[0]
    if not struct or not ds.get('series'): raise RuntimeError(f'IMF IRFCL {indicator}: empty dataset')
    sdim=struct.get('dimensions',{}).get('series',[]) or []
    odim=struct.get('dimensions',{}).get('observation',[]) or []
    country_dim=next((d for d in sdim if d.get('id') in ('COUNTRY','REF_AREA')),None)
    country_pos=sdim.index(country_dim) if country_dim in sdim else -1
    time_dim=next((d for d in odim if d.get('id')=='TIME_PERIOD'),None)
    if country_dim is None or country_pos<0 or time_dim is None: raise RuntimeError(f'IMF IRFCL {indicator}: dimension schema unavailable')
    countries=country_dim.get('values',[]) or []
    times=[v.get('value') or v.get('id') for v in (time_dim.get('values',[]) or [])]
    out={}
    for key,series in ds.get('series',{}).items():
        ...
        cid=countries[idx].get('id'); name=countries[idx].get('name') or cid
        if not cid: continue
        rec=out.setdefault(cid,{'name':name,'byMonth':{}})
        for obs_key,obs_val in (series.get('observations') or {}).items():
            try: period=times[int(obs_key)]; val=float(obs_val[0])
            except Exception: continue
            if period and np.isfinite(val): rec['byMonth'][str(period)]=val
    if len(out)<20: raise RuntimeError(f'IMF IRFCL {indicator}: only {len(out)} countries')
    return out


def _cb_real():
    """Central-bank gold changes: WGC official download first, IMF IRFCL direct second.

    No synthetic values are generated. IMF IRFCL is used as the direct official
    statistical fallback because WGC states its country reserve series are
    compiled primarily from IMF IFS and supplemented with official sources.
    """
    c=_inst_cache_get('cb')
    if c is not None:return c
    last_exc=None
    # 1) WGC official workbook locator, if accessible from the runtime.
    wgc_urls=[]
    if WGC_CENTRAL_BANK_CHANGES_URL: wgc_urls.append(WGC_CENTRAL_BANK_CHANGES_URL)
    try:
        page_r=_inst_http('https://www.gold.org/goldhub/data/gold-reserves-by-country',timeout=20)
        hrefs=re.findall(r'(?:href|data-href)=["\']([^"\']+)',page_r.text,flags=re.I)
        for href in hrefs:
            if '.xlsx' in href.lower() and ('change' in href.lower() or 'official' in href.lower()):
                wgc_urls.insert(0,requests.compat.urljoin(page_r.url,href))
    except Exception:
        pass
    for url in dict.fromkeys(wgc_urls):
        try:
            r=_inst_http(url,timeout=30,headers={'Referer':'https://www.gold.org/goldhub/data/gold-reserves-by-country','Accept':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,*/*'})
            if not r.content.startswith(b'PK\x03\x04'): raise RuntimeError('WGC response is not XLSX')
            try:
                xl=pd.ExcelFile(io.BytesIO(r.content)); tables=[(sh,pd.read_excel(xl,sheet_name=sh)) for sh in xl.sheet_names]
            except Exception as exc:
                if 'openpyxl' not in str(exc).lower(): raise
                tables=_read_xlsx_without_openpyxl(r.content)
            for sh,d in tables:
                if d.empty: continue
                cols=list(d.columns); date_cols=[]
                for col in cols:
                ...
                dc,parsed=max(date_cols,key=lambda x:x[1].notna().sum()); latest=parsed.max(); mask=parsed==latest
                candidates=[]
                for col in cols:
                    if col==dc: continue
                    nm=str(col).lower()
                    if any(k in nm for k in ('change','net','purchase','sale','tonne')) and '%' not in nm:
                        vals=pd.to_numeric(d.loc[mask,col],errors='coerce')
                        if vals.notna().sum(): candidates.append((col,float(vals.sum())))
                if candidates:
                    _,total=max(candidates,key=lambda x:abs(x[1]))
                    out={'source':'World Gold Council official / IMF IFS','period':latest.strftime('%Y-%m-%d'),'net_change_tonnes':float(total),'direction':'BUY' if total>0 else 'SELL' if total<0 else 'NEUTRAL','reported_only':True}
                    out.update(_inst_status(latest,45,'World Gold Council / IMF'))
                    return _inst_cache_put('cb',out)
        except Exception as exc: last_exc=exc
    # 2) Direct IMF IRFCL: official monthly gold holdings in fine troy ounces.
    try:
        raw=_imf_irfcl_monthly(IMF_GOLD_FTO_INDICATOR)
        months=sorted({m for rec in raw.values() for m in rec.get('byMonth',{})})
        if len(months)<2: raise RuntimeError('IMF IRFCL gold series has fewer than two months')
        latest=months[-1]; prior=months[-2]
        current=sum(rec['byMonth'].get(latest,0.0) for rec in raw.values())
        previous=sum(rec['byMonth'].get(prior,0.0) for rec in raw.values())
        delta_tonnes=(current-previous)/TROY_OZ_PER_TONNE
        asof=pd.to_datetime(latest+'-01' if len(latest)==7 else latest,errors='coerce')
        out={'source':'IMF IRFCL (World Gold Council-aligned official reserve data)','period':str(latest),'net_change_tonnes':float(delta_tonnes),'direction':'BUY' if delta_tonnes>0 else 'SELL' if delta_tonnes<0 else 'NEUTRAL','reported_only':True,'countries_covered':len(raw)}
        out.update(_inst_status(asof,90,'IMF IRFCL'))
        return _inst_cache_put('cb',out)
    except Exception as exc:
        last_exc=exc
    raise RuntimeError(f'WGC/IMF central-bank data unavailable: {last_exc}')
...
def _macro_real():
        ...
        try:
            ...
            if result:
                ...
                if price is not None:
                    ...
                    exp=('رفع محتمل' if implied>float((lo+hi)/2)+0.05 else 'خفض محتمل' if implied<float((lo+hi)/2)-0.05 else 'استقرار محتمل') if lo is not None and hi is not None else 'متاحة من ZQ'
                    fedwatch_status=_inst_status(asof,2,'Yahoo Finance ZQ=F (CME ZQ market-data fallback)') if not pd.isna(asof) else {'status':'FRESH','fresh':True,'age_days':0,'asof':None,'source':'Yahoo Finance ZQ=F (CME ZQ market-data fallback)'}
    ...
    out={'source':'Official NY Fed API + Cboe + Federal Reserve H.10 + U.S. Treasury' ,'policy':pol,'fed_lower':lo,'fed_upper':hi,'effr':effr,
         ...
         'fed_futures_implied':fedwatch_status.get('implied_rate')}
...
def run_institutional_self_tests():
    ...
    check('version', VERSION.startswith('v18.56-'), VERSION)
