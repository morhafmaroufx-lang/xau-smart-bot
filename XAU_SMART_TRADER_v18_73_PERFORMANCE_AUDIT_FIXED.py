...
AUDITOR_WEEKLY_TZ = os.getenv("AUDITOR_WEEKLY_TZ", "Asia/Damascus")
...
CREATE TABLE IF NOT EXISTS Live_Trades (
    ...
    risk_reward REAL,
    timeframe TEXT,
    trade_type TEXT NOT NULL DEFAULT 'MAIN',
    sr_side TEXT,
    sr_level REAL,
    sr_trigger TEXT,
    sr_strength REAL,
...
CREATE TABLE IF NOT EXISTS Weekly_Trade_Reports (
...
CREATE INDEX IF NOT EXISTS idx_weekly_reports_end ON Weekly_Trade_Reports(week_end);
...
class Database:
    ...
    def initialize(self) -> None:
        with self._lock, self.connect() as conn:
            ...
            migrations = {
                "trade_type": "ALTER TABLE Live_Trades ADD COLUMN trade_type TEXT NOT NULL DEFAULT 'MAIN'",
                "sr_side": "ALTER TABLE Live_Trades ADD COLUMN sr_side TEXT",
                "sr_level": "ALTER TABLE Live_Trades ADD COLUMN sr_level REAL",
                "sr_trigger": "ALTER TABLE Live_Trades ADD COLUMN sr_trigger TEXT",
                "sr_strength": "ALTER TABLE Live_Trades ADD COLUMN sr_strength REAL",
            }
            for col, ddl in migrations.items():
                if col not in cols: conn.execute(ddl)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_live_trades_type_time ON Live_Trades(trade_type, signal_time)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_live_trades_status_time ON Live_Trades(status, signal_time)")
            conn.commit()
...
class TradeAuditor:
    ...
    def register(self, trade: TradeInput, trade_type: str="MAIN", sr_meta: Optional[dict[str,Any]]=None) -> bool:
        direction=trade.direction.upper(); trade_type=str(trade_type or "MAIN").upper(); sr_meta=sr_meta or {}
        if direction not in {"BUY","SELL"}: raise ValueError("direction must be BUY or SELL")
        if trade_type not in {"MAIN","SR"}: raise ValueError("trade_type must be MAIN or SR")
        ...
        with self.db.transaction() as conn:
            ...
            conn.execute("""INSERT INTO Live_Trades(trade_id,signal_time,direction,entry,sl,tp1,tp2,tp3,tp_final,score,quality,risk_reward,timeframe,trade_type,sr_side,sr_level,sr_trigger,sr_strength,expiry_time,status,final_result,snapshot_json,created_at,updated_at,current_price,last_price_time) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(
                trade.trade_id,trade.signal_time,direction,trade.entry,trade.sl,trade.tp1,trade.tp2,trade.tp3,trade.tp_final,trade.score,trade.quality,trade.risk_reward,trade.timeframe,trade_type,sr_meta.get("side"),sr_meta.get("level"),sr_meta.get("trigger"),sr_meta.get("strength"),trade.expiry_time,"ACTIVE","OPEN",self.db.jdump(trade.analysis_snapshot),now,now,trade.entry,trade.signal_time))
            conn.execute("INSERT INTO Trade_Events(trade_id,event_type,event_time,price,metadata_json) VALUES(?,?,?,?,?)",(trade.trade_id,"ENTRY",trade.signal_time,trade.entry,self.db.jdump({"source":"signal"})))
        return True

    def _update_excursions(self,conn,trade,point):
        entry=float(trade['entry']); adverse=max(0,entry-point.price) if trade['direction']=='BUY' else max(0,point.price-entry); favorable=max(0,point.price-entry) if trade['direction']=='BUY' else max(0,entry-point.price)
        if adverse>float(trade.get('max_adverse_excursion') or 0): conn.execute("UPDATE Live_Trades SET max_adverse_excursion=?,max_drawdown=?,mae_price=?,mae_time=?,max_drawdown_price=?,max_drawdown_time=? WHERE trade_id=?",(adverse,adverse,point.price,point.timestamp,point.price,point.timestamp,trade['trade_id']))
        if favorable>float(trade.get('max_favorable_excursion') or 0): conn.execute("UPDATE Live_Trades SET max_favorable_excursion=?,mfe_price=?,mfe_time=? WHERE trade_id=?",(favorable,point.price,point.timestamp,trade['trade_id']))

    def _event(self,conn,trade,event_type,point,metadata=None):
        return conn.execute("INSERT OR IGNORE INTO Trade_Events(trade_id,event_type,event_time,price,metadata_json) VALUES(?,?,?,?,?)",(trade['trade_id'],event_type,point.timestamp,point.price,self.db.jdump(metadata))).rowcount>0

    def _close_sl(self,conn,trade,point):
        self._event(conn,trade,'SL',point,{"source":point.source})
        had_target=conn.execute("SELECT 1 FROM Trade_Events WHERE trade_id=? AND event_type IN ('TP1','TP2','TP3','TP_FINAL') LIMIT 1",(trade['trade_id'],)).fetchone()
        final='PARTIAL_SUCCESS' if had_target else 'FAILED'
        conn.execute("UPDATE Live_Trades SET status='CLOSED',final_result=?,close_time=?,updated_at=? WHERE trade_id=?",(final,point.timestamp,point.timestamp,trade['trade_id']))

    def observe_tick(self,point):
        changed=[]
        with self.db.transaction() as conn:
            for row in conn.execute("SELECT * FROM Live_Trades WHERE status='ACTIVE'").fetchall():
                trade=dict(row); self._update_excursions(conn,trade,point); conn.execute("UPDATE Live_Trades SET current_price=?,last_price_time=?,updated_at=? WHERE trade_id=?",(point.price,point.timestamp,point.timestamp,trade['trade_id']))
                if trade.get('expiry_time') and parse_dt(point.timestamp)>=parse_dt(trade['expiry_time']):
                    self._event(conn,trade,'EXPIRY',point,{"reason":"expiry_time"}); conn.execute("UPDATE Live_Trades SET status='CLOSED',final_result='EXPIRED',close_time=?,updated_at=? WHERE trade_id=?",(point.timestamp,point.timestamp,trade['trade_id'])); changed.append(trade['trade_id']); continue
                direction=trade['direction']; sl=float(trade['sl']); targets=levels_for(trade); hit_sl=point.price<=sl if direction=='BUY' else point.price>=sl; hits=[(l,x) for l,x in targets if (point.price>=x if direction=='BUY' else point.price<=x)]
                if hit_sl: self._close_sl(conn,trade,point); changed.append(trade['trade_id']); continue
                events={r[0] for r in conn.execute("SELECT event_type FROM Trade_Events WHERE trade_id=?",(trade['trade_id'],)).fetchall()}; new=False
                for label,target in hits:
                    if label not in events: new=self._event(conn,trade,label,point,{"source":point.source}) or new
                if targets and targets[-1][0] in {r[0] for r in conn.execute("SELECT event_type FROM Trade_Events WHERE trade_id=?",(trade['trade_id'],)).fetchall()}:
                    conn.execute("UPDATE Live_Trades SET status='CLOSED',final_result='FULL_SUCCESS',close_time=?,updated_at=? WHERE trade_id=?",(point.timestamp,point.timestamp,trade['trade_id'])); changed.append(trade['trade_id'])
                elif new: conn.execute("UPDATE Live_Trades SET final_result='PARTIAL_SUCCESS',updated_at=? WHERE trade_id=?",(point.timestamp,trade['trade_id'])); changed.append(trade['trade_id'])
        return changed
    ...
    def observe_bar(self,bar):
        ...
        with self.db.transaction() as conn:
            for row in conn.execute("SELECT * FROM Live_Trades WHERE status='ACTIVE'").fetchall():
                trade=dict(row); direction=trade['direction']; sl=float(trade['sl']); targets=levels_for(trade); sl_hit=bar.low<=sl if direction=='BUY' else bar.high>=sl; hits=[(l,x) for l,x in targets if (bar.high>=x if direction=='BUY' else bar.low<=x)]
                point=PricePoint(bar.timestamp,bar.close,bar.source,bar.high,bar.low)
                if sl_hit and hits:
                    self._event(conn,trade,'AMBIGUOUS',point,{"reason":"same_bar_order_unknown"}); conn.execute("UPDATE Live_Trades SET status='CLOSED',final_result='AMBIGUOUS',close_time=?,updated_at=? WHERE trade_id=?",(bar.timestamp,bar.timestamp,trade['trade_id'])); changed.append(trade['trade_id']); continue
                if sl_hit: self._close_sl(conn,trade,point); changed.append(trade['trade_id']); continue
                new=False
                for label,target in hits: new=self._event(conn,trade,label,PricePoint(bar.timestamp,target,bar.source,bar.high,bar.low),{"mode":"bar"}) or new
                if hits and hits[-1][0]==targets[-1][0]: conn.execute("UPDATE Live_Trades SET status='CLOSED',final_result='FULL_SUCCESS',close_time=?,updated_at=? WHERE trade_id=?",(bar.timestamp,bar.timestamp,trade['trade_id'])); changed.append(trade['trade_id'])
                elif new: conn.execute("UPDATE Live_Trades SET final_result='PARTIAL_SUCCESS',updated_at=? WHERE trade_id=?",(bar.timestamp,trade['trade_id'])); changed.append(trade['trade_id'])
        return changed
...
class Metrics:
    ...
    def _stats(self,items):
        total=len(items); full=sum(x['final_result']=='FULL_SUCCESS' for x in items); partial=sum(x['final_result']=='PARTIAL_SUCCESS' for x in items); failed=sum(x['final_result']=='FAILED' for x in items); expired=sum(x['final_result']=='EXPIRED' for x in items); ambiguous=sum(x['final_result']=='AMBIGUOUS' for x in items); closed=full+failed+partial+expired+ambiguous; decisive=full+partial+failed
        def avg(k):
            v=[float(x[k]) for x in items if x.get(k) is not None]; return sum(v)/len(v) if v else 0.0
        return {'total':total,'open':max(0,total-closed),'closed':closed,'full_success':full,'partial_success':partial,'failed':failed,'expired':expired,'ambiguous':ambiguous,'win_rate_pct':100*(full+partial)/decisive if decisive else 0.0,'full_target_rate_pct':100*full/closed if closed else 0.0,'avg_score':avg('score'),'avg_rr':avg('risk_reward'),'avg_drawdown':avg('max_drawdown'),'avg_mae':avg('max_adverse_excursion'),'avg_mfe':avg('max_favorable_excursion')}
    def _load(self):
        with self.db.connect() as c: return [dict(r) for r in c.execute('SELECT * FROM Live_Trades ORDER BY signal_time').fetchall()]
    def report(self):
        trades=self._load(); main=[x for x in trades if x.get('trade_type','MAIN')!='SR']; sr=[x for x in trades if x.get('trade_type','MAIN')=='SR']; return {'trades':self._stats(trades),'main_trades':self._stats(main),'sr_trades':self._stats(sr)}
    def period_report(self,start,end):
        tz=ZoneInfo(AUDITOR_WEEKLY_TZ); items=[]
        for x in self._load():
            try: dt=parse_dt(x['signal_time']).astimezone(tz)
            except Exception: continue
            if start<=dt<end: items.append(x)
        main=[x for x in items if x.get('trade_type','MAIN')!='SR']; sr=[x for x in items if x.get('trade_type','MAIN')=='SR']; return {'trades':self._stats(items),'main_trades':self._stats(main),'sr_trades':self._stats(sr),'period':{'start':start.isoformat(),'end':end.isoformat()}}


# --- Embedded Auditor reports ---
class Reporter:
    def __init__(self,metrics): self.metrics=metrics
    def render(self,r,title):
        t=r['trades']; m=r['main_trades']; sr=r['sr_trades']
        return '\n'.join([f'🛡️ XAU SMART TRADER — {title}','━━━━━━━━━━━━━━━━━━','','📊 الصفقات',f"• الإجمالي: {t['total']}",f"• المفتوحة: {t['open']} | المغلقة: {t['closed']}",f"• نجاح كامل: {t['full_success']}",f"• نجاح جزئي: {t['partial_success']}",f"• فشل: {t['failed']}",f"• منتهية: {t['expired']}",f"• غامضة: {t['ambiguous']}",f"• Win Rate: {t['win_rate_pct']:.2f}%",f"• Full Target Rate: {t['full_target_rate_pct']:.2f}%",f"• متوسط الجودة: {t['avg_score']:.2f} نقطة",f"• متوسط R:R: {t['avg_rr']:.2f}",f"• متوسط MAE: {t['avg_mae']:.4f}",f"• متوسط MFE: {t['avg_mfe']:.4f}",f"• متوسط Drawdown: {t['avg_drawdown']:.4f}",'','🎯 الصفقات الرئيسية',f"• الإجمالي: {m['total']} | Win Rate: {m['win_rate_pct']:.2f}%",f"• كامل: {m['full_success']} | جزئي: {m['partial_success']} | فشل: {m['failed']}",'','📐 صفقات الدعم والمقاومة S/R',f"• الإجمالي: {sr['total']} | Win Rate: {sr['win_rate_pct']:.2f}%",f"• كامل: {sr['full_success']} | جزئي: {sr['partial_success']} | فشل: {sr['failed']}",'','🔒 المراقب مستقل ولا يغيّر قرارات التداول أو العتبات.'])
    def daily(self):
        tz=ZoneInfo(AUDITOR_WEEKLY_TZ); n=datetime.now(tz); st=n.replace(hour=0,minute=0,second=0,microsecond=0); return self.render(self.metrics.period_report(st,st+timedelta(days=1)),'التقرير اليومي')
    def weekly(self,when=None):
        tz=ZoneInfo(AUDITOR_WEEKLY_TZ); n=(when or datetime.now(tz)).astimezone(tz); st=(n-timedelta(days=n.weekday())).replace(hour=0,minute=0,second=0,microsecond=0); en=st+timedelta(days=5); r=self.metrics.period_report(st,en); return self.render(r,f'التقرير الأسبوعي — {st.date()} إلى {(en-timedelta(seconds=1)).date()}'),r

class PerformanceAuditor:
    def __init__(self,db_path=AUDITOR_DB_PATH,price_engine=None):
        self.db=Database(db_path); self.price_engine=price_engine or LivePriceEngine(); self.trade_auditor=TradeAuditor(self.db); self.metrics=Metrics(self.db); self.reporter=Reporter(self.metrics); self._stop=threading.Event(); self._thread=None
    @staticmethod
    def _now(): return datetime.now(timezone.utc).isoformat()
    def register_trade(self,*,trade_id=None,signal_time=None,direction,entry,sl,tp1=None,tp2=None,tp3=None,tp_final=None,score=None,quality=None,risk_reward=None,timeframe=None,expiry_time=None,analysis_snapshot=None,trade_type='MAIN',sr_meta=None):
        signal_time=signal_time or self._now(); trade_id=trade_id or f"TRD_{uuid.uuid4().hex}"; targets=[x for x in (tp1,tp2,tp3,tp_final) if x is not None];
        if not targets: raise ValueError('At least one target is required')
        final=tp_final if tp_final is not None else targets[-1]; expiry_time=expiry_time or (parse_dt(signal_time)+timedelta(hours=AUDITOR_DEFAULT_TRADE_EXPIRY_HOURS)).isoformat(); obj=TradeInput(trade_id,signal_time,direction,float(entry),float(sl),tp1,tp2,tp3,final,score,quality,risk_reward,timeframe,expiry_time,analysis_snapshot or {}); self.trade_auditor.register(obj,trade_type=trade_type,sr_meta=sr_meta); return trade_id
    ...
    def observe_bar(self,**kw): return self.trade_auditor.observe_bar(BarPoint(kw['timestamp'],float(kw['open']),float(kw['high']),float(kw['low']),float(kw['close']),kw.get('source','simulation')))
    def metrics_report(self): return self.metrics.report()
    def daily_report(self): return self.reporter.daily()
    def weekly_report(self,persist=True):
        text,r=self.reporter.weekly(); key=r['period']['start'][:10]
        if persist:
            with self.db.transaction() as c: c.execute('INSERT OR REPLACE INTO Weekly_Trade_Reports(report_key,week_start,week_end,generated_at,report_json,report_text) VALUES(?,?,?,?,?,?)',(key,r['period']['start'],r['period']['end'],self._now(),self.db.jdump(r),text))
        return text,r
    def _maybe_finalize_week(self):
        tz=ZoneInfo(AUDITOR_WEEKLY_TZ); n=datetime.now(tz)
        if n.weekday()!=5: return
        st=(n-timedelta(days=5)).replace(hour=0,minute=0,second=0,microsecond=0); key=st.date().isoformat()
        with self.db.connect() as c: exists=c.execute('SELECT 1 FROM Weekly_Trade_Reports WHERE report_key=?',(key,)).fetchone()
        if not exists: self.weekly_report(persist=True)

    def run_forever(self):
        while not self._stop.is_set():
            try:
                with self.db.connect() as c: active=c.execute("SELECT COUNT(*) FROM Live_Trades WHERE status='ACTIVE'").fetchone()[0]
                if active: self.trade_auditor.observe_tick(self.price_engine.get_price(force=True))
            except Exception: AUDITOR_LOGGER.exception('TRADE_AUDIT_CYCLE_ERROR')
            try: self._maybe_finalize_week()
            except Exception: AUDITOR_LOGGER.exception('WEEKLY_REPORT_ERROR')
...
VERSION = "v18.68-INSTITUTIONAL-IMF-RAW-RESPONSE-PARSER-FORENSICS-WGC-ARABIC-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
def _auditor_register_analysis_direct(*args, **kwargs):
    """Disabled: monitor scope is trades and reports only."""
    return False


def _auditor_register_analysis(*args, **kwargs):
    """Disabled: monitor scope is trades and reports only."""
    return False


def _auditor_register_sr_trade(trade, snapshot, setup_id):
    if PERFORMANCE_AUDITOR is None or not setup_id: return False
    side='SUPPORT' if trade.get('direction')=='BUY' else 'RESISTANCE'
    meta={'side':side,'level':None,'trigger':trade.get('trigger'),'strength':trade.get('score')}
    return _auditor_enqueue(('sr_trade',dict(trade),dict(snapshot or {}),setup_id,meta))
    ...
    while not AUDITOR_WORKER_STOP.is_set():
        ...
        try:
            ...
            if kind == "trade":
                ...
                tr,snap,sid,meta=task[1],task[2],task[3],task[4]
                PERFORMANCE_AUDITOR.register_trade(trade_id=sid,signal_time=snap.get('time') or datetime.now(timezone.utc).isoformat(),direction=tr['direction'],entry=tr['entry'],sl=tr['sl'],tp1=tr.get('tp1'),tp2=tr.get('tp2'),tp3=tr.get('tp3'),tp_final=tr.get('tp3') or tr.get('tp2') or tr.get('tp1'),score=tr.get('score'),quality='S/R',risk_reward=tr.get('rr'),timeframe='M15',analysis_snapshot=snap,trade_type='SR',sr_meta=meta)
            elif kind == "analysis":
...
async def reply(update, text):
    """إرسال موثوق لرسائل Telegram الطويلة، خصوصاً التحليل الكامل."""
    try:
...
async def full_analysis(update, context):
    ...
    try:
        ...
        if q:
            try:
                await q.answer("جارٍ إعداد التحليل الكامل…")
            except Exception:
        ...
        await reply(update, analysis_text)
    except Exception as e:
        logger.exception("Full analysis execution error")
        await reply(update, f"❌ تعذر تنفيذ التحليل الكامل.\nالسبب: {e}")
...
async def audit_report(update, context):
    """مراقب الأداء المحدث — نفس النظام البصري للمراقب القديم، مع بيانات الصفقات و S/R فقط."""
    ...
    if not (is_admin_chat(user_id) or is_admin_chat(update.effective_chat.id)):
        ...
        return
    if PERFORMANCE_AUDITOR is None:
        await reply(update, "🛡️ XAU SMART TRADER — مراقب الأداء\n\n🔴 محرك المراقبة غير متاح حالياً.")
        return
    try:
        report = await asyncio.to_thread(PERFORMANCE_AUDITOR.metrics_report)
        t = report.get("trades", {})
        sr = report.get("sr_trades", {})
        main = report.get("main_trades", {})
        lines = [
            "🛡️ XAU SMART TRADER — مراقب الأداء", "━━━━━━━━━━━━━━━━━━", "",
            "📊 الصفقات", f"• الإجمالي: {t.get('total', 0)}", f"• المفتوحة: {t.get('open', 0)} | المغلقة: {t.get('closed', 0)}",
            f"• نجاح كامل: {t.get('full_success', 0)}", f"• نجاح جزئي: {t.get('partial_success', 0)}", f"• فشل: {t.get('failed', 0)}",
            f"• منتهية: {t.get('expired', 0)}", f"• غامضة: {t.get('ambiguous', 0)}",
            f"• Win Rate: {float(t.get('win_rate_pct', 0) or 0):.2f}%", f"• Full Target Rate: {float(t.get('full_target_rate_pct', 0) or 0):.2f}%",
            f"• متوسط الجودة: {float(t.get('avg_score', 0) or 0):.2f} نقطة", f"• متوسط R:R: {float(t.get('avg_rr', 0) or 0):.2f}",
            f"• متوسط MAE: {float(t.get('avg_mae', 0) or 0):.4f}", f"• متوسط MFE: {float(t.get('avg_mfe', 0) or 0):.4f}", f"• متوسط Drawdown: {float(t.get('avg_drawdown', 0) or 0):.4f}", "",
            "🎯 أداء الصفقات الرئيسية", f"• الإجمالي: {main.get('total', 0)} | المغلقة: {main.get('closed', 0)}",
            f"• نجاح كامل: {main.get('full_success', 0)} | نجاح جزئي: {main.get('partial_success', 0)}", f"• فشل: {main.get('failed', 0)} | منتهية: {main.get('expired', 0)}",
            f"• Win Rate: {float(main.get('win_rate_pct', 0) or 0):.2f}%", f"• متوسط الجودة: {float(main.get('avg_score', 0) or 0):.2f} نقطة", f"• متوسط R:R: {float(main.get('avg_rr', 0) or 0):.2f}", "",
            "📐 أداء صفقات الدعم والمقاومة S/R", f"• الإجمالي: {sr.get('total', 0)} | المغلقة: {sr.get('closed', 0)}",
            f"• نجاح كامل: {sr.get('full_success', 0)} | نجاح جزئي: {sr.get('partial_success', 0)}", f"• فشل: {sr.get('failed', 0)} | منتهية: {sr.get('expired', 0)}",
            f"• Win Rate: {float(sr.get('win_rate_pct', 0) or 0):.2f}%", f"• Full Target Rate: {float(sr.get('full_target_rate_pct', 0) or 0):.2f}%",
            f"• متوسط الجودة: {float(sr.get('avg_score', 0) or 0):.2f} نقطة", f"• متوسط R:R: {float(sr.get('avg_rr', 0) or 0):.2f}", "",
            "📋 التقارير", "• التقرير اليومي: متابعة نتائج يوم التداول.", "• التقرير الأسبوعي: النتائج الكاملة لأسبوع التداول.", "",
            "🔒 المراقب مستقل ولا يغيّر قرارات التداول أو العتبات.",
        ]
        buttons = [[InlineKeyboardButton("📅 التقرير اليومي", callback_data="audit_daily"), InlineKeyboardButton("📆 التقرير الأسبوعي", callback_data="audit_weekly")], [InlineKeyboardButton("🔄 تحديث", callback_data="menu_audit"), InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")]]
...
async def audit_daily_report(update, context):
    if PERFORMANCE_AUDITOR is None: return await reply(update,'🛡️ المراقب غير متاح حالياً.')
    await reply(update, await asyncio.to_thread(PERFORMANCE_AUDITOR.daily_report))

async def audit_weekly_report(update, context):
    if PERFORMANCE_AUDITOR is None: return await reply(update,'🛡️ المراقب غير متاح حالياً.')
    text,_=await asyncio.to_thread(PERFORMANCE_AUDITOR.weekly_report)
    await reply(update,text)
...
async def callback_router(update, context):
    ...
    callback_routes = {
        ...
        "audit_daily": audit_daily_report,
        "audit_weekly": audit_weekly_report,
    }
...
async def send_market_session_alerts():
    """تنبيهات مستقلة ونظيفة: قبل الافتتاح بـ15 دقيقة + بداية التداخل فقط."""
    now = now_damascus()
    ...
    # 1) Pre-open: تنبيه واحد عند 15 دقيقة تقريباً فقط، وليس عند الافتتاح.
    for name, label, zone_name, open_hour, _duration in SESSION_SPECS:
        ...
        minutes_to_open = (opening - local).total_seconds() / 60.0
        remaining = int(round(minutes_to_open))
        if remaining != 15:
            continue
        key = f"PREOPEN:{name}:{local.date().isoformat()}"
        if SESSION_ALERT_STATE.get(key):
            continue
        text = (
            f"🔔 افتتاح سوق {label} بعد 15 دقيقة\n\n"
            f"📊 حالة السوق\n{state}\n{status_note}\n\n"
            f"🕐 الوقت الحالي: {now.strftime('%H:%M')} دمشق"
        )
        results = await _dispatch_broadcast(recipients, text, kind="session_preopen")
        if any(x is True for x in results):
            SESSION_ALERT_STATE[key] = True

    # 2) Overlaps: تنبيه واحد فقط عند انتقال زوج من غير متداخل إلى متداخل.
...
async def start_bot():
    ...
    APPLICATION.add_handler(CommandHandler("audit_daily", audit_daily_report))
    APPLICATION.add_handler(CommandHandler("audit_weekly", audit_weekly_report))
...
def sr_analyze():
    ...
    try: sr=support_resistance(df,lookback=min(120,len(df)))
    except Exception as exc:
    ...
    cands=[x for x in (_sr_candidate_setup(df,sr,"BUY"),_sr_candidate_setup(df,sr,"SELL")) if x]
    if not cands:
    ...
    cands.sort(key=lambda x:x["score"],reverse=True); best=cands[0]; trade=_sr_build_trade(best)
    if not trade:
...
def _sr_insert_trade(trade, snapshot):
    ...
    try:
        ...
        if cur.rowcount == 1:
            ...
            conn.commit()
            _auditor_register_sr_trade(trade, snapshot, setup_id)
...
async def sr_button(update, context):
    try:
        ...
        await reply(update,f"❌ تعذر تنفيذ استراتيجية S/R حالياً.\n🧪 رمز التشخيص: SR_HANDLER\nتفاصيل: {type(exc).__name__}: {exc}")
        ...
        try:
            if SR_AUTO_ENABLED and not market_closed_reason():
                ...
                if result.get("signal"):
                    ...
                    if is_new:
                        await _sr_notify_all("🚨 صفقة S/R مستقلة جديدة\n" + _sr_format(result) + f"\n\n🆔 {setup_id}")
            try:
                ...
                for row,status,res,price in changed:
                    await _sr_notify_all(f"📐 تحديث صفقة S/R\n🆔 {row['setup_id']}\n💰 السعر: {price}\n📌 النتيجة: {res}\n📊 الحالة: {status}")
            except Exception:
...
def _sr_install_telegram_handler():
    APPLICATION.add_handler(CommandHandler("sr", sr_command), group=0)
    APPLICATION.add_handler(
        MessageHandler(filters.TEXT & filters.Regex(r"^📐 استراتيجية S/R$"), _sr_button_guarded),
        group=0,
    )
...
async def integrated_start_bot():
    ...
    APPLICATION.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, router), group=1)
    await APPLICATION.initialize()
    await APPLICATION.start()
    await _start_alert_dispatcher()
    await APPLICATION.bot.set_webhook(url=WEBHOOK_URL, allowed_updates=["message", "callback_query"], drop_pending_updates=True)
    ...
    logger.info("Webhook: %s", WEBHOOK_URL)
    asyncio.create_task(auto_loop())
    asyncio.create_task(_sr_auto_loop())
    if PERFORMANCE_AUDITOR is not None:
        try:
            _start_auditor_bridge_worker()
            PERFORMANCE_AUDITOR.start_background()
...
def run_v18_69_sr_self_tests():
    ...
    check("exact S/R button", 'filters.Regex(r"^📐 استراتيجية S/R$")' in source)
