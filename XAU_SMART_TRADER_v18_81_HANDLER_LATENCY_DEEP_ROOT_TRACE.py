...
# ============================================================
# HANDLER LATENCY ROOT TRACE v18.79
# ============================================================
...
class Metrics:
    ...
    def system_snapshot(self,queue_depth=0,queue_capacity=0,queue_drops=0):
        ...
        return {"uptime_s":now-self.started_mono,"cpu_pct":cpu_pct,"rss_mb":rss,"event_loop_lag_ms":_percentile(loops,95),"event_loop_lag_max_ms":max(loops) if loops else 0.0,"request_avg_ms":sum(req)/len(req) if req else 0.0,"request_p95_ms":_percentile(req,95),"request_max_ms":max(req) if req else 0.0,"request_count":req_count,"handler_avg_ms":sum(handlers)/len(handlers) if handlers else 0.0,"handler_p95_ms":_percentile(handlers,95),"handler_max_ms":max(handlers) if handlers else 0.0,"handler_count":handler_count,"handler_trace_count":trace_count,"throughput_per_min":throughput,"price_latency_ms":sum(pls)/len(pls) if pls else self.last_price_latency_ms,"db_write_latency_ms":sum(dbs)/len(dbs) if dbs else 0.0,"price_age_s":price_age,"queue_depth":queue_depth,"queue_capacity":queue_capacity,"queue_drops":queue_drops,"error_count":errors,"alert_count":alerts,"self_heal_count":heals,"auditor_heartbeat":hb}
    @staticmethod
    def _rss_mb():
        try:
...
async def reply(update, text):
    ...
    try:
        with handler_trace_stage("telegram_reply:format_header"):
            payload = with_market_header(text)
        with handler_trace_stage("telegram_reply:split_text"):
            parts = _split_telegram_text(payload)
        markup = None
        q = getattr(update, "callback_query", None)
        if q:
            try:
                with handler_trace_stage("telegram_reply:callback_answer"):
                    await q.answer()
            except Exception:
            ...
            with handler_trace_stage("telegram_reply:edit_message"):
                await q.edit_message_text(parts[0], reply_markup=markup)
            for part in parts[1:]:
                with handler_trace_stage("telegram_reply:send_part"):
...
async def audit_report(update, context):
    ...
    try:
        ...
        d = report.get("daily", {})
        w = report.get("weekly", {})
        root_trace = report.get("handler_root_trace", {})
        latest_traces = root_trace.get("latest", []) if isinstance(root_trace, dict) else []
        stage_aggregates = root_trace.get("stage_aggregates", []) if isinstance(root_trace, dict) else []
        lines = [
            ...
            "🔎 آخر Root Traces",
            *[
                f"• {t.get('trace_id','—')} | {t.get('handler','—')} | {t.get('elapsed_ms',0):.1f} ms | {t.get('outcome','—')}"
                for t in latest_traces[:5]
            ],
            *[
                f"• أبطأ مرحلة: {s.get('stage','—')} | avg {s.get('avg_ms',0):.1f} ms | max {s.get('max_ms',0):.1f} ms"
                for s in stage_aggregates[:3]
            ],
            "",
