...
AUTO_ENABLED = True
# TEMPORARY: تعطيل حجب التداول بسبب تعطل مصدر تقويم الأخبار (403).
# يعاد تفعيل الفلتر بعد إصلاح/استبدال مصدر الأخبار.
...
async def auto_loop():
    ...
    while True:
        try:
            ...
            if not record:
                for token in quota_tokens.values(): _release_trade_quota(token)
