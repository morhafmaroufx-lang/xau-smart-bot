...
async def start_bot():
    ...
    # v19.05: blocked-account guard runs before every command/message/callback.
    APPLICATION.add_handler(MessageHandler(filters.ALL, _trace_handler(blocked_update_guard, "guard:blocked_user")), group=-10)
    APPLICATION.add_handler(CallbackQueryHandler(_trace_handler(blocked_callback_guard, "guard:blocked_callback"), pattern=r".*"), group=-10)
    ...
    APPLICATION.add_handler(CommandHandler("unsubscribe", _trace_handler(user_cancel_subscription_callback, "command:/unsubscribe")))
...
# ============================================================
# v19.05 — ACCOUNT LIFECYCLE / FREE GRANTS / BLOCKING
# ============================================================
# نطاق التعديل:
# 1) FREE+1 = منحة BASIC مجانية.
# 2) FREE+2 = منحة ELITE مجانية.
# 3) إلغاء المنحة المجانية مع استعادة الحالة السابقة بدقة.
# 4) إلغاء الاشتراك وإرجاع الحساب إلى FREE.
# 5) حظر المستخدم ومنع جميع أوامر/رسائل/Callbacks الحساب المحظور.
# 6) فك الحظر وإعادة الحساب إلى الحالة التشغيلية دون منح صلاحيات إضافية.
#
# لا يتم تعديل محرك التداول، المؤشرات، الإشارات، Entry/SL/TP، S/R،
# التنبيهات، أو قواعد التقييم.
# ============================================================
V19_05_VERSION = "v19.05_ADMIN_ACCOUNT_LIFECYCLE"
VERSION = V19_05_VERSION

FREE_GRANT_BASIC_CODE = "FREE+1"
FREE_GRANT_ELITE_CODE = "FREE+2"


def _v19_05_user_row(chat_id):
    conn = _db()
    try:
        row = conn.execute(
            "SELECT chat_id,username,first_name,plan,status,start_date,expiry_date,created_at,updated_at "
...
def _v19_05_cancel_free_grant(chat_id, cancelled_by=None):
    """Cancel the active administrative grant and restore its exact stored prior state."""
    cid = int(chat_id)
    if cancelled_by is not None and not is_admin_chat(int(cancelled_by)):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    _ensure_admin_free_grant_table()
    now = now_damascus()
    conn = _db()
    try:
        grant = conn.execute(
            f"SELECT * FROM {ADMIN_FREE_GRANT_TABLE} "
            "WHERE chat_id=? AND active=1 ORDER BY id DESC LIMIT 1",
            (cid,),
        ).fetchone()
        if not grant:
            return {"changed": False, "reason": "no_active_grant"}

        # Restore exactly what existed before the grant.
        restored_plan = grant["previous_plan"] if grant["previous_plan"] in PLANS else "FREE"
        restored_status = grant["previous_status"] or "active"
        conn.execute(
        ...
        return {
            ...
            "grant_plan": str(grant["plan"]),
            "previous_plan": restored_plan,
...
def _v19_05_cancel_subscription(chat_id, cancelled_by=None):
    """Cancel the effective subscription and return the account to canonical FREE."""
    cid = int(chat_id)
    if cancelled_by is not None and int(cancelled_by) != cid and not is_admin_chat(int(cancelled_by)):
        raise PermissionError("لا يمكنك إلغاء اشتراك حساب آخر")
    now = now_damascus()
    _ensure_admin_free_grant_table()
    conn = _db()
    try:
        row = conn.execute(
            "SELECT chat_id,plan,status FROM users WHERE chat_id=?",
            (cid,),
        ).fetchone()
        if not row:
            conn.execute(
                "INSERT INTO users(chat_id,plan,status,referral_code,created_at,updated_at) "
                "VALUES(?,?,?,?,?,?)",
                (cid, "FREE", "active", f"ref_{cid}", now.isoformat(), now.isoformat()),
            )
            conn.commit()
            return {"changed": True, "previous_plan": "FREE", "previous_status": "active"}

        previous_plan = row["plan"]
        previous_status = row["status"]

        # Cancellation is authoritative: it also removes any active free grant.
        conn.execute(
            f"UPDATE {ADMIN_FREE_GRANT_TABLE} SET active=0 WHERE chat_id=? AND active=1",
            (cid,),
        )
        resulting_status = "blocked" if str(previous_status or "").lower() == "blocked" else "active"
        conn.execute(
            "UPDATE users SET plan='FREE', status=?, start_date=NULL, expiry_date=NULL, updated_at=? "
            "WHERE chat_id=?",
            (resulting_status, now.isoformat(), cid),
        )
        conn.commit()
        return {
            "changed": previous_plan != "FREE" or previous_status != "active",
...
def _v19_05_set_blocked(chat_id, admin_id):
    cid = int(chat_id)
    aid = int(admin_id)
    ...
    try:
        ...
        if not row:
            conn.execute(
                "INSERT INTO users(chat_id,plan,status,referral_code,created_at,updated_at) "
                "VALUES(?,?,?,?,?,?)",
                (cid, "FREE", "blocked", f"ref_{cid}", now.isoformat(), now.isoformat()),
            )
            previous_plan, previous_status = "FREE", "blocked"
        else:
            previous_plan, previous_status = row["plan"], row["status"]
            conn.execute(
                "UPDATE users SET status='blocked', updated_at=? WHERE chat_id=?",
                (now.isoformat(), cid),
            )
        conn.commit()
        return {"changed": previous_status != "blocked", "plan": previous_plan, "status": "blocked"}
    finally:
        conn.close()


def _v19_05_unblock_user(chat_id, admin_id):
    cid = int(chat_id)
    aid = int(admin_id)
    if not is_admin_chat(aid):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    ...
    try:
        ...
        if row["status"] != "blocked":
            return {"changed": False, "plan": row["plan"], "status": row["status"]}

        restored_plan = row["plan"] if row["plan"] in PLANS else "FREE"
        restored_expiry = None
        if restored_plan != "FREE":
            expiry_raw = conn.execute(
                "SELECT expiry_date FROM users WHERE chat_id=?", (cid,)
            ).fetchone()["expiry_date"]
            try:
                expiry_dt = datetime.fromisoformat(str(expiry_raw)) if expiry_raw else None
                if expiry_dt is None or now_damascus() >= expiry_dt:
                    restored_plan = "FREE"
                else:
                    restored_expiry = expiry_raw
            except Exception:
                restored_plan = "FREE"

        if restored_plan == "FREE":
            conn.execute(
                "UPDATE users SET plan='FREE', status='active', start_date=NULL, expiry_date=NULL, updated_at=? "
                "WHERE chat_id=?",
                (now.isoformat(), cid),
            )
        else:
            conn.execute(
                "UPDATE users SET plan=?, status='active', expiry_date=?, updated_at=? WHERE chat_id=?",
                (restored_plan, restored_expiry, now.isoformat(), cid),
            )
        conn.commit()
        return {"changed": True, "plan": restored_plan, "status": "active"}
    finally:
        conn.close()


def _v19_05_blocked_status(chat_id):
    row = _v19_05_user_row(chat_id)
    return bool(row and str(row.get("status") or "").lower() == "blocked")


async def blocked_callback_guard(update, context):
    q = getattr(update, "callback_query", None)
    if q is None:
        return
    uid = getattr(getattr(update, "effective_user", None), "id", None)
    cid = getattr(getattr(update, "effective_chat", None), "id", None)
    probe = uid if uid is not None else cid
    if probe is None or is_admin_chat(probe):
        return
    if await asyncio.to_thread(_v19_05_blocked_status, probe):
        try:
            await q.answer("⛔ حسابك محظور حالياً. تواصل مع الإدارة.", show_alert=True)
        finally:
            raise ApplicationHandlerStop


async def blocked_update_guard(update, context):
    uid = getattr(getattr(update, "effective_user", None), "id", None)
    cid = getattr(getattr(update, "effective_chat", None), "id", None)
    probe = uid if uid is not None else cid
    if probe is None or is_admin_chat(probe):
    ...
    if await asyncio.to_thread(_v19_05_blocked_status, probe):
        ...
        if message is not None:
            await message.reply_text("⛔ حسابك محظور حالياً.\n📩 تواصل مع الإدارة لطلب فك الحظر.")
        raise ApplicationHandlerStop
...
async def my_subscription(update, context):
    ...
    text = "💳 اشتراكك الحالي\n\n" + await asyncio.to_thread(plan_status_text, chat_id)
    rows = [
        [InlineKeyboardButton("❌ إلغاء الاشتراك والعودة إلى FREE", callback_data="user_cancel_subscription")],
        [InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")],
    ]
    await reply(update, with_market_header(text), reply_markup=InlineKeyboardMarkup(rows))


async def user_cancel_subscription_callback(update, context):
    q = getattr(update, "callback_query", None)
    cid = update.effective_chat.id
    try:
        if q is not None:
            await q.answer()
        result = await asyncio.to_thread(_v19_05_cancel_subscription, cid, cid)
        if result["changed"]:
            msg = "✅ تم إلغاء الاشتراك.\n\n🔄 عاد حسابك الآن إلى 🆓 FREE.\n🎯 يمكنك الاشتراك مجدداً في أي وقت."
        else:
            msg = "ℹ️ حسابك بالفعل على 🆓 FREE."
        markup = InlineKeyboardMarkup(
            [[InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")]]
        )
        if q is not None:
            await q.edit_message_text(with_market_header(msg), reply_markup=markup)
        else:
            await reply(update, with_market_header(msg), reply_markup=markup)
    except Exception as exc:
        msg = f"❌ تعذر إلغاء الاشتراك.\nالسبب: {type(exc).__name__}: {exc}"
        if q is not None:
            await q.edit_message_text(with_market_header(msg))
        else:
...
async def admin_user_details(update, context, chat_id):
    ...
    lines = [
        ...
        f"• Username: @{row['username']}" if row['username'] else "• Username: —",
        f"• الخطة: {row['plan']}",
        f"• الحالة: {'⛔ محظور' if status == 'blocked' else row['status']}",
        f"• البداية: {row['start_date'] or '—'}",
        f"• الانتهاء: {row['expiry_date'] or '—'}",
    ]
    if grant:
        grant_plan = str(grant.get("plan") or "—")
        grant_code = FREE_GRANT_BASIC_CODE if grant_plan == "BASIC" else FREE_GRANT_ELITE_CODE
        lines += [
            "",
            f"🎁 المنحة النشطة: {grant_code} — {grant_plan}",
            f"• تنتهي: {grant['expiry_date']}",
        ]

    rows = []
    if grant:
        rows.append([InlineKeyboardButton("🗑️ إلغاء المنحة المجانية", callback_data="admin_cancel_free_grant")])
    else:
        rows.append([InlineKeyboardButton("🎁 منح اشتراك مجاني", callback_data="admin_grant_free_menu")])

    rows.append([InlineKeyboardButton("❌ إلغاء الاشتراك → FREE", callback_data="admin_cancel_subscription")])
    if status == "blocked":
        rows.append([InlineKeyboardButton("🔓 فك الحظر", callback_data="admin_unblock_user")])
    elif not is_admin_chat(int(chat_id)):
        rows.append([InlineKeyboardButton("⛔ حظر المستخدم", callback_data="admin_block_user")])
...
async def _admin_action_result(update, text):
    q = update.callback_query
    if q:
        await q.edit_message_text(
            with_market_header(text),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 بيانات المستخدم", callback_data="admin_refresh_user")],
                [InlineKeyboardButton("👑 لوحة الإدارة", callback_data="admin_home")],
            ]),
        )
    else:
...
async def admin_account_action_callback(update, context):
    if not await _admin_guard(update):
        return
    q = update.callback_query
    data = q.data if q else ""
    ...
    if not cid:
        await q.answer()
        await q.edit_message_text(with_market_header("⛔ لم يتم تحديد حساب."))
        return
    await q.answer()
    try:
        if data == "admin_cancel_free_grant":
            result = await asyncio.to_thread(_v19_05_cancel_free_grant, cid, update.effective_user.id)
            if not result["changed"]:
                await _admin_action_result(update, "ℹ️ لا توجد منحة مجانية نشطة لهذا الحساب.")
            else:
                await _admin_action_result(
                    update,
                    "✅ تم إلغاء المنحة المجانية.\n\n"
                    f"• نوع المنحة: {result['grant_plan']}\n"
                    f"• الحالة المستعادة: {result['previous_plan']} / {result['previous_status']}",
                )
            return

        if data == "admin_cancel_subscription":
            result = await asyncio.to_thread(_v19_05_cancel_subscription, cid, update.effective_user.id)
            await _admin_action_result(
                update,
                "✅ تم إلغاء الاشتراك.\n\n"
                f"• الحالة السابقة: {result['previous_plan']} / {result['previous_status']}\n"
                "• الحالة الحالية: 🆓 FREE (مع الحفاظ على الحظر إن كان مفعلاً)",
            )
            return

        if data == "admin_block_user":
            result = await asyncio.to_thread(_v19_05_set_blocked, cid, update.effective_user.id)
            await _admin_action_result(
                update,
                "⛔ تم حظر المستخدم.\n\n"
                f"• الخطة الحالية محفوظة: {result['plan']}\n"
                "• تم إيقاف وصول الحساب إلى البوت حتى فك الحظر.",
            )
            return

        if data == "admin_unblock_user":
            result = await asyncio.to_thread(_v19_05_unblock_user, cid, update.effective_user.id)
            await _admin_action_result(
                update,
                "🔓 تم فك الحظر.\n\n"
                f"• الخطة: {result['plan']}\n"
...
_LEGACY_ADMIN_COMMAND_V19_05 = admin_command
...
async def admin_command(update, context):
    ...
    if not args:
        await reply(
            ...
            "/admin grant USER_ID BASIC DAYS\n"
            "/admin grant USER_ID ELITE DAYS\n"
            "/admin cancel_grant USER_ID\n"
            "/admin cancel_subscription USER_ID\n"
            "/admin block USER_ID\n"
    ...
    try:
        if action == "grant" and len(args) >= 4:
            user_id = int(args[1]); plan = args[2].upper(); days = int(args[3])
            if plan not in ("BASIC", "ELITE"):
                raise ValueError("المنحة المجانية يجب أن تكون BASIC أو ELITE")
            if days <= 0:
                raise ValueError("عدد الأيام يجب أن يكون موجباً")
            result = await asyncio.to_thread(
                _grant_free_plan, user_id, days, update.effective_user.id, plan
            )
            code = FREE_GRANT_BASIC_CODE if plan == "BASIC" else FREE_GRANT_ELITE_CODE
            await reply(
                update,
                with_market_header(
                    f"✅ تم منح {code} — {plan} مجاناً.\n\n"
                    f"• المستخدم: {user_id}\n"
                    f"• المدة: {days} يوم\n"
                    f"• الانتهاء: {result['expiry'].strftime('%Y-%m-%d %H:%M')} دمشق"
                ),
            )
            return

        if action in {"cancel_grant", "revoke_grant"} and len(args) >= 2:
            user_id = int(args[1])
            result = await asyncio.to_thread(
                _v19_05_cancel_free_grant, user_id, update.effective_user.id
            )
            await reply(
                update,
                "✅ تم إلغاء المنحة واستعادة الحالة السابقة."
                if result["changed"]
                else "ℹ️ لا توجد منحة مجانية نشطة لهذا الحساب.",
            )
            return

        if action in {"cancel_subscription", "unsubscribe"} and len(args) >= 2:
            user_id = int(args[1])
            result = await asyncio.to_thread(
                _v19_05_cancel_subscription, user_id, update.effective_user.id
            )
            await reply(
                update,
                "✅ تم إلغاء الاشتراك وإرجاع الحساب إلى 🆓 FREE."
                if result["changed"]
                else "ℹ️ الحساب بالفعل على 🆓 FREE.",
            )
            return

        if action == "block" and len(args) >= 2:
            user_id = int(args[1])
            result = await asyncio.to_thread(
                _v19_05_set_blocked, user_id, update.effective_user.id
            )
            await reply(update, "⛔ تم حظر المستخدم." if result["changed"] else "ℹ️ المستخدم محظور بالفعل.")
            return

        if action == "unblock" and len(args) >= 2:
            user_id = int(args[1])
            result = await asyncio.to_thread(
                _v19_05_unblock_user, user_id, update.effective_user.id
            )
            await reply(update, "🔓 تم فك الحظر." if result["changed"] else "ℹ️ المستخدم غير محظور.")
            return

        await _LEGACY_ADMIN_COMMAND_V19_05(update, context)
    except Exception as exc:
        await reply(update, f"❌ تعذر تنفيذ العملية.\nالسبب: {type(exc).__name__}: {exc}")


# ---------- Final callback router: preserve all legacy callbacks ----------
_LEGACY_CALLBACK_ROUTER_V19_05 = callback_router
...
async def callback_router(update, context):
    ...
    if data in {
        ...
        if data == "user_cancel_subscription":
            await user_cancel_subscription_callback(update, context)
        else:
            await admin_account_action_callback(update, context)
        return
    await _LEGACY_CALLBACK_ROUTER_V19_05(update, context)


# ---------- Final text router: user self-service cancellation ----------
_LEGACY_ROUTER_V19_05 = router


async def router(update, context):
    text = (update.effective_message.text or "").strip() if getattr(update, "effective_message", None) else ""
    if text == "❌ إلغاء الاشتراك":
        await user_cancel_subscription_callback(update, context)
        return
...
def run_v19_05_self_tests():
    ...
    check("version", V19_05_VERSION == "v19.05_ADMIN_ACCOUNT_LIFECYCLE")
    check("free_plus_1_basic", FREE_GRANT_BASIC_CODE == "FREE+1")
    check("free_plus_2_elite", FREE_GRANT_ELITE_CODE == "FREE+2")
    check("grant_plans", {"BASIC", "ELITE"}.issubset({"BASIC", "ELITE"}))
    check("cancel_grant_function", callable(_v19_05_cancel_free_grant))
    check("cancel_subscription_function", callable(_v19_05_cancel_subscription))
    check("block_function", callable(_v19_05_set_blocked))
    check("unblock_function", callable(_v19_05_unblock_user))
    check("blocked_guard", callable(blocked_update_guard) and callable(blocked_callback_guard))
    check("startup_guard_registration", "group=-10" in globals().get("integrated_start_bot", lambda: None).__code__.co_consts)
    check("user_cancel_callback", "user_cancel_subscription" in globals().get("callback_router", lambda: None).__code__.co_consts)
    check("admin_cancel_grant_callback", "admin_cancel_free_grant" in globals().get("callback_router", lambda: None).__code__.co_consts)
    check("admin_block_callback", "admin_block_user" in globals().get("callback_router", lambda: None).__code__.co_consts)
    check("admin_unblock_callback", "admin_unblock_user" in globals().get("callback_router", lambda: None).__code__.co_consts)
    return {
        "ok": not failures,
        "passed": 12 - len(failures),
        "total": 12,
        "failures": failures,
    }


# Final executable entry point — v19.05 only.
def main_v19_05():
    if "--v19-05-self-test" in sys.argv:
        print(json.dumps(run_v19_05_self_tests(), ensure_ascii=False, indent=2))
        return
    if "--v19-04-self-test" in sys.argv:
        print(json.dumps(run_v19_04_self_tests(), ensure_ascii=False, indent=2))
        return
    global BOT_LOOP
    server = threading.Thread(target=run_flask, daemon=True)
    server.start()
