...
VERSION = "v18.48-INSTITUTIONAL-DATA-INTEGRITY-HARDENED-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIX-MENU-MARKETS-RUNTIME-CORRECTED"
...
WGC_CENTRAL_BANK_CHANGES_URL = "https://www.gold.org/download/file/7741/Changes_latest_as_of_Sep2026_IFS.xlsx"
...
def _inst_http(url,params=None,timeout=None):
    try:
        r=requests.get(url,params=params,timeout=timeout or INSTITUTIONAL_HTTP_TIMEOUT,headers={
            "User-Agent":"XAU-Smart-Trader/18.48 institutional",
            "Accept":"*/*",
        })
    except requests.RequestException as exc:
        raise RuntimeError(f"HTTP request failed [{url}]: {type(exc).__name__}: {exc}") from exc
    if not r.ok:
        raise RuntimeError(f"HTTP {r.status_code} [{url}] content_type={r.headers.get('content-type','')}")
    return r
...
def _fred(ids):
    """FRED reader with a multi-series path plus per-series fallback.

    fredgraph can occasionally reject/partially serve a multi-series request.
    One failed series must never erase otherwise valid institutional data.
    """
    ...
    frames=[]
    try:
        r=_inst_http(FRED_CSV_URL,{'id':','.join(ids)})
        d=pd.read_csv(io.StringIO(r.text))
        if 'DATE' in d.columns:
            d['DATE']=pd.to_datetime(d['DATE'],errors='coerce')
            for col in d.columns:
                if col!='DATE': d[col]=pd.to_numeric(d[col],errors='coerce')
            d=d.dropna(subset=['DATE']).sort_values('DATE')
            if len(d): frames.append(d)
    except Exception as exc:
        logging.getLogger("institutional").warning("FRED bulk request failed: %s",exc)
    if frames:
        base=frames[0].set_index('DATE')
        missing=[x for x in ids if x not in base.columns]
    else:
        base=pd.DataFrame(index=pd.DatetimeIndex([],name='DATE')); missing=ids[:]
    for sid in missing:
        try:
            r=_inst_http(FRED_CSV_URL,{'id':sid})
            d=pd.read_csv(io.StringIO(r.text))
            if 'DATE' not in d.columns: continue
            d['DATE']=pd.to_datetime(d['DATE'],errors='coerce')
            value_cols=[x for x in d.columns if x!='DATE']
            if not value_cols: continue
            col=value_cols[0]
            d[col]=pd.to_numeric(d[col],errors='coerce')
            d=d.dropna(subset=['DATE']).set_index('DATE')[[col]].rename(columns={col:sid})
            base=base.join(d,how='outer')
        except Exception as exc:
            logging.getLogger("institutional").warning("FRED series %s failed: %s",sid,exc)
    if base.empty:
        raise RuntimeError("FRED unavailable for requested series")
    base=base.sort_index().reset_index()
    return _inst_cache_put(key,base)


def _yahoo(symbol,range_value='1mo'):
    key=f'yahoo:{symbol}:{range_value}'; c=_inst_cache_get(key)
    if c is not None:return c
    url=YAHOO_CHART_URL.format(symbol=requests.utils.quote(symbol,safe=''))
    payload=_inst_http(url,{'range':range_value,'interval':'1d','events':'history'}).json()
    item=((payload.get('chart') or {}).get('result') or [None])[0]
    if not item: raise RuntimeError(f'Yahoo unavailable: {symbol}')
    q=((item.get('indicators') or {}).get('quote') or [{}])[0]
    rows=[]
    for ts,cl in zip(item.get('timestamp') or [],q.get('close') or []):
        if cl is not None:
            rows.append((datetime.fromtimestamp(int(ts),tz=timezone.utc),float(cl)))
    if not rows: raise RuntimeError(f'Yahoo empty: {symbol}')
    return _inst_cache_put(key,pd.DataFrame(rows,columns=['date','close']))
...
def _find_date_column(df):
    """Find CFTC/report date columns across schema/name variations."""
    if df is None or df.empty:
        return None
    cols=list(df.columns)
    normalized={c:_norm_colname(c) for c in cols}
    # Prefer explicit CFTC date fields.
    for c,n in normalized.items():
        if n in {
            'report_date_as_yyyy_mm_dd','report_date_as_mm_dd_yyyy',
            'as_of_date_in_form_yyymmdd','as_of_date_in_form_yyyymmdd',
            'report_date','as_of_date','asof_date',
            'report_date_as_yyyy_mm_dd_yyyy_mm_dd'
        } or 'report_date' in n or n.startswith('as_of_date'):
            return c
    # Fallback: a column with a high proportion of parseable dates.
    best=None; best_count=0
    for c in cols:
        parsed=pd.to_datetime(df[c],errors='coerce')
        count=int(parsed.notna().sum())
        if count>=2 and count>best_count:
            best=(c,parsed); best_count=count
    return best[0] if best else None


def _attach_report_date(df):
    datecol=_find_date_column(df)
    if datecol is None:
        return df
    norm=_norm_colname(datecol)
    raw=df[datecol].astype(str).str.strip()
    parsed=pd.to_datetime(df[datecol],errors='coerce')
    # CFTC compact YYYYMMDD/YYMMDD fields must be parsed explicitly; generic
    # pandas parsing can interpret integer dates as nanoseconds since epoch.
    if 'yyymmdd' in norm or norm.endswith('yyyymmdd'):
        ...
        for fmt in ('%Y%m%d','%y%m%d','%m%d%Y'):
            q=pd.to_datetime(digits,format=fmt,errors='coerce')
            if q.notna().sum()>0:
                parsed=q; break
    df=df.copy(); df['_date']=parsed
    return df.dropna(subset=['_date']).sort_values('_date')
...
def _cftc_zip(url):
    ...
    d=_attach_report_date(d)
    if d.empty:
        raise RuntimeError('CFTC GOLD report date field unavailable/unparseable')
...
def _cot_real():
    ...
    if pd.isna(report_date):
        raise RuntimeError('CFTC GOLD report has values but no valid report date')
    out={'source':'CFTC Disaggregated + Legacy COT','report_date':pd.Timestamp(report_date).strftime('%Y-%m-%d'),
    ...
    return _inst_cache_put('cot',out)


def _gld_real():
    c=_inst_cache_get('gld')
    if c is not None:return c
    r=_inst_http(SPDR_GLD_HISTORY_URL,timeout=25)
    if 'spreadsheet' not in (r.headers.get('content-type','').lower()) and not r.content[:4] == b'PK\x03\x04':
        raise RuntimeError('SPDR GLD archive did not return XLSX')
    xl=pd.ExcelFile(io.BytesIO(r.content)); best=None
    for sh in xl.sheet_names:
        try:d=pd.read_excel(xl,sheet_name=sh)
        except Exception:continue
        cols={str(x).strip().lower():x for x in d.columns}
        dc=next((cols[k] for k in cols if 'date' in k or 'as of' in k),None)
        oc=next((cols[k] for k in cols if any(t in k for t in ('gold holdings','gold ounces','ounces of gold','gold oz')) and 'share' not in k),None)
        sc=next((cols[k] for k in cols if any(t in k for t in ('shares outstanding','total shares','shares')) and 'price' not in k),None)
        if dc is not None and (oc is not None or sc is not None):
            d['_date']=pd.to_datetime(d[dc],errors='coerce'); d=d.dropna(subset=['_date']).sort_values('_date')
            if len(d)>1: best=(d,oc,sc); break
    if best is None: raise RuntimeError('SPDR GLD archive schema unavailable')
    d,oc,sc=best; a,b=d.iloc[-1],d.iloc[-2]
    ao=pd.to_numeric(a[oc],errors='coerce') if oc is not None else np.nan; bo=pd.to_numeric(b[oc],errors='coerce') if oc is not None else np.nan
    ass=pd.to_numeric(a[sc],errors='coerce') if sc is not None else np.nan; bss=pd.to_numeric(b[sc],errors='coerce') if sc is not None else np.nan
    direction='IN' if pd.notna(ao) and pd.notna(bo) and ao>bo else 'OUT' if pd.notna(ao) and pd.notna(bo) and ao<bo else 'NEUTRAL'
    shares_dir='IN' if pd.notna(ass) and pd.notna(bss) and ass>bss else 'OUT' if pd.notna(ass) and pd.notna(bss) and ass<bss else 'NEUTRAL'
    if direction != 'NEUTRAL' and shares_dir not in ('NEUTRAL',direction): direction='UNCERTAIN'
    out={'source':'SPDR Gold Shares official archive','date':a['_date'].strftime('%Y-%m-%d'),
         'gold_oz':float(ao) if pd.notna(ao) else None,
         'gold_oz_delta':float(ao-bo) if pd.notna(ao) and pd.notna(bo) else None,
         'shares_delta':float(ass-bss) if pd.notna(ass) and pd.notna(bss) else None,
         'shares_direction':shares_dir,'flow_proxy':direction}
    out.update(_inst_status(a['_date'],5,'SPDR GLD'))
...
def _cb_real():
    ...
    if c is not None:return c
    urls=[WGC_CENTRAL_BANK_CHANGES_URL]
    # The WGC page is the authoritative locator and the hard-coded file is a
    # deterministic fallback. This prevents a monthly filename change from
    # disabling the source permanently.
    try:
        page=_inst_http('https://www.gold.org/goldhub/data/gold-reserves-by-country',timeout=20).text
        found=re.findall(r'https?://[^\"\' ]+Changes[^\"\' ]+\.xlsx',page,flags=re.I)
        urls=[found[0]]+urls if found else urls
    except Exception:
        pass
    last_exc=None
    for url in dict.fromkeys(urls):
        try:
            r=_inst_http(url,timeout=25)
            dset=pd.ExcelFile(io.BytesIO(r.content))
            for sh in dset.sheet_names:
                try:d=pd.read_excel(dset,sheet_name=sh)
                except Exception:continue
                if d.empty:continue
                cols=list(d.columns)
                # Layout A: dates are rows, change is a numeric column.
                date_cols=[]
                for col in cols:
                    parsed=pd.to_datetime(d[col],errors='coerce')
                    if parsed.notna().sum() >= max(2,int(len(d)*0.25)): date_cols.append((col,parsed))
                if date_cols:
                    dc,parsed=max(date_cols,key=lambda x:x[1].notna().sum())
                    latest=parsed.max(); mask=parsed==latest
                    numeric=[]
                    for col in cols:
                        if col==dc: continue
                        name=str(col).lower()
                        if any(k in name for k in ('change','net','purchase','sale','tonne')) and '%' not in name:
                            vals=pd.to_numeric(d.loc[mask,col],errors='coerce')
                            if vals.notna().sum(): numeric.append((col,vals.sum()))
                    if numeric:
                        col,total=max(numeric,key=lambda x:abs(float(x[1])))
                        out={'source':'World Gold Council / IMF IFS','period':latest.strftime('%Y-%m-%d'),'net_change_tonnes':float(total),'direction':'BUY' if total>0 else 'SELL' if total<0 else 'NEUTRAL'}
                        out.update(_inst_status(latest,45,'World Gold Council / IMF'))
                        return _inst_cache_put('cb',out)
                # Layout B: dates are columns; choose the latest date-like column.
                date_named=[]
                for col in cols:
                    parsed=pd.to_datetime(str(col),errors='coerce')
                    if not pd.isna(parsed): date_named.append((col,parsed))
                if date_named:
                    dc,latest=max(date_named,key=lambda x:x[1])
                    vals=pd.to_numeric(d[dc],errors='coerce').dropna()
                    if len(vals):
                        total=float(vals.sum())
                        out={'source':'World Gold Council / IMF IFS','period':pd.Timestamp(latest).strftime('%Y-%m-%d'),'net_change_tonnes':total,'direction':'BUY' if total>0 else 'SELL' if total<0 else 'NEUTRAL'}
                        out.update(_inst_status(latest,45,'World Gold Council / IMF'))
                        return _inst_cache_put('cb',out)
            raise RuntimeError('WGC central-bank change schema unavailable')
        except Exception as exc:
            last_exc=exc
    raise RuntimeError(f'WGC central-bank data unavailable: {last_exc}')


def _macro_real():
    c=_inst_cache_get('macro')
    if c is not None:return c
    ids=['DFEDTARL','DFEDTARU','EFFR','DGS10','DFII10','VIXCLS','DTWEXBGS']
    f=_fred(ids)
    f=f.sort_values('DATE')
    a=f.iloc[-1]; b=f.iloc[-2] if len(f)>1 else a
    def _series_status(series,max_age=3):
        if series not in f.columns:
            return {'status':'UNAVAILABLE','fresh':False,'age_days':None,'asof':None,'source':'FRED '+series}
        q=f[['DATE',series]].dropna()
        if q.empty:
            return {'status':'UNAVAILABLE','fresh':False,'age_days':None,'asof':None,'source':'FRED '+series}
        return _inst_status(q['DATE'].iloc[-1],max_age,'FRED '+series)
    lo=float(a.DFEDTARL) if pd.notna(a.DFEDTARL) else None; hi=float(a.DFEDTARU) if pd.notna(a.DFEDTARU) else None
    plo=float(b.DFEDTARL) if pd.notna(b.DFEDTARL) else lo; phi=float(b.DFEDTARU) if pd.notna(b.DFEDTARU) else hi
    delta=((lo+hi)-(plo+phi))/2 if all(v is not None for v in (lo,hi,plo,phi)) else 0
    pol='HOLD' if abs(delta)<1e-9 else ('HAWKISH' if delta>0 else 'DOVISH')
    zq=None; zq_df=None
    try:
        zq_df=_yahoo('ZQ=F','10d'); px=float(zq_df.close.iloc[-1]); zq=100-px if 90<px<100 else None
    except Exception: pass
    exp='غير متاحة'
    if zq is not None and lo is not None and hi is not None:
        mid=(lo+hi)/2
        exp='رفع محتمل' if zq-mid>=.10 else 'خفض محتمل' if zq-mid<=-.10 else 'استقرار محتمل'
    dxy=None; dxy_ch=None; dxy_asof=None
    try:
        q=_yahoo('DX-Y.NYB','1mo'); dxy=float(q.close.iloc[-1]); dxy_ch=(float(q.close.iloc[-1])/float(q.close.iloc[0])-1)*100; dxy_asof=q.date.iloc[-1]
    except Exception: pass
    spx=None; ma=None
    try:q=_yahoo('^GSPC','3mo'); spx=float(q.close.iloc[-1]); ma=float(q.close.tail(20).mean())
    except Exception:pass
    vix=float(a.VIXCLS) if pd.notna(a.VIXCLS) else None
    regime='غير متاح' if spx is None or ma is None else ('RISK-ON' if spx>=ma and (vix is None or vix<20) else 'RISK-OFF' if spx<ma and vix is not None and vix>=20 else 'MIXED')
    asof=a.get('DATE')
    out={'source':'FRED + secondary market feed','policy':pol,'fed_lower':lo,'fed_upper':hi,'effr':float(a.EFFR) if pd.notna(a.EFFR) else None,
         'series_status':{sid:_series_status(sid,3) for sid in ids},
         'rate_expectation':exp,'fed_futures_implied':zq,'rate_expectation_source':'Fed Funds futures (secondary feed)' if zq is not None else 'غير متاح',
         'dxy':dxy,'dxy_change_pct':dxy_ch,'dxy_asof':_inst_iso(dxy_asof),'usd_broad':float(a.DTWEXBGS) if pd.notna(a.DTWEXBGS) else None,
         'us10y':float(a.DGS10) if pd.notna(a.DGS10) else None,'real10y':float(a.DFII10) if pd.notna(a.DFII10) else None,
         'vix':vix,'risk_regime':regime}
    out.update(_inst_status(asof,3,'FRED'))
    out['dxy_status']=_inst_status(dxy_asof,3,'Yahoo DXY') if dxy_asof is not None else {'status':'UNAVAILABLE','fresh':False,'age_days':None,'asof':None,'source':'Yahoo DXY'}
    out['fed_futures_status']=_inst_status(zq_df.date.iloc[-1],3,'Fed Funds futures secondary') if zq_df is not None and zq is not None and hasattr(zq_df,'date') else {'status':'UNAVAILABLE','fresh':False,'age_days':None,'asof':None,'source':'Fed Funds futures secondary'}
...
def institutional_analysis(df):
    ...
    with ThreadPoolExecutor(max_workers=4,thread_name_prefix='institutional') as pool:
        ...
        for future in as_completed(futures):
            ...
            try:sources[key]=future.result()
                ...
                sources[key]={'error':str(e),'status':'UNAVAILABLE','source':None,'last_attempt_at':attempt_at}; errors.append(f'{key}: {e}')
    for _src in sources.values(): _src.setdefault('last_attempt_at',attempt_at)
    m=sources.get('macro',{}); c=sources.get('cot',{}); g=sources.get('gld',{}); cb=sources.get('central_banks',{})
    # Invariant: the exact source objects shown in the report are the objects
    # used for scoring. No parallel/reconstructed data path is permitted.
    if c.get('managed_money_net') is not None and c.get('status')=='UNKNOWN':
        errors.append('CFTC integrity: values present but freshness status UNKNOWN')
    ...
    macro_fresh=(m.get('series_status',{}).get('DFEDTARL',{}).get('status')=='FRESH' and m.get('series_status',{}).get('DFEDTARU',{}).get('status')=='FRESH')
    if macro_fresh and m.get('policy')=='DOVISH':add(15,'BUY','السياسة النقدية تميل للتيسير')
    elif macro_fresh and m.get('policy')=='HAWKISH':add(15,'SELL','السياسة النقدية تميل للتشدد')
    elif macro_fresh and m.get('policy')=='HOLD':add(15,None,'السياسة النقدية: تثبيت، دون اتجاه جديد')
    if m.get('fed_futures_status',{}).get('status')=='FRESH' and m.get('rate_expectation')=='خفض محتمل':add(15,'BUY','توقعات الفائدة تميل للخفض')
    elif m.get('fed_futures_status',{}).get('status')=='FRESH' and m.get('rate_expectation')=='رفع محتمل':add(15,'SELL','توقعات الفائدة تميل للرفع')
    elif m.get('fed_futures_status',{}).get('status')=='FRESH' and m.get('rate_expectation')=='استقرار محتمل':add(15,None,'توقعات الفائدة تميل للاستقرار')
    real=m.get('real10y')
    if m.get('series_status',{}).get('DFII10',{}).get('status')=='FRESH' and real is not None:
    ...
    if m.get('series_status',{}).get('VIXCLS',{}).get('status')=='FRESH' and (vix is not None or risk in ('RISK-ON','RISK-OFF')):
    ...
    if c.get('status')=='FRESH' and cot_value is not None:
        ...
        if available < 20.0:
            errors.append('CFTC integrity: fresh COT contribution was not added to coverage')
    ...
    if cb.get('status')=='FRESH' and net is not None:
...
def build_analysis(include_trade=True):
    ...
    L += ['','🧭 الصورة الكبرى',f"W1: {mtf['w1'].get('direction','غير متاح')}",f"D1: {mtf['d1'].get('direction','غير متاح')}",f"H4: {mtf['h4'].get('direction','غير متاح')}",f"H1: {mtf['h1'].get('direction','غير متاح')}",f"M15: {mtf['m15'].get('direction','غير متاح')}",f"M5: {mtf['m5'].get('direction','غير متاح')}",'','🌐 حالة السوق الكلي',f'النظام: {regime}',f"DXY: {v(m.get('dxy'))}",f"تغير DXY: {v(m.get('dxy_change_pct'),'%')}",f"Broad USD: {v(m.get('usd_broad'))}",f"US10Y: {v(m.get('us10y'),'%')}",f"Real 10Y: {v(m.get('real10y'),'%')}",f"VIX: {v(m.get('vix'))}",'','💰 السعر والمستويات',f"السعر الحالي: {result['price']:.2f}",f"🟢 S1: {format_zone(levels,'support1')}",f"🟢 S2: {format_zone(levels,'support2')}",f"🟢 S3: {format_zone(levels,'support3')}",f"🔴 R1: {format_zone(levels,'resistance1')}",f"🔴 R2: {format_zone(levels,'resistance2')}",f"🔴 R3: {format_zone(levels,'resistance3')}",'','💧 السيولة',f"الانحياز: {mtf['m15']['liquidity']['bias']}",f"Buy-side الأقرب: {fmt(mtf['m15']['liquidity']['nearest_buy'])}",f"Sell-side الأقرب: {fmt(mtf['m15']['liquidity']['nearest_sell'])}",f"السحب: {mtf['m15']['liquidity']['sweep_text']}",f"إعادة الاختبار: {liquidity_retest_summary(mtf['m15']['liquidity'])}",'','🏦 التحليل المؤسسي — بيانات حقيقية',f"🎯 الاتجاه المؤسسي: {inst.get('direction','غير متاح')}",f"💪 قوة الدليل المؤسسي: {v(inst.get('strength'))}/100",f"📡 تغطية المصادر: {v(inst.get('coverage_pct'))}%",f"🚦 حالة البيانات المؤسسية: {v(inst.get('status'))}",'','🏛️ البنوك المركزية',f"السياسة النقدية: {v(m.get('policy'))}",f"الفائدة المستهدفة: {v(m.get('fed_lower'),'%')} — {v(m.get('fed_upper'),'%')}",f"الفائدة الفعلية EFFR: {v(m.get('effr'),'%')}",f"توقع السوق للفائدة: {v(m.get('rate_expectation'))}",f"مصدر التوقع: {v(m.get('rate_expectation_source'))}",f"Fed Funds Futures: {v(m.get('fed_futures_implied'),'%')}",f"مشتريات الذهب الرسمية: {v(cb.get('net_change_tonnes'),' طن')}",f"اتجاه المشتريات: {v(cb.get('direction'))}",'','🏦 ETF / صناديق الذهب',f"GLD آخر تحديث: {v(g.get('date'))}",f"حيازة الذهب: {v(g.get('gold_oz'))} أونصة",f"تغير الحيازة: {v(g.get('gold_oz_delta'))} أونصة",f"تغير الأسهم: {v(g.get('shares_delta'))}",f"اتجاه الحيازة: {v(g.get('flow_proxy'))}",'⚠️ تغير الحيازة ليس تدفقاً نقدياً؛ التدفق النقدي المباشر غير متاح من المصدر الحالي.','','📑 COT — تمركزات الذهب',f"تاريخ التقرير: {v(c.get('report_date'))}",f"Commercial صافي: {v(c.get('legacy_commercial_net'))}",f"Commercial تغير أسبوعي: {v(c.get('legacy_commercial_delta'))}",f"Non-Commercial صافي: {v(c.get('legacy_noncommercial_net'))}",f"Non-Commercial تغير أسبوعي: {v(c.get('legacy_noncommercial_delta'))}",f"Managed Money صافي: {v(c.get('managed_money_net'))}",f"Managed Money تغير أسبوعي: {v(c.get('managed_money_delta'))}",'','📈 السوق الكلي',f"DXY: {v(m.get('dxy'))}",f"تغير DXY: {v(m.get('dxy_change_pct'),'%')}",f"Broad USD: {v(m.get('usd_broad'))}",f"US10Y: {v(m.get('us10y'),'%')}",f"Real 10Y: {v(m.get('real10y'),'%')}",f"VIX: {v(m.get('vix'))}",f"Risk-On / Risk-Off: {regime}",'','🚦 بوابات التنفيذ',f"📊 الاتجاه: {'🟢 داعم' if result['direction'] in ('BUY','SELL') else '🟡 غير محسوم'}",f"🧠 الهيكل: {'🟢 داعم' if mtf['h1'].get('structure') in ('صاعد','هابط') else '🟡 محايد'}",f"💧 السيولة: {'🟢 مؤكدة' if 'لا يوجد' not in str(mtf['m15']['liquidity'].get('sweep_text','')) else '🟡 غير مكتملة'}",f"🔄 إعادة الاختبار: {liquidity_retest_summary(mtf['m15']['liquidity'])}",f"📰 الأخبار: {result['news']}",f"🛡️ التنفيذ: {'🟢 READY' if result.get('execution_state')=='READY' else '🟡 WAIT'}",'','⏳ ما الذي ننتظره؟','• اكتمال سحب السيولة','• إعادة اختبار مؤكدة','• تأكيد M15/M5','• عدم تعارض واضح مع العامل المؤسسي','','📡 صحة مصادر التحليل المؤسسي']
    for key,label in (('macro','FRED / السوق الكلي'),('cot','CFTC COT'),('gld','SPDR GLD'),('central_banks','World Gold Council / IMF')):
        src_item=(src.get(key) or {}); st=src_item.get('status'); icon='🟢 متاح' if src_item.get('source') and st=='FRESH' else '🟠 قديم' if st=='STALE' else '🟡 غير محسوم' if st=='UNKNOWN' else '🔴 غير متاح'; detail=[]
        if src_item.get('asof'): detail.append(f"آخر تحديث {src_item.get('asof')}")
        if src_item.get('age_days') is not None: detail.append(f"عمر {src_item.get('age_days')} يوم")
        if src_item.get('error'): detail.append(str(src_item.get('error')))
        L.append(f"{label}: {icon}" + (f" — {' | '.join(detail)}" if detail else ''))
        if key=='macro':
            for sid,ss in (src_item.get('series_status') or {}).items():
                if ss.get('status')!='FRESH': L.append(f"  • {sid}: {ss.get('status','UNKNOWN')}")
    if inst.get('errors'): L.append('⚠️ ' + ' | '.join(str(x) for x in inst.get('errors',[])[:4]))
...
async def institutional_menu(update, context):
    ...
    try:
        ...
        body=("🧠 التحليل المؤسسي\n\n"
              ...
              "📡 المصادر: FRED / CFTC / SPDR / World Gold Council / Yahoo\n"
