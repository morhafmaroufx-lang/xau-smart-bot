...
VERSION = "v18.102_TRADE_MONITOR_PERSISTENCE_RECONCILIATION_FIXED"
...
class WeeklyDecisionValidator:
    ...
    def validate(cls, decision, strict=False):
        ...
        for key, expected_direction in (("bullish_scenario", "BUY"), ("bearish_scenario", "SELL")):
            ...
            if normalized and normalized not in {"CONFIRMED", "WATCH", "FAILED", "UNAVAILABLE", "RETEST_PENDING"}:
...
def _weekly_confirmation_contract(conf):
    conf = dict(conf or {})
    raw = conf.get("status", "غير متاح")
    conf["normalized_status"] = WeeklyDecisionValidator.CONFIRMATION_MAP.get(raw, "UNAVAILABLE")
...
def build_weekly_decision(frames_data, levels, price, d1_df=None, generated_at=None):
    ...
    if d1_df is None:
        try:
            ...
            d1_df = None
    bull_conf = _weekly_confirmation_contract(_breakout_confirmation(d1_df, r1, "BUY", "weekly", atr))
    bear_conf = _weekly_confirmation_contract(_breakout_confirmation(d1_df, s1, "SELL", "weekly", atr))
...
def _weekly_decision_compatibility_scenarios(decision):
    ...
    def _trigger_text(sc):
        ...
        status = conf.get("status", "غير متاح")
    ...
    if state == "BULLISH" and direction == "BUY":
        ...
        primary = {
            ...
            "quality": 0,
            "mechanism": d.get("state_change_reason") or "الفريمات الاستراتيجية لا تمنح أفضلية موحدة؛ القرار يعتمد على تأكيد كسر أحد طرفي النطاق.",
            "trigger": _trigger_text(bullish) + "\nأو\n" + _trigger_text(bearish),
            "targets": [],
            "stop": None,
            "factors": [],
        }
        alternative = {
            ...
            "quality": 0,
            "mechanism": "قد يبقى الذهب داخل نطاق القرار حتى يظهر كسر مؤكد بإغلاق D1.",
            "trigger": "توسع واضح في الزخم والحجم ثم كسر نطاق القرار مع تأكيد D1.",
            "targets": [x for x in ((d.get("decision_range") or {}).get("low"), (d.get("decision_range") or {}).get("high")) if x is not None],
            "stop": None,
            "factors": [],
        }
