...
def _ux_main_keyboard():
    ...
    return [
        ...
        ["📈 مراقب أداء الصفقات", "🖥️ مراقب أداء النظام"],
    ]
...
async def trade_performance_monitor(update, context):
    """📈 مراقب أداء الصفقات — قراءة فقط لنتائج صفقات MAIN و S/R والتقارير."""
    ...
    try:
        ...
        lines = [
            "📈 XAU SMART TRADER — مراقب أداء الصفقات", "",
            ...
            "📊 جودة الأداء",
            f"• Win Rate: {metric(t.get('win_rate_pct'),'.2f','%')}",
            f"• Full Target Rate: {metric(t.get('full_target_rate_pct'),'.2f','%')}",
            f"• متوسط R:R: {metric(t.get('avg_rr'),'.2f')}",
            f"• متوسط MAE: {metric(t.get('avg_mae'),'.4f')}",
            f"• متوسط MFE: {metric(t.get('avg_mfe'),'.4f')}",
            f"• متوسط Drawdown: {metric(t.get('avg_drawdown'),'.4f')}", "",
            "🔎 المتابعة الحالية",
            f"• صفقات تحت المراقبة: {m.get('trades_total',0)}",
            f"• صفقات نشطة: {m.get('trades_active',0)}",
            f"• تحليلات تحت المراقبة: {m.get('analyses_total',0)}",
            f"• تحليلات نشطة: {m.get('analyses_active',0)}", "",
            "📅 دقة التحليل اليومي",
            f"• التحليلات: {d.get('total',0)} | المطابقة: {d.get('matched',0)}",
            f"• الدقة: {metric(d.get('accuracy_pct'),'.2f','%')}",
            f"• الاتجاه: {metric(d.get('direction_accuracy_avg'),'.2f','%')}",
            f"• النطاق: {metric(d.get('range_accuracy_avg'),'.2f','%')}",
            f"• الهدف: {metric(d.get('target_accuracy_avg'),'.2f','%')}", "",
            "📆 دقة التحليل الأسبوعي",
            f"• التحليلات: {w.get('total',0)} | المطابقة: {w.get('matched',0)}",
            f"• الدقة: {metric(w.get('accuracy_pct'),'.2f','%')}",
            f"• الاتجاه: {metric(w.get('direction_accuracy_avg'),'.2f','%')}",
            f"• النطاق: {metric(w.get('range_accuracy_avg'),'.2f','%')}",
            f"• الهدف: {metric(w.get('target_accuracy_avg'),'.2f','%')}", "",
            "🔒 مراقب قراءة فقط — لا يغيّر الإشارات أو الصفقات أو الاستراتيجية."
        ]
        buttons = [[InlineKeyboardButton("🔄 تحديث أداء الصفقات", callback_data="menu_trade_performance")],
        ...
        await reply(update, f"📈 تعذر إنشاء مراقب أداء الصفقات.\nالسبب: {e}")
...
async def help_menu(update, context):
    body=("⚡ التحليل السريع — قراءة لحظية مباشرة.\n"
          ...
          "📈 مراقب أداء الصفقات — متابعة نتائج الصفقات والاستراتيجية.\n"
...
async def router(update, context):
    ...
    routes = {
        ...
        "📈 مراقب أداء الصفقات": trade_performance_monitor,
