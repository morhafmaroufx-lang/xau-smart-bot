...
VERSION = "v18.39-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIXED"
...
# News refresh is optional. Analysis/execution always remain local-only.
# Default LOCAL_ONLY prevents a provider dependency during beta unless explicitly enabled.
NEWS_BACKGROUND_REFRESH_ENABLED = os.getenv("NEWS_BACKGROUND_REFRESH_ENABLED", "0").strip().lower() in {"1", "true", "yes", "on"}
...
def news_calendar_status(now=None):
    """Local-only freshness/status metadata for UX and diagnostics."""
    now = now or _now_utc()
    with _LOCK:
        events, source, error = _load_local_events()
        if not events:
            return {"state": "UNKNOWN", "source": source, "age_minutes": None, "error": error or "no local calendar available"}
        future = [_parse_dt(e.get("time")) for e in events]
        future = [x for x in future if x and x >= now]
        horizon_minutes = None
        if future:
            horizon_minutes = int((max(future) - now).total_seconds() / 60.0)
        if source == "bootstrap-official-schedule":
            state = "BOOTSTRAP"
        elif source == "manual":
            state = "FRESH"
        else:
            state = "FRESH"
        return {"state": state, "source": source, "age_minutes": None, "coverage_minutes": horizon_minutes, "error": None}
...
def get_risk(now=None):
    ...
    try:
        ...
        calendar_meta = news_calendar_status(now)
        if upcoming:
...
def start_background_refresh():
    ...
    if not NEWS_BACKGROUND_REFRESH_ENABLED:
        LOGGER.info("NEWS_BACKGROUND_REFRESH_DISABLED: LOCAL_ONLY mode")
        return
...
def _daily_execution_core():
    ...
    # Terminal gates take precedence over score-based candidate state.
    if liquidity_blocked:
        ...
        state = "READY"
    elif direction in ("BUY","SELL") and setup_score >= MIN_TRADE_SCORE:
        state = "CANDIDATE"
    else:
        state = "WATCH"
    ...
    return {"signal":ready,"execution_state":state,"direction":direction,"score":setup_score,"scenario_score":int(scenario_score),"setup_score":setup_score,"quality":quality,"quality_icon":quality_icon,"price":price,"levels":levels,"liquidity_blocked":liquidity_blocked,"liquidity_state":liquidity_state,"liquidity":liquidity,"factors":factors,"setup_reasons":setup_reasons,"trade":candidate if ready else None,"news_blocked":news_blocked,"news":news_text,"news_state":("BLOCKED" if news_blocked else "CLEAR"),"daily":{"h1":h1,"m15":m15,"m5":m5,"buy_score":buy_score,"sell_score":sell_score,"buy_factors":buy_factors,"sell_factors":sell_factors}}
...
async def quick_analysis(update, context):
    ...
    try:
        ...
        execution_quality = {"READY": "جاهزة للتنفيذ", "CANDIDATE": "مرشحة — تحتاج استكمال", "BLOCKED": "محجوبة", "INVALIDATED": "ملغاة", "WATCH": "قيد المراقبة"}.get(execution_state, "غير جاهزة للتنفيذ")
        ...
        if execution_state == "BLOCKED":
            conclusion = "السيناريو قد يكون صالحاً تحليلياً، لكن بوابة التنفيذ محجوبة حالياً؛ لا تنفيذ حتى زوال سبب الحجب."
        elif execution_state == "INVALIDATED":
            conclusion = "تم إلغاء صلاحية الإعداد بسبب حالة السيولة؛ انتظر تكوين إعداد جديد."
        elif conflict:
            conclusion = "السياق الأعلى H4 معاكس لاتجاه التنفيذ القصير؛ هذا تحذير سياقي وليس بوابة تنفيذ مستقلة."
        elif execution_state == "CANDIDATE":
        ...
        text = (
            ...
            f"🧠 سياق H4: {structure_dir}" + (" — معاكس لاتجاه التنفيذ القصير (تحذير سياقي)." if conflict else " — متوافق نسبياً مع اتجاه التنفيذ القصير.") + "\n"
            ...
            f"🚦 حالة التنفيذ: {execution_state} — {execution_quality}\n"
