...
VERSION = "v18.46-INSTITUTIONAL-SOURCES-AUDITED-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIX-MENU-MARKETS-RUNTIME-CORRECTED"
...
def _inst_cache_get(key):
    with INSTITUTIONAL_LOCK:
        ...
        if x and time.time()-x[0] < INSTITUTIONAL_CACHE_SECONDS: return x[1]
    return None

def _inst_cache_put(key,value):
    with INSTITUTIONAL_LOCK: INSTITUTIONAL_CACHE[key]=(time.time(),value)
    return value

def _inst_http(url,params=None,timeout=None):
    r=requests.get(url,params=params,timeout=timeout or INSTITUTIONAL_HTTP_TIMEOUT,headers={"User-Agent":"XAU-Smart-Trader/18.46 institutional"})
    r.raise_for_status(); return r

def _fred(ids):
    key='fred:'+','.join(ids); c=_inst_cache_get(key)
    if c is not None: return c
    r=_inst_http(FRED_CSV_URL,{'id':','.join(ids)})
    d=pd.read_csv(io.StringIO(r.text)); d['DATE']=pd.to_datetime(d['DATE'],errors='coerce')
    for col in d.columns:
        if col!='DATE': d[col]=pd.to_numeric(d[col],errors='coerce')
    d=d.dropna(subset=['DATE']).sort_values('DATE')
    return _inst_cache_put(key,d)
...
def _yahoo(symbol,range_value='1mo'):
    ...
    for ts,cl in zip(item.get('timestamp') or [],q.get('close') or []):
        if cl is not None: rows.append((datetime.fromtimestamp(int(ts),tz=timezone.utc),float(cl)))
...
def _cftc_zip(url):
    ...
    with zipfile.ZipFile(io.BytesIO(r.content)) as z:
        ...
        if not name: raise RuntimeError('CFTC archive has no table')
        with z.open(name) as f: d=pd.read_csv(f,encoding='latin-1',low_memory=False)
    code=next((c for c in d.columns if c.lower()=='cftc_contract_market_code'),None)
    if code: d=d[d[code].astype(str).str.strip()=='088691']
    if d.empty:
        namecol=next((c for c in d.columns if c.lower()=='market_and_exchange_names'),None)
        if namecol: d=d[d[namecol].astype(str).str.upper().str.contains('GOLD - COMMODITY EXCHANGE INC',na=False)]
    if d.empty: raise RuntimeError('CFTC GOLD row unavailable')
    datecol=next((c for c in d.columns if c.lower() in ('report_date_as_yyyy_mm_dd','report_date_as_mm_dd_yyyy','as_of_date_in_form_yyymmdd')),None)
    if datecol: d['_date']=pd.to_datetime(d[datecol],errors='coerce'); d=d.sort_values('_date')
...
def _cot_real():
    ...
    out={'source':'CFTC Disaggregated + Legacy COT','report_date':str(r.get('_date',''))[:10], 'managed_money_net':ml-ms if ml is not None and ms is not None else None,'managed_money_delta':(ml-ms)-(pml-pms) if all(v is not None for v in (ml,ms,pml,pms)) else None}
    if leg is not None:
        ...
        out.update({'legacy_noncommercial_net':nl-ns if nl is not None and ns is not None else None,'legacy_commercial_net':cl-cs if cl is not None and cs is not None else None,'legacy_noncommercial_delta':(nl-ns)-(pnl-pns) if all(v is not None for v in (nl,ns,pnl,pns)) else None,'legacy_commercial_delta':(cl-cs)-(pcl-pcs) if all(v is not None for v in (cl,cs,pcl,pcs)) else None})
    return _inst_cache_put('cot',out)

def _gld_real():
    c=_inst_cache_get('gld');
    if c is not None:return c
    r=_inst_http(SPDR_GLD_HISTORY_URL,timeout=25); xl=pd.ExcelFile(io.BytesIO(r.content)); best=None
    for sh in xl.sheet_names:
        ...
        oc=next((cols[k] for k in cols if ('gold holdings' in k or 'gold ounces' in k or 'ounces' in k) and 'share' not in k),None)
        sc=next((cols[k] for k in cols if 'shares outstanding' in k or 'total shares' in k),None)
    ...
    d,oc,sc=best; a,b=d.iloc[-1],d.iloc[-2]; ao=pd.to_numeric(a[oc],errors='coerce') if oc is not None else np.nan; bo=pd.to_numeric(b[oc],errors='coerce') if oc is not None else np.nan; ass=pd.to_numeric(a[sc],errors='coerce') if sc is not None else np.nan; bss=pd.to_numeric(b[sc],errors='coerce') if sc is not None else np.nan
    return _inst_cache_put('gld',{'source':'SPDR Gold Shares official archive','date':a['_date'].strftime('%Y-%m-%d'),'gold_oz':float(ao) if pd.notna(ao) else None,'gold_oz_delta':float(ao-bo) if pd.notna(ao) and pd.notna(bo) else None,'shares_delta':float(ass-bss) if pd.notna(ass) and pd.notna(bss) else None,'flow_proxy':'IN' if pd.notna(ao) and pd.notna(bo) and ao>bo else 'OUT' if pd.notna(ao) and pd.notna(bo) and ao<bo else 'NEUTRAL'})

def _cb_real():
    c=_inst_cache_get('cb');
    if c is not None:return c
    r=_inst_http(WGC_CENTRAL_BANK_CHANGES_URL,timeout=25); d=pd.read_excel(io.BytesIO(r.content)); candidates=[]
    for col in d.columns:
        if any(k in str(col).lower() for k in ('change','net','purchase','sale')):
            v=pd.to_numeric(d[col],errors='coerce').dropna()
            if len(v): candidates.append((str(col),float(v.sum())))
    if not candidates: raise RuntimeError('WGC central-bank change column unavailable')
    period,total=max(candidates,key=lambda x:abs(x[1]))
    return _inst_cache_put('cb',{'source':'World Gold Council / IMF IFS','period':period,'net_change_tonnes':total,'direction':'BUY' if total>0 else 'SELL' if total<0 else 'NEUTRAL'})

def _macro_real():
    c=_inst_cache_get('macro');
    if c is not None:return c
    f=_fred(['DFEDTARL','DFEDTARU','EFFR','DGS10','DFII10','VIXCLS']); a=f.iloc[-1]; b=f.iloc[-2] if len(f)>1 else a
    lo=float(a.DFEDTARL) if pd.notna(a.DFEDTARL) else None; hi=float(a.DFEDTARU) if pd.notna(a.DFEDTARU) else None; plo=float(b.DFEDTARL) if pd.notna(b.DFEDTARL) else lo; phi=float(b.DFEDTARU) if pd.notna(b.DFEDTARU) else hi
    pol='HOLD' if lo is None or hi is None or abs(((lo+hi)-(plo+phi))/2)<1e-9 else ('HAWKISH' if ((lo+hi)-(plo+phi))/2>0 else 'DOVISH')
    zq=None
    try:
        q=_yahoo('ZQ=F','10d'); px=float(q.close.iloc[-1]); zq=100-px if 90<px<100 else None
    except Exception: pass
    exp='غير متاحة' if zq is None or lo is None or hi is None else ('رفع محتمل' if zq-(lo+hi)/2>=.10 else 'خفض محتمل' if zq-(lo+hi)/2<=-.10 else 'استقرار محتمل')
    dxy=None
    try:dxy=float(_yahoo('DX-Y.NYB').close.iloc[-1])
    except Exception:pass
    spx=None; ma=None
    try:q=_yahoo('^GSPC','3mo'); spx=float(q.close.iloc[-1]); ma=float(q.close.tail(20).mean())
    except Exception:pass
    vix=float(a.VIXCLS) if pd.notna(a.VIXCLS) else None; regime='غير متاح' if spx is None or ma is None else ('RISK-ON' if spx>=ma and (vix is None or vix<20) else 'RISK-OFF' if spx<ma and vix is not None and vix>=20 else 'MIXED')
    return _inst_cache_put('macro',{'source':'FRED + Yahoo market data','policy':pol,'fed_lower':lo,'fed_upper':hi,'effr':float(a.EFFR) if pd.notna(a.EFFR) else None,'rate_expectation':exp,'fed_futures_implied':zq,'dxy':dxy,'us10y':float(a.DGS10) if pd.notna(a.DGS10) else None,'real10y':float(a.DFII10) if pd.notna(a.DFII10) else None,'vix':vix,'risk_regime':regime})
...
def institutional_analysis(df):
    ...
    with ThreadPoolExecutor(max_workers=4, thread_name_prefix="institutional") as pool:
        ...
        for future in as_completed(futures):
            ...
            except Exception as e: sources[key]={'error':str(e)}; errors.append(f'{key}: {e}')
    m=sources.get('macro',{}); c=sources.get('cot',{}); g=sources.get('gld',{}); cb=sources.get('central_banks',{}); bull=bear=0; factors=[]
    if m.get('policy')=='DOVISH':bull+=12;factors.append('السياسة النقدية تميل للتيسير')
    elif m.get('policy')=='HAWKISH':bear+=12;factors.append('السياسة النقدية تميل للتشدد')
    if m.get('rate_expectation')=='خفض محتمل':bull+=8;factors.append('توقعات الفائدة تميل للخفض')
    elif m.get('rate_expectation')=='رفع محتمل':bear+=8;factors.append('توقعات الفائدة تميل للرفع')
    if m.get('real10y') is not None:
        if m['real10y']<1.5:bull+=8;factors.append(f"العائد الحقيقي منخفض ({m['real10y']:.2f}%)")
        elif m['real10y']>2.5:bear+=8;factors.append(f"العائد الحقيقي مرتفع ({m['real10y']:.2f}%)")
    if m.get('dxy') is not None:
        try:q=_yahoo('DX-Y.NYB'); ch=(q.close.iloc[-1]/q.close.iloc[0]-1)*100
        except Exception:ch=0
        if ch<=-1:bull+=8;factors.append(f'الدولار ضعيف خلال الشهر ({ch:+.1f}%)')
        elif ch>=1:bear+=8;factors.append(f'الدولار قوي خلال الشهر ({ch:+.1f}%)')
    if m.get('vix') is not None and m['vix']>=25:bull+=5;factors.append('VIX مرتفع — طلب ملاذ محتمل')
    if m.get('risk_regime')=='RISK-OFF':bull+=5;factors.append('Risk-Off')
    elif m.get('risk_regime')=='RISK-ON':bear+=3;factors.append('Risk-On')
    n=c.get('legacy_noncommercial_net'); cm=c.get('legacy_commercial_net')
    if n is not None: bull+=10 if n>0 else 0; bear+=10 if n<0 else 0; factors.append('Non-Commercial '+('صافي شراء' if n>0 else 'صافي بيع'))
    if cm is not None: bull+=4 if cm<0 else 0; bear+=4 if cm>0 else 0
    if g.get('flow_proxy')=='IN':bull+=10;factors.append('GLD: زيادة في الحيازة')
    elif g.get('flow_proxy')=='OUT':bear+=10;factors.append('GLD: انخفاض في الحيازة')
    net=cb.get('net_change_tonnes')
    if net is not None: bull+=12 if net>0 else 0; bear+=12 if net<0 else 0; factors.append(f'البنوك المركزية: {net:+.0f} طن')
    direction='BUY' if bull>bear else 'SELL' if bear>bull else 'WAIT'; quality=int(min(100,round(abs(bull-bear)*2)))
    return {'direction':direction,'quality':quality,'bull':bull,'bear':bear,'institutional_score':quality,'factors':factors,'sources':sources,'errors':errors,'context':'تحليل مؤسسي متعدد المصادر؛ البيانات غير المتاحة لا تُستبدل بقيم مصطنعة.'}

def institutional_adjustment(direction,institutional):
    if direction not in ('BUY','SELL'):return 0,[]
    if institutional.get('direction')==direction:return min(10,int(round(float(institutional.get('quality',0))/10))),['البيانات المؤسسية الحقيقية متوافقة مع الاتجاه']
    if institutional.get('direction') in ('BUY','SELL'):return -min(10,int(round(float(institutional.get('quality',0))/20))),['البيانات المؤسسية الحقيقية تعارض الاتجاه']
    return 0,['البيانات المؤسسية لا تمنح أفضلية واضحة']
...
def evaluate_signal():
    ...
    adj, adj_factors = institutional_adjustment(core.get("direction"), institutional) if institutional else (0, [])
...
def build_analysis(include_trade=True):
    ...
    L=[f'🤖 XAU SMART TRADER {VERSION}','📊 التحليل الهيكلي والكمّي والمؤسسي','','🎯 القرار',direction,f"🧠 قوة السيناريو: {result.get('scenario_score',result['score'])}/100",f"🎯 الإعداد التنفيذي: {result.get('setup_score',result['score'])}/100",f"🚦 حالة التنفيذ: {result.get('execution_state','WATCH')}",'','🔎 سبب القرار']
...
async def institutional_menu(update, context):
    ...
    try:
        ...
        body=("🧠 التحليل المؤسسي\n\n"
              ...
              f"💪 القوة: {inst.get('quality','غير متاح')}/100\n\n"
              "🏛️ البنوك المركزية\n"
