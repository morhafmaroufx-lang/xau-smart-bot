...
def liquidity_retest_summary(liq):
    ...
    level = r.get("level")
    if state == "CONFIRMED":
        return f"🟢 إعادة الاختبار ناجحة عند {fmt(level)}"
    if state == "RETEST":
        return f"🟡 إعادة الاختبار قيد التقييم عند {fmt(level)}"
    if state == "BOS":
        return f"🟠 كسر الهيكل مؤكد — بانتظار إعادة الاختبار عند {fmt(level)}"
    if state == "DISPLACEMENT":
        return "🟡 الاندفاع مؤكد — بانتظار تأكيد كسر الهيكل"
    if state == "SWEEP":
        return f"🔵 بانتظار الاندفاع بعد سحب السيولة عند {fmt(level)}"
    if state == "INVALIDATED" or r.get("result") == "FAILED":
        return f"🔴 إعادة الاختبار فاشلة — تم إبطال المستوى {fmt(level)}"
    if state == "CONSUMED":
        return "⚪ تم استهلاك إعادة الاختبار المؤكدة — بانتظار Sweep جديد"
    if state == "EXPIRED" or r.get("result") == "EXPIRED":
        return "⚪ انتهى عمر حالة السيولة — بانتظار Sweep جديد"
...
def _parse_news_datetime(value, time_value=None):
    """تحويل وقت الخبر من المنطقة الزمنية المصدرية إلى توقيت دمشق."""
    try:
...
def get_news_events(force=False):
    """جلب تقويم اقتصادي عالي التأثير من CSV مع مصدر أساسي واحتياطي.

    المنطقة الزمنية المصدرية قابلة للضبط عبر NEWS_SOURCE_TZ، والافتراضي America/New_York؛ ثم يتم التحويل إلى توقيت دمشق.
    ...
    # المصدر الأساسي: CSV عام لتقويم Forex Factory عبر nfs.faireconomy.media.
    # المصدر الاحتياطي: رابط Forex Factory المباشر عند توفره.
    urls = []
...
def build_daily_analysis():
    ...
    buy, _ = _scenario_score(frames, [("H1",35),("M15",30),("M5",25)], "BUY", levels, price, m15["atr"], "daily")
    sell, _ = _scenario_score(frames, [("H1",35),("M15",30),("M5",25)], "SELL", levels, price, m15["atr"], "daily")
...
async def trade_now(update, context):
    ...
    try:
        ...
        if not can_receive_trade(chat_id):
            ...
            return
        quota_token = _reserve_trade_quota(chat_id)
        if not quota_token:
            await reply(update, "⛔ تم الوصول إلى حد الصفقات في باقتك الحالية.\n\n🚀 استخدم /plans للترقية.")
            return
...
async def send_market_session_alerts():
    ...
    now = now_damascus()
    recipients = _alert_recipients("market_alerts")
...
async def send_news_alerts():
    """تنبيهات الأخبار عالية التأثير قبلها وحين تفعيل الحظر وبعد انتهائه."""
    if not NEWS_FILTER_ENABLED or not APPLICATION:
        return
    recipients = _alert_recipients("news_alerts")
    if not recipients:
        return
    try:
        events = get_news_events()
    ...
    for event in events:
        ...
        if -1 <= delta <= NEWS_BEFORE_MIN:
            ...
            text = (f"📰 تنبيه خبر عالي التأثير\n━━━━━━━━━━━━━━━━━━\n"
                    ...
                    f"🚫 الحماية: حظر التداول {NEWS_BEFORE_MIN} دقيقة قبل الخبر و{NEWS_AFTER_MIN} دقيقة بعده.")
            for cid in recipients:
                try:
                    await APPLICATION.bot.send_message(chat_id=cid, text=with_market_header(text))
                except Exception:
