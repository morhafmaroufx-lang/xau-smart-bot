...
async def send_market_session_alerts():
    """تنبيه نظيف قبل الافتتاح بـ15 دقيقة + نظام مستقل لتداخل الجلسات."""
    now = now_damascus()
    ...
    # 1) تنبيه ما قبل افتتاح كل سوق بـ 15 دقيقة.
    # نافذة دقيقة: 15 دقيقة قبل الافتتاح، وتتحمل دورة auto_loop ذات 15 دقيقة.
    for name, label, zone_name, open_hour, _duration in SESSION_SPECS:
        local = now.astimezone(ZoneInfo(zone_name))
        if local.weekday() >= 5:
            continue

        # نحسب موعد الافتتاح التالي محلياً، ثم الفرق بالدقائق.
        opening = local.replace(hour=open_hour, minute=0, second=0, microsecond=0)
        minutes_to_open = (opening - local).total_seconds() / 60.0
        if not (0 < minutes_to_open <= 15):
            continue
        ...
        remaining = max(1, int(round(minutes_to_open)))
        text = (
            f"🔔 باقي {remaining} دقيقة لافتتاح سوق {label}\n\n"
            f"{state}\n"
            f"{status_note}\n"
            f"🕐 الوقت الحالي: {now.strftime('%H:%M')} دمشق"
        )
        results = await _dispatch_broadcast(
            recipients, with_market_header(text), kind="session_preopen"
        )
        if any(x is True for x in results):
            SESSION_ALERT_STATE[key] = True

    # 2) نظام مستقل وذكي لتنبيهات التداخلات.
    # يرسل مرة واحدة عند بداية كل تداخل، مع الحالة العامة فقط دون ضوضاء بصرية.
    for left, right in _session_overlap_pairs(now):
        left_name, left_label = left
        right_name, right_label = right
        pair = f"{left_name}+{right_name}"

        # نحدد أن التنبيه حدث عند بداية نافذة التداخل اعتماداً على أول دورة تراها.
        date_key = now.strftime("%Y-%m-%d")
        key = f"OVERLAP:{pair}:{date_key}"
        if SESSION_OVERLAP_ALERT_STATE.get(key):
            continue

        # لا نرسل تنبيه التداخل إلا خلال أول 15 دقيقة من بداية التداخل.
        # نستخدم transition-state لمنع تنبيه متأخر إذا بدأ البوت أثناء التداخل.
        overlap_start_detected = False
        for _, _, zone_name, open_hour, duration_hours in SESSION_SPECS:
            # بداية التداخل تُحسب بدقة من انتقال عدد الجلسات النشطة بين الدورات.
            pass

        # حساب بداية التداخل عملياً عبر مقارنة الحالة قبل 15 دقيقة.
        previous = now - timedelta(minutes=15)
        current_pairs = {f"{a[0]}+{b[0]}" for a, b in _session_overlap_pairs(now)}
        previous_pairs = {f"{a[0]}+{b[0]}" for a, b in _session_overlap_pairs(previous)}
        overlap_start_detected = pair in current_pairs and pair not in previous_pairs

        if not overlap_start_detected:
            continue

        text = (
            f"🔶 بدأ تداخل جلسات الأسواق\n\n"
            f"{left_label} + {right_label}\n"
            f"{state}\n"
            f"📊 السيولة والتقلب قد يرتفعان خلال فترة التداخل.\n"
            f"🕐 دمشق: {now.strftime('%H:%M')}"
        )
        results = await _dispatch_broadcast(
            recipients, with_market_header(text), kind="session_overlap"
        )
        if any(x is True for x in results):
...
async def send_news_alerts():
    ...
    for event in events:
        ...
        if -1 <= delta <= NEWS_BEFORE_MIN:
            ...
            results = await _dispatch_broadcast(recipients, with_market_header(text), kind="news")
            if any(x is True for x in results):
...
async def _sr_notify_all(text):
    ...
    await _dispatch_broadcast(recipients, with_market_header(text), kind="sr")
