        ...
        with self._lock, self.connect() as conn:
            ...
            conn.execute("""INSERT INTO payment_config
                ...
                (PAYMENT_NETWORK, PAYMENT_CHAIN_ID, PAYMENT_CURRENCY,
                 PAYMENT_USDT_CONTRACT, 0, PAYMENT_RECEIVER_ADDRESS,
...
def _payment_atomic(usdt_amount):
    q = Decimal(10) ** PAYMENT_TOKEN_DECIMALS
...
def _payment_order_text(order):
    ...
    lines = [
        ...
        "",
        f"المبلغ المطلوب:",
        f"{_payment_money(Decimal(order['final_price_atomic']) / (Decimal(10) ** PAYMENT_TOKEN_DECIMALS))} USDT",
        "",
        f"الشبكة:",
        "BNB Smart Chain",
        "BEP-20",
...
async def _payment_order_status_ui(update, context, order_id):
    ...
    amount = Decimal(int(order["final_price_atomic"])) / (Decimal(10) ** PAYMENT_TOKEN_DECIMALS)
    text = (
        "💳 حالة طلب الدفع\n\n"
...
def run_v19_45_payment_integration_self_tests():
    ...
    try:
        ...
        assert _payment_atomic(Decimal("30.00")) > 0
...
VERSION = "v19.48_PAYMENT_BSC_USDT_CONTRACT_CONFIGURED"
