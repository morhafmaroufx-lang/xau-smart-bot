...
VERSION = "v18.51-INSTITUTIONAL-DATA-GATEWAY-HARDENED-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
TREASURY_YIELD_STATIC_XML = "https://home.treasury.gov/sites/default/files/interest-rates/yield.xml"
TREASURY_REAL_STATIC_XML = "https://home.treasury.gov/sites/default/files/interest-rates/real_yield.xml"
...
def _treasury_curve(real=False):
    """Official U.S. Treasury 10Y reader with two official endpoints and schema-tolerant XML parsing.

    The Treasury documents both the API-style XML endpoint and the static XML files.
    We try the static feed first because it is deterministic, then the documented API feed.
    ...
    urls=[
        (TREASURY_REAL_STATIC_XML if real else TREASURY_YIELD_STATIC_XML, None),
        ((TREASURY_REAL_XML if real else TREASURY_YIELD_XML), {"data":typ,"field_tdr_date_value":str(year)}),
    ]
    last_exc=None
    for url,params in urls:
        try:
            r=_inst_http(url,params,timeout=20) if params else _inst_http(url,timeout=20)
            root=ET.fromstring(r.content)
            rows=[]
            for item in root.iter():
                children={}
                for child in list(item):
                    key=re.sub(r'[^a-z0-9]','',child.tag.split('}')[-1].lower())
                    children[key]=(child.text or '').strip()
                if not children: continue
                date_raw=(children.get('date') or children.get('recorddate') or
                           children.get('newrecorddate') or children.get('recorddatevalue'))
                # Treasury schema names have appeared as TC10Yr / TIPSReal10 / 
                # 10Yr / tenyear across documented feeds. Normalize aggressively.
                candidates=[]
                for k,v in children.items():
                    if not v or v.upper() in ('N/A','NA','NULL',''):
                        continue
                    if any(token in k for token in ('tc10yr','10yr','tenyear','10year','tipsreal10','real10')):
                        candidates.append(v)
                if not date_raw or not candidates: continue
                dt=pd.to_datetime(date_raw,errors='coerce')
                if pd.isna(dt): continue
                val=None
                for raw in candidates:
                    try:
                        x=float(str(raw).replace(',','').strip())
                        if np.isfinite(x): val=x; break
                    except Exception: pass
                if val is not None: rows.append((dt,val))
            if rows:
                d=pd.DataFrame(rows,columns=['DATE','VALUE']).dropna(subset=['DATE']).sort_values('DATE')
                d=d.drop_duplicates(subset=['DATE'],keep='last')
                if not d.empty:
                    dt=d.iloc[-1].DATE
                    if abs((pd.Timestamp(datetime.now(timezone.utc)).tz_localize(None)-pd.Timestamp(dt).tz_localize(None)).days) <= 10:
                        return float(d.iloc[-1].VALUE), dt
                    raise RuntimeError(('Treasury real' if real else 'Treasury nominal')+' 10Y feed returned stale data')
            raise RuntimeError(('Treasury real' if real else 'Treasury nominal')+' 10Y XML parse failed')
        except Exception as exc:
            last_exc=exc
    raise RuntimeError(f"Treasury {'real' if real else 'nominal'} 10Y unavailable: {last_exc}")
...
def _cftc_zip(url):
    ...
    # Hard integrity gate: COT dates must be plausible report dates.
    now=pd.Timestamp(datetime.now(timezone.utc).date())
    d=d[(d['_date']>=pd.Timestamp('2010-01-01')) & (d['_date']<=now+pd.Timedelta(days=3))]
    if d.empty: raise RuntimeError('CFTC report dates failed integrity validation')
...
def _cot_real():
    ...
    dis=None; dis_err=None
    # Current-year archive can lag publication. Try current year then prior year,
    # always keeping the newest validated report returned by the source.
    for yy in (y,y-1):
        try:
            candidate=_cftc_zip(CFTC_DISAGG_ZIP_TEMPLATE.format(year=yy))
            if dis is None or candidate['_date'].max() > dis['_date'].max(): dis=candidate
        except Exception as exc: dis_err=exc
    if dis is None: raise RuntimeError(f'CFTC Disaggregated unavailable: {dis_err}')
    leg=None
    for yy in (y,y-1):
        try:
            candidate=_cftc_zip(CFTC_LEGACY_ZIP_TEMPLATE.format(year=yy))
            if leg is None or candidate['_date'].max() > leg['_date'].max(): leg=candidate
        except Exception: pass
    ...
    out={'source':'CFTC Disaggregated + Legacy COT','report_date':str(report_date)[:10],
         ...
         'integrity_valid': bool(report_date is not None and pd.Timestamp('2010-01-01') <= pd.Timestamp(report_date) <= pd.Timestamp(datetime.now(timezone.utc).date()) + pd.Timedelta(days=3))}
...
def _macro_real():
    ...
    if c is not None:return c
    errors=[]
    # Federal Reserve policy/rates/VIX/broad USD are official FRED observations.
    try:
        f=_fred(['DFEDTARL','DFEDTARU','EFFR','VIXCLS','DTWEXBGS'])
    except Exception as exc:
        f=pd.DataFrame(columns=['DATE']); errors.append('FRED: '+str(exc))
    lo,lo_date=_latest_series_value(f,'DFEDTARL'); hi,hi_date=_latest_series_value(f,'DFEDTARU')
    effr,effr_date=_latest_series_value(f,'EFFR'); vix,vix_date=_latest_series_value(f,'VIXCLS'); broad_usd,broad_usd_date=_latest_series_value(f,'DTWEXBGS')
    ...
    out={'source':'FRED official + U.S. Treasury official','policy':pol,'fed_lower':lo,'fed_upper':hi,'effr':effr,
    ...
    out['series_status']={
        ...
        'effr':_inst_status(effr_date,3,'FRED EFFR'), 'us10y':_inst_status(us10y_date,3,'U.S. Treasury 10Y'),
        'real10y':_inst_status(real10y_date,3,'U.S. Treasury Real 10Y'), 'vix':_inst_status(vix_date,3,'FRED VIXCLS'),
        'usd_broad':_inst_status(broad_usd_date,3,'FRED DTWEXBGS')}
