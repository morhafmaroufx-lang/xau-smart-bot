...
SR_VERSION = "v18.71-SR-REENGINEERED"
...
def _sr_candidate_setup(df, sr, direction):
    ...
    for zone in levels[:4]:
        ...
        candidates.append({"direction":direction,"entry":price,"zone":zone,"atr":atr,"width":width,"score":round(confirmation,2),"trigger":trigger,"scenario":data,"scenarios":{k:v for k,v in scenarios},"opposite_levels":opposite,"levels":lv,"structure":struct.get(direction,0)})
...
def sr_analyze():
    ...
    if not cands:
        lv=_sr_levels(sr); price=_sr_num(df["close"].iloc[-1]); nearest=sorted(((abs(v-price),k,v) for k,v in lv.items() if v is not None))[:4]
        watch=" | ".join(f"{k}={v:.3f}" for _,k,v in nearest)
        return {"ok":True,"signal":None,"sr":sr,"df":df,"reason":"لا توجد إشارة مؤكدة حالياً. تتم مراقبة أقرب مستويات S/R حتى يكتمل أحد السيناريوهات الأربعة.","watch":watch}
    cands.sort(key=lambda x:x["score"],reverse=True); best=cands[0]; trade=_sr_build_trade(best)
    if not trade:
        return {"ok":True,"signal":None,"sr":sr,"df":df,"candidate":best,"reason":f"تم تأكيد سيناريو {best['trigger']} لكن هندسة Entry/SL/TP أو R:R لم تكن صالحة؛ الصفقة مرفوضة حمايةً لرأس المال."}
    return {"ok":True,"signal":trade,"sr":sr,"df":df,"candidate":best}
