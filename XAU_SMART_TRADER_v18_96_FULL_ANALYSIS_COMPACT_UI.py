...
VERSION = "v18.96-FULL-ANALYSIS-COMPACT-UI"
...
def _full_analysis_markup():
    return _ux_markup([
        [("➕ لماذا؟","full_why"),("➕ السيولة","full_liquidity")],
        [("➕ المستويات","full_levels"),("➕ المؤسسي","full_institutional")],
        [("➕ الاقتصاد","full_macro"),("➕ جودة البيانات","full_quality")],
...
async def callback_router(update, context):
    ...
    if data in routes:
        await routes[data]()
        return
    if data.startswith("full_") and data in {"full_why","full_liquidity","full_levels","full_institutional","full_macro","full_quality","full_all"}:
        await full_analysis_detail(update, context, data.replace("full_", "", 1))
        return
    if data == "full_back":
...
# ============================================================
# v18.94 FINAL RUNTIME ENTRY POINT
