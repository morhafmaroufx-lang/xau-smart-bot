...
VERSION = "v18.59-INSTITUTIONAL-SOURCE-HEALTH-US-CALENDAR-SPDR-IMF-WGC-CROSS-VALIDATED-FEDFUNDS-ARABIC-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
def _imf_irfcl_monthly(indicator):
    """Fetch IMF IRFCL monthly series using verified SDMX 3.0 key semantics."""
    urls=[
        f"{IMF_IRFCL_DATAFLOW}/*.{indicator}.*.M?dimensionAtObservation=TIME_PERIOD&attributes=dsd&measures=all&includeHistory=false",
        f"{IMF_IRFCL_DATAFLOW}/*.{indicator}.*.M?dimensionAtObservation=TIME_PERIOD&attributes=dsd&measures=all",
    ]
    last=None
    for url in urls:
        for attempt in range(3):
            try:
                r=_inst_http(url,timeout=90,headers={'User-Agent':'Mozilla/5.0','Accept':'application/json'})
                payload=r.json(); data=payload.get('data') or {}
                struct=(data.get('structures') or [{}])[0]; ds=(data.get('dataSets') or [{}])[0]
                if not struct or not ds.get('series'): raise RuntimeError('empty dataset')
                ...
                if country_dim is None or country_pos<0 or time_dim is None: raise RuntimeError('dimension schema unavailable')
                ...
                raise RuntimeError(f'only {len(out)} countries')
            except Exception as exc:
                last=exc
...
def run_institutional_self_tests():
    ...
    check('version', VERSION.startswith('v18.59-'), VERSION)
