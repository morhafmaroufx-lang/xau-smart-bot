...
class Metrics:
    ...
    def record_handler(self,latency_ms): self.record_request(latency_ms); self.handler_count+=1
    ...
    def system_snapshot(self,queue_depth=0,queue_capacity=0,queue_drops=0):
        with self._lock:
            ...
            req=list(self.latencies); loops=list(self.loop_lags); pls=list(self.price_latencies); dbs=list(self.db_latencies)
            req_count=self.request_count; errors=self.error_count; alerts=self.alert_count; heals=self.self_heal_count; hb=self.heartbeat; last_price=self.last_price_mono
        cpu_pct=self._cpu_pct(); rss=self._rss_mb(); price_age=max(0.0,now-last_price) if last_price else None
        return {"uptime_s":now-self.started_mono,"cpu_pct":cpu_pct,"rss_mb":rss,"event_loop_lag_ms":_percentile(loops,95),"event_loop_lag_max_ms":max(loops) if loops else 0.0,"request_avg_ms":sum(req)/len(req) if req else 0.0,"request_p95_ms":_percentile(req,95),"request_max_ms":max(req) if req else 0.0,"request_count":req_count,"throughput_per_min":throughput,"price_latency_ms":sum(pls)/len(pls) if pls else self.last_price_latency_ms,"db_write_latency_ms":sum(dbs)/len(dbs) if dbs else 0.0,"price_age_s":price_age,"queue_depth":queue_depth,"queue_capacity":queue_capacity,"queue_drops":queue_drops,"error_count":errors,"alert_count":alerts,"self_heal_count":heals,"auditor_heartbeat":hb}
    @staticmethod
    def _rss_mb():
        try:
...
class PerformanceAuditor:
    ...
    def _evaluate_health(self,s):
        ...
        if s['request_p95_ms']>=AUDITOR_LATENCY_CRIT_MS: checks.append(('CRITICAL','REQUEST_LATENCY'))
        elif s['request_p95_ms']>=AUDITOR_LATENCY_WARN_MS: checks.append(('WARNING','REQUEST_LATENCY'))
...
async def audit_report(update, context):
    ...
    try:
        ...
        lines = [
            ...
            f"• Latency P95: {system.get('request_p95_ms', 0):.1f} ms",
...
def _auditor_alert_callback(level, reason, snapshot):
    ...
    try:
        ...
        text = (f"🛡️ تنبيه مراقب الأداء\n\nالمستوى: {level}\nالمشكلة: {reason}\n"
                ...
                f"Latency P95: {snapshot.get('request_p95_ms', 0):.1f} ms\n"
...
# ============================================================
# v18.74 SRE RELIABILITY HARDENING — APPEND-ONLY
