...
def support_resistance(df, lookback=120, current_price=None):
    x = df.tail(min(lookback, len(df)))
    current = sf(current_price if current_price is not None else x["close"].iloc[-1])
...
async def quick_analysis(update, context):
    ...
    try:
        df15 = get_bars("15m", 200)
        m15 = analyze(df15)
        price = live_price()["price"]
        d = "🟢 صعود قوي" if m15["direction"] == "BUY" else "🔴 هبوط قوي" if m15["direction"] == "SELL" else "🟡 محايد / انتظار"

        # مستويات لحظية: أقرب دعم/مقاومة من إطار M15 مع احترام السعر الحالي
        levels = support_resistance(df15, current_price=price)
        s1 = levels.get("support1", {}).get("price") if levels.get("support1") else None
        r1 = levels.get("resistance1", {}).get("price") if levels.get("resistance1") else None

        # الهدف ووقف الخسارة: نستخدم أقرب مستوى هيكلي متاح بدل أرقام ثابتة
        trade = m15.get("trade") or {}
        entry = trade.get("entry", price)
        sl = trade.get("sl")
        tp1 = trade.get("tp1")
        if m15["direction"] == "BUY":
            target = r1 or tp1
            stop = s1 or sl
            decision = "🟢 شراء سكالبينج"
        elif m15["direction"] == "SELL":
            target = s1 or tp1
            stop = r1 or sl
            decision = "🔴 بيع سكالبينج"
        else:
            target = r1 or s1 or tp1
            stop = s1 or r1 or sl
        ...
        rsi = float(m15["rsi"])
        if rsi >= 70:
            rsi_text = f"{rsi:.1f} — تشبع شرائي، الحذر من المطاردة"
        elif rsi >= 60:
            rsi_text = f"{rsi:.1f} — زخم شرائي قوي"
        elif rsi <= 30:
            rsi_text = f"{rsi:.1f} — تشبع بيعي، الحذر من مطاردة البيع"
        elif rsi <= 40:
            rsi_text = f"{rsi:.1f} — زخم بيعي"
        else:
            rsi_text = f"{rsi:.1f} — زخم متوازن"

        macd = float(m15["macd"])
        macd_text = "إيجابي" if macd > 0 else "سلبي" if macd < 0 else "محايد"
        structure = m15.get("structure", "محايد")
        direction_word = "الصعود" if m15["direction"] == "BUY" else "الهبوط" if m15["direction"] == "SELL" else "الحركة"

        text = (
            f"⚡ التحليل السريع {VERSION}\n\n"
            f"🎯 الاتجاه اللحظي: {d}\n"
            f"📊 EMA 9/21: {'إيجابي' if m15['direction'] == 'BUY' else 'سلبي' if m15['direction'] == 'SELL' else 'غير مؤكد'}\n"
            f"📈 RSI: {rsi_text}\n"
            f"📉 MACD: {macd_text} ({macd:.2f})\n"
            f"💪 قوة الإشارة: {m15['score']} نقطة\n\n"
            f"💰 السعر الحالي: {price:.2f}\n"
            f"📍 دعم 1: {s1:.2f}\n" if s1 is not None else "💰 السعر الحالي: %.2f\n📍 دعم 1: غير متاح\n" % price
        )
        text += (
            f"🔴 مقاومة 1: {r1:.2f}\n" if r1 is not None else "🔴 مقاومة 1: غير متاحة\n"
        )
        text += (
            f"🎯 الهدف اللحظي: {target:.2f}\n" if target is not None else "🎯 الهدف اللحظي: غير متاح\n"
        )
        text += (
            f"🛑 وقف الخسارة: {stop:.2f}\n" if stop is not None else "🛑 وقف الخسارة: غير متاح\n"
        )
        text += (
            f"\n⚡ التوصية الفورية: {decision}\n"
            f"🧠 الهيكل: {structure}\n"
            f"🔎 الخلاصة: {'استمرار ' + direction_word + ' مع تأكيد الزخم.' if m15['direction'] != 'WAIT' else 'لا يوجد تأكيد كافٍ للدخول الآن.'}"
        )
