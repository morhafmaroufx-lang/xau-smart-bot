...
class PerformanceAuditor:
    ...
    def metrics_report(self):
        r=self.metrics.report(); r['system']=self.metrics.system_snapshot(queue_depth=_safe_queue_depth(),queue_capacity=_safe_queue_capacity(),queue_drops=_safe_queue_drops()); r['handler_root_trace']=self.metrics.handler_trace_report(); r['update_lifecycle']=self.metrics.update_lifecycle_report(); return r
...
VERSION = "v18.80-HANDLER-LATENCY-ROOT-TRACE-FIXED"
...
def _execution_readiness(direction, scenario_score, setup_score, alignment, liquidity_blocked, rr_ok, trigger_confirmed, data_quality, conflicts):
    """Independent execution readiness score; scenario strength alone cannot create READY."""
    if direction not in ("BUY","SELL"):
        return 0
    score=0
    score += min(25, max(0, scenario_score) * 0.25)
    score += 20 if alignment else 8
    score += 20 if trigger_confirmed else 0
    score += 15 if rr_ok else 0
    score += 10 if not liquidity_blocked else 0
    score += min(10, max(0,data_quality-50)*0.2)
    score -= min(20, 5*len(conflicts or []))
    return int(max(0,min(100,round(score))))

def _data_quality_score(frames_map, price, levels):
    checks=[]
    for name,x in frames_map.items():
        if isinstance(x,dict) and x: checks.append(name)
    score=min(70, len(checks)*10)
    if price and float(price)>0: score+=15
    if isinstance(levels,dict) and levels: score+=10
    if all((frames_map.get(k) or {}).get('atr') for k in ('H1','M15','M5') if k in frames_map): score+=5
    return int(max(0,min(100,score)))

def _conflict_engine(direction, frames_map, price, levels):
    conflicts=[]
    if direction not in ('BUY','SELL'): return conflicts
    expected='صاعد' if direction=='BUY' else 'هابط'
    for name in ('H4','H1','M15','M5'):
        x=frames_map.get(name) or frames_map.get(name.lower()) or {}
        if x.get('structure') in ('صاعد','هابط') and x.get('structure')!=expected:
            conflicts.append(f'هيكل {name} يعارض السيناريو')
...
def _daily_execution_core():
    ...
    execution_readiness=_execution_readiness(direction, scenario_score, setup_score, alignment, liquidity_blocked, rr_ok, trigger_confirmed, data_quality, conflicts)
    ...
    return {"signal":ready,"execution_state":state,"direction":direction,"score":setup_score,"scenario_score":int(scenario_score),"setup_score":setup_score,"execution_readiness":execution_readiness,"data_quality_score":data_quality,"conflicts":conflicts,"quality":quality,"quality_icon":quality_icon,"price":price,"levels":levels,"liquidity_blocked":liquidity_blocked,"liquidity_state":liquidity_state,"liquidity":liquidity,"factors":factors,"setup_reasons":setup_reasons,"trade":candidate if ready else None,"news_blocked":False,"news":news_text,"daily":{"h1":h1,"m15":m15,"m5":m5,"buy_score":buy_score,"sell_score":sell_score,"buy_factors":buy_factors,"sell_factors":sell_factors}}
...
def evaluate_signal():
    ...
    final_conflicts=_conflict_engine(core.get('direction'), {"H4":h4,"H1":h1,"M15":m15,"M5":m5}, core.get('price'), core.get('levels'))
    result['conflicts']=final_conflicts
    result['execution_readiness']=_execution_readiness(core.get('direction'), int(core.get('scenario_score',0)), int(core.get('setup_score',0)), sum(1 for x in (h1,m15,m5) if x.get('direction')==core.get('direction'))==3, bool(core.get('liquidity_blocked')), bool(core.get('trade')), bool((m15.get('liquidity',{}).get('retest',{}) or {}).get('state')=='CONFIRMED'), int(core.get('data_quality_score',0)), final_conflicts)
...
def _scenario_score(frames_data, frames, direction, levels, price, atr, horizon=None):
    """Scenario score 0..100: trend 20, structure 20, liquidity 20, momentum 15, trigger 15, location 10.
    Categories are capped to prevent double-counting correlated indicators.
    """
    if direction not in ("BUY", "SELL"):
        return 0, ["لا يوجد اتجاه قابل للتقييم"]
    factors=[]
    expected_structure = "صاعد" if direction == "BUY" else "هابط"
    names=[n for n,_ in frames if n in frames_data]
    xs=[frames_data[n] for n in names]
    support=sum(1 for x in xs if x.get("direction")==direction)
    oppose=sum(1 for x in xs if x.get("direction") in ("BUY","SELL") and x.get("direction")!=direction)
    trend=0
    if xs:
        ratio=support/len(xs)
        trend=20 if ratio>=0.99 else 15 if ratio>=0.66 else 8 if ratio>=0.34 else 0
        if trend: factors.append(f"الاتجاه متعدد الأطر يدعم السيناريو ({support}/{len(xs)})")
        if oppose: factors.append(f"تعارض اتجاهي على {oppose} إطار")
    structure_support=sum(1 for x in xs if x.get("structure")==expected_structure)
    structure=20 if structure_support==len(xs) and xs else 15 if structure_support>=2 else 8 if structure_support==1 else 0
    if structure: factors.append(f"الهيكل {expected_structure} داعم ({structure_support}/{len(xs)})")
    # Liquidity: one primary frame to avoid counting the same event repeatedly.
    if horizon is None:
        horizon = "weekly" if any(name == "W1" for name,_ in frames) else "daily"
    liq_name = "H1" if horizon == "daily" else "D1"
    liq = frames_data.get(liq_name, {}).get("liquidity", {}) or {}
    sweep=liq.get("sweep")
    wanted="SELL_SIDE_SWEEP" if direction=="BUY" else "BUY_SIDE_SWEEP"
    retest_state=(liq.get("retest",{}) or {}).get("state")
    liquidity=0
    if sweep==wanted:
        liquidity=12; factors.append("سحب السيولة يدعم الاتجاه")
        if retest_state=="CONFIRMED": liquidity=20; factors.append("إعادة اختبار السيولة مؤكدة")
        elif retest_state in ("RETEST","DISPLACEMENT","BOS"): liquidity=16; factors.append("تسلسل السيولة قيد التأكيد")
    elif retest_state=="INVALIDATED":
        factors.append("حالة السيولة ملغاة")
    # Momentum: use independent ADX/DI direction + MACD relation + RSI context, capped 15.
    m=frames_data.get("M15", xs[-1] if xs else {}) or {}
    ...
    momentum=0
    di_ok=(direction=="BUY" and plus>minus) or (direction=="SELL" and minus>plus)
    macd_ok=(direction=="BUY" and macd>sig) or (direction=="SELL" and macd<sig)
    rsi_ok=(direction=="BUY" and rsi>=50) or (direction=="SELL" and rsi<=50)
    if adx>=25 and di_ok: momentum+=7
    elif adx>=20 and di_ok: momentum+=5
    if macd_ok: momentum+=5
    if rsi_ok: momentum+=3
    momentum=min(15,momentum)
    if momentum: factors.append(f"الزخم يدعم السيناريو ({momentum}/15)")
    # Trigger: confirmed retest/sweep is strongest; otherwise breakout confirmation metadata when available.
    trigger=0
    if retest_state=="CONFIRMED" and sweep==wanted: trigger=15
    elif sweep==wanted: trigger=7
    if trigger: factors.append(f"المحفز التنفيذي ({trigger}/15)")
    # Location: reward space to the next opposing level, penalize entering directly into an obstacle.
    location=0
    if direction=="BUY":
        obstacle=nearest_resistance(levels, price)
        anchor=nearest_support(levels, price)
    else:
        obstacle=nearest_support(levels, price)
        anchor=nearest_resistance(levels, price)
    atr_v=max(float(atr or 0), 1e-9)
    space=(abs(float(obstacle)-price)/atr_v) if obstacle is not None else 2.0
    anchor_dist=(abs(price-float(anchor))/atr_v) if anchor is not None else 99.0
    if obstacle is not None and space>=2.0: location=10
    elif obstacle is not None and space>=1.2: location=7
    elif obstacle is not None and space>=0.7: location=4
    else: factors.append("العائق السعري قريب — جودة الموقع ضعيفة")
    if anchor is not None and anchor_dist<=0.25:
        location=min(location,5); factors.append("السعر ملاصق لمستوى حساس")
    if location: factors.append(f"جودة موقع الدخول ({location}/10)")
...
if __name__ == "__main__":
    pass  # deferred to end so append-only runtime layers are defined first
...
# Runtime must occur after all append-only overrides are defined.
if __name__ == '__main__':
    main_v18_69_sr()
