...
SR_VERSION = "v18.69-SR"
SR_DB_PATH = os.getenv("SR_DB_PATH", "xau_sr_trades.db")
SR_MIN_SCORE = float(os.getenv("SR_MIN_SCORE", "80"))
...
def _sr_false_breakout_features(df, zone_price, direction, width, atr):
    """Detect a completed rejection through a S/R zone on the latest candle."""
    ...
    if None in (o, h, l, c, pc):
        return {"score": 0, "confirmed": False}
    zone = max(width, atr * SR_ZONE_ATR)
    if direction == "BUY":
        pierced = l < zone_price - min(zone * 0.20, atr * 0.10)
        reclaimed = c > zone_price and c > o
        wick = max(0.0, min(o, c) - l)
        body = abs(c - o)
        confirmed = pierced and reclaimed and pc <= zone_price + zone
    else:
        pierced = h > zone_price + min(zone * 0.20, atr * 0.10)
        reclaimed = c < zone_price and c < o
    ...
    score = 0
    if pierced: score += 25
    if reclaimed: score += 25
    if wick >= max(body, atr * 0.05): score += 15
    if confirmed: score += 15
    return {"score": min(80, score), "confirmed": confirmed, "pierced": pierced, "reclaimed": reclaimed}


def _sr_breakout_retest_features(df, zone_price, direction, width, atr):
    """Detect breakout followed by a controlled retest and close back with direction."""
    if df is None or len(df) < 5 or zone_price is None or atr is None or atr <= 0:
        return {"score": 0, "confirmed": False}
    last = df.iloc[-1]
    prev = df.iloc[-2]
    prior = df.iloc[-4:-2]
    lc = _sr_num(last.get("close")); pc = _sr_num(prev.get("close"))
    ph = _sr_num(prev.get("high")); pl = _sr_num(prev.get("low"))
    if None in (lc, pc, ph, pl) or prior.empty:
        return {"score": 0, "confirmed": False}
    zone = max(width, atr * SR_ZONE_ATR)
    score = 0
    if direction == "BUY":
        broke = pc > zone_price + zone * 0.10
        retest = pl <= zone_price + zone and pl >= zone_price - zone * 1.5
        held = lc > zone_price
    else:
        broke = pc < zone_price - zone * 0.10
        retest = ph >= zone_price - zone and ph <= zone_price + zone * 1.5
        held = lc < zone_price
    confirmed = broke and retest and held
    if broke: score += 30
    if retest: score += 25
    if held: score += 25
    return {"score": min(80, score), "confirmed": confirmed, "broke": broke, "retest": retest, "held": held}


def _sr_candidate_setup(df, sr, direction):
    """Create a self-contained S/R candidate using only price/S/R geometry."""
    if df is None or len(df) < 20 or not sr:
        return None
    price = _sr_num(df["close"].iloc[-1])
    atr = _sr_num(sr.get("atr"))
    if price is None or atr is None or atr <= 0:
        return None
    lv = _sr_levels(sr)
    width = max(atr * SR_ZONE_ATR, price * 0.00025)
    supports = sorted([v for k,v in lv.items() if k.startswith("support") and v is not None and v < price], reverse=True)
    resistances = sorted([v for k,v in lv.items() if k.startswith("resistance") and v is not None and v > price])
    if direction == "BUY":
        zone = supports[0] if supports else None
        opposite = resistances
    else:
        zone = resistances[0] if resistances else None
        opposite = supports
    if zone is None or not _sr_near(price, zone, max(width, atr * 0.60)):
        # A breakout-retest can be farther from the level than a simple rejection.
        pass
    fb = _sr_false_breakout_features(df, zone, direction, width, atr) if zone is not None else {"score":0,"confirmed":False}
    br = _sr_breakout_retest_features(df, zone, direction, width, atr) if zone is not None else {"score":0,"confirmed":False}
    proximity = max(0.0, 1.0 - abs(price - zone) / max(width * 2.0, atr)) if zone is not None else 0.0
    score = 30 * proximity + min(25, max(fb["score"], br["score"]) * 0.40)
    if fb.get("confirmed"): score += 20
    if br.get("confirmed"): score += 20
    if not (fb.get("confirmed") or br.get("confirmed")):
        return None
    score = min(100.0, score + 10.0)
    if score < SR_MIN_SCORE:
        return None
    return {
        "direction": direction,
        "entry": price,
        ...
        "width": width,
        "score": round(score, 2),
        "trigger": "FALSE_BREAKOUT" if fb.get("confirmed") else "BREAKOUT_RETEST",
        "false_breakout": fb,
        "breakout_retest": br,
...
def _sr_build_trade(candidate):
    """Build independent Entry/SL/TP strictly from S/R geometry; no main trade builder."""
    if not candidate:
        return None
    entry = _sr_num(candidate.get("entry")); zone = _sr_num(candidate.get("zone")); atr = _sr_num(candidate.get("atr"))
    direction = candidate.get("direction")
    if None in (entry, zone, atr) or atr <= 0 or direction not in ("BUY", "SELL"):
        return None
    opposite = [_sr_num(x) for x in candidate.get("opposite_levels", []) if _sr_num(x) is not None]
    if not opposite:
        return None
    if direction == "BUY":
        targets = sorted([x for x in opposite if x > entry])
        sl = zone - atr * SR_SL_BUFFER_ATR
    else:
        targets = sorted([x for x in opposite if x < entry], reverse=True)
        sl = zone + atr * SR_SL_BUFFER_ATR
    if not targets:
        return None
    risk = abs(entry - sl)
    if risk <= 0 or risk > atr * SR_MAX_RISK_ATR:
        return None
    rr1 = abs(targets[0] - entry) / risk
    if rr1 < SR_MIN_RR:
        return None
    tps = targets[:3]
    while len(tps) < 3:
    ...
    return {
        ...
        "tp1": round(tps[0], 3), "tp2": round(tps[1], 3) if tps[1] is not None else None,
        "tp3": round(tps[2], 3) if tps[2] is not None else None,
        "rr": round(rr1, 2), "score": candidate["score"],
        "direction": direction, "trigger": candidate["trigger"],
        "zone": zone, "atr": atr, "levels": candidate["levels"],
    }
...
def sr_analyze():
    """Independent S/R analysis with explicit failure classification."""
    try:
        df = get_bars("15m", limit=300)
    except Exception as exc:
        logger.exception("S/R data acquisition failed")
        return {"ok": False, "error_code": "DATA_FETCH", "reason": f"تعذر جلب بيانات M15: {type(exc).__name__}: {exc}"}
    if df is None or getattr(df, "empty", True) or len(df) < 30:
        return {"ok": False, "error_code": "DATA_EMPTY", "reason": "بيانات M15 غير كافية لتشغيل S/R."}
    required={"open","high","low","close"}; missing=sorted(required.difference(df.columns))
    if missing:
        return {"ok": False, "error_code": "DATA_SCHEMA", "reason": f"أعمدة M15 مفقودة: {', '.join(missing)}"}
    try:
        sr=support_resistance(df, lookback=min(120,len(df)))
    except Exception as exc:
        logger.exception("S/R level engine failed")
        return {"ok": False, "error_code": "SR_ENGINE", "reason": f"فشل حساب مستويات S/R: {type(exc).__name__}: {exc}"}
    if not isinstance(sr,dict):
        return {"ok": False, "error_code": "SR_SCHEMA", "reason": "محرك S/R أعاد بنية غير صالحة."}
    candidates=[x for x in (_sr_candidate_setup(df,sr,"BUY"),_sr_candidate_setup(df,sr,"SELL")) if x]
    if not candidates:
        return {"ok": True,"signal":None,"sr":sr,"df":df,"reason":"لا توجد إشارة S/R مستقلة مكتملة وفق شروط الارتداد/الاختراق الكاذب/إعادة الاختبار."}
    candidates.sort(key=lambda x:x["score"],reverse=True)
    trade=_sr_build_trade(candidates[0])
    if not trade:
        return {"ok":True,"signal":None,"sr":sr,"df":df,"candidate":candidates[0],"reason":"وجدت إشارة S/R، لكن لا يوجد TP من S/R المقابل يحقق R:R المطلوب وحدود المخاطرة."}
    return {"ok":True,"signal":trade,"sr":sr,"df":df,"candidate":candidates[0]}
...
def _sr_format(result):
    ...
    if not signal:
        ...
        lines += [
            ...
            f"⭐ جودة الإشارة: {signal['score']:.0f} نقطة",
            f"🧭 المحفز: {signal['trigger']}",
    ...
    for key,label in (("support1","S1"),("support2","S2"),("support3","S3"),("resistance1","R1"),("resistance2","R2"),("resistance3","R3")):
        ...
        if p is not None: lines.append(f"{label}: {p}")
    return "\n".join(lines)
...
async def _sr_notify_all(text):
    ...
    for cid in recipients:
        try:
            ...
            logger.exception("S/R notification error for %s", cid)
...
def run_v18_70_sr_runtime_unit_tests():
    ...
    c={"direction":"BUY","entry":100.2,"zone":99.8,"atr":0.5,"score":90,"trigger":"FALSE_BREAKOUT","opposite_levels":[101.5,102.2,103.0],"levels":{}}
    tr=_sr_build_trade(c); check("entry/sl/tp geometry",tr is not None and tr["sl"]<tr["entry"]<tr["tp1"] and tr["rr"]>=SR_MIN_RR,str(tr))
