...
async def callback_router(update, context):
    ...
    elif data == "nav_analyses":
        await analyses_menu(update, context)
    elif data == "nav_trades":
        await trades_menu(update, context)
    elif data == "nav_market":
        await market_news_menu(update, context)
    elif data == "nav_institutional":
        await institutional_menu(update, context)
    elif data == "nav_liquidity":
...
async def _nav_send(update, text, buttons=None, edit=False):
    markup = InlineKeyboardMarkup(buttons) if buttons else None
    message = getattr(update, "message", None)
    query = getattr(update, "callback_query", None)
    if edit and query and query.message:
        await query.edit_message_text(with_market_header(text), reply_markup=markup)
    elif message:
        await message.reply_text(with_market_header(text), reply_markup=markup)
    elif query and query.message:
        await query.message.reply_text(with_market_header(text), reply_markup=markup)


def _nav_buttons(rows, include_home=True):
    buttons = [[InlineKeyboardButton(label, callback_data=data)] for label, data in rows]
    if include_home:
...
async def analyses_menu(update, context):
    ...
    buttons = _nav_buttons([
        ("📊 التحليل الكامل", "menu_full_analysis"),
        ("📝 التقرير التوضيحي اليومي", "menu_daily_report"),
        ("📅 التقرير التوضيحي الأسبوعي", "menu_weekly_report"),
    ])
    await _nav_send(update, "📊 التحليلات\n━━━━━━━━━━━━━━━━━━\nاختر نوع التحليل:", buttons, edit=bool(getattr(update, "callback_query", None)))


async def trades_menu(update, context):
    buttons = _nav_buttons([
        ("🎯 صفقة الآن", "menu_trade_now"),
        ("📜 سجل الصفقات", "menu_trade_history"),
        ("🔔 حالة تنبيهات الصفقات", "menu_trade_alerts"),
    ])
    await _nav_send(update, "🎯 الصفقات\n━━━━━━━━━━━━━━━━━━\nاختر العملية:", buttons, edit=bool(getattr(update, "callback_query", None)))
...
async def market_news_menu(update, context):
    ...
    buttons = _nav_buttons([
        ...
        ("💰 سعر الذهب", "menu_gold_price"),
    ])
    await _nav_send(update, "🌍 السوق والأخبار\n━━━━━━━━━━━━━━━━━━\nاختر القسم:", buttons, edit=bool(getattr(update, "callback_query", None)))
...
async def institutional_menu(update, context):
    ...
    try:
        ...
        await _nav_send(update, "🏦 التحليل المؤسسي\n━━━━━━━━━━━━━━━━━━\n🧠 Institutional Context\n" + str(context_text) + "\n\n📡 Institutional Data\n⚪ البيانات المؤسسية المباشرة تعتمد على توفر مصدر مباشر؛ لا يتم اختلاق بيانات غير متاحة.", [[InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")]], edit=bool(getattr(update, "callback_query", None)))
    except Exception as e:
...
async def liquidity_menu(update, context):
    ...
    try:
        ...
        await _nav_send(update, text, [[InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")]], edit=bool(getattr(update, "callback_query", None)))
    except Exception as e:
        await reply(update, f"🎯 مناطق السيولة\n\n❌ تعذر تحديث السيولة: {e}")


async def help_menu(update, context):
    await _nav_send(update, "ℹ️ المساعدة\n━━━━━━━━━━━━━━━━━━\nاستخدم القائمة الرئيسية للوصول إلى التحليل، الصفقات، السوق والأخبار، التحليل المؤسسي ومناطق السيولة.\n\n⚠️ جميع الإشارات تحليلية وليست ضماناً للربح.", [[InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")]], edit=bool(getattr(update, "callback_query", None)))
