...
# Telegram transport: polling is the safe default for this single-process Render
# service because it removes the Flask↔asyncio webhook bridge as a failure point.
# Set BOT_TRANSPORT=webhook only when the Render webhook URL is explicitly verified.
...
async def start_bot():
    ...
    if BOT_TRANSPORT == "polling":
        # Explicitly remove any old webhook before polling. Keeping an old webhook
        # causes Telegram 409 conflicts and results in a completely silent bot.
        await APPLICATION.bot.delete_webhook(drop_pending_updates=False)
        await APPLICATION.updater.start_polling(allowed_updates=allowed_updates, drop_pending_updates=False)
