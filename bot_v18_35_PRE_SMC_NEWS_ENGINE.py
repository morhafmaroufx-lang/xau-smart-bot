...
def _fetch_tradingeconomics_news():
    ...
    key = NEWS_TE_API_KEY or "guest:guest"
    now = now_damascus()
    ...
    response = requests.get(url, params={"c": key}, timeout=NEWS_HTTP_TIMEOUT,
                            headers={"User-Agent": "XAU-Smart-Trader/18.35"})
...
def get_news_events(force=False):
    """Multi-provider economic calendar with persistent cache and fail-safe execution."""
    global NEWS_CACHE
    now_ts = time.time()
    ...
    for source, fetcher in providers:
        try:
            events = fetcher()
            if not events:
                raise RuntimeError("empty normalized event set")
            events.sort(key=lambda e: e["time"])
            NEWS_CACHE = {"time": now_ts, "events": events, "source": source, "status": "ONLINE"}
    ...
    cached = _load_persisted_news_cache()
    if cached:
        age_min = (now_ts - cached["saved_at"]) / 60.0
        if age_min <= NEWS_MAX_CACHE_AGE_MIN:
            NEWS_CACHE = {"time": now_ts, "events": cached["events"], "source": "cache", "status": "STALE_VALID"}
            logger.warning("NEWS_CACHE_FALLBACK age=%.1fmin source=%s", age_min, cached.get("source"))
            return cached["events"]

    raise RuntimeError("تعذر تحديث تقويم الأخبار عبر جميع المصادر؛ " + " | ".join(errors))
...
def news_filter():
    ...
    try:
        ...
        return False, "🟢 لا يوجد حالياً خبر عالي التأثير ضمن نافذة الحجب."
    except Exception as exc:
