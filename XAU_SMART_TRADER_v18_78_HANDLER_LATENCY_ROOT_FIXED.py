...
class Metrics:
    ...
    def system_snapshot(self,queue_depth=0,queue_capacity=0,queue_drops=0):
        ...
        return {"uptime_s":now-self.started_mono,"cpu_pct":cpu_pct,"rss_mb":rss,"event_loop_lag_ms":_percentile(loops,95),"event_loop_lag_max_ms":max(loops) if loops else 0.0,"request_avg_ms":sum(req)/len(req) if req else 0.0,"request_p95_ms":_percentile(req,95),"request_max_ms":max(req) if req else 0.0,"request_count":req_count,"handler_avg_ms":sum(handlers)/len(handlers) if handlers else 0.0,"handler_p95_ms":_percentile(handlers,95),"handler_max_ms":max(handlers) if handlers else 0.0,"handler_count":handler_count,"throughput_per_min":throughput,"price_latency_ms":sum(pls)/len(pls) if pls else self.last_price_latency_ms,"db_write_latency_ms":sum(dbs)/len(dbs) if dbs else 0.0,"price_age_s":price_age,"queue_depth":queue_depth,"queue_capacity":queue_capacity,"queue_drops":queue_drops,"error_count":errors,"alert_count":alerts,"self_heal_count":heals,"auditor_heartbeat":hb}
    @staticmethod
    def _rss_mb():
        try:
...
VERSION = "v18.78-HANDLER-LATENCY-ROOT-FIXED"
