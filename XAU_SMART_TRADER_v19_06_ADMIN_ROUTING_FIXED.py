...
# v19.06 — إصلاح توجيه أزرار الإدارة مباشرةً
# يمنع سقوط admin_users إلى Routers قديمة متعددة التعريفات.
V19_06_VERSION = "v19.06_ADMIN_ROUTING_FIXED"
...
async def callback_router(update, context):
    ...
    # المسارات الإدارية تُعالج هنا مباشرةً، ولا تمر عبر أي Router قديم.
    admin_routes = {
    ...
    if data in admin_routes:
        await q.answer()
        if not await _admin_guard(update):
            return
        await admin_routes[data](update, context)
        return

    # إدارة المنح والحساب المحدد.
    if data in {
        "admin_grant_free_menu", "admin_grant_free_basic", "admin_grant_free_elite",
        "admin_cancel_free_grant", "admin_cancel_subscription",
        "admin_block_user", "admin_unblock_user", "admin_user_back"
    }:
        await q.answer()
        if not await _admin_guard(update):
            return
        cid = context.user_data.get("admin_selected_user")
        if data == "admin_grant_free_menu":
            await admin_user_grant_menu(update, context); return
        if data == "admin_grant_free_basic":
            context.user_data["admin_grant_plan"] = "BASIC"
            context.user_data["admin_mode"] = "grant_free_plan"
        ...
        if data == "admin_grant_free_elite":
            ...
            await reply(update, "💎 تم اختيار ELITE مجانًا (FREE+2).\n\n✏️ أرسل عدد الأيام، مثلاً: 7 أو 15 أو 30 أو 90"); return
        if data == "admin_user_back":
            await admin_user_details(update, context, cid) if cid else await admin_users(update, context); return
        if not cid:
            await reply(update, "⛔ لم يتم تحديد حساب."); return
        if data == "admin_cancel_free_grant":
            result = await asyncio.to_thread(_cancel_free_grant, cid, update.effective_user.id)
            await reply(update, "ℹ️ لا توجد باقة مجانية إدارية نشطة لهذا الحساب." if not result.get("ok") else "✅ تم إلغاء الباقة المجانية واستعادة الحالة السابقة.")
        elif data == "admin_cancel_subscription":
            await asyncio.to_thread(_cancel_subscription, cid, update.effective_user.id)
            await reply(update, "✅ تم إلغاء اشتراك الحساب وإعادته إلى FREE.")
        elif data == "admin_block_user":
            await asyncio.to_thread(_set_account_blocked, cid, update.effective_user.id, True)
            await reply(update, "⛔ تم حظر الحساب.")
        elif data == "admin_unblock_user":
            await asyncio.to_thread(_set_account_blocked, cid, update.effective_user.id, False)
            await reply(update, "🟢 تم فك حظر الحساب.")
        await admin_user_details(update, context, cid)
        return

    # باقي أزرار المستخدمين العامة.
    await _LEGACY_CALLBACK_ROUTER(update, context)
