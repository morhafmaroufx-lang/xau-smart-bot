    ...
    def _init_db(self):
        with self._db_lock:
            ...
            try:
                ...
                for name, typ in (("claimed_at","REAL"),("send_started_at","REAL"),("delivery_latency_ms","REAL"),("idempotency_key","TEXT")):
    ...
    def enqueue(self, chat_id, text, *, kind='generic', priority=50, max_attempts=ALERT_MAX_ATTEMPTS, idempotency_key=None):
        ...
        with self._db_lock:
            ...
            try:
                ...
                if pending >= ALERT_MAX_PENDING and int(priority) < 80:
                    logger.error('ALERT_BACKPRESSURE_REJECT | kind=%s pending=%s',kind,pending); return None
                c.execute('INSERT INTO alert_outbox(alert_id,chat_id,message,kind,priority,status,attempts,max_attempts,created_at,last_attempt,next_retry,sent_at,last_error,idempotency_key) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    (aid,cid,payload,str(kind),int(priority),'PENDING',0,max(1,int(max_attempts)),now,None,now,None,None,idem))
                c.commit()
            finally: c.close()
        self._wake.set()
...
VERSION = "v19.30_RELIABILITY_PERFORMANCE_ALERT_HARDENING"
...
def _v1929_m15_scan_self_tests():
    failures = []
    if VERSION != "v19.30_RELIABILITY_PERFORMANCE_ALERT_HARDENING":
        failures.append("version")
