...
# ============================================================
# v19.29 — M15 CLOSED-CANDLE SCAN GATE / DUPLICATE PROTECTION
# ============================================================
# القواعد:
...
VERSION = "v19.29_M15_CLOSED_CANDLE_SCAN_GATE"
...
def get_bars(interval, limit=300):
    """v19.29: لا تُعاد الشمعة الجارية عند طلب M15؛ بقية الفريمات دون تغيير."""
...
def _v1929_claim_m15_scan(scan_key):
    """Claim ذريّ واحد: الشمعة نفسها لا يمكن أن تبدأ دورة تداول ثانية."""
    ...
    with M15_SCAN_STATE_LOCK:
        ...
        try:
            conn.execute("PRAGMA busy_timeout=10000")
            cur = conn.execute(
                "INSERT OR IGNORE INTO m15_scan_state(scan_key, scanned_at) VALUES(?,?)",
                (str(scan_key), now),
            )
            conn.commit()
            claimed = cur.rowcount == 1
            if claimed:
                logger.info("M15_SCAN_CLAIMED | candle=%s", scan_key)
            else:
                logger.info("M15_SCAN_SKIPPED_DUPLICATE_CANDLE | candle=%s", scan_key)
            return claimed
        finally:
            conn.close()
...
def _v1929_m15_scan_self_tests():
    failures = []
    if VERSION != "v19.29_M15_CLOSED_CANDLE_SCAN_GATE":
        failures.append("version")
        ...
        try:
            ...
            if closed_reason:
                ...
                try:
                    ...
                    await asyncio.to_thread(_v1929_claim_m15_scan, closed_candle_key)
                    logger.info("M15 candle marked during market closure | candle=%s reason=%s",
                                closed_candle_key, closed_reason)
                except Exception:
