...
VERSION = "v18.52-INSTITUTIONAL-DATA-GATEWAY-CFTC-TREASURY-HARDENED-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
def _treasury_curve(real=False):
    """Official U.S. Treasury XML yield curve reader."""
    ...
    ns={'m':'http://schemas.datacontract.org/2004/07/TreasuryData'}
    rows=[]
    for item in root.iter():
        children={re.sub(r'[^a-z0-9]','',k.tag.split('}')[-1].lower()): (k.text or '').strip() for k in list(item)}
        if not children: continue
        date=children.get('date') or children.get('recorddate') or children.get('newrecorddate')
        y10=(children.get('tenyear') or children.get('10year') or children.get('tenyearrate')
             or next((v for k,v in children.items() if ('10yr' in k or '10year' in k) and not any(x in k for x in ('20yr','30yr','20year','30year'))),None))
        if date and y10:
            try: rows.append((pd.to_datetime(date,errors='coerce'),float(y10)))
            except Exception: pass
    if not rows: raise RuntimeError(('Treasury real' if real else 'Treasury nominal')+' 10Y XML parse failed')
