...
# ============================================================
# v19.55 — G.Y TRIGGER ROLE CONSISTENCY HARDENING
# Scope: G.Y/S-R candidate interpretation only.
# Purpose: prevent BREAKOUT_RETEST / SR_FLIP from being inferred
# from a level whose original S/R role is incompatible with the
# directional scenario. REJECTION / FALSE_BREAKOUT stay anchored
# to the original S/R side.
# MAIN, subscription, quota, alerts, persistence, PCS and Entry/SL/TP
# geometry are not rewritten by this block.
# ============================================================
V19_55_GY_TRIGGER_ROLE_CONTRACT = "ENFORCED"
V19_55_GY_TRIGGER_ROLE_VERSION = "v19.55_GY_TRIGGER_ROLE_TIMING_HARDENED"


def _v1955_gy_trigger_role_compatible(direction, level_key, trigger):
    """Return whether a G.Y trigger is semantically compatible with the source S/R role.

    BUY:
      - REJECTION / FALSE_BREAKOUT must act on a support.
      - BREAKOUT_RETEST / SR_FLIP must act on a resistance that was broken
        and is now being treated as support.

    SELL:
      - REJECTION / FALSE_BREAKOUT must act on a resistance.
      - BREAKOUT_RETEST / SR_FLIP must act on a support that was broken
        and is now being treated as resistance.

    This is a semantic provenance gate; it does not alter direction, geometry,
    score, R:R, SL or TP formulas after a valid candidate exists.
    """
    d = str(direction or "").upper().strip()
    key = str(level_key or "").lower().strip()
    t = str(trigger or "").upper().strip()
    if d == "BUY":
        expected = "resistance" if t in {"BREAKOUT_RETEST", "SR_FLIP"} else "support" if t in {"REJECTION", "FALSE_BREAKOUT"} else None
    elif d == "SELL":
        expected = "support" if t in {"BREAKOUT_RETEST", "SR_FLIP"} else "resistance" if t in {"REJECTION", "FALSE_BREAKOUT"} else None
    else:
        return False
    return expected is not None and key.startswith(expected)


def _v1955_gy_allowed_triggers(direction, level_key):
    """Return only triggers that are valid for the source level role and direction."""
    d = str(direction or "").upper().strip()
    key = str(level_key or "").lower().strip()
    if d == "BUY":
        return ("BREAKOUT_RETEST", "SR_FLIP") if key.startswith("resistance") else ("REJECTION", "FALSE_BREAKOUT") if key.startswith("support") else ()
    if d == "SELL":
        return ("BREAKOUT_RETEST", "SR_FLIP") if key.startswith("support") else ("REJECTION", "FALSE_BREAKOUT") if key.startswith("resistance") else ()
    return ()


def _v1955_gy_trigger_role_self_test():
    cases = [
        ...
        ("SELL", "resistance1", "BREAKOUT_RETEST", False),
        ("SELL", "support1", "REJECTION", False),
    ]
    results=[]
    for direction, key, trigger, expected in cases:
        actual=_v1955_gy_trigger_role_compatible(direction, key, trigger)
        results.append({"direction":direction,"level_key":key,"trigger":trigger,"expected":expected,"actual":actual,"ok":actual is expected})
    return {"ok":all(x["ok"] for x in results),"version":V19_55_GY_TRIGGER_ROLE_VERSION,"results":results}
...
def _sr_candidate_setup(df, sr, direction):
    ...
    for zone_key, zone in level_items[:4]:
        ...
        allowed=set(_v1955_gy_allowed_triggers(direction,zone_key))
        scenarios=[(trigger,data) for trigger,data in raw_scenarios if trigger in allowed]
        # Provenance is part of the G.Y trigger contract: a trigger whose
        # required S/R role does not match the actual source level is not a candidate.
        scenarios=[
            (trigger,data) for trigger,data in scenarios
            if _v1955_gy_trigger_role_compatible(direction,zone_key,trigger)
        ]
        ...
        candidates.append({"direction":direction,"entry":price,"zone":zone,"zone_key":zone_key,"zone_role":("RESISTANCE" if zone_key.lower().startswith("resistance") else "SUPPORT"),"trigger_role_contract":V19_55_GY_TRIGGER_ROLE_CONTRACT,"atr":atr,"width":width,"score":round(confirmation,2),"trigger":trigger,"scenario":data,"scenarios":{k:v for k,v in scenarios},"opposite_levels":opposite,"levels":lv,"structure":struct.get(direction,0),"df":df.copy()})
...
def _sr_build_trade(candidate):
    ...
    return {
        ...
        "zone_role":candidate.get("zone_role"),"trigger_role_contract":candidate.get("trigger_role_contract"),
        "atr":candidate["atr"],"levels":candidate["levels"],"geometry":g,
    }
        ...
        while self._running:
            ...
            try:
                ...
                for idx,part in enumerate(parts,1):
                    msg=await self.bot.send_message(chat_id=job['chat_id'],text=part)
...
VERSION = "v19.55_GY_TRIGGER_ROLE_TIMING_HARDENED"
...
# ============================================================
# v19.54 — EXECUTION ACCURACY HARDENING
...
# ============================================================
V19_54_VERSION = "v19.54_EXECUTION_ACCURACY_HARDENED"
...
VERSION = V19_55_GY_TRIGGER_ROLE_VERSION
GY_RUNTIME_PATCH_VERSION = V19_55_GY_TRIGGER_ROLE_VERSION
