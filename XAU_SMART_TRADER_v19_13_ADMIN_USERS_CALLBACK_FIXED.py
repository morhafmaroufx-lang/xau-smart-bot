...
# v19.12: defer process startup until ALL append-only overrides are defined.


# ============================================================
# v19.11 — ADMIN ACCOUNT STATUS CONTROL — SAFE APPEND ONLY
# Built on v19.04 without replacing startup/router infrastructure.
# ============================================================
V19_11_VERSION = "v19.11_ADMIN_ACCOUNT_STATUS_CONTROL_SAFE"
V19_13_VERSION = "v19.13_ADMIN_USERS_CALLBACK_FIXED"


def _v19_11_free_plus_label(grant):
    if not grant:
        return None
    plan = str(grant.get("plan", "")).upper()
    return {"BASIC": "FREE+1", "ELITE": "FREE+2"}.get(plan)


def _v19_11_effective_grant(chat_id):
    _ensure_admin_free_grant_table()
    _reconcile_admin_free_grant(chat_id)
    return _active_admin_free_grant(chat_id)


# Only extend feature access with the account-blocked guard.
_V19_11_LEGACY_HAS_FEATURE = has_feature
...
def has_feature(chat_id, feature):
    ...
    if row and str(row["status"] or "").lower() == "blocked":
        return False
    return _V19_11_LEGACY_HAS_FEATURE(chat_id, feature)


# Account status display: FREE+1 / FREE+2 are overlays, not paid plans.
_V19_11_LEGACY_PLAN_STATUS_TEXT = plan_status_text

def plan_status_text(chat_id):
    if is_admin_chat(chat_id):
        return _V19_11_LEGACY_PLAN_STATUS_TEXT(chat_id)
    row = get_member(chat_id)
    if row and str(row["status"] or "").lower() == "blocked":
        return "⛔ الحساب محظور\n🔒 الوصول إلى خدمات التداول والاشتراك موقوف من الإدارة."
    grant = _v19_11_effective_grant(chat_id)
    if grant and grant.get("plan") in ("BASIC", "ELITE"):
        expiry = datetime.fromisoformat(str(grant["expiry_date"]))
        remaining = max(0, int((expiry - now_damascus()).total_seconds()))
        days, rem = divmod(remaining, 86400)
        hours = rem // 3600
        label = _v19_11_free_plus_label(grant)
        plan = str(grant["plan"])
        return (
...
def _v19_11_cancel_free_grant(chat_id, admin_id):
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    _ensure_admin_free_grant_table()
    cid = int(chat_id)
...
def _v19_11_cancel_subscription(chat_id, admin_id):
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    cid = int(chat_id)
    _ensure_admin_free_grant_table()
    ...
    try:
        ...
        if not row:
            return False
        # Explicit subscription cancellation also cancels any active free grant.
        conn.execute(
            "UPDATE users SET plan='FREE',status='active',start_date=NULL,expiry_date=NULL,updated_at=? WHERE chat_id=?",
            (now_damascus().isoformat(), cid),
        )
...
def _v19_11_set_blocked(chat_id, blocked, admin_id):
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    cid = int(chat_id)
...
async def admin_user_grant_menu(update, context):
    ...
    await _admin_render(
        ...
        "اختر الباقة ثم أرسل عدد الأيام.",
        [
            [("🥉 منح BASIC مجانًا", "v19_11_grant_basic")],
            [("💎 منح ELITE مجانًا", "v19_11_grant_elite")],
            [("🔙 بيانات المستخدم", "v19_11_user_back")],
        ],
    )
...
async def admin_user_details(update, context, chat_id):
    if not await _admin_guard(update):
        return
    grant = await asyncio.to_thread(_v19_11_effective_grant, chat_id)
    row = await asyncio.to_thread(_admin_user_lookup, chat_id)
    if not row:
        await reply(update, "🔎 لم يتم العثور على هذا الحساب.")
        return
    ...
    if grant:
        lines += [
            f"• المنحة: {'BASIC' if grant['plan']=='BASIC' else 'ELITE'} مجانية",
            f"• انتهاء المنحة: {grant['expiry_date']}",
        ]
    controls = [
        [("🎁 منح اشتراك مجاني", "v19_11_grant_menu")],
        [("❌ إلغاء الباقة المجانية", "v19_11_cancel_grant")],
        [("🚫 إلغاء اشتراك الحساب", "v19_11_cancel_subscription")],
        [("🟢 فك حظر الحساب", "v19_11_unblock")] if str(row["status"]).lower() == "blocked" else [("⛔ حظر الحساب", "v19_11_block")],
        [("🔙 إدارة المستخدمين", "admin_users"), ("👑 لوحة الإدارة", "admin_home")],
    ]
    await _admin_render(update, "\n".join(lines), controls)
...
async def admin_users(update, context):
    if not await _admin_guard(update):
        return
    # Idempotent initialization; this is the only extra DB object required.
    await asyncio.to_thread(_ensure_admin_free_grant_table)
    total, rows = await asyncio.to_thread(_admin_users_snapshot)
    conn = _db()
    try:
        grants = conn.execute(
            f"SELECT plan,status,COUNT(*) n FROM users u JOIN {ADMIN_FREE_GRANT_TABLE} g "
            "ON g.chat_id=u.chat_id WHERE g.active=1 GROUP BY g.plan,u.status ORDER BY g.plan,u.status"
        ).fetchall()
    finally:
        conn.close()
    lines = ["👥 إدارة المستخدمين", "", f"• إجمالي الحسابات: {total}"]
...
_V19_11_LEGACY_CALLBACK = callback_router
async def callback_router(update, context):
    q = getattr(update, "callback_query", None)
    if not q:
        return
    data = (q.data or "").strip()
    # v19.13: direct, isolated routing for the إدارة المستخدمين button.
    # This prevents admin_users from falling through legacy callback routers.
    if data == "admin_users":
        await q.answer()
        await admin_users(update, context)
        return
    if data == "v19_11_grant_menu":
        await q.answer(); await admin_user_grant_menu(update, context); return
    if data in {"v19_11_grant_basic", "v19_11_grant_elite"}:
        await q.answer()
        context.user_data["admin_grant_plan"] = "BASIC" if data.endswith("basic") else "ELITE"
        context.user_data["admin_mode"] = "v19_11_grant_days"
        await reply(update, "✏️ أرسل عدد الأيام المطلوب، مثل: 7 أو 30 أو 90")
        return
    if data == "v19_11_user_back":
        await q.answer()
        cid = context.user_data.get("admin_selected_user")
        if cid:
            await admin_user_details(update, context, cid)
            ...
            await admin_users(update, context)
        return
    if data in {"v19_11_cancel_grant", "v19_11_cancel_subscription", "v19_11_block", "v19_11_unblock"}:
        await q.answer()
        if not await _admin_guard(update):
            return
        cid = context.user_data.get("admin_selected_user")
        ...
        try:
            if data == "v19_11_cancel_grant":
                ok = await asyncio.to_thread(_v19_11_cancel_free_grant, cid, update.effective_user.id)
                msg = "✅ تم إلغاء الباقة المجانية واستعادة الحالة السابقة." if ok else "ℹ️ لا توجد باقة مجانية نشطة."
            elif data == "v19_11_cancel_subscription":
                ok = await asyncio.to_thread(_v19_11_cancel_subscription, cid, update.effective_user.id)
                msg = "✅ تم إلغاء اشتراك الحساب وإعادته إلى FREE." if ok else "🔎 لم يتم العثور على الحساب."
            elif data == "v19_11_block":
                ok = await asyncio.to_thread(_v19_11_set_blocked, cid, True, update.effective_user.id)
                msg = "⛔ تم حظر الحساب." if ok else "🔎 لم يتم العثور على الحساب."
            else:
                ok = await asyncio.to_thread(_v19_11_set_blocked, cid, False, update.effective_user.id)
                msg = "🟢 تم فك حظر الحساب." if ok else "🔎 لم يتم العثور على الحساب."
            await reply(update, msg)
            await admin_user_details(update, context, cid)
        except Exception as exc:
            logger.exception("v19.11 admin action failed")
            await reply(update, f"❌ تعذر تنفيذ العملية.\nالسبب: {type(exc).__name__}: {exc}")
        return
    await _V19_11_LEGACY_CALLBACK(update, context)


_V19_11_LEGACY_ROUTER = router
async def router(update, context):
    text = (update.effective_message.text or "").strip() if getattr(update, "effective_message", None) else ""
    if context.user_data.get("admin_mode") == "v19_11_grant_days":
        if not _admin_ok(update):
        ...
        if not text.isdigit() or int(text) <= 0:
            await reply(update, "⚠️ أرسل عدد أيام صحيحًا أكبر من صفر."); return
        cid = context.user_data.get("admin_selected_user")
        plan = context.user_data.get("admin_grant_plan")
        context.user_data.pop("admin_mode", None); context.user_data.pop("admin_grant_plan", None)
        if not cid or plan not in {"BASIC", "ELITE"}:
            await reply(update, "⛔ لم يتم تحديد الحساب أو نوع المنحة."); return
        try:
            result = await asyncio.to_thread(_grant_free_plan, cid, int(text), update.effective_user.id, plan)
            marker = "FREE+1" if plan == "BASIC" else "FREE+2"
            await reply(update, with_market_header(
                f"✅ تم منح {marker} بنجاح\n\n• الحساب: {cid}\n• المنحة: {'BASIC' if plan=='BASIC' else 'ELITE'} مجانية\n• المدة: {int(text)} يوم\n• الانتهاء: {result['expiry'].strftime('%Y-%m-%d %H:%M')} دمشق"
            ))
        except Exception as exc:
            logger.exception("v19.11 grant failed")
            await reply(update, f"❌ تعذر تنفيذ المنحة.\nالسبب: {type(exc).__name__}: {exc}")
        return
    await _V19_11_LEGACY_ROUTER(update, context)


# IMPORTANT: do not replace integrated_start_bot(). The working v19.04 startup
# already resolves the global callback_router/router names at runtime.
def run_v19_12_startup_order_self_test():
    source = Path(__file__).read_text(encoding="utf-8")
    first_main = source.find("if __name__ == \"__main__\":")
    last_main = source.rfind("if __name__ == \"__main__\":")
    return {"ok": first_main == last_main, "first_main": first_main, "last_main": last_main}
...
def run_v19_11_self_tests():
    ...
    def check(name, cond):
        if not cond: failures.append(name)
    check("version", V19_11_VERSION == "v19.11_ADMIN_ACCOUNT_STATUS_CONTROL_SAFE")
    check("free_plus_1", _v19_11_free_plus_label({"plan": "BASIC"}) == "FREE+1")
    check("free_plus_2", _v19_11_free_plus_label({"plan": "ELITE"}) == "FREE+2")
    check("admin_users", callable(admin_users))
    check("grant_menu", callable(admin_user_grant_menu))
    check("cancel_grant", callable(_v19_11_cancel_free_grant))
    check("cancel_subscription", callable(_v19_11_cancel_subscription))
    check("block", callable(_v19_11_set_blocked))
    check("callback", "v19_11_grant_menu" in callback_router.__code__.co_consts)
    check("router", "v19_11_grant_days" in router.__code__.co_consts)
    check("startup_unchanged", integrated_start_bot is not None)
    return {"ok": not failures, "passed": 11-len(failures), "total": 11, "failures": failures}


def run_v19_13_admin_users_callback_self_test():
    source = Path(__file__).read_text(encoding="utf-8")
    checks = {
        "version": "v19.13_ADMIN_USERS_CALLBACK_FIXED" in source,
        "direct_admin_users_route": 'if data == "admin_users":' in source and 'await admin_users(update, context)' in source,
        "single_main_guard": source.count('if __name__ == "__main__":') == 1,
    }
    return {"ok": all(checks.values()), "checks": checks}


def main_v19_13():
    if "--v19-13-admin-users-self-test" in sys.argv:
        print(json.dumps(run_v19_13_admin_users_callback_self_test(), ensure_ascii=False, indent=2)); return
    if "--v19-12-self-test" in sys.argv:
        print(json.dumps(run_v19_12_startup_order_self_test(), ensure_ascii=False, indent=2)); return
