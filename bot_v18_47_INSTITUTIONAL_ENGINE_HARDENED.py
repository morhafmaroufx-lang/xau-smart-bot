...
VERSION = "v18.47-INSTITUTIONAL-ENGINE-HARDENED-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIX-MENU-MARKETS-RUNTIME-CORRECTED"
...
def _inst_http(url,params=None,timeout=None):
    r=requests.get(url,params=params,timeout=timeout or INSTITUTIONAL_HTTP_TIMEOUT,headers={
        "User-Agent":"XAU-Smart-Trader/18.47 institutional",
        "Accept":"*/*",
    })
    r.raise_for_status()
    return r
...
def _cftc_zip(url):
    ...
    if datecol:
        d['_date']=pd.to_datetime(d[datecol],errors='coerce'); d=d.dropna(subset=['_date']).sort_values('_date')
