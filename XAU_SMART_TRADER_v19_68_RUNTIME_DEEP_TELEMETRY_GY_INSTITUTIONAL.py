...
AUDITOR_TRADE_RECONCILE_SECONDS = max(30.0, float(os.getenv("AUDITOR_TRADE_RECONCILE_SECONDS", "300")))
...
class Metrics:
    ...
    def _sr_monitor_report(self):
        """Read-only G.Y monitor backed directly by the persistent S/R DB.

        Every G.Y setup is counted from ``sr_trades``; TP1/TP2 states are
        retained as partial-success states while the trade remains monitored,
        and CLOSED rows are classified by their final result. No strategy
        function is called and no trade record is deleted here.
        """
        result = {
        ...
        try:
            ...
            with sqlite3.connect(sr_path, timeout=5, check_same_thread=False) as conn:
                conn.row_factory = sqlite3.Row
                rows = [dict(r) for r in conn.execute(
                    "SELECT status, result FROM sr_trades ORDER BY created_at ASC"
                ).fetchall()]
            ...
            for row in rows:
                ...
                if status == "OPEN":
                    result["active"] += 1
                if status == "TP1_HIT" or status == "TP2_HIT":
...
# ============================================================
# v19.55 — COMPLETE ARABIC PRESENTATION LOCALIZATION BOUNDARY
# ============================================================
# تعريب طبقة العرض فقط. لا يغيّر أي قيمة داخلية أو callback أو DB أو منطق تداول.
# G.Y محمي بالكامل ولا تتم ترجمته.
# ============================================================

_USER_UI_EXACT_AR = {
    "XAU SMART TRADER PERFORMANCE AUDIT": "تدقيق أداء XAU SMART TRADER",
    "SCORE PERFORMANCE": "أداء الدرجات",
    "DAILY ANALYSIS": "التحليل اليومي",
    "WEEKLY ANALYSIS": "التحليل الأسبوعي",
    "Full Target Rate": "معدل تحقيق الهدف الكامل",
    "Average R:R": "متوسط المخاطرة/العائد",
    "Average Drawdown": "متوسط التراجع السعري",
    "Average MAE": "متوسط أقصى حركة عكسية",
    "Average MFE": "متوسط أقصى حركة مواتية",
    "Direction Avg": "متوسط دقة الاتجاه",
    "Range Avg": "متوسط دقة النطاق",
    "Target Avg": "متوسط دقة الهدف",
    "Not Matched": "غير متطابق",
    "Partial Success": "نجاح جزئي",
    "Full Success": "نجاح كامل",
    ...
    "Matched": "متطابق",
    "Accuracy": "الدقة",
    "Structure + Momentum + Volume + Fibonacci + FVG": "هيكل السوق + الزخم + الحجم + فيبوناتشي + فجوة القيمة العادلة",
    "Buy-side": "جانب الشراء",
    "Sell-side": "جانب البيع",
    "TRADE:": "الصفقة:",
    "Chat ID": "معرف المحادثة",
    "ID الخاص بحسابك": "المعرف الخاص بحسابك",
    "Daily Analysis": "التحليل اليومي",
    "Weekly Analysis": "التحليل الأسبوعي",
    "Performance Monitor": "مراقب الأداء",
    "System Performance": "أداء النظام",
    "Trade Performance": "أداء الصفقات",
    "Scheduler errors": "أخطاء المجدول",
    "Total: ": "الإجمالي: ",
    "Closed: ": "المغلقة: ",
    "Full Success: ": "نجاح كامل: ",
    "Partial Success: ": "نجاح جزئي: ",
    ...
    "PAYMENT_DETECTED": "تم رصد الدفع",
    "VERIFYING": "قيد التحقق",
    "CONFIRMED": "مؤكد",
    "ACTIVATING": "قيد التفعيل",
    "MANUAL_REVIEW": "مراجعة إدارية",
    "READY": "جاهز",
    "WAIT": "انتظار",
    "UNKNOWN": "غير معروف",
    "BUY": "شراء",
    ...
    "RETEST": "إعادة الاختبار",
    "Breakout Retest": "اختراق وإعادة اختبار",
    "Breakout": "الاختراق",
    "BREAKOUT": "الاختراق",
    "Liquidity": "السيولة",
    ...
    "daily": "اليومي",
    "weekly": "الأسبوعي",
    "User-Agent": "معرّف العميل",
    "US Producer Price Index (PPI)": "مؤشر أسعار المنتجين الأمريكي (PPI)",
    "US Consumer Price Index (CPI)": "مؤشر أسعار المستهلك الأمريكي (CPI)",
    "FOMC Rate Decision": "قرار الفائدة للاحتياطي الفيدرالي الأمريكي",
    "CME FedWatch": "CME FedWatch",
    "LONDON_NEW_YORK_OVERLAP": "تداخل لندن ونيويورك",
    "LONDON": "لندن",
    "NEW_YORK": "نيويورك",
}

import re as _ui_re
_UI_EXACT_PATTERN = re.compile(
    "|".join(sorted((re.escape(k) for k in _USER_UI_EXACT_AR), key=len, reverse=True))
)
_UI_WORD_PATTERN = re.compile(r"\\b[A-Za-z][A-Za-z0-9_+/-]*\\b")


def localize_user_text(text):
    """تعريب النص الخارج إلى المستخدم فقط، مع إبقاء G.Y والأكواد الداخلية دون تغيير."""
    if text is None:
        return text
    value = str(text)
    # حماية G.Y صراحةً من أي معالجة مستقبلية.
    protected = {}
    def _protect(match):
        key = f"__GY_PROTECTED_{len(protected)}__"
        protected[key] = match.group(0)
        return key
    value = re.sub(r"(?<![A-Za-z0-9])G\\.Y(?![A-Za-z0-9])", _protect, value, flags=re.IGNORECASE)
    value = _UI_EXACT_PATTERN.sub(lambda m: _USER_UI_EXACT_AR[m.group(0)], value)

    # تعريب مصطلحات العرض المتبقية فقط. الأكواد التجارية BASIC/ELITE/FREE وW1/D1/H4...
    # وXAU/USD وUSDT وBSC وBEP-20 تبقى كما هي لأنها معرفات/رموز معتمدة.
    word_map = {
        "Total": "الإجمالي", "Closed": "المغلقة", "Failed": "الفاشلة", "Expired": "المنتهية",
        "Ambiguous": "الملتبسة", "Matched": "متطابق", "Success": "نجاح", "Partial": "جزئي",
        "READY": "جاهز", "WAIT": "انتظار", "OPEN": "مفتوحة", "CLOSED": "مغلقة", "ACTIVE": "نشطة",
        "PENDING": "قيد الانتظار", "SENT": "تم الإرسال", "BUY": "شراء", "SELL": "بيع",
        "Entry": "الدخول", "SL": "وقف الخسارة", "TP1": "الهدف 1", "TP2": "الهدف 2", "TP3": "الهدف 3",
        "Score": "الدرجة", "Quality": "الجودة", "Retest": "إعادة الاختبار", "RETEST": "إعادة الاختبار",
        "Breakout": "الاختراق", "BREAKOUT": "الاختراق", "Liquidity": "السيولة", "Structure": "هيكل السوق",
        "Momentum": "الزخم", "Volume": "الحجم", "Confirmation": "التأكيد", "Signal": "الإشارة",
        "Trade": "الصفقة", "Market": "السوق", "News": "الأخبار", "Daily": "اليومي", "Weekly": "الأسبوعي",
        "System": "النظام", "Monitor": "المراقب", "Performance": "الأداء", "Audit": "التدقيق",
        "Error": "خطأ", "Warning": "تحذير", "Time": "الوقت", "Price": "السعر", "Session": "الجلسة",
        "Risk": "المخاطرة", "Reward": "العائد", "Account": "الحساب", "User": "المستخدم", "Rate": "المعدل",
        "Average": "المتوسط", "Drawdown": "التراجع السعري", "MAE": "MAE", "MFE": "MFE",
        "private": "الخاصة", "at": "عند", "with": "مع", "and": "و", "or": "أو", "only": "فقط",
        "current": "الحالي", "point": "نقطة", "points": "نقاط", "day": "يوم", "days": "أيام",
        "month": "شهر", "months": "أشهر", "plan": "الباقة", "status": "الحالة", "remaining": "المتبقي",
        "expiry": "الانتهاء", "network": "الشبكة", "address": "العنوان", "order": "الطلب",
        "payment": "الدفع", "database": "قاعدة البيانات", "request": "الطلب", "subscription": "الاشتراك",
        "valid": "صالح", "invalid": "غير صالح", "not": "ليس", "send": "إرسال", "sent": "تم الإرسال",
        "failed": "فشل", "success": "نجاح",
        "Full": "كامل", "Partial": "جزئي", "UNKNOWN": "غير معروف", "MATCHED": "متطابق", "FAILED": "فشل",
        "EXPIRED": "منتهٍ", "AMBIGUOUS": "ملتبس", "NOT_MATCHED": "غير متطابق", "FULL": "كامل",
    }
    for src, dst in word_map.items():
        value = re.sub(rf"(?<![A-Za-z0-9_]){re.escape(src)}(?![A-Za-z0-9_])", dst, value, flags=re.IGNORECASE if src not in {"MAE","MFE"} else 0)

    # مستويات الدعم/المقاومة الظاهرة للمستخدم. تبقى المفاتيح الداخلية دون تغيير.
    value = re.sub(r"(?<![A-Za-z0-9])S([1-3])(?![A-Za-z0-9])", lambda m: f"دعم {m.group(1)}", value, flags=re.IGNORECASE)
    value = re.sub(r"(?<![A-Za-z0-9])R([1-3])(?![A-Za-z0-9])", lambda m: f"مقاومة {m.group(1)}", value, flags=re.IGNORECASE)
    value = re.sub(r"(?<![A-Za-z0-9])S/R(?![A-Za-z0-9])", "الدعم/المقاومة", value, flags=re.IGNORECASE)

    # تعريب مفاتيح تقرير الأداء من الشكل key=value دون لمس القيم الرقمية.
    key_map = {"total": "الإجمالي", "full": "كامل", "failed": "فشل", "partial": "جزئي", "full_rate": "معدل الكامل"}
    for src, dst in key_map.items():
        value = re.sub(rf"(?<![A-Za-z0-9_]){re.escape(src)}(?==)", dst, value, flags=re.IGNORECASE)
...
def with_market_header(text):
    return market_header() + localize_user_text(text)
    ...
    async def _deliver(self, job):
        ...
        while job.attempts < job.max_attempts:
            try:
                ...
                for part in parts:
                    await self.bot.send_message(chat_id=job.chat_id, text=localize_user_text(part))
                return True
    ...
    try:
        ...
        if q:
            ...
            for index, part in enumerate(parts):
                with handler_trace_stage("telegram_reply:deep:send_part_http", part_index=index, parts_total=len(parts), chars=len(part)):
                    await update.message.reply_text(part, reply_markup=(reply_markup if index == 0 else None))
        else:
            if isinstance(trace, dict):
...
async def _performance_report_for_admin(update):
    """حارس المراقب + مصالحة فورية قبل العرض.

    سجل الصفقات الرئيسي (trades.db) هو مصدر الحقيقة؛ لا نعتمد على أن عامل
    Auditor سبق أن نفّذ دورة المصالحة قبل أن يضغط المدير على المراقب.
    المصالحة هنا لا تغيّر أي قرار تداول ولا شروط دخول، وإنما تضمن أن
    Live_Trades ونتائجها محفوظة وقابلة للعرض حتى بعد إعادة التشغيل.
    """
    user_id = update.effective_user.id if update.effective_user else update.effective_chat.id
    ...
    if PERFORMANCE_AUDITOR is None:
        ...
        return None
    try:
        # Forced read-through reconciliation: never wait for the background
        # auditor cycle and never depend on an in-memory trade list.
        reconcile = await asyncio.to_thread(_auditor_reconcile_from_main_history)
        PERFORMANCE_AUDITOR._last_trade_reconcile_stats = dict(reconcile)
    except Exception as exc:
        logging.getLogger(__name__).exception("AUDITOR_TRADE_MONITOR_FORCED_RECONCILE_ERROR")
        PERFORMANCE_AUDITOR._last_trade_reconcile_stats = {
            "scanned": 0, "inserted": 0, "updated": 0,
            "errors": 1, "error": type(exc).__name__
        }
...
def webhook():
    ...
    loop_running = bool(BOT_LOOP and BOT_LOOP.is_running())
    app_running = getattr(APPLICATION, "running", None) if APPLICATION is not None else False
    # During PTB startup APPLICATION exists before Application.start() completes.
    # A 503 here makes Telegram retry safely instead of calling process_update() too early.
    if (not app_present) or (not loop_running) or (app_running is False):
        logger.error(
            "WEBHOOK_NOT_READY | application=%s loop_running=%s app_running=%s",
            app_present, loop_running, app_running,
        )
...
def _trade_alert_idempotency_key(chat_id, signature):
    """مفتاح حتمي للتنبيه التجاري: نفس المستخدم + نفس بصمة الصفقة = نفس Alert.
    لا يعتمد على UUID أو السعر اللحظي أو وقت التنفيذ.
    """
    if not signature:
        return None
    raw = json.dumps(
        {"chat_id": int(chat_id), "signature": list(signature)},
        ensure_ascii=False, separators=(",", ":"), sort_keys=True, default=str,
    )
    digest = hashlib.sha256(raw.encode("utf-8")).hexdigest()
    return f"TRADE:{int(chat_id)}:{digest}"
...
def _init_alert_event_state_db():
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        conn.execute("""CREATE TABLE IF NOT EXISTS alert_event_state(
            event_key TEXT NOT NULL, chat_id INTEGER NOT NULL, status TEXT NOT NULL DEFAULT 'PENDING',
            attempts INTEGER NOT NULL DEFAULT 0, last_error TEXT, updated_at REAL NOT NULL,
            PRIMARY KEY(event_key,chat_id))""")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_alert_event_state_status ON alert_event_state(status,updated_at)")
        conn.commit()
    finally: conn.close()

def _reconcile_alert_event_recipient(event_key,chat_id):
    """مزامنة حالة الحدث مع Outbox دون انتظار Telegram داخل Scheduler."""
    idem=f"{event_key}:{int(chat_id)}"
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        row=conn.execute("SELECT status FROM alert_outbox WHERE idempotency_key=?",(idem,)).fetchone()
        if not row:
            return None
        state=str(row[0])
        mapped='SENT' if state=='SENT' else 'FAILED' if state=='FAILED' else 'QUEUED'
        conn.execute("UPDATE alert_event_state SET status=?,last_error=NULL,updated_at=? WHERE event_key=? AND chat_id=?",(mapped,time.time(),event_key,int(chat_id)))
        conn.commit()
        return mapped
    finally: conn.close()

def _claim_alert_event_recipient(event_key,chat_id):
    _init_alert_event_state_db()
    now=time.time(); cid=int(chat_id)
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000"); conn.execute("BEGIN IMMEDIATE")
        row=conn.execute("SELECT status,updated_at,attempts FROM alert_event_state WHERE event_key=? AND chat_id=?",(event_key,cid)).fetchone()
        if row:
            status=str(row[0]); age=now-float(row[1] or 0)
            if status=='SENT':
                conn.commit(); return False
            if status in ('SENDING','QUEUED') and age < 180:
                conn.commit(); return False
            # حالة قديمة عالقة بعد تعطل العملية/الشبكة: يسمح بإعادة المحاولة.
            conn.execute("UPDATE alert_event_state SET status='SENDING',attempts=attempts+1,updated_at=?,last_error=NULL WHERE event_key=? AND chat_id=?",(now,event_key,cid))
        else:
            conn.execute("INSERT INTO alert_event_state(event_key,chat_id,status,attempts,updated_at) VALUES(?,?, 'SENDING',1,?)",(event_key,cid,now))
        conn.commit(); return True
    except Exception:
        conn.rollback(); raise
    finally: conn.close()

def _finish_alert_event_recipient(event_key,chat_id,status,error=None):
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        conn.execute("UPDATE alert_event_state SET status=?,last_error=?,updated_at=? WHERE event_key=? AND chat_id=?",(str(status),None if not error else str(error)[:500],time.time(),event_key,int(chat_id)))
        conn.commit()
    finally: conn.close()

ALERT_MAX_ATTEMPTS = int(os.getenv("ALERT_MAX_ATTEMPTS", "8"))

async def _dispatch_event_broadcast(recipients,text,*,kind,event_key,max_attempts=ALERT_MAX_ATTEMPTS):
    """إدخال سريع إلى Outbox؛ لا ينتظر Telegram ولا يحجز Scheduler."""
    if not APPLICATION:
        return []
    dispatcher=ALERT_DISPATCHER or await _start_alert_dispatcher()
    if not dispatcher:
        return [False for _ in sorted(set(recipients or []))]
    results=[]
    for cid in sorted(set(recipients or [])):
        cid=int(cid)
        try:
            reconciled=await asyncio.to_thread(_reconcile_alert_event_recipient,event_key,cid)
            if reconciled=='SENT':
                results.append(True); continue
            if reconciled=='QUEUED':
                results.append(True); continue
            if not await asyncio.to_thread(_claim_alert_event_recipient,event_key,cid):
                # موجود قيد الإرسال؛ لا نكرر الحدث.
                results.append(True); continue
            ok=await _dispatch_alert(cid,text,kind=kind,max_attempts=max_attempts,wait_delivery=False,idempotency_key=f"{event_key}:{cid}")
            await asyncio.to_thread(_finish_alert_event_recipient,event_key,cid,'QUEUED' if ok else 'FAILED',None if ok else 'dispatch failed')
            results.append(bool(ok))
        except Exception as exc:
            try: await asyncio.to_thread(_finish_alert_event_recipient,event_key,cid,'FAILED',exc)
            except Exception: pass
...
async def send_market_session_alerts():
    ...
    for name, label, zone_name, open_hour, _duration in SESSION_SPECS:
        ...
        # Durable per-recipient state is authoritative; memory is only a fast cache.
        text = (
            f"🔔 افتتحت جلسة {label}\n\n"
            f"📊 حالة السوق\n{state}\n{status_note}\n\n"
            f"🕐 الوقت الحالي: {now.strftime('%H:%M')} دمشق"
        )
        results = await _dispatch_event_broadcast(recipients, text, kind="session_open", event_key=key)
        if results and all(x is True for x in results): SESSION_ALERT_STATE[key] = True
    ...
    for pair, (left, right) in current_pairs.items():
        ...
        results = await _dispatch_event_broadcast(recipients, text, kind="session_overlap", event_key=key)
        if results and all(x is True for x in results): SESSION_OVERLAP_ALERT_STATE[key] = True
...
async def send_news_alerts():
    ...
    for event in events:
        ...
        if -1 <= delta <= NEWS_BEFORE_MIN:
            ...
            results = await _dispatch_event_broadcast(recipients, text, kind="news", event_key=key)
            if results and all(x is True for x in results): NEWS_ALERT_STATE[key] = True
    ...
    try:
        ...
        async def _send():
            for chat_id in tuple(ADMIN_IDS):
                try:
                    await APPLICATION.bot.send_message(chat_id=chat_id, text=localize_user_text(text))
                except Exception:
                    pass
...
def _gy_sr_completeness(sr, price):
    """G.Y-only S/R Completeness Contract.

    The contract deliberately requires at least one valid support AND one valid
    resistance relative to the current price. It does NOT require all six
    levels, does NOT require S1/R1 specifically, and never fabricates missing
    levels. This contract is intentionally isolated from MAIN.
    """
    lv = _sr_levels(sr if isinstance(sr, dict) else {})
    p = _sr_num(price)
    supports = sorted(
        {v for k, v in lv.items() if k.startswith("support") and v is not None and p is not None and v < p}
    , reverse=True)
    resistances = sorted(
        {v for k, v in lv.items() if k.startswith("resistance") and v is not None and p is not None and v > p}
    )
    has_support = bool(supports)
    has_resistance = bool(resistances)
    if has_support and has_resistance:
        state = "FULL"
    elif has_support:
        state = "PARTIAL_SUPPORT"
    elif has_resistance:
        state = "PARTIAL_RESISTANCE"
    else:
        state = "EMPTY"
    return {
        "state": state,
        "executable": has_support and has_resistance,
        "supports": supports,
...
# ============================================================
# v19.57 — G.Y S/R TRIGGER-ROLE CONTRACT
# Preserve S/R level provenance and allow only semantically valid
# trigger/level-role combinations. No direction inversion, no
# Entry/SL/TP/R:R/Score/PCS formulas changed.
# ============================================================
V19_57_GY_TRIGGER_ROLE_CONTRACT = "ENFORCED"
V19_57_GY_TRIGGER_ROLE_VERSION = "v19.57_GY_TRIGGER_ROLE_CONTRACT"


def _v1957_gy_trigger_role_compatible(direction, level_key, trigger):
    direction = str(direction or "").upper()
    level_key = str(level_key or "").lower()
    trigger = str(trigger or "").upper()
    is_support = level_key.startswith("support")
    is_resistance = level_key.startswith("resistance")
    if direction == "BUY":
        return (is_support and trigger in {"REJECTION", "FALSE_BREAKOUT"}) or (is_resistance and trigger in {"BREAKOUT_RETEST", "SR_FLIP"})
    if direction == "SELL":
        return (is_resistance and trigger in {"REJECTION", "FALSE_BREAKOUT"}) or (is_support and trigger in {"BREAKOUT_RETEST", "SR_FLIP"})
    return False


def _v1957_gy_allowed_triggers(direction, level_key):
    direction = str(direction or "").upper()
    level_key = str(level_key or "").lower()
    is_support = level_key.startswith("support")
    is_resistance = level_key.startswith("resistance")
    if direction == "BUY" and is_support:
        return ("REJECTION", "FALSE_BREAKOUT")
    if direction == "BUY" and is_resistance:
        return ("BREAKOUT_RETEST", "SR_FLIP")
    if direction == "SELL" and is_resistance:
        return ("REJECTION", "FALSE_BREAKOUT")
    if direction == "SELL" and is_support:
        return ("BREAKOUT_RETEST", "SR_FLIP")
    return ()


def _v1957_gy_trigger_role_self_test():
    cases = [
        ("BUY", "support1", "REJECTION", True),
        ("BUY", "support2", "FALSE_BREAKOUT", True),
        ("BUY", "resistance1", "BREAKOUT_RETEST", True),
        ("BUY", "resistance2", "SR_FLIP", True),
        ("SELL", "resistance1", "REJECTION", True),
        ("SELL", "resistance2", "FALSE_BREAKOUT", True),
        ("SELL", "support1", "BREAKOUT_RETEST", True),
        ("SELL", "support2", "SR_FLIP", True),
        ("BUY", "resistance1", "REJECTION", False),
        ("BUY", "support1", "BREAKOUT_RETEST", False),
        ("SELL", "support1", "REJECTION", False),
        ("SELL", "resistance1", "SR_FLIP", False),
    ]
    failures = []
    for idx, (direction, level_key, trigger, expected) in enumerate(cases, 1):
        got = _v1957_gy_trigger_role_compatible(direction, level_key, trigger)
        if got is not expected:
            failures.append({"case": idx, "expected": expected, "got": got, "direction": direction, "level_key": level_key, "trigger": trigger})
    return {"ok": not failures, "version": V19_57_GY_TRIGGER_ROLE_VERSION, "cases": len(cases), "failures": failures}


def _sr_candidate_setup(df, sr, direction):
    """Multi-scenario S/R candidate with preserved level provenance.

    The level role is part of the candidate contract so a trigger cannot be
    attached to a semantically incompatible support/resistance level.
    """
    ...
    sr_contract=_gy_sr_completeness(sr, price)
    if not sr_contract["executable"]:
        return None
    lv=_sr_levels(sr); width=max(atr*SR_ZONE_ATR,price*0.00025)
    level_items=sorted([(k,v) for k,v in lv.items() if v is not None], key=lambda kv:abs(kv[1]-price))
    if not level_items: return None
    struct=_sr_structure_bias(df)
    candidates=[]
    for zone_key, zone in level_items[:4]:
        zone_role = "SUPPORT" if zone_key.startswith("support") else "RESISTANCE" if zone_key.startswith("resistance") else "UNKNOWN"
        ...
        raw_scenarios={"FALSE_BREAKOUT":fb,"BREAKOUT_RETEST":br,"REJECTION":rej,"SR_FLIP":flip}
        allowed=_v1957_gy_allowed_triggers(direction,zone_key)
        scenarios=[(name,raw_scenarios[name]) for name in allowed]
        if not scenarios:
            continue
        scenarios.sort(key=lambda x:x[1].get("score",0),reverse=True); trigger,data=scenarios[0]
        if not _v1957_gy_trigger_role_compatible(direction,zone_key,trigger):
            continue
        ...
        candidates.append({"direction":direction,"entry":price,"zone":zone,"zone_key":zone_key,"zone_role":zone_role,"atr":atr,"width":width,"score":round(confirmation,2),"trigger":trigger,"scenario":data,"scenarios":{k:v for k,v in scenarios},"opposite_levels":opposite,"levels":lv,"structure":struct.get(direction,0),"trigger_role_contract":V19_57_GY_TRIGGER_ROLE_CONTRACT,"df":df.copy()})
...
# ============================================================
# v19.41 — GEOMETRY REJECTION DIAGNOSTICS CONTRACT
# Scope: observability only. The v19.40 geometry decision implementation
# above is preserved as the sole decision reference and is called unchanged.
# No Entry/SL/TP/R:R/Score/S/R/PCS/M15 condition is changed here.
# ============================================================
VERSION = "v19.42_W1_NON_BLOCKING_GY_PCS"
GEOMETRY_DIAGNOSTICS_CONTRACT = "DIAGNOSTIC_ONLY"
_v1941_geometry_decision_reference = _sr_geometry_diagnostics

def _v1941_geometry_diagnostic_enrich(candidate, base):
    """Add failure-stage metadata without re-evaluating or changing geometry."""
    out=dict(base or {})
    ok=bool(out.get("ok"))
    out.setdefault("contract", GEOMETRY_DIAGNOSTICS_CONTRACT)
    if ok:
        out.update({
            "fail_code": None, "fail_stage": "COMPLETE",
            "entry_status": "PASS", "sl_status": "PASS",
            "risk_status": "PASS", "tp_status": "PASS", "rr_status": "PASS",
        })
        return out
    reason=str(out.get("reason") or "")
    if not candidate:
        code,stage="CANDIDATE_INVALID","CANDIDATE"
        statuses=("NOT_EVALUATED",)*5
    elif "ابتعد عن منطقة التفاعل" in reason:
        code,stage="ENTRY_INVALID","ENTRY"
        statuses=("FAIL","NOT_EVALUATED","NOT_EVALUATED","NOT_EVALUATED","NOT_EVALUATED")
    elif "إبطال بنيوي صالح" in reason:
        code,stage="INVALID_STRUCTURAL_SL","SL"
        statuses=("PASS","FAIL","NOT_EVALUATED","NOT_EVALUATED","NOT_EVALUATED")
    elif "المخاطرة تساوي صفراً" in reason:
        code,stage="RISK_ZERO","RISK"
        statuses=("PASS","PASS","FAIL","NOT_EVALUATED","NOT_EVALUATED")
    elif "المخاطرة البنيوية غير صالحة" in reason:
        code,stage="RISK_TOO_LARGE","RISK"
        statuses=("PASS","PASS","FAIL","NOT_EVALUATED","NOT_EVALUATED")
    elif "لا يوجد مستوى S/R مقابل" in reason:
        code,stage="NO_VALID_SR_TARGET","TP"
        statuses=("PASS","PASS","PASS","FAIL","NOT_EVALUATED")
    elif "فشلت شرط R:R" in reason:
        code,stage="RR_TOO_LOW","RR"
        statuses=("PASS","PASS","PASS","PASS","FAIL")
    else:
        code,stage="GEOMETRY_REJECTED_UNCLASSIFIED","GEOMETRY"
        statuses=("NOT_EVALUATED",)*5
    out.update({
        "fail_code": code, "fail_stage": stage,
        "entry_status": statuses[0], "sl_status": statuses[1],
        "risk_status": statuses[2], "tp_status": statuses[3], "rr_status": statuses[4],
    })
    return out

def _sr_geometry_diagnostics(candidate):
    """Compatibility wrapper: exact v19.40 decision + diagnostics metadata only."""
    base=_v1941_geometry_decision_reference(candidate)
    return _v1941_geometry_diagnostic_enrich(candidate, base)

def _v1941_geometry_contract_self_test():
    """Static contract test proving the reference decision is still the execution source."""
    import ast as _ast
    ...
    check("diagnostic contract marker", GEOMETRY_DIAGNOSTICS_CONTRACT=="DIAGNOSTIC_ONLY")
    check("reference function preserved", "_v1941_geometry_decision_reference = _sr_geometry_diagnostics" in source)
    check("active wrapper exists", "_sr_geometry_diagnostics" in names)
    check("builder still consumes geometry", "candidate[\"geometry\"]=g" in source and "if not g.get(\"ok\"): return None" in source)
    check("no strategy gate constants changed", all(x in source for x in ("SR_MIN_RR","SR_MAX_RISK_ATR","SR_ZONE_ATR","SR_SL_BUFFER_ATR")))
    return {"passed":sum(x["status"]=="PASS" for x in checks),"total":len(checks),"ok":all(x["status"]=="PASS" for x in checks),"results":checks}
...
def _sr_build_trade(candidate):
    ...
    return {
        ...
        "trigger":candidate["trigger"],"zone":candidate["zone"],"zone_key":candidate.get("zone_key"),
        "zone_role":candidate.get("zone_role"),"atr":candidate["atr"],
        "levels":candidate["levels"],"geometry":g,
        "trigger_role_contract":candidate.get("trigger_role_contract", V19_57_GY_TRIGGER_ROLE_CONTRACT),
    }
...
def _sr_monitor_diagnostics(df, sr):
    """Transparent G.Y S/R monitoring state; incomplete structures are never
    presented as fully actionable opportunities."""
    try:
        price=_sr_num(df["close"].iloc[-1]); atr=_sr_num(sr.get("atr")); lv=_sr_levels(sr)
        if price is None or atr is None or atr<=0:
            return {"price":price,"atr":atr,"items":[],"best":None,"sr_completeness":{"state":"EMPTY","executable":False,"supports":[],"resistances":[]}}
        completeness=_gy_sr_completeness(sr, price)
        ...
        # A one-sided S/R map remains useful for monitoring, but it is explicitly
        # non-executable until the opposite side is present.
        if not completeness["executable"]:
            for item in items:
                item["state"] = "INCOMPLETE_SR"
                if completeness["state"] == "PARTIAL_SUPPORT":
                    item["reason"] = "الدعم متاح، لكن لا توجد مقاومة صالحة فوق السعر؛ لا يمكن بناء صفقة G.Y كاملة."
                elif completeness["state"] == "PARTIAL_RESISTANCE":
                    item["reason"] = "المقاومة متاحة، لكن لا يوجد دعم صالح تحت السعر؛ لا يمكن بناء صفقة G.Y كاملة."
                else:
                    item["reason"] = "لا توجد بنية S/R صالحة بالنسبة للسعر الحالي."
        return {"price":price,"atr":atr,"width":width,"items":items,"best":items[0] if items else None,"sr_completeness":completeness}
    except Exception as exc:
...
def sr_analyze():
    ...
    with handler_trace_stage("sr_analyze:post_sr_validation"):
        ...
        sr_completeness=_gy_sr_completeness(sr, _sr_num(df["close"].iloc[-1]))
    ...
    with handler_trace_stage("sr_analyze:candidate_filter"):
        ...
        if not cands:
            if not sr_completeness["executable"]:
                return {"ok":True,"signal":None,"sr":sr,"df":df,"monitor":monitor,"sr_completeness":sr_completeness,"reason":"لا توجد صفقة G.Y قابلة للتنفيذ لأن بنية S/R غير مكتملة بالنسبة للسعر الحالي؛ يلزم دعم صالح تحت السعر ومقاومة صالحة فوق السعر. لا يتم اختلاق المستوى المفقود."}
            return {"ok":True,"signal":None,"sr":sr,"df":df,"monitor":monitor,"sr_completeness":sr_completeness,"reason":"لا توجد إشارة مؤكدة حالياً. محرك المراقبة يحدد المستوى والسيناريو والشرط الناقص قبل الدخول."}
    ...
    with handler_trace_stage("sr_analyze:build_trade"):
        ...
        if not trade:
            ...
            geometry_code=geometry.get("fail_code") or "GEOMETRY_REJECTED_UNCLASSIFIED"
            geometry_reason=geometry.get("reason") or "لم يتوفر سبب تشخيصي تفصيلي."
            return {"ok":True,"signal":None,"sr":sr,"df":df,"candidate":best,"monitor":monitor,
                    "geometry_rejection":geometry,
                    "reason":f"تم تأكيد سيناريو {best['trigger']} لكن هندسة Entry/SL/TP أو R:R لم تكن صالحة؛ الصفقة مرفوضة حمايةً لرأس المال.\n🔎 سبب الرفض [{geometry_code}]: {geometry_reason}"}
    with handler_trace_stage("sr_analyze:final_payload"):
        return {"ok":True,"signal":trade,"sr":sr,"df":df,"candidate":best,"monitor":monitor,"sr_completeness":sr_completeness}
...
async def integrated_start_bot():
    ...
    logger.info("Webhook: %s", WEBHOOK_URL)
    asyncio.create_task(auto_loop())
    SR_AUTO_TASK = asyncio.create_task(_v1935_gy_auto_loop(), name="gy-private-trade-auto")
    if PERFORMANCE_AUDITOR is not None:
        try:
            _start_auditor_bridge_worker()
            PERFORMANCE_AUDITOR.set_self_healer(_auditor_self_heal)
...
class PersistentAlertDispatcher:
    ...
    def __init__(self, bot, db_path=ALERT_DB_PATH, workers=4):
        ...
        # DB initialization is deferred to start() and executed via asyncio.to_thread.
        # This constructor must never perform synchronous SQLite I/O in the Event Loop.
    def _conn(self):
    ...
    def _init_db(self):
        with self._db_lock:
            ...
            try:
                ...
                # Crash/restart recovery: a fully delivered multi-part alert is terminal even if
                # the process died before the final durable status update.  Partially
                # delivered alerts remain retryable (at-least-once semantics).
                c.execute("UPDATE alert_outbox SET status='SENT',sent_at=COALESCE(sent_at,?),last_error=NULL WHERE status='SENDING' AND parts_total IS NOT NULL AND parts_total>0 AND COALESCE(parts_sent,0)>=parts_total", (time.time(),))
                c.execute("UPDATE alert_outbox SET status='PENDING',next_retry=? WHERE status='SENDING' AND NOT (parts_total IS NOT NULL AND parts_total>0 AND COALESCE(parts_sent,0)>=parts_total)", (time.time(),))
                c.commit()
            finally: c.close()
    def enqueue(self, chat_id, text, *, kind='generic', priority=50, max_attempts=ALERT_MAX_ATTEMPTS, idempotency_key=None):
        try: cid=int(chat_id)
        except Exception: return None
        payload=str(text or '').strip()
        if not payload: return None
        now=time.time(); aid='al_'+uuid.uuid4().hex
        parts_total=max(1,len(_split_telegram_text(payload)))
        with self._db_lock:
            c=self._conn()
            try:
...
async def _dispatch_broadcast(recipients,text,*,kind='generic',max_attempts=ALERT_MAX_ATTEMPTS,event_key=None):
    results=[]
    for cid in set(recipients or []):
        idem = f"{event_key}:{int(cid)}" if event_key else None
        results.append(await _dispatch_alert(cid,text,kind=kind,max_attempts=max_attempts,idempotency_key=idem))
    return results
...
def live_price():
    ...
    # Single-flight for synchronous callers running in worker threads.
...
class PersistentAlertDispatcher:
        ...
        with self._db_lock:
            ...
            try:
                c.execute("""CREATE TABLE IF NOT EXISTS alert_outbox(
                    ...
                    created_at REAL NOT NULL, last_attempt REAL, next_retry REAL NOT NULL,
                    sent_at REAL, last_error TEXT)""")
                # Safe telemetry/idempotency migrations for databases created by earlier versions.
                cols={r[1] for r in c.execute("PRAGMA table_info(alert_outbox)").fetchall()}
                for name, typ in (("claimed_at","REAL"),("send_started_at","REAL"),("delivery_latency_ms","REAL"),("idempotency_key","TEXT"),("parts_total","INTEGER"),("parts_sent","INTEGER"),("telegram_message_ids","TEXT")):
                    if name not in cols: c.execute(f"ALTER TABLE alert_outbox ADD COLUMN {name} {typ}")
                c.execute('CREATE UNIQUE INDEX IF NOT EXISTS ux_alert_idempotency ON alert_outbox(idempotency_key) WHERE idempotency_key IS NOT NULL')
                c.execute("""CREATE TABLE IF NOT EXISTS alert_trade_links(
                    alert_id TEXT PRIMARY KEY, chat_id INTEGER NOT NULL, signature TEXT NOT NULL,
                    consume_quota INTEGER NOT NULL DEFAULT 0, quota_token TEXT,
                    created_at REAL NOT NULL, finalized_at REAL
                )""")
    ...
    def enqueue(self, chat_id, text, *, kind='generic', priority=50, max_attempts=ALERT_MAX_ATTEMPTS, idempotency_key=None):
        ...
        with self._db_lock:
            ...
            try:
                ...
                if idem:
                    existing = c.execute("SELECT alert_id,status,attempts,max_attempts FROM alert_outbox WHERE idempotency_key=?", (idem,)).fetchone()
                    if existing:
                        status=str(existing['status'])
                        if status in {'SENT','PENDING','SENDING'}:
                            return str(existing['alert_id'])
                        if status == 'FAILED' and int(existing['attempts'] or 0) < int(existing['max_attempts'] or max_attempts):
                            c.execute("UPDATE alert_outbox SET status='PENDING',next_retry=?,last_error=NULL WHERE alert_id=?", (now,str(existing['alert_id'])))
                            c.commit(); self._wake.set(); return str(existing['alert_id'])
                        return None
                pending=c.execute("SELECT COUNT(*) FROM alert_outbox WHERE status IN ('PENDING','SENDING')").fetchone()[0]
                if pending >= ALERT_MAX_PENDING and int(priority) < 80:
                    logger.error('ALERT_BACKPRESSURE_REJECT | kind=%s pending=%s',kind,pending); return None
                c.execute('INSERT INTO alert_outbox(alert_id,chat_id,message,kind,priority,status,attempts,max_attempts,created_at,last_attempt,next_retry,sent_at,last_error,idempotency_key,parts_total,parts_sent,telegram_message_ids) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    (aid,cid,payload,str(kind),int(priority),'PENDING',0,max(1,int(max_attempts)),now,None,now,None,None,idem,parts_total,0,None))
                c.commit()
            finally: c.close()
        self._wake.set()
        with self._telemetry_lock:self._telemetry["enqueued"]+=1
        logger.info('ALERT_ENQUEUED | id=%s kind=%s idem=%s',aid,kind,bool(idem)); return aid
    def enqueue_trade_with_link(self, chat_id, text, signature, consume_quota, quota_token, *, kind='trade', priority=100, max_attempts=ALERT_MAX_ATTEMPTS, idempotency_key=None):
        """ذرية داخل xau_alert_queue.db: Outbox + alert_trade_links في transaction واحدة.
        لا تشمل Telegram أو subscriptions.db أو trades.db، لأنها قواعد بيانات منفصلة.
        """
        try:
            cid=int(chat_id)
        except Exception:
            return None
        payload=str(text or '').strip()
        if not payload or not signature or not idempotency_key:
            return None
        now=time.time()
        ...
        with self._db_lock:
            ...
            try:
                ...
                existing=c.execute("SELECT alert_id,status,attempts,max_attempts FROM alert_outbox WHERE idempotency_key=?",(idem,)).fetchone()
                if existing:
                    aid=str(existing['alert_id'])
                    # Repair a missing link from a crash/restart using the same deterministic alert id.
                    c.execute("INSERT OR IGNORE INTO alert_trade_links(alert_id,chat_id,signature,consume_quota,quota_token,created_at,finalized_at) VALUES(?,?,?,?,?,?,NULL)",
                              (aid,cid,str(signature),1 if consume_quota else 0,None if quota_token is None else str(quota_token),now))
                    status=str(existing['status'])
                    if status=='FAILED' and int(existing['attempts'] or 0) < int(existing['max_attempts'] or max_attempts):
                        c.execute("UPDATE alert_outbox SET status='PENDING',next_retry=?,last_error=NULL WHERE alert_id=?",(now,aid))
                    c.commit(); self._wake.set(); return aid

                pending=c.execute("SELECT COUNT(*) FROM alert_outbox WHERE status IN ('PENDING','SENDING')").fetchone()[0]
                if pending >= ALERT_MAX_PENDING and int(priority) < 80:
                    c.rollback(); logger.error('ALERT_BACKPRESSURE_REJECT | kind=%s pending=%s',kind,pending); return None
                aid='al_'+hashlib.sha256(idem.encode('utf-8')).hexdigest()[:32]
                parts_total=max(1,len(_split_telegram_text(payload)))
                c.execute('INSERT INTO alert_outbox(alert_id,chat_id,message,kind,priority,status,attempts,max_attempts,created_at,last_attempt,next_retry,sent_at,last_error,idempotency_key,parts_total,parts_sent,telegram_message_ids) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    (aid,cid,payload,str(kind),int(priority),'PENDING',0,max(1,int(max_attempts)),now,None,now,None,None,idem,parts_total,0,None))
                c.execute('INSERT INTO alert_trade_links(alert_id,chat_id,signature,consume_quota,quota_token,created_at,finalized_at) VALUES(?,?,?,?,?,?,NULL)',
                    (aid,cid,str(signature),1 if consume_quota else 0,None if quota_token is None else str(quota_token),now))
                c.commit()
            except Exception:
                try:c.rollback()
        ...
        self._wake.set()
        with self._telemetry_lock:self._telemetry['enqueued']+=1
        logger.info('ALERT_ENQUEUED_ATOMIC_TRADE | id=%s kind=%s idem=%s',aid,kind,idem)
        return aid
    ...
    async def _worker(self,idx):
        while self._running:
            ...
            try:
                parts=_split_telegram_text(job['message'])
                for idx,part in enumerate(parts,1):
                    msg=await self.bot.send_message(chat_id=job['chat_id'],text=localize_user_text(part))
                    # Persist each successful Telegram side effect immediately.
                    # This narrows the crash window and lets restart recovery prove
                    # that all parts were already delivered before marking SENT.
                    try:
                        mid=getattr(msg,'message_id',None)
                        with self._db_lock:
                            c=self._conn()
                            try:
                                row=c.execute("SELECT telegram_message_ids FROM alert_outbox WHERE alert_id=?",(job['alert_id'],)).fetchone()
                                ids=[]
                                if row and row['telegram_message_ids']:
                                    try: ids=json.loads(row['telegram_message_ids'])
                                    except Exception: ids=[]
                                if mid is not None: ids.append(int(mid))
                                c.execute("UPDATE alert_outbox SET parts_sent=?,telegram_message_ids=? WHERE alert_id=? AND status='SENDING'",(idx,json.dumps(ids,separators=(',',':')),job['alert_id']))
                                c.commit()
                            finally: c.close()
                    except Exception:
                        logger.exception('ALERT_PART_DURABILITY_UPDATE_FAILED | id=%s part=%s',job['alert_id'],idx)
                await asyncio.to_thread(self._result,job,True)
...
async def _dispatch_broadcast(recipients,text,*,kind='generic',max_attempts=ALERT_MAX_ATTEMPTS,event_key=None):
    # كل مستلم يملك مفتاح idempotency مستقلاً؛ نجاح مستلم لا يحسم حالة بقية المستلمين.
    async def _one(cid):
        idem = f"{event_key}:{int(cid)}" if event_key else None
        return await _dispatch_alert(cid,text,kind=kind,max_attempts=max_attempts,idempotency_key=idem)
    return await asyncio.gather(*[_one(cid) for cid in sorted(set(recipients or []))],return_exceptions=False)
...
async def auto_loop():
    ...
    while True:
        try:
            ...
            record, is_new = await asyncio.to_thread(register_trade, result)
            if not record:
                await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,False,"register_trade returned no record",60.0)
...
async def _resolve_alert_trade_link(link):
    ...
    if status == 'SENT':
        await asyncio.to_thread(_complete_notification, link['chat_id'], link['signature'])
    ...
    if status in ('FAILED', None):
        await asyncio.to_thread(_cancel_notification_claim, link['chat_id'], link['signature'])
...
async def _send_trade_notification(chat_id,result,record,title,consume_quota=False,quota_token=None):
    """إرسال تنبيه صفقة مع Claim غير حاجب، Outbox/Link ذريين ومفتاح idempotency حتمي.

    دلالة التسليم: At-Least-Once. لا ندّعي Exactly-Once لأن Telegram Side Effect
    خارجي عن SQLite؛ عند crash بين Telegram وdurable checkpoint قد تتكرر الرسالة.
    """
    signature=_trade_notification_signature(record)
    if not signature:return False
    if _notification_already_sent(chat_id,signature):return False
    claimed = await asyncio.to_thread(_claim_notification, chat_id, signature)
    if not claimed:return False
    quota_token=quota_token if consume_quota else None
    if consume_quota and not quota_token:
        await asyncio.to_thread(_cancel_notification_claim,chat_id,signature)
        return False
    aid = None
    try:
        payload=_format_trade_message(result,title,record)
        d=ALERT_DISPATCHER or await _start_alert_dispatcher()
        if not d: raise RuntimeError('Alert dispatcher unavailable')
        idem = _trade_alert_idempotency_key(chat_id, signature)
        # Outbox row + alert_trade_links row are committed by one SQLite transaction.
        aid=await asyncio.to_thread(
            d.enqueue_trade_with_link, chat_id, payload, signature,
            consume_quota, quota_token, kind='trade', priority=100, max_attempts=6,
            idempotency_key=idem,
        )
        if not aid: raise RuntimeError('Atomic trade alert enqueue/link failed')
        delivered=await d.wait_delivery(aid,ALERT_DELIVERY_WAIT_SECONDS)
        if delivered is True:
        ...
        # Once the atomic link exists, reconciliation owns cleanup.
        if aid is None:
            await asyncio.to_thread(_cancel_notification_claim,chat_id,signature)
            if consume_quota: await asyncio.to_thread(_release_trade_quota,quota_token)
        else:
            status = await asyncio.to_thread(ALERT_DISPATCHER.delivery_status,aid) if ALERT_DISPATCHER else None
            if status is None:
                await asyncio.to_thread(_cancel_notification_claim,chat_id,signature)
                if consume_quota: await asyncio.to_thread(_release_trade_quota,quota_token)
        logger.exception('Signal delivery failed for %s',chat_id);return False
...
async def _start_alert_dispatcher():
    ...
    await asyncio.to_thread(_init_alert_trade_links)
    await _reconcile_alert_trade_links_once()
...
async def _dispatch_alert(chat_id,text,*,kind='generic',max_attempts=ALERT_MAX_ATTEMPTS,wait_delivery=False,delivery_timeout=ALERT_DELIVERY_WAIT_SECONDS,idempotency_key=None):
    d=ALERT_DISPATCHER or await _start_alert_dispatcher()
    if not d:return False
    ...
    aid=await asyncio.to_thread(d.enqueue,chat_id,text,kind=kind,priority=priority,max_attempts=max_attempts,idempotency_key=idempotency_key)
    if not aid:return False
...
def plans_text():
    return "\n".join([
        ...
        "💎 النخبة — $35 / شهر", "• كل الأساسية + التحليل المتكامل + التحليل المؤسسي + تنبيهات التداول + تنبيهات الأخبار + حالة مخاطر الأخبار", "• 50 صفقة شهرياً", "",
...
def _admin_users_snapshot():
    conn=_db()
    try:
        # إحصائيات إدارة المستخدمين تعرض الحسابات المشتركة فقط.
        # حسابات الإدارة تبقى موجودة وقابلة للبحث والإدارة، لكنها لا تُحتسب
        # ضمن إجمالي الحسابات أو توزيع الباقات.
        admin_ids=tuple(int(x) for x in ADMIN_IDS)
        if admin_ids:
            placeholders=",".join("?" for _ in admin_ids)
            rows=conn.execute(
                f"SELECT plan,status,COUNT(*) n FROM users WHERE chat_id NOT IN ({placeholders}) GROUP BY plan,status ORDER BY plan,status",
                admin_ids,
            ).fetchall()
            total=conn.execute(
                f"SELECT COUNT(*) FROM users WHERE chat_id NOT IN ({placeholders})",
                admin_ids,
            ).fetchone()[0]
        else:
...
async def integrated_start_bot():
    ...
    APPLICATION.add_handler(CommandHandler("sr",_trace_handler(sr_command,"command:/sr")),group=0)
    APPLICATION.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"^🎯 صفقة G\.Y الخاصة$"),_trace_handler(_sr_button_guarded,"message:gy_private_trade")),group=0)
    # دعم المشتركين بمسار مباشر في المجموعة 0، قبل الراوتر العام.
    APPLICATION.add_handler(MessageHandler(filters.TEXT & filters.Regex(r"^🆘 الدعم$"),_trace_handler(support_button,"message:support")),group=0)
    APPLICATION.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,_trace_handler(_payment_duration_message_direct_handler,"message:payment_duration_direct")),group=0)
    APPLICATION.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND,_trace_handler(router,"message:router")),group=1)
    ...
    # Canonical G.Y automatic engine: lifecycle + confirmed setup generation.
    SR_AUTO_TASK=asyncio.create_task(_v1935_gy_auto_loop(),name="gy-private-trade-auto")
    if PERFORMANCE_AUDITOR is not None:
        try:
            ...
            # Persist/restore the monitor view from the canonical main trade DB immediately.
            try:
                PERFORMANCE_AUDITOR._last_trade_reconcile_stats = await asyncio.to_thread(_auditor_reconcile_from_main_history)
            except Exception:
                logging.getLogger(__name__).exception("AUDITOR_INITIAL_TRADE_RECONCILE_ERROR")
...
async def admin_users(update, context):
    ...
    try:
        grants = conn.execute(
            f"SELECT g.plan AS plan, u.status AS status, COUNT(*) AS n FROM users u JOIN {ADMIN_FREE_GRANT_TABLE} g ON g.chat_id=u.chat_id "
...
async def callback_router(update, context):
    """Final canonical callback router for v19.13.

    Important: every administration keyboard callback is routed explicitly here.
    The previous v19.12 layer delegated unknown callbacks to the legacy router,
    which did not contain admin_users/admin_dashboard routes. This made the
    "👥 إدارة المستخدمين" button appear dead.
    """
    q = update.callback_query
    if not q:
        return
    ...
    fn = admin_routes.get(data)
    if fn is not None:
        await fn(update, context)
        return

    if data in {"v19_13_grant_menu", "v19_12_grant_menu", "v19_11_grant_menu", "admin_grant_free_menu"}:
        await admin_user_grant_menu(update, context)
        return

    if data in {"admin_grant_free_basic", "admin_grant_free_elite"}:
        plan = "BASIC" if data.endswith("basic") else "ELITE"
        context.user_data["admin_grant_plan"] = plan
        context.user_data["admin_mode"] = "grant_free_plan"
        label = "BASIC" if plan == "BASIC" else "ELITE"
        await reply(update, f"{'🥉' if plan == 'BASIC' else '💎'} تم اختيار {label} مجانًا.\n\n✏️ أرسل عدد الأيام، مثلاً: 7 أو 15 أو 30 أو 90")
        return

    if data == "admin_user_back":
        cid = context.user_data.get("admin_selected_user")
        if cid:
            await admin_user_details(update, context, cid)
        else:
            await admin_users(update, context)
        return

    if data in {"v19_13_cancel_grant", "v19_12_cancel_grant", "v19_11_cancel_grant"}:
        cid = context.user_data.get("admin_selected_user")
        try:
            ...
            logger.exception("v19.13 cancel grant failed")
            await reply(update, f"❌ تعذر الإلغاء.\nالسبب: {type(exc).__name__}: {exc}")
        return

    if data in {"v19_13_cancel_subscription", "v19_12_cancel_subscription", "v19_11_cancel_subscription"}:
        cid = context.user_data.get("admin_selected_user")
        try:
            ...
            logger.exception("v19.13 cancel subscription failed")
            await reply(update, f"❌ تعذر إلغاء الاشتراك.\nالسبب: {type(exc).__name__}: {exc}")
        return

    if data in {"v19_13_block", "v19_12_block", "v19_11_block", "v19_13_unblock", "v19_12_unblock", "v19_11_unblock"}:
        cid = context.user_data.get("admin_selected_user")
        blocked = data.endswith("block") and not data.endswith("unblock")
        try:
            ...
            logger.exception("v19.13 block action failed")
            await reply(update, f"❌ تعذر تنفيذ العملية.\nالسبب: {type(exc).__name__}: {exc}")
        return
...
V19_13_VERSION = "v19.13_ADMIN_ACCOUNT_STATUS_CONTROL_FIXED"

def run_v19_13_self_tests():
    failures=[]
    def check(name, cond):
        if not cond: failures.append(name)
    check("version", V19_13_VERSION == "v19.13_ADMIN_ACCOUNT_STATUS_CONTROL_FIXED")
    check("basic_plan_exists", "BASIC" in PLANS)
    check("elite_plan_exists", "ELITE" in PLANS)
    ...
    check("callback_controls", "v19_13_block" in callback_router.__code__.co_consts)
    check("admin_users_route", "admin_users" in callback_router.__code__.co_consts)
    check("admin_dashboard_route", "admin_dashboard" in callback_router.__code__.co_consts)
    return {"ok": not failures, "passed": 15-len(failures), "total": 15, "failures": failures}

def main_v19_13():
    if "--v19-13-self-test" in sys.argv:
        print(json.dumps(run_v19_13_self_tests(), ensure_ascii=False, indent=2)); return
    global BOT_LOOP
    server = threading.Thread(target=run_flask, daemon=True); server.start()
    loop = asyncio.new_event_loop(); BOT_LOOP = loop; asyncio.set_event_loop(loop)
    try:
...
# ============================================================
# v19.17 — ACCOUNT STATUS + ACCOUNT LIFECYCLE NOTIFICATIONS + ID BUTTON
# ============================================================
# هذا القسم إضافي فقط: لا يغيّر منطق التداول أو شروط الإشارة أو S/R.
# ============================================================
V19_17_VERSION = "v19.18_ACCOUNT_STATUS_NOTIFICATIONS_ID_ACTIVE"
ACCOUNT_NOTIFICATION_TABLE = "account_notifications"
ACCOUNT_NOTIFICATION_INTERVAL = 3600
ACCOUNT_NOTIFICATION_TASK = None


def _ensure_account_notification_table():
    conn = _db()
    try:
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS {ACCOUNT_NOTIFICATION_TABLE} (
                chat_id INTEGER NOT NULL,
        ...
        """)
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{ACCOUNT_NOTIFICATION_TABLE}_chat ON {ACCOUNT_NOTIFICATION_TABLE}(chat_id, sent_at)")
        conn.commit()
    finally:
        conn.close()


def _claim_account_notification(chat_id, event_key):
    if not event_key:
        return False
    _ensure_account_notification_table()
    conn = _db()
    try:
        cur = conn.execute(
            f"INSERT OR IGNORE INTO {ACCOUNT_NOTIFICATION_TABLE}(chat_id,event_key,sent_at) VALUES(?,?,?)",
            (int(chat_id), str(event_key), now_damascus().isoformat()),
        )
        conn.commit()
...
def _release_account_notification(chat_id, event_key):
    conn = _db()
    try:
        conn.execute(
            f"DELETE FROM {ACCOUNT_NOTIFICATION_TABLE} WHERE chat_id=? AND event_key=?",
            (int(chat_id), str(event_key)),
        )
        conn.commit()
    finally:
        conn.close()
...
async def _send_account_notification(chat_id, event_key, text):
    """إرسال إشعار حساب ذريًا لمنع التكرار بين الجدولة والأحداث الإدارية."""
    if not APPLICATION or not text or is_admin_chat(chat_id):
        return False
    claimed = await asyncio.to_thread(_claim_account_notification, chat_id, event_key)
    if not claimed:
        return False
    try:
        await APPLICATION.bot.send_message(chat_id=int(chat_id), text=with_market_header(text))
        return True
    except Exception:
        await asyncio.to_thread(_release_account_notification, chat_id, event_key)
        logger.exception("ACCOUNT_NOTIFICATION_SEND_FAILED chat_id=%s event=%s", chat_id, event_key)
        return False


def _account_status_snapshot(chat_id):
    """لقطة خفيفة لحالة اشتراك المستخدم، مستقلة عن قرارات التداول."""
    if is_admin_chat(chat_id):
        return {
            "plan": "VIP", "label": "VIP — إدارة", "status": "admin",
            "status_ar": "👑 حساب إداري", "expiry": None, "remaining": None,
            "grant": None,
        }
    _reconcile_admin_free_grant(chat_id)
    row = get_member(chat_id)
    ...
    if not row:
        return {
            "plan": "FREE", "label": "مجانية", "status": "active",
            "status_ar": "🟢 فعال", "expiry": None, "remaining": None, "grant": None,
        }
    plan = str(row["plan"] or "FREE").upper()
    status_value = str(row["status"] or "active").lower()
    labels = {
        "FREE": "FREE — مجانية",
        "BASIC": "BASIC",
        "PRO": "PRO",
    ...
    grant_label = None
    if grant:
        grant_label = "FREE+1 — BASIC مجانية" if grant.get("plan") == "BASIC" else "FREE+2 — ELITE مجانية"
    return {
        "plan": plan,
        "label": grant_label or labels.get(plan, plan),
        "status": status_value,
        "status_ar": status_labels.get(status_value, f"ℹ️ {status_value}"),
        "expiry": expiry,
        "remaining": remaining,
        "grant": grant,
    }
...
async def _account_status_text(chat_id):
    snap = await asyncio.to_thread(_account_status_snapshot, int(chat_id))
    lines = ["👤 حالة الحساب"]
    lines.append(f"• الباقة: {snap['label']}")
    lines.append(f"• الحالة: {snap['status_ar']}")
    if snap.get("grant"):
        lines.append("• النوع: 🎁 منحة مجانية مؤقتة")
    if snap.get("expiry"):
        lines.append(f"• الانتهاء: {snap['expiry'].strftime('%Y-%m-%d %H:%M')} دمشق")
        lines.append(f"• المتبقي: {_format_remaining(snap.get('remaining'))}")
    else:
        lines.append("• الصلاحية: لا يوجد تاريخ انتهاء")
    return "\n".join(lines)


async def status(update, context):
    if not await feature_guard("status")(update, context):
        return
    chat_id = int(update.effective_chat.id)
    await asyncio.to_thread(_ensure_user, update)
    account_text = await _account_status_text(chat_id)
    await reply(update, (
        f"🟢 XAU SMART TRADER {VERSION}\n\n"
        "حالة النظام: يعمل\nTelegram: متصل\nFlask: يعمل\nالبيانات: Biquote OHLC\n"
        "التحليل: W1/D1/H4/H1/M15\nالهيكل: مفعّل\nالحجم: مفعّل\nRSI: مفعّل\nMACD: مفعّل\nADX: مفعّل\nFibonacci: مفعّل\nFVG: مفعّل\n"
        "الأخبار: تنبيهات تلقائية فقط — لا حجب للتداول\n\n"
        f"🎯 حد الإشارة: {SIGNAL_THRESHOLD} نقطة\n"
        f"🔥 الإشارة القوية: {STRONG_THRESHOLD} نقطة\n"
        "📝 التقرير التوضيحي: يومي + أسبوعي\n\n"
        + account_text
    ))


async def _notify_account_event(chat_id, event_key, text):
    try:
        await _send_account_notification(int(chat_id), event_key, text)
    except Exception:
        logger.exception("ACCOUNT_EVENT_NOTIFICATION_FAILED chat_id=%s event=%s", chat_id, event_key)


def _account_notification_snapshot(now):
    """كل SQLite I/O الخاص بالـ scheduler في دالة sync واحدة تُنفذ خارج Event Loop."""
    _ensure_account_notification_table()
    _ensure_admin_free_grant_table()
    conn = _db()
    try:
        if ADMIN_IDS:
            rows = conn.execute(
                "SELECT chat_id,plan,status,expiry_date FROM users WHERE chat_id NOT IN (%s)" % ",".join("?" for _ in ADMIN_IDS),
                tuple(int(x) for x in ADMIN_IDS),
            ).fetchall()
        else:
            rows = conn.execute("SELECT chat_id,plan,status,expiry_date FROM users").fetchall()
        users=[dict(r) for r in rows]
        grants=conn.execute(
            f"SELECT id,chat_id,plan,expiry_date FROM {ADMIN_FREE_GRANT_TABLE} WHERE active=0 AND expiry_date<=?",
            (now.isoformat(),),
        ).fetchall()
        return users,[dict(r) for r in grants]
    finally:
        conn.close()


async def _account_notification_scheduler():
    """مراقب خفيف لحالة الاشتراكات؛ لا ينفذ SQLite داخل Event Loop."""
    global ACCOUNT_NOTIFICATION_TASK
    try:
        while True:
            try:
                now = now_damascus()
                users, grants = await asyncio.to_thread(_account_notification_snapshot, now)

                for g in grants:
                    cid = int(g["chat_id"])
                    plan_label = "BASIC" if g["plan"] == "BASIC" else "ELITE"
                    await _notify_account_event(
                        cid,
                        f"grant_expired:{g['id']}:{g['expiry_date']}",
                        f"⌛ انتهت المنحة المجانية\n\n• الباقة: FREE+{'1' if plan_label == 'BASIC' else '2'} ({plan_label})\n• انتهت: {str(g['expiry_date'])[:16].replace('T',' ')} دمشق\n\nتمت إعادة حسابك إلى حالته السابقة تلقائيًا.",
                    )
                ...
                for row in users:
                    ...
                    if status_value == "expired":
                        await _notify_account_event(
                            cid,
                            f"subscription_expired:{expiry_raw or 'unknown'}",
                            "⌛ انتهت مدة حسابك\n\nتمت إعادة حسابك إلى الحالة المجانية. يمكنك طلب الترقية من قسم الباقات.",
                        )
                        continue
                    if status_value != "active" or not expiry_raw:
                        continue
                    try:
                        expiry = datetime.fromisoformat(str(expiry_raw))
                        if expiry.tzinfo is None:
                    ...
                    if remaining <= 0:
                        continue
                    thresholds = [(7*86400, "7d"), (3*86400, "3d"), (86400, "1d"), (6*3600, "6h")]
                    for threshold, code in thresholds:
                        if remaining <= threshold:
                            await _notify_account_event(
                                cid,
                                f"expiry_warning:{expiry.isoformat()}:{code}",
                                f"⚠️ تنبيه انتهاء الحساب\n\n• الباقة: {row.get('plan') or '—'}\n• المتبقي: {_format_remaining(remaining)}\n• الانتهاء: {expiry.strftime('%Y-%m-%d %H:%M')} دمشق\n\nيمكنك التجديد من قسم 💳 الباقات.",
                            )
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("ACCOUNT_NOTIFICATION_SCHEDULER_ERROR")
            await asyncio.sleep(max(300, ACCOUNT_NOTIFICATION_INTERVAL))
    finally:
        ACCOUNT_NOTIFICATION_TASK = None


async def _start_account_notification_scheduler():
    global ACCOUNT_NOTIFICATION_TASK
    if ACCOUNT_NOTIFICATION_TASK and not ACCOUNT_NOTIFICATION_TASK.done():
        return
    ACCOUNT_NOTIFICATION_TASK = asyncio.create_task(
        _account_notification_scheduler(), name="account-lifecycle-notifications"
    )


async def _stop_account_notification_scheduler():
    global ACCOUNT_NOTIFICATION_TASK
    task = ACCOUNT_NOTIFICATION_TASK
    ACCOUNT_NOTIFICATION_TASK = None
...
# زر مستقل لإظهار ID الخاص بالمستخدم دون كشف أي بيانات أخرى.
async def account_id_button(update, context):
    await user_id_command(update, context)


_prev_ux_main_keyboard_v19_17 = _ux_main_keyboard

def _ux_main_keyboard_v19_17():
    kb = [list(row) for row in _prev_ux_main_keyboard_v19_17()]
    if not any("🆔 ID الخاص بحسابك" in cell for row in kb for cell in row):
        kb.append(["🆔 ID الخاص بحسابك"])
    return kb

_ux_main_keyboard = _ux_main_keyboard_v19_17

_prev_router_v19_17 = router
async def router(update, context):
    text = (update.effective_message.text or "").strip() if getattr(update, "effective_message", None) else ""
    if text == "🆔 ID الخاص بحسابك":
        await account_id_button(update, context)
        return
    await _prev_router_v19_17(update, context)


_prev_admin_callback_router_v19_17 = callback_router
async def callback_router(update, context):
    """إضافة إشعارات دورة حياة الحساب مع الحفاظ على موجه الإدارة السابق."""
    data = getattr(getattr(update, "callback_query", None), "data", "") or ""
    if data in {"v19_13_cancel_grant", "v19_12_cancel_grant", "v19_11_cancel_grant"}:
        cid = context.user_data.get("admin_selected_user")
        await _prev_admin_callback_router_v19_17(update, context)
        if cid:
            await _notify_account_event(int(cid), f"grant_cancelled:{now_damascus().strftime('%Y%m%d%H%M%S')}",
                                        "❌ تم إلغاء المنحة المجانية\n\nتمت إعادة حسابك إلى الحالة السابقة.")
        return
    if data in {"v19_13_cancel_subscription", "v19_12_cancel_subscription", "v19_11_cancel_subscription"}:
        cid = context.user_data.get("admin_selected_user")
        await _prev_admin_callback_router_v19_17(update, context)
        if cid:
            await _notify_account_event(int(cid), f"subscription_cancelled:{now_damascus().strftime('%Y%m%d%H%M%S')}",
                                        "❌ تم إلغاء اشتراك حسابك\n\nأصبحت حالتك الحالية: FREE. يمكنك طلب الترقية من قسم الباقات.")
        return
    if data in {"v19_13_block", "v19_12_block", "v19_11_block", "v19_13_unblock", "v19_12_unblock", "v19_11_unblock"}:
        cid = context.user_data.get("admin_selected_user")
        blocked = data.endswith("block") and not data.endswith("unblock")
        await _prev_admin_callback_router_v19_17(update, context)
        if cid:
            event = "account_blocked" if blocked else "account_unblocked"
            text = "⛔ تم حظر حسابك من قبل الإدارة.\n\nتم إيقاف الوصول إلى خدمات الحساب حتى فك الحظر." if blocked else "🟢 تم فك حظر حسابك.\n\nأصبح بإمكانك استخدام خدمات حسابك مجددًا وفق باقتك الحالية."
            await _notify_account_event(int(cid), f"{event}:{now_damascus().strftime('%Y%m%d%H%M%S')}", text)
        return
    await _prev_admin_callback_router_v19_17(update, context)

# إشعار فوري عند منح BASIC/ELITE المجانية.
_prev_legacy_router_v19_17 = router
async def router(update, context):
    text = (update.effective_message.text or "").strip() if getattr(update, "effective_message", None) else ""
    mode = context.user_data.get("admin_mode") if getattr(context, "user_data", None) is not None else None
    if text == "🆔 ID الخاص بحسابك":
        await account_id_button(update, context)
        return
    if mode in {"grant_free_plan", "grant_free_elite_days"} and text.isdigit() and int(text) > 0:
        cid = context.user_data.get("admin_selected_user")
        grant_plan = context.user_data.get("admin_grant_plan", "BASIC") if mode == "grant_free_plan" else "ELITE"
        result = None
        try:
            # تنفيذ نفس مسار المنحة القائم، ثم إرسال الإشعار للمشترك.
            grant_fn = _grant_free_plan if mode == "grant_free_plan" else _grant_free_elite
            result = await asyncio.to_thread(grant_fn, cid, int(text), update.effective_user.id, grant_plan) if mode == "grant_free_plan" and cid else (await asyncio.to_thread(grant_fn, cid, int(text), update.effective_user.id) if cid else None)
        except Exception:
            # لا نكرر رسالة الخطأ هنا؛ الموجه الأصلي يتولى المسار.
            result = None
        if result:
            context.user_data.pop("admin_mode", None)
            context.user_data.pop("admin_grant_plan", None)
            label = "FREE+1 — BASIC" if grant_plan == "BASIC" else "FREE+2 — ELITE"
            expiry = result.get("expiry")
            expiry_text = expiry.strftime('%Y-%m-%d %H:%M') if hasattr(expiry, 'strftime') else str(expiry)[:16].replace('T',' ')
            await reply(update, with_market_header(
                f"✅ تم منح {grant_plan} مجانًا بنجاح\n\n• Chat ID: {cid}\n• المدة: {int(text)} يوم\n• تنتهي: {expiry_text} دمشق"
            ))
            await _notify_account_event(int(cid), f"grant_received:{grant_plan}:{expiry}",
                                        f"🎁 تم منحك {label} مجانًا\n\n• المدة: {int(text)} يوم\n• تنتهي: {expiry_text} دمشق\n\nتم تفعيل المزايا تلقائيًا على حسابك.")
            return
    await _prev_legacy_router_v19_17(update, context)


# إعادة تعريف الإقلاع النهائي لإضافة مراقب دورة حياة الحساب فقط.
_prev_integrated_start_bot_v19_17 = integrated_start_bot
async def integrated_start_bot():
    await _prev_integrated_start_bot_v19_17()
    await _start_account_notification_scheduler()

# ============================================================
# v19.19 — إصلاح ظهور زر ID في لوحة المشترك
# السبب: دالة start الفعالة تستخدم _contract_main_keyboard،
# بينما v19.18 عدّلت _ux_main_keyboard فقط.
# هذا التعديل لا يغيّر منطق التداول أو الاشتراكات أو التنبيهات.
# ============================================================
_prev_contract_main_keyboard_v19_19 = _contract_main_keyboard

def _contract_main_keyboard_v19_19(chat_id=None):
    rows = [list(row) for row in _prev_contract_main_keyboard_v19_19(chat_id)]
    if not any("🆔 ID الخاص بحسابك" in cell for row in rows for cell in row):
        rows.append(["🆔 ID الخاص بحسابك"])
    return rows

_contract_main_keyboard = _contract_main_keyboard_v19_19



# ============================================================
# v19.20 — FREE SERVICE ONE-TIME QUOTAS
# Scope: FREE users only, four services only.
# No change to trade quotas, trading logic, S/R strategy, or paid plans.
# ============================================================
VERSION = "v19.20_FREE_SERVICE_QUOTAS_FIXED"

_FREE_SERVICE_QUOTAS = {
...
def _free_service_usage_count(chat_id, feature):
    if is_admin_chat(chat_id):
        return 0
    feature = _canonical_feature(feature)
    if feature not in _FREE_SERVICE_QUOTAS:
        return 0
    conn = _db()
    try:
        return int(conn.execute(
            "SELECT COUNT(*) FROM usage WHERE chat_id=? AND feature=?",
            (chat_id, "free:" + _FREE_SERVICE_QUOTAS[feature])
        ).fetchone()[0])
    finally:
        conn.close()


def _consume_free_service_once(chat_id, feature):
    """Atomically claim the single lifetime FREE allowance for one service."""
    if is_admin_chat(chat_id):
        return True
    feature = _canonical_feature(feature)
    if feature not in _FREE_SERVICE_QUOTAS:
    ...
    if plan not in PLANS:
        plan = 'FREE'
    # Paid plans and active FREE+ grants are not subject to FREE service quotas.
    if plan != 'FREE':
        return True
    try:
        if _active_admin_free_grant(chat_id):
            return True
    except Exception:
        pass
    conn = _db()
    try:
        conn.execute("BEGIN IMMEDIATE")
        marker = "free:" + _FREE_SERVICE_QUOTAS[feature]
        exists = conn.execute(
            "SELECT 1 FROM usage WHERE chat_id=? AND feature=? LIMIT 1",
            (chat_id, marker)
        ).fetchone()
        if exists:
            conn.rollback()
        ...
        conn.execute(
            "INSERT INTO usage(chat_id, feature, used_at) VALUES(?,?,?)",
            (chat_id, marker, now_damascus().isoformat())
        )
        conn.commit()
        return True
    except Exception:
...
async def _free_quota_guard(update, feature):
    """Permission + one-time FREE quota guard, with paid/granted plans untouched."""
    await asyncio.to_thread(_ensure_user, update)
    chat_id = update.effective_chat.id
    user_id = update.effective_user.id if update.effective_user else chat_id
    if is_admin_chat(user_id):
        return True
    canonical = _canonical_feature(feature)
    if not await asyncio.to_thread(has_feature, chat_id, canonical):
        await reply(update, "🔒 هذه الميزة غير متاحة ضمن باقتك الحالية.\n\n💳 استخدم /plans لمعرفة الباقات.")
        return False
    row = await asyncio.to_thread(get_member, chat_id)
    plan = row['plan'] if row else 'FREE'
    if plan not in PLANS:
        plan = 'FREE'
    if plan != 'FREE':
    ...
    try:
        ...
        pass
    claimed = await asyncio.to_thread(_consume_free_service_once, chat_id, canonical)
    if claimed:
        return True
    labels = {
        "quick_analysis": "⚡ التحليل السريع",
        "daily_analysis": "🧭 القراءة اليومية للسوق",
        "weekly_analysis": "🧭 القراءة الأسبوعية للسوق",
        "sr": "💧 السيولة والدعم والمقاومة",
    }
    label = labels.get(canonical, "هذه الخدمة")
    await reply(update, f"ℹ️ استنفدت استخدامك المجاني لـ {label}.\n\n💳 يمكنك الترقية إلى باقة مدفوعة لاستخدام الخدمة دون هذا القيد.")
    return False


# Wrap only the four FREE-quota services. The wrappers preserve their
# original function bodies and therefore do not alter trading/analysis logic.
_v19_19_quick_analysis = quick_analysis
async def quick_analysis(update, context):
    if not await _free_quota_guard(update, "quick_analysis"):
        return
    return await _v19_19_quick_analysis(update, context)

_v19_19_daily_analysis = daily_analysis
async def daily_analysis(update, context):
    if not await _free_quota_guard(update, "daily_analysis"):
        return
    return await _v19_19_daily_analysis(update, context)

_v19_19_weekly_analysis = weekly_analysis
async def weekly_analysis(update, context):
    if not await _free_quota_guard(update, "weekly_analysis"):
        return
    return await _v19_19_weekly_analysis(update, context)

_v19_19_show_levels = show_levels
async def show_levels(update, context):
    if not await _free_quota_guard(update, "sr"):
        return
    return await _v19_19_show_levels(update, context)


# ============================================================
# v19.21 — FREE S/R QUOTA ROUTING FIX
# Scope: fix the actual main-menu S/R route (liquidity_menu).
# v19.20 guarded show_levels(), but the production main menu routes
# "💧 السيولة والدعم" to liquidity_menu().
# No trading/SR calculation logic is changed.
# ============================================================
VERSION = "v19.21_FREE_SR_QUOTA_ROUTING_FIXED"

_v19_20_liquidity_menu = liquidity_menu
async def liquidity_menu(update, context):
    if not await _free_quota_guard(update, "sr"):
        return
    return await _v19_20_liquidity_menu(update, context)


# ============================================================
# v19.22 — USER UX / AUTO TRADE CLASSIC FORMAT ONLY
# Scope: requested UI and notification presentation changes only.
# Preserves v19.21 FREE service quotas exactly.
# Does NOT change entry/SL/TP rules, scoring logic, S/R engine,
# G.Y calculation/strategy, trade quotas, or subscription grants.
# ============================================================
VERSION = "v19.22_USER_UX_AUTO_TRADE_CLASSIC_FIXED"

# 1) Final main keyboard contract:
#    - ID button: subscribers only, never admin.
#    - G.Y private button: admin only.
#    - Support button: available to all accounts.
_prev_contract_main_keyboard_v19_22 = _contract_main_keyboard

def _contract_main_keyboard(chat_id=None):
    rows = [list(row) for row in _prev_contract_main_keyboard_v19_22(chat_id)]
    admin = False
    try:
    ...
    cleaned = []
    for row in rows:
        row = [cell for cell in row if cell not in {
            "🆔 ID الخاص بحسابك",
            "🎯 صفقة G.Y الخاصة",
            "🆘 الدعم",
        }]
    ...
    if admin:
        # الإدارة لا ترى زر الدعم.
        rows.append(["🎯 صفقة G.Y الخاصة"])
    else:
        rows.append(["🆔 ID الخاص بحسابك"])
        rows.append(["🆘 الدعم"])
    return rows

# 2) Account ID: include the account name and the exact requested instruction.
async def user_id_command(update, context):
    chat = getattr(update, "effective_chat", None)
    message = getattr(update, "effective_message", None)
    user = getattr(update, "effective_user", None)
    ...
    last_name = str(getattr(user, "last_name", "") or "").strip() if user else ""
    account_name = " ".join(x for x in (first_name, last_name) if x).strip() or "غير متوفر"
    await message.reply_text(
        "🆔 معرف حسابك في Telegram\n\n"
        f"👤 الاسم: {account_name}\n"
        f"🔢 الرقم: `{chat_id}`\n\n"
        "📌 أرسل هذا الرقم مع الاسم إلى قسم الدعم عند طلب تفعيل أو منحة اشتراك.",
        parse_mode="Markdown"
    )

# 3) Support button / route. No trading or subscription mutation is performed here.
def _support_contact_url(contact):
    """تحويل ADMIN_CONTACT إلى رابط Telegram، أو استخدام معرف الإدارة الموجود في ADMIN_IDS."""
    value = str(contact or "").strip()
    if value:
        if value.startswith(("https://t.me/", "http://t.me/", "https://telegram.me/", "http://telegram.me/")):
            return value
        username = value[1:].strip() if value.startswith("@") else value
        if username and all(ch.isalnum() or ch == "_" for ch in username):
            return f"https://t.me/{username}"

    # لا نعتمد على ADMIN_CONTACT وحده؛ بيانات الإدارة موجودة فعلياً في ADMIN_IDS.
    # رابط tg:// يفتح محادثة حساب الإدارة مباشرة عندما لا يوجد username مهيأ.
    try:
        admin_id = next(iter(sorted(ADMIN_IDS)))
        if int(admin_id) > 0:
...
async def support_button(update, context):
    ...
    if is_admin_chat(chat_id):
        await reply(update, "⛔ قسم الدعم غير متاح لحسابات الإدارة.")
        return

    contact = ADMIN_CONTACT
    contact_url = _support_contact_url(contact)
    if contact_url:
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("📩 التواصل مع الدعم", url=contact_url)]])
        contact_line = "📩 يمكنك التواصل مباشرةً مع قسم الدعم عبر الزر أدناه."
    else:
        keyboard = None
        contact_line = "📩 لا توجد وسيلة تواصل مع الإدارة مهيأة حالياً."

    await update.effective_message.reply_text(
        with_market_header(        "🆘 قسم الدعم\n\n"
        "للطلبات والاستفسارات المتعلقة بالحساب والاشتراكات والمنح المجانية.\n\n"
        "📌 عند طلب تفعيل أو منحة اشتراك، أرسل الاسم ورقم ID الخاص بحسابك.\n\n"
        f"{contact_line}",
        ),
...
# 4) G.Y manual button remains available to administration only.
_prev_sr_button_v19_22 = sr_button
async def sr_button(update, context):
    chat_id = getattr(getattr(update, "effective_chat", None), "id", None)
    user_id = getattr(getattr(update, "effective_user", None), "id", chat_id)
    if not (is_admin_chat(chat_id) or is_admin_chat(user_id)):
        await reply(update, "⛔ هذه الخدمة مخصصة للإدارة فقط.")
        return
    return await _prev_sr_button_v19_22(update, context)

# 5) Classic automatic trade message: direction + entry + SL + targets + R:R + quality.
#    No explanatory analysis/factors/retest text is included.
def _format_trade_message(result, title="🚨 إشارة ذهب", record=None):
    result = result or {}
    trade = record or result.get("trade") or {}
    direction_code = str(result.get("direction") or trade.get("direction") or "").upper()
    direction = "🟢 شراء" if direction_code == "BUY" else "🔴 بيع" if direction_code == "SELL" else "⚪ غير محدد"
    score = int(max(0, min(100, sf(result.get("score", trade.get("score", 0)), 0))))
    quality, quality_icon = trade_quality(score)
    status = str(trade.get("status", "NEW"))
    status_ar = {"NEW":"جديدة", "ACTIVE":"نشطة", "TP1":"TP1 تحقق", "TP1_HIT":"TP1 تحقق", "TP2":"TP2 تحقق", "TP2_HIT":"TP2 تحقق", "CLOSED":"مغلقة"}.get(status, status)
    ...
    lines = [
        ...
        "",
        f"{direction} — XAU/USD",
        f"⭐ جودة الصفقة: {score}/100 — {quality_icon} {quality}",
        f"💰 الدخول: {num('entry'):.2f}",
        f"🛑 وقف الخسارة: {num('sl'):.2f}",
        f"🎯 الهدف 1: {num('tp1'):.2f}",
        f"🎯 الهدف 2: {num('tp2'):.2f}",
        f"🎯 الهدف 3: {num('tp3'):.2f}",
        f"⚖️ R:R: 1:{num('rr'):.2f}",
    ]
...
# 6) Automatic G.Y notifications use the same classic trade format.
def _sr_gy_alert_payload(trade, snapshot):
    score = sf(trade.get("score", 0), 0)
    quality, quality_icon = trade_quality(int(max(0, min(100, round(score)))))
    direction_code = str(trade.get("direction", "")).upper()
    direction = "🟢 شراء" if direction_code == "BUY" else "🔴 بيع" if direction_code == "SELL" else "⚪ غير محدد"
    setup_id = trade.get("_setup_id", "PENDING")
    return (
        "🚨 صفقة G.Y الخاصة\n\n"
        f"{direction} — XAU/USD\n"
        f"⭐ جودة الصفقة: {int(round(score))}/100 — {quality_icon} {quality}\n"
        f"💰 الدخول: {sf(trade.get('entry'), 0):.2f}\n"
        f"🛑 وقف الخسارة: {sf(trade.get('sl'), 0):.2f}\n"
        f"🎯 الهدف 1: {sf(trade.get('tp1'), 0):.2f}\n"
        f"🎯 الهدف 2: {sf(trade.get('tp2'), 0):.2f}\n"
        f"🎯 الهدف 3: {sf(trade.get('tp3'), 0):.2f}\n"
        f"⚖️ R:R: 1:{sf(trade.get('rr'), 0):.2f}\n\n"
        f"🆔 {setup_id}"
    )

# 8) Remove the obsolete subscriber-facing G.Y mention from the plan description.
_prev_plans_text_v19_22 = plans_text
def plans_text():
    text = _prev_plans_text_v19_22()
    return text.replace(" + 🎯 صفقة G.Y الخاصة", "").replace(" + 🎯 صفقة G.Y الخاصة +", " +")


# 7) Final text router: preserve existing routing while adding the new UI routes.
_prev_router_v19_22 = router
async def router(update, context):
    text = (update.effective_message.text or "").strip() if getattr(update, "effective_message", None) else ""
    if text == "🆘 الدعم":
    ...
    if text == "🎯 صفقة G.Y الخاصة":
        ...
        await sr_button(update, context)
        return
    return await _prev_router_v19_22(update, context)




# ============================================================
# v19.26 — FINAL COMMERCIAL PACKAGES / DAILY QUOTA / MEMBER & ADMIN UX
# Scope: commercial contract, package presentation, daily trade quota,
#         G.Y Elite access, subscription request lifecycle, account UX,
#         admin dashboard organization, and account-notification startup.
# Explicitly does NOT alter trading/entry/SL/TP/scoring/S/R calculation logic.
# ============================================================
VERSION = "v19.27_SUBSCRIPTION_ADMIN_MENU_UX_FIXED"

# ---------- 1) Canonical commercial contract ----------
PLANS = {
    "FREE": {
        "name": "🆓 FREE — المجانية",
        "price": 0,
        ...
        "trade_period": "daily",
        "features": {
            "gold_price", "markets", "status", "quick_analysis", "sr",
            "daily_analysis", "weekly_analysis", "trade_history",
        },
    },
    "BASIC": {
        "name": "🟢 BASIC — الأساسية",
        ...
        "features": {
            "gold_price", "markets", "status", "quick_analysis", "sr",
            "daily_analysis", "weekly_analysis", "trade_history",
            "trade_alerts", "news_alerts", "news_risk_status",
        },
    },
    "ELITE": {
        "name": "🟣 ELITE — النخبة",
        ...
        "features": {
            "gold_price", "markets", "status", "quick_analysis", "sr",
            "daily_analysis", "weekly_analysis", "trade_history",
            "full_analysis", "institutional", "gy_private_trade",
            "trade_alerts", "news_alerts", "news_risk_status",
        },
    },
}
...
TRADE_LIMIT_TEXT = {
    "FREE": "صفقة واحدة يوميًا",
    "BASIC": "5 صفقات يوميًا",
    "ELITE": "جميع الصفقات دون حد يومي",
}

PLAN_DESCRIPTIONS = {
    "FREE": (
        "مناسبة لتجربة النظام يوميًا.\n\n"
        "• 🎯 صفقة واحدة يوميًا\n"
        "• ⚡ التحليل السريع\n"
        "• 🧭 القراءة اليومية للسوق\n"
        "• 🧭 القراءة الأسبوعية للسوق\n"
    ...
    "BASIC": (
        "مناسبة للاستخدام المنتظم ومتابعة السوق يوميًا.\n\n"
        "• 🎯 حتى 5 صفقات يوميًا\n"
        "• ⚡ التحليل السريع\n"
        "• 🧭 القراءة اليومية للسوق\n"
        "• 🧭 القراءة الأسبوعية للسوق\n"
    ...
    "ELITE": (
        "الباقة الكاملة للمستخدم المتقدم.\n\n"
        "• 🎯 جميع الصفقات دون حد يومي\n"
        "• ⭐ صفقة G.Y الخاصة — حصرية للنخبة\n"
        "• 🏦 التحليل المؤسسي — حصري للنخبة\n"
        "• ⚡ التحليل السريع\n"
        "• 🧭 القراءة اليومية للسوق\n"
        "• 🧭 القراءة الأسبوعية للسوق\n"
        "• 💧 تحليل مستويات الدعم والمقاومة\n"
        "• 📜 سجل الصفقات الكامل\n"
        "• 📊 إحصائيات التداول\n"
        "• 📑 التقارير والمزايا المتقدمة"
    ),
}

PLAN_PERIOD_PRICES = {
    "BASIC": {"monthly": 15, "3_months": 39, "6_months": 69},
    "ELITE": {"monthly": 39, "3_months": 99, "6_months": 179},
}


def _v1926_plan_key(chat_id):
    if is_admin_chat(chat_id):
        return "ADMIN"
    row = get_member(chat_id)
    plan = str(row["plan"] if row else "FREE").upper()
    return plan if plan in PLANS else "FREE"


def _v1926_effective_grant(chat_id):
    try:
        _reconcile_admin_free_grant(chat_id)
        grant = _active_admin_free_grant(chat_id)
        if grant and grant.get("plan") in {"BASIC", "ELITE"}:
        ...
        logger.exception("v19.26 grant reconciliation error")
    return None


# ---------- 2) Daily trade quota, atomic and concurrency-safe ----------
def _v1926_day_start():
    now = now_damascus()
...
def usage_count(chat_id, plan=None):
    if is_admin_chat(chat_id):
        return 0
    grant = _v1926_effective_grant(chat_id)
    row = get_member(chat_id)
    effective_plan = (grant.get("plan") if grant else (row["plan"] if row else "FREE"))
    effective_plan = str(effective_plan).upper()
    if effective_plan not in PLANS:
        effective_plan = "FREE"
    day_start = _v1926_day_start().isoformat()
    conn = _db()
    try:
        return int(conn.execute(
            "SELECT COUNT(*) FROM usage WHERE chat_id=? AND feature='trade' AND used_at>=?",
            (int(chat_id), day_start),
        ).fetchone()[0])
    finally:
        conn.close()
...
def trade_quota(chat_id):
    ...
    grant = _v1926_effective_grant(chat_id)
    row = get_member(chat_id)
    plan = str(grant.get("plan") if grant else (row["plan"] if row else "FREE")).upper()
    if plan not in PLANS:
        plan = "FREE"
    return plan, usage_count(chat_id, plan), PLANS[plan]["trade_limit"]
...
def consume_trade(chat_id):
    if is_admin_chat(chat_id):
        return True
    token = _reserve_trade_quota(chat_id)
    return bool(token)


def _reserve_trade_quota(chat_id):
    """Atomic daily reservation. One SQLite transaction is the quota gate."""
    if is_admin_chat(chat_id):
        return {"chat_id": int(chat_id), "reserved": True, "admin": True}
    conn = None
    cid = int(chat_id)
    now = now_damascus()
    ...
    try:
        ...
        conn.execute("BEGIN IMMEDIATE")
        row = conn.execute(
            "SELECT plan,status,expiry_date FROM users WHERE chat_id=?", (cid,)
        ).fetchone()
        if not row:
            conn.rollback()
            return None
        ...
        plan = str(row["plan"] or "FREE").upper()
        # Active administrative grant overrides the stored commercial plan.
        grant = conn.execute(
            f"SELECT plan,expiry_date FROM {ADMIN_FREE_GRANT_TABLE} "
            "WHERE chat_id=? AND active=1 ORDER BY id DESC LIMIT 1", (cid,)
        ).fetchone()
        if grant:
            try:
                gexp = datetime.fromisoformat(str(grant["expiry_date"]))
                if gexp.tzinfo is None:
                    gexp = gexp.replace(tzinfo=DAMASCUS)
                if gexp > now:
                    plan = str(grant["plan"] or "FREE").upper()
            except Exception:
                pass
        ...
        if limit is None:
            conn.commit()
            return {"chat_id": cid, "reserved": True, "admin": False, "unlimited": True}
        ...
        conn.commit()
        return {"chat_id": cid, "reserved": True, "admin": False, "usage_id": cur.lastrowid}
    except Exception:
        if conn:
            try:
                conn.rollback()
                ...
                pass
        logger.exception("v19.26 atomic daily trade quota error for %s", cid)
        return None
    finally:
        if conn:
            conn.close()
...
# ---------- 3) Account status / package presentation ----------
def plan_status_text(chat_id):
    if is_admin_chat(chat_id):
        return (
            "👑 الإدارة — صلاحية مستقلة عن الاشتراك\n"
            "🎯 الصفقات: غير محدودة\n"
            "🏦 التحليل المؤسسي: متاح\n"
            "⭐ G.Y الخاصة: متاحة"
        )
    grant = _v1926_effective_grant(chat_id)
    row = get_member(chat_id)
    ...
    used = usage_count(chat_id, plan)
    limit = info["trade_limit"]
    remaining = "♾️ غير محدودة" if limit is None else str(max(0, int(limit) - used))
    expiry = grant.get("expiry_date") if grant else (row["expiry_date"] if row else None)
    expiry_text = str(expiry)[:16].replace("T", " ") + " دمشق" if expiry else "لا يوجد انتهاء — مجانية"
    grant_line = "\n🎁 النوع: منحة مجانية" if grant else ""
    return (
        f"{info['name']}\n"
        f"💰 السعر: {'مجاني' if info['price'] == 0 else '$' + str(info['price']) + ' / 30 يومًا'}\n"
        f"🎯 الصفقات اليوم: {remaining}\n"
        f"📌 الحد: {TRADE_LIMIT_TEXT[plan]}\n"
        f"📅 الانتهاء: {expiry_text}"
        f"{grant_line}"
    )
...
def plans_text():
    return (
        ...
        "🏦 التحليل المؤسسي — حصري للنخبة\n"
        "⭐ صفقة G.Y الخاصة — حصرية للنخبة\n\n"
        "🎁 FREE+1 = منحة BASIC مجانية\n"
        "🎁 FREE+2 = منحة ELITE مجانية\n\n"
        "ℹ️ الصفقات المجانية والمدفوعة تُحتسب يوميًا وفق باقتك.\n"
        "ℹ️ سجل الصفقات والتقارير لا تُحذف عند انتهاء الاشتراك."
    )
...
async def plans(update, context):
    await asyncio.to_thread(_ensure_user, update)
    keyboard = [
        [InlineKeyboardButton("🆓 FREE — المجانية", callback_data="plan_FREE")],
        [InlineKeyboardButton("🟢 BASIC — $15", callback_data="plan_BASIC")],
        [InlineKeyboardButton("🟣 ELITE — $39 ⭐", callback_data="plan_ELITE")],
    ]
    text = plans_text() + "\n\n⬇️ اختر الباقة لمعرفة التفاصيل:"
    markup = InlineKeyboardMarkup(keyboard)
    if update.callback_query:
        await update.callback_query.edit_message_text(with_market_header(text), reply_markup=markup)
    else:
        await update.effective_message.reply_text(with_market_header(text), reply_markup=markup)


async def plan_callback(update, context):
    query = update.callback_query
    await query.answer()
    plan_key = str(query.data or "").replace("plan_", "", 1).upper()
    if plan_key not in PLANS:
        await query.edit_message_text(with_market_header("❌ الباقة غير موجودة."))
        return
    plan = PLANS[plan_key]
    body = PLAN_DESCRIPTIONS[plan_key]
    prices = PLAN_PERIOD_PRICES.get(plan_key)
    price_lines = f"💰 السعر: {'مجاني' if plan_key == 'FREE' else '$' + str(plan['price']) + ' / 30 يومًا'}\n"
    if prices:
        price_lines += (
            f"📦 3 أشهر: ${prices['3_months']}\n"
            f"📦 6 أشهر: ${prices['6_months']}\n"
        )
    text = f"{plan['name']}\n{price_lines}🎯 {TRADE_LIMIT_TEXT[plan_key]}\n\n{body}"
    buttons = []
    if plan_key != "FREE":
        buttons.append([InlineKeyboardButton(f"📩 طلب الاشتراك في {plan['name']}", callback_data=f"request_{plan_key}")])
    buttons.append([InlineKeyboardButton("🔙 العودة للباقات", callback_data="back_to_plans")])
    buttons.append([InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")])
    await query.edit_message_text(with_market_header(text), reply_markup=InlineKeyboardMarkup(buttons))


# ---------- 4) Subscriber account menu ----------
async def my_subscription(update, context):
    chat_id = update.effective_chat.id
    await asyncio.to_thread(_ensure_user, update)
    status = await asyncio.to_thread(plan_status_text, chat_id)
    buttons = [
        [InlineKeyboardButton("📋 الباقات", callback_data="member_plans")],
        [InlineKeyboardButton("🆔 ID الخاص بحسابك", callback_data="member_id")],
        [InlineKeyboardButton("🆘 الدعم", callback_data="member_support")],
        [InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")],
    ]
    await update.effective_message.reply_text(
        with_market_header("👤 حسابي\n\n" + status),
        reply_markup=InlineKeyboardMarkup(buttons),
    )


# ---------- 5) G.Y private trade = ELITE only (admin remains allowed) ----------
_v1926_gy_implementation = _prev_sr_button_v19_22
async def sr_button(update, context):
    chat_id = getattr(getattr(update, "effective_chat", None), "id", None)
    user_id = getattr(getattr(update, "effective_user", None), "id", chat_id)
    if is_admin_chat(chat_id) or is_admin_chat(user_id):
        return await _v1926_gy_implementation(update, context)
    await asyncio.to_thread(_ensure_user, update)
    if not await asyncio.to_thread(has_feature, chat_id, "gy_private_trade"):
        await reply(update, "🔒 🎯 صفقة G.Y الخاصة متاحة حصريًا لباقة ELITE.\n\n💳 افتح 📋 الباقات للترقية إلى ELITE.")
        return
    return await _v1926_gy_implementation(update, context)


# ---------- 6) Main keyboard: preserve current layout; only move trade history inside trades
_v1926_prev_contract_main_keyboard = _contract_main_keyboard

def _contract_main_keyboard(chat_id=None):
    rows = [list(r) for r in _v1926_prev_contract_main_keyboard(chat_id)]
    # Remove the history button from the main menu without rearranging the remaining rows.
    cleaned = []
    for row in rows:
        row = [cell for cell in row if cell != "📜 سجل الصفقات"]
        if row:
            cleaned.append(row)
    rows = cleaned
    # G.Y is now a commercial ELITE feature, not a subscriber-hidden admin-only feature.
    if not is_admin_chat(chat_id):
        # Existing v19.22 removed G.Y for subscribers; add it only to ELITE users.
        if _v1926_plan_key(chat_id) == "ELITE" and not any("🎯 صفقة G.Y الخاصة" in c for r in rows for c in r):
            rows.append(["🎯 صفقة G.Y الخاصة"])
    return rows


# ---------- 7) Subscription request lifecycle + duplicate protection ----------
def _ensure_v1926_subscription_request_schema():
    conn = _db()
    try:
        ...
        for col, typ in additions.items():
            if col not in cols:
                conn.execute(f"ALTER TABLE subscription_requests ADD COLUMN {col} {typ}")
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_subscription_requests_pending "
            "ON subscription_requests(chat_id,status,requested_at)"
        )
        conn.commit()
    finally:
        conn.close()
...
def _v1926_pending_request(chat_id):
    _ensure_v1926_subscription_request_schema()
    conn = _db()
    try:
        row = conn.execute(
            "SELECT * FROM subscription_requests WHERE chat_id=? AND status='PENDING' "
...
async def subscription_request_callback(update, context):
    query = update.callback_query
    await query.answer()
    plan_key = str(query.data or "").replace("request_", "", 1).upper()
    if plan_key not in {"BASIC", "ELITE"}:
        await query.edit_message_text(with_market_header("❌ طلب الاشتراك غير صالح."))
        return
    chat_id = int(query.message.chat_id)
    await asyncio.to_thread(_ensure_user, update)
    _ensure_v1926_subscription_request_schema()
    pending = await asyncio.to_thread(_v1926_pending_request, chat_id)
    if pending:
        await query.edit_message_text(with_market_header(
            f"⏳ لديك طلب اشتراك قيد المراجعة بالفعل.\n\n"
            f"📦 الباقة: {PLANS.get(str(pending['plan']).upper(), PLANS['BASIC'])['name']}\n"
            f"🕐 الطلب: {str(pending['requested_at'])[:16].replace('T',' ')} دمشق\n\n"
    ...
    try:
        conn.execute(
            "INSERT INTO subscription_requests(chat_id,plan,requested_at,status) VALUES(?,?,?,'PENDING')",
            (chat_id, plan_key, now),
        )
        conn.commit()
    finally:
        conn.close()
    ...
    contact = ADMIN_CONTACT
    contact_text = f"\n\n📩 التواصل مع الدعم: {contact}" if contact else ""
    await query.edit_message_text(with_market_header(
        f"📩 طلب الاشتراك — {PLANS[plan_key]['name']}\n\n"
        f"💰 السعر: ${PLANS[plan_key]['price']} / 30 يومًا\n"
        f"🎯 {TRADE_LIMIT_TEXT[plan_key]}\n\n"
        "✅ تم تسجيل طلبك بنجاح.\n"
        "⏳ سيبقى الطلب قيد المراجعة حتى تأكيد الإدارة."
        f"{contact_text}"
    ))


# ---------- 8) Admin subscription-request center ----------
async def admin_subscription_requests(update, context):
    if not await _admin_guard(update):
        return
    _ensure_v1926_subscription_request_schema()
    conn = _db()
    try:
        rows = conn.execute(
            "SELECT id,chat_id,plan,requested_at FROM subscription_requests "
            "WHERE status='PENDING' ORDER BY requested_at ASC LIMIT 30"
        ).fetchall()
    finally:
        conn.close()
    if not rows:
        text = "📝 طلبات الاشتراك\n\nلا توجد طلبات معلقة حاليًا."
        buttons = [[InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_home")]]
        await _admin_render(update, text, buttons)
        return
    lines = ["📝 طلبات الاشتراك", ""]
    buttons = []
    for r in rows:
        plan = str(r["plan"]).upper()
        label = "🟢 BASIC" if plan == "BASIC" else "🟣 ELITE"
        lines.append(f"#{r['id']} — {label} — Chat ID: {r['chat_id']} — {str(r['requested_at'])[:16].replace('T',' ')}")
        buttons.append([InlineKeyboardButton(f"#{r['id']} {label}", callback_data=f"admin_req_view:{r['id']}")])
    buttons.append([InlineKeyboardButton("🔙 لوحة الإدارة", callback_data="admin_home")])
    await _admin_render(update, "\n".join(lines), buttons)


async def _admin_request_view(update, context, request_id):
    if not await _admin_guard(update):
        return
    _ensure_v1926_subscription_request_schema()
    conn = _db()
    try:
        row = conn.execute("SELECT * FROM subscription_requests WHERE id=?", (int(request_id),)).fetchone()
    finally:
        conn.close()
    if not row:
        await reply(update, "❌ لم يتم العثور على طلب الاشتراك.")
        return
    plan = str(row["plan"]).upper()
    text = (
        f"📝 طلب اشتراك #{row['id']}\n\n"
        f"🆔 Chat ID: {row['chat_id']}\n"
        f"📦 الباقة: {PLANS.get(plan, PLANS['BASIC'])['name']}\n"
        f"💰 السعر: ${PLANS.get(plan, PLANS['BASIC'])['price']} / 30 يومًا\n"
        f"🕐 الطلب: {str(row['requested_at'])[:16].replace('T',' ')} دمشق\n"
        f"📌 الحالة: {row['status']}"
    )
    buttons = []
    if row["status"] == "PENDING":
        buttons.append([
            InlineKeyboardButton("✅ اعتماد 30 يومًا", callback_data=f"admin_req_approve:{row['id']}"),
            InlineKeyboardButton("❌ رفض", callback_data=f"admin_req_reject:{row['id']}"),
        ])
    buttons.append([InlineKeyboardButton("🔙 طلبات الاشتراك", callback_data="admin_subscription_requests")])
    await _admin_render(update, text, buttons)


async def _admin_request_approve(update, context, request_id):
    if not await _admin_guard(update):
        return
    _ensure_v1926_subscription_request_schema()
    conn = _db()
    try:
        ...
        if not row or row["status"] != "PENDING":
            await reply(update, "ℹ️ الطلب غير معلق أو غير موجود.")
            return
        plan = str(row["plan"]).upper()
        if plan not in {"BASIC", "ELITE"}:
            await reply(update, "❌ الباقة المرتبطة بالطلب غير صالحة.")
            return
        cid = int(row["chat_id"])
        now = now_damascus()
        expiry = now + timedelta(days=30)
        existing = conn.execute("SELECT plan,expiry_date FROM users WHERE chat_id=?", (cid,)).fetchone()
        base = now
        if existing and existing["expiry_date"] and str(existing["plan"]).upper() in {"BASIC", "ELITE"}:
            try:
                old_exp = datetime.fromisoformat(str(existing["expiry_date"]))
                if old_exp.tzinfo is None:
                    old_exp = old_exp.replace(tzinfo=DAMASCUS)
                if old_exp > now:
                    base = old_exp
            except Exception:
                pass
        ...
        conn.execute(
            "INSERT OR IGNORE INTO users(chat_id,plan,status,referral_code,created_at,updated_at) VALUES(?,?,?,?,?,?)",
            (cid, "FREE", "active", f"ref_{cid}", now.isoformat(), now.isoformat()),
        )
        conn.execute(
            "UPDATE users SET plan=?,status='active',start_date=?,expiry_date=?,updated_at=? WHERE chat_id=?",
            (plan, now.isoformat(), expiry.isoformat(), now.isoformat(), cid),
        )
        conn.execute(
            "UPDATE subscription_requests SET status='APPROVED',approved_by=?,approved_at=?,processed_at=? WHERE id=?",
            (int(update.effective_user.id), now.isoformat(), now.isoformat(), int(request_id)),
        )
        conn.commit()
    finally:
        conn.close()
    await _notify_account_event(
        cid,
        f"subscription_approved:{request_id}",
        f"✅ تم تفعيل اشتراكك بنجاح\n\n📦 الباقة: {PLANS[plan]['name']}\n📅 حتى: {expiry.strftime('%Y-%m-%d %H:%M')} دمشق\n\nشكرًا لاختيارك XAU SMART TRADER.",
    )
    q = getattr(update, "callback_query", None)
    if q:
        await q.answer("تم اعتماد الطلب")
    await _admin_request_view(update, context, request_id)


async def _admin_request_reject(update, context, request_id):
    if not await _admin_guard(update):
        return
    _ensure_v1926_subscription_request_schema()
    conn = _db()
    try:
        ...
        cid = int(row["chat_id"])
        conn.execute(
            "UPDATE subscription_requests SET status='REJECTED',rejected_by=?,rejected_at=?,processed_at=? WHERE id=?",
            (int(update.effective_user.id), now.isoformat(), now.isoformat(), int(request_id)),
        )
        conn.commit()
    finally:
    ...
    await _notify_account_event(
        cid,
        f"subscription_rejected:{request_id}",
        "❌ تم رفض طلب الاشتراك من قبل الإدارة.\n\nيمكنك التواصل مع الدعم لمعرفة التفاصيل أو تقديم طلب جديد لاحقًا.",
    )
    q = getattr(update, "callback_query", None)
    if q:
        await q.answer("تم رفض الطلب")
    await _admin_request_view(update, context, request_id)


# ---------- 9) Organized admin menu, preserving existing tools ----------
async def admin_dashboard(update, context):
    if not await _admin_guard(update):
        return
    rows = [
        [("👥 المستخدمون", "admin_users"), ("💳 الاشتراكات", "admin_subscriptions")],
        [("🎁 المنح المجانية", "admin_grants")],
        [("📝 طلبات الاشتراك", "admin_subscription_requests")],
        [("📊 أداء التداول", "admin_trade_performance"), ("📈 أداء النظام", "admin_system_performance")],
        [("🔔 مراقبة التنبيهات", "admin_alerts")],
        [("🔍 التدقيق اليومي", "admin_audit_daily"), ("📅 التدقيق الأسبوعي", "admin_audit_weekly")],
        [("🗄️ قاعدة البيانات", "admin_database")],
        [("⚙️ النظام والمهام", "admin_scheduler")],
        [("📜 سجل الإدارة", "admin_help")],
        [("🟢 حالة النظام", "admin_system")],
        [("ℹ️ المساعدة", "admin_help")],
        [("🔙 الرئيسية", "nav_home")],
    ]
    await _admin_render(update, "👑 لوحة الإدارة\n\nاختر القسم المطلوب:", rows)


async def admin_grants(update, context):
    if not await _admin_guard(update):
        return
    text = (
        "🎁 المنح المجانية\n\n"
        "FREE+1 = BASIC مجانية\n"
        "FREE+2 = ELITE مجانية\n\n"
        "يمكن منح BASIC أو ELITE لأي مستخدم من إدارة المستخدمين، مع حفظ الباقة السابقة وإعادتها بعد انتهاء المنحة."
    )
    await _admin_render(update, text, [[("👥 إدارة المستخدمين", "admin_users")], [("🔙 لوحة الإدارة", "admin_home")]])


# ---------- 10) Callback additions ----------
_v1926_previous_callback_router = callback_router
async def callback_router(update, context):
    q = getattr(update, "callback_query", None)
    data = str(getattr(q, "data", "") or "")
    if data == "member_plans":
        await q.answer()
        await plans(update, context)
        return
    if data == "member_id":
        ...
        await user_id_command(update, context)
        return
    if data == "member_support":
        await q.answer()
        await support_button(update, context)
        return
    if data == "admin_grants":
        await q.answer()
        await admin_grants(update, context)
        return
    if data == "admin_subscription_requests":
        await q.answer()
        await admin_subscription_requests(update, context)
        return
    if data.startswith("admin_req_view:"):
        await q.answer()
        await _admin_request_view(update, context, data.split(":", 1)[1])
        return
    if data.startswith("admin_req_approve:"):
        await _admin_request_approve(update, context, data.split(":", 1)[1])
        return
    if data.startswith("admin_req_reject:"):
        await _admin_request_reject(update, context, data.split(":", 1)[1])
        return
    await _v1926_previous_callback_router(update, context)


# ---------- 11) Router additions for public G.Y and package/account buttons ----------
_v1926_previous_router = router
async def router(update, context):
    ...
    if text == "👤 الاشتراك":
        ...
        return
    return await _v1926_previous_router(update, context)


# ---------- 12) Start lifecycle scheduler correctly before the long-running base loop ----------
_v1926_original_base_start = globals().get("_prev_integrated_start_bot_v19_17")
if _v1926_original_base_start is not None:
    async def _v1926_patched_base_start():
        async def _delayed_lifecycle_start():
            try:
                ...
                logger.exception("v19.26 account lifecycle scheduler bootstrap error")
        asyncio.create_task(_delayed_lifecycle_start(), name="account-lifecycle-bootstrap")
        await _v1926_original_base_start()
    _prev_integrated_start_bot_v19_17 = _v1926_patched_base_start


# ---------- 13) Deterministic self-tests ----------
def run_v19_26_contract_self_tests():
    failures = []
    def check(name, condition):
        if not condition:
            failures.append(name)
    check("three_plans", set(PLANS) == {"FREE", "BASIC", "ELITE"})
    check("free_daily", PLANS["FREE"]["trade_limit"] == 1 and PLANS["FREE"]["trade_period"] == "daily")
    check("basic_daily", PLANS["BASIC"]["trade_limit"] == 5 and PLANS["BASIC"]["trade_period"] == "daily")
    check("elite_unlimited", PLANS["ELITE"]["trade_limit"] is None)
    check("basic_price", PLANS["BASIC"]["price"] == 15)
    check("elite_price", PLANS["ELITE"]["price"] == 39)
    check("elite_institutional", "institutional" in PLANS["ELITE"]["features"] and "institutional" not in PLANS["BASIC"]["features"])
    check("elite_gy", "gy_private_trade" in PLANS["ELITE"]["features"] and "gy_private_trade" not in PLANS["BASIC"]["features"] and "gy_private_trade" not in PLANS["FREE"]["features"])
    check("free_plus_one", "BASIC" in {"BASIC", "ELITE"})
    check("free_plus_two", "ELITE" in {"BASIC", "ELITE"})
    check("daily_text", "صفقة واحدة يوميًا" in TRADE_LIMIT_TEXT["FREE"] and "5 صفقات يوميًا" in TRADE_LIMIT_TEXT["BASIC"])
    return {"ok": not failures, "version": VERSION, "failures": failures}



# ============================================================
# v19.27 — SUBSCRIPTION / ADMIN MENU UX + CALLBACK HARDENING
# ============================================================
# نطاق التعديل فقط:
# 1) 👤 الاشتراك = عرض حالة الاشتراك فقط، بلا باقات/ID/دعم/رئيسية.
# 2) إصلاح مسار 📝 طلبات الاشتراك الإداري بمسار Callback مستقل مبكر.
# 3) إزالة زر 🏠 الرئيسية من النوافذ الداخلية.
# 4) توحيد قائمة الإدارة والقائمة الرئيسية على صفوف متجاورة قدر الإمكان.
# 5) لا تعديل على محرك التداول أو الحصص أو SL/TP أو S/R أو القرارات.
# ============================================================

# ---------- 1) Subscription button = status only ----------
async def my_subscription(update, context):
    """يعرض حالة الاشتراك فقط. لا يفتح الباقات أو الدعم أو ID."""
    chat_id = update.effective_chat.id
    await asyncio.to_thread(_ensure_user, update)
    status = await asyncio.to_thread(plan_status_text, chat_id)
    text = "👤 حالة الاشتراك\n\n" + status
    q = getattr(update, "callback_query", None)
    if q:
        await q.edit_message_text(with_market_header(text))
    else:
        await update.effective_message.reply_text(with_market_header(text))

# ---------- 2) Main menu: preserve existing order/content ----------
def _ux_main_keyboard():
    return [
        ["⚡ التحليل السريع", "🧠 التحليل المتكامل"],
        ["📝 التقرير اليومي", "📅 التقرير الأسبوعي"],
...
# ---------- 3) No "🏠 الرئيسية" in internal user windows ----------
def _ux_section_rows(section):
    if section == "analyses":
        return [[("🧠 التحليل المتكامل", "menu_full_analysis"),
                 ("⚡ التحليل السريع", "nav_quick")],
                [("📑 التقارير اليومية والأسبوعية", "nav_reports")]]
    if section == "reports":
        return [[("📝 التقرير التوضيحي اليومي", "menu_daily_report"),
                 ("📅 التقرير التوضيحي الأسبوعي", "menu_weekly_report")],
                [("📜 سجل الصفقات", "menu_trade_history")]]
    if section == "trades":
        return [[("📜 سجل الصفقات", "menu_trade_history"),
                 ("🔔 تنبيهات الصفقات", "menu_trade_alerts")]]
    if section == "market":
        return [[("💰 سعر الذهب", "menu_gold_price"),
                 ("📰 الأخبار", "menu_news")],
                [("🌍 الجلسات والأسواق", "menu_markets")]]
    return []

async def _ux_render(update, title, body, section=None):
    text = f"{title}\n{body}" if body else title
    rows = _ux_section_rows(section) if section else []
    markup = _ux_markup(rows) if rows else None
    q = getattr(update, "callback_query", None)
    if q:
        await q.edit_message_text(with_market_header(text), reply_markup=markup)
    elif getattr(update, "message", None):
...
# ---------- 4) Remove home button from full-analysis windows ----------
def _full_analysis_markup():
    return _ux_markup([[("➕ التفاصيل الكاملة", "full_all")]])

# ---------- 5) Remove home button from known package/detail views ----------
# Re-override package detail so navigation stays inside the package flow.
async def plan_callback(update, context):
    query = update.callback_query
    await query.answer()
    plan_key = str(query.data or "").replace("plan_", "", 1).upper()
    ...
    price_lines = f"💰 السعر: {'مجاني' if plan_key == 'FREE' else '$' + str(plan['price']) + ' / 30 يومًا'}\n"
    if prices:
        price_lines += f"📦 3 أشهر: ${prices['3_months']}\n📦 6 أشهر: ${prices['6_months']}\n"
    text = f"{plan['name']}\n{price_lines}🎯 {TRADE_LIMIT_TEXT[plan_key]}\n\n{body}"
    buttons = []
    if plan_key != "FREE":
        buttons.append([InlineKeyboardButton(f"📩 طلب الاشتراك في {plan['name']}", callback_data=f"request_{plan_key}")])
...
async def subscription_request_callback(update, context):
    ...
    if pending:
        await query.edit_message_text(with_market_header(
            ...
            [InlineKeyboardButton("🔙 العودة للباقات", callback_data="back_to_plans")]
        ]))
        return
    now = now_damascus().isoformat()
    conn = _db()
...
# ---------- 6) Admin dashboard: compact two-column layout ----------
async def admin_dashboard(update, context):
    if not await _admin_guard(update):
        return
    rows = [
        [("👥 المستخدمون", "admin_users"), ("💳 الاشتراكات", "admin_subscriptions")],
        [("🎁 المنح المجانية", "admin_grants"), ("📝 طلبات الاشتراك", "admin_subscription_requests")],
        [("📊 أداء التداول", "admin_trade_performance"), ("📈 أداء النظام", "admin_system_performance")],
        [("🔔 مراقبة التنبيهات", "admin_alerts"), ("🔍 التدقيق اليومي", "admin_audit_daily")],
        [("📅 التدقيق الأسبوعي", "admin_audit_weekly"), ("🗄️ قاعدة البيانات", "admin_database")],
        [("⚙️ النظام والمهام", "admin_scheduler"), ("🟢 حالة النظام", "admin_system")],
        [("ℹ️ المساعدة", "admin_help")],
    ]
    await _admin_render(update, "👑 لوحة الإدارة\n\nاختر القسم المطلوب:", rows)

# ---------- 7) Admin request center: robust direct callback path ----------
async def _v1927_admin_subscription_requests_callback(update, context):
    q = getattr(update, "callback_query", None)
    if not q:
        return
    try:
...
async def admin_subscription_requests(update, context):
    ...
    try:
        ...
        conn.close()
    if not rows:
        await _admin_render(update, "📝 طلبات الاشتراك\n\nلا توجد طلبات معلقة حاليًا.",
                            [[("🔙 لوحة الإدارة", "admin_home")]])
        return
    lines = ["📝 طلبات الاشتراك", ""]
    buttons = []
    pair = []
    for r in rows:
        ...
        label = "🟢 BASIC" if plan == "BASIC" else "🟣 ELITE"
        lines.append(f"#{r['id']} — {label} — Chat ID: {r['chat_id']} — {str(r['requested_at'])[:16].replace('T',' ')}")
        pair.append(InlineKeyboardButton(f"#{r['id']} {label}", callback_data=f"admin_req_view:{r['id']}"))
        if len(pair) == 2:
            buttons.append(pair)
            pair = []
    if pair:
...
# ---------- 9) Remove home button from admin grant screen ----------
async def admin_grants(update, context):
    if not await _admin_guard(update):
        return
    text = (
    ...
    await _admin_render(update, text, [[("👥 إدارة المستخدمين", "admin_users")],
                                       [("🔙 لوحة الإدارة", "admin_home")]])

# ---------- 10) Final callback router: explicit admin + member routes ----------
_v1927_previous_callback_router = callback_router
async def callback_router(update, context):
    q = getattr(update, "callback_query", None)
    data = str(getattr(q, "data", "") or "")
    if not q:
        return
    if data == "admin_subscription_requests":
        await _v1927_admin_subscription_requests_callback(update, context)
        return
    if data.startswith("admin_req_view:"):
        try: await q.answer()
        except Exception: pass
    ...
    if data == "admin_grants":
        ...
        await admin_grants(update, context)
        return
    await _v1927_previous_callback_router(update, context)

# ---------- 11) Install one early, filtered admin-request callback handler ----------
def _v1927_install_admin_request_handler():
    """Register a dedicated early handler so this critical admin button cannot be shadowed."""
    app = globals().get("APPLICATION")
    if app is None:
        return
    if globals().get("_V1927_ADMIN_REQUEST_HANDLER_INSTALLED", False):
        return
    try:
        handler = CallbackQueryHandler(
            _trace_handler(_v1927_admin_subscription_requests_callback, "callback:admin_subscription_requests_direct"),
            pattern=r"^admin_subscription_requests$",
        )
        app.add_handler(handler, group=0)
        globals()["_V1927_ADMIN_REQUEST_HANDLER_INSTALLED"] = True
    except Exception:
        logger.exception("v19.27 failed to install admin subscription request handler")

_v1927_original_base_start = globals().get("_prev_integrated_start_bot_v19_17")
if _v1927_original_base_start is not None:
    async def _v1927_patched_base_start():
        _v1927_install_admin_request_handler()
        await _v1927_original_base_start()
    _prev_integrated_start_bot_v19_17 = _v1927_patched_base_start

# ---------- 12) Contract self-test ----------
def run_v19_27_contract_self_tests():
    failures = []
    if VERSION != "v19.27_SUBSCRIPTION_ADMIN_MENU_UX_FIXED":
        failures.append("version")
    if _ux_main_keyboard()[-1] != ["🛡️ مراقب الصفقات", "🖥️ مراقب أداء النظام"]:
        failures.append("main_keyboard_shape")
    if any("🏠 الرئيسية" in str(row) for row in _ux_section_rows("analyses")):
        failures.append("section_home_button")
    if set(PLANS) != {"FREE", "BASIC", "ELITE"}:
        failures.append("plans")
    return {"ok": not failures, "version": VERSION, "failures": failures}


# ============================================================
# v19.28 — OFFICIAL MARKET READINGS RESTORED / REPORT ROUTING FIX
# ============================================================
# الهدف: إعادة مساري «القراءة اليومية للسوق» و«القراءة الأسبوعية للسوق»
# إلى العقد الرسمي المعتمد في v18.109 دون إعادة تصميم المحتوى.
#
# قواعد هذه الطبقة:
# 1) 🧭 القراءة اليومية للسوق -> build_daily_report()
# 2) 🧭 القراءة الأسبوعية للسوق -> build_weekly_report()
# 3) منع ظهور build_daily_analysis/build_weekly_analysis القديمتين للمستخدم.
# 4) التقرير الأسبوعي يمر عبر WeeklyDecision -> Validator -> formatter الرسمي.
# 5) التقرير اليومي قراءة تحليلية فقط، بلا صفقة أو دخول أو SL/TP/R:R.
# 6) لا تغيير في محرك التداول أو S/R أو الباقات أو الحصص.
# 7) إبقاء المسارات القديمة داخلياً للتوافق، لكن لا تُستخدم من واجهة القراءة.
# ============================================================

VERSION = "v19.28_OFFICIAL_MARKET_READINGS_RESTORED"

# ---------- القراءة اليومية الرسمية ----------
async def daily_analysis(update, context):
    """واجهة «🧭 القراءة اليومية للسوق»؛ المصدر الرسمي هو build_daily_report."""
    if not await feature_guard("daily_analysis")(update, context):
        return
    if not await _free_quota_guard(update, "daily_analysis"):
        return
    try:
        ...
        logger.exception("Official daily market reading error")
        await reply(update, f"❌ تعذر إنشاء 🧭 القراءة اليومية للسوق.\nالسبب: {e}")


# ---------- القراءة الأسبوعية الرسمية ----------
async def weekly_analysis(update, context):
    """واجهة «🧭 القراءة الأسبوعية للسوق»؛ المصدر الرسمي هو build_weekly_report."""
    if not await feature_guard("weekly_analysis")(update, context):
        return
    if not await _free_quota_guard(update, "weekly_analysis"):
        return
    try:
        ...
        logger.exception("Official weekly market reading error")
        await reply(update, f"❌ تعذر إنشاء 🧭 القراءة الأسبوعية للسوق.\nالسبب: {e}")


# ---------- أسماء التقرير القديمة تُوجّه إلى القراءة الرسمية ----------
async def daily_report(update, context):
    return await daily_analysis(update, context)
...
# ---------- القائمة الرئيسية: الأسماء الرسمية فقط ----------
def _ux_main_keyboard():
    """القائمة الرئيسية النهائية؛ لا تعرض التسميات القديمة للتحليل اليومي/الأسبوعي."""
    return [
        ["⚡ التحليل السريع", "🧠 التحليل المتكامل"],
        ["🧭 القراءة اليومية للسوق", "🧭 القراءة الأسبوعية للسوق"],
        ["🌍 السوق والأخبار", "🎯 الصفقات"],
...
# ---------- نافذة التقارير: صفّان متجاوران بلا زر رئيسية ----------
def _ux_section_rows(section):
    if section == "analyses":
        return [
            [("🧠 التحليل المتكامل", "menu_full_analysis"),
             ("⚡ التحليل السريع", "nav_quick")],
            [("🧭 القراءة اليومية للسوق", "menu_daily_report"),
             ("🧭 القراءة الأسبوعية للسوق", "menu_weekly_report")],
        ]
    if section == "reports":
        return [
            [("🧭 القراءة اليومية للسوق", "menu_daily_report"),
    ...
    if section == "market":
        return [
            [("💰 سعر الذهب", "menu_gold_price"), ("📰 الأخبار", "menu_news")],
            [("🌍 الجلسات والأسواق", "menu_markets")],
        ]
    return []


# ---------- Callback router: القراءة الرسمية قبل أي مسار قديم ----------
_v1928_previous_callback_router = callback_router
async def callback_router(update, context):
    q = getattr(update, "callback_query", None)
    data = str(getattr(q, "data", "") or "") if q else ""
    if data == "menu_daily_report":
        try:
            await q.answer()
        except Exception:
            pass
    ...
    if data == "menu_weekly_report":
        ...
        return
    return await _v1928_previous_callback_router(update, context)


# ---------- Text router: التسميات الرسمية فقط، مع قبول القديمة للتوافق ----------
_v1928_previous_router = router
async def router(update, context):
    text = (getattr(getattr(update, "effective_message", None), "text", "") or "").strip()
    if text in {"🧭 القراءة اليومية للسوق", "📝 التقرير اليومي"}:
        await daily_analysis(update, context)
        return
    if text in {"🧭 القراءة الأسبوعية للسوق", "📅 التقرير الأسبوعي"}:
        await weekly_analysis(update, context)
        return
    return await _v1928_previous_router(update, context)


# ---------- اختبارات عقدية حتمية ----------
def run_v19_28_market_readings_contract_self_tests():
    failures = []
    kb = _ux_main_keyboard()
    rows = _ux_section_rows("reports")
    analyses = _ux_section_rows("analyses")

    if VERSION != "v19.28_OFFICIAL_MARKET_READINGS_RESTORED":
        failures.append("version")
    if ["🧭 القراءة اليومية للسوق", "🧭 القراءة الأسبوعية للسوق"] not in kb:
        failures.append("main_readings_labels")
    if [
        ("🧭 القراءة اليومية للسوق", "menu_daily_report"),
        ("🧭 القراءة الأسبوعية للسوق", "menu_weekly_report"),
    ] not in rows:
        failures.append("reports_row")
    if [
        ("🧭 القراءة اليومية للسوق", "menu_daily_report"),
        ("🧭 القراءة الأسبوعية للسوق", "menu_weekly_report"),
    ] not in analyses:
        failures.append("analyses_row")
    if any("📝 التقرير التوضيحي اليومي" in str(row) or "📅 التقرير التوضيحي الأسبوعي" in str(row) for row in rows):
        failures.append("legacy_report_labels")
    if "build_daily_report" not in globals() or "build_weekly_report" not in globals():
        failures.append("official_builders_missing")
    if "build_daily_analysis" not in globals() or "build_weekly_analysis" not in globals():
        failures.append("legacy_builders_missing_for_compatibility")
    return {"ok": not failures, "version": VERSION, "failures": failures}



# ============================================================
# v19.30 — M15 reliability/performance hardening; M15 closed-candle gate preserved
# ============================================================
# القواعد:
# 1) الفحص الآلي للصفقات الجديدة يتم مرة واحدة فقط لكل شمعة M15 مكتملة.
# 2) الشمعة غير المكتملة لا تدخل في تقييم M15 الخاص بالتنفيذ.
# 3) إغلاق شمعة M15 يفتح فرصة لإعادة التحليل، وليس أمراً بإرسال صفقة.
# 4) جميع شروط الدخول الحالية تبقى كما هي دون تخفيف أو رفع مصطنع.
# 5) إذا لم تستوفِ الإشارة الشروط الحالية فلا توجد صفقة.
# 6) تغيّر score/quality وحده لا ينشئ صفقة جديدة.
# 7) حالة آخر شمعة مفحوصة تُحفظ في SQLite لمنع تكرارها بعد إعادة التشغيل.
# ============================================================

VERSION = "v19.32_ALERT_DELIVERY_CRASH_RESTART_PERFORMANCE_HARDENED"
M15_SCAN_INTERVAL_MINUTES = 15
M15_SCAN_STATE_DB = TRADE_DB_PATH
M15_SCAN_STATE_LOCK = threading.RLock()

# v19.30 runtime singleton/coordination guards. These guards do not alter
# trading conditions; they only prevent duplicate background execution.
MAIN_AUTO_LOOP_RUNNING = False
MAIN_AUTO_LOOP_LOCK = threading.RLock()
ALERT_SCHEDULER_TASK = None
ALERT_SCHEDULER_LOCK = threading.RLock()
_PRICE_FETCH_LOCK = threading.Lock()
INSTITUTIONAL_SNAPSHOT_LOCK = threading.RLock()
INSTITUTIONAL_SNAPSHOT = {"ts": 0.0, "value": None}
INSTITUTIONAL_SNAPSHOT_TTL = max(30.0, float(os.getenv("INSTITUTIONAL_SNAPSHOT_TTL", "300")))


def _v1929_m15_completed_frame(df):
    """إرجاع الشموع M15 المكتملة فقط؛ لا نستخدم الشمعة الجارية في قرار التنفيذ."""
    if df is None or df.empty or "openTime" not in df.columns:
        raise ValueError("بيانات M15 غير صالحة أو لا تحتوي على openTime.")
    out = df.copy()
    ...
    completed_cutoff = current_bucket - pd.Timedelta(minutes=15)
    out = out[out["openTime"] <= completed_cutoff].copy()
    if len(out) < MIN_BARS:
        raise ValueError("لا توجد شموع M15 مكتملة كافية للتحليل.")
    return out


_v1929_base_get_bars = get_bars


def get_bars(interval, limit=300):
    """v19.30: لا تُعاد الشمعة الجارية عند طلب M15؛ بقية الفريمات دون تغيير."""
    df = _v1929_base_get_bars(interval, limit)
    if str(interval).lower() in ("15m", "m15"):
        return _v1929_m15_completed_frame(df)
    return df


def _v1929_init_scan_state_db():
    with M15_SCAN_STATE_LOCK:
        conn = sqlite3.connect(M15_SCAN_STATE_DB, timeout=10)
        try:
            conn.execute("PRAGMA busy_timeout=10000")
            conn.execute("""
                CREATE TABLE IF NOT EXISTS m15_scan_state (
            ...
            """)
            cols={r[1] for r in conn.execute("PRAGMA table_info(m15_scan_state)").fetchall()}
            for name,typ in (("status","TEXT"),("attempts","INTEGER"),("retry_after","REAL"),("last_error","TEXT")):
                if name not in cols: conn.execute(f"ALTER TABLE m15_scan_state ADD COLUMN {name} {typ}")
            conn.execute("UPDATE m15_scan_state SET status='DONE' WHERE status IS NULL")
            conn.commit()
        finally:
            conn.close()


def _v1929_latest_completed_m15_key():
    df = _v1929_m15_completed_frame(_v1929_base_get_bars("15m", 60))
    ts = pd.to_datetime(df.iloc[-1]["openTime"], utc=True)
    return ts.isoformat()


def _v1929_claim_m15_scan(scan_key):
    """Claim ذريّ واحد مع إعادة محاولة مضبوطة عند فشل التحليل؛ دلالة M15 لا تتغير."""
    _v1929_init_scan_state_db()
    now_iso=now_damascus().isoformat(); now=time.time(); key=str(scan_key)
    with M15_SCAN_STATE_LOCK:
        conn=sqlite3.connect(M15_SCAN_STATE_DB,timeout=10)
        try:
            conn.execute("PRAGMA busy_timeout=10000"); conn.execute("BEGIN IMMEDIATE")
            row=conn.execute("SELECT status,attempts,retry_after FROM m15_scan_state WHERE scan_key=?",(key,)).fetchone()
            if row and row[0]=='DONE':
                conn.commit(); logger.info("M15_SCAN_SKIPPED_DONE | candle=%s",key); return False
            if row and row[0]=='RUNNING':
                conn.commit(); logger.info("M15_SCAN_SKIPPED_RUNNING | candle=%s",key); return False
            if row and float(row[2] or 0)>now:
                conn.commit(); return False
            attempts=int(row[1] or 0) if row else 0
            if attempts>=3:
                conn.commit(); logger.error("M15_SCAN_RETRY_LIMIT | candle=%s attempts=%s",key,attempts); return False
            if row:
                conn.execute("UPDATE m15_scan_state SET scanned_at=?,status='RUNNING',attempts=attempts+1,retry_after=0,last_error=NULL WHERE scan_key=?",(now_iso,key))
            else:
                conn.execute("INSERT INTO m15_scan_state(scan_key,scanned_at,status,attempts,retry_after,last_error) VALUES(?,?, 'RUNNING',1,0,NULL)",(key,now_iso))
            conn.commit(); logger.info("M15_SCAN_CLAIMED | candle=%s attempt=%s",key,attempts+1); return True
        except Exception:
            conn.rollback(); raise
        finally: conn.close()

def _v1930_m15_scan_finish(scan_key,ok,error=None,retry_delay=60.0):
    status='DONE' if ok else 'FAILED'
    retry_after=0.0 if ok else time.time()+max(5.0,float(retry_delay))
    conn=sqlite3.connect(M15_SCAN_STATE_DB,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        conn.execute("UPDATE m15_scan_state SET status=?,retry_after=?,last_error=? WHERE scan_key=?",(status,retry_after,None if ok else str(error)[:500],str(scan_key)))
        conn.commit()
    finally: conn.close()


def _v1930_perf_log(stage, started, **fields):
    try:
        payload=" ".join(f"{k}={v}" for k,v in fields.items())
        logger.info("PERF_STAGE | stage=%s latency_ms=%.1f%s", stage, (time.perf_counter()-started)*1000.0, (" "+payload) if payload else "")
    except Exception:
        pass


def _v1929_m15_scan_self_tests():
    failures = []
    if VERSION != "v19.31_ALERT_DELIVERY_CRASH_RESTART_PERFORMANCE_HARDENED":
        failures.append("version")
    if M15_SCAN_INTERVAL_MINUTES != 15:
        failures.append("interval")
    if not callable(_v1929_m15_completed_frame):
        failures.append("completed_frame")
    if not callable(_v1929_claim_m15_scan):
        failures.append("scan_claim")
    return {"ok": not failures, "version": VERSION, "failures": failures}


async def _ensure_alert_scheduler_task():
    global ALERT_SCHEDULER_TASK
    with ALERT_SCHEDULER_LOCK:
        task=ALERT_SCHEDULER_TASK
        if task is not None and not task.done():
            return task
        task=asyncio.create_task(_alert_scheduler_loop(),name="alert-timing-scheduler")
        ALERT_SCHEDULER_TASK=task
        logger.info("ALERT_SCHEDULER_TASK_CREATED | task=%s",id(task))
        return task

async def auto_loop():
    """الدورة النهائية: تحديث الصفقات مستمر، أما الصفقات الجديدة فبعد كل M15 مكتملة فقط."""
    global MAIN_AUTO_LOOP_RUNNING
    with MAIN_AUTO_LOOP_LOCK:
        if MAIN_AUTO_LOOP_RUNNING:
            logger.warning("MAIN_AUTO_LOOP_DUPLICATE_BLOCKED")
            return
        MAIN_AUTO_LOOP_RUNNING=True
    await _ensure_alert_scheduler_task()
    _v1929_init_scan_state_db()
    try:
        while True:
            try:
                # تحديث الصفقات القائمة والتنبيهات لا ينتظران إغلاق M15.
                _t_stage=time.perf_counter()
                changed_records = await asyncio.to_thread(update_trade_results)
                _v1930_perf_log("update_trade_results", _t_stage)
                _t_stage=time.perf_counter()
                eligible = await asyncio.to_thread(alert_subscribers)
                _v1930_perf_log("alert_subscribers", _t_stage)
                SUBSCRIBERS.update(eligible)
                SUBSCRIBERS.intersection_update(eligible)
                ...
                if SUBSCRIBERS and changed_records:
                    for record in changed_records:
                        ...
                        await asyncio.gather(*[
                            _send_trade_notification(chat_id, result_update, record,
                                                     "🔄 تحديث الصفقة — حالة جديدة",
                                                     consume_quota=False)
                            for chat_id in list(SUBSCRIBERS)
                        ], return_exceptions=True)
                ...
                closed_reason = await asyncio.to_thread(market_closed_reason)
                if closed_reason:
                    # استهلاك آخر شمعة مكتملة أثناء الإغلاق يمنع إعادة فحصها كصفقة
                    # جديدة عند أول دورة بعد عودة السوق. لا يتم تحليل أو إرسال صفقة هنا.
                    try:
                        closed_candle_key = await asyncio.to_thread(_v1929_latest_completed_m15_key)
                        claimed_closed = await asyncio.to_thread(_v1929_claim_m15_scan, closed_candle_key)
                        if claimed_closed:
                            await asyncio.to_thread(_v1930_m15_scan_finish, closed_candle_key, True)
                        logger.info("M15 candle marked during market closure | candle=%s reason=%s claimed=%s",
                                    closed_candle_key, closed_reason, claimed_closed)
                    except Exception:
                        logger.exception("M15 closure-state reconciliation failed")
                    await asyncio.sleep(10)
                    continue

                # لا يبدأ تحليل صفقة جديدة إلا عند ظهور شمعة M15 مكتملة جديدة.
                _t_stage=time.perf_counter()
                candle_key = await asyncio.to_thread(_v1929_latest_completed_m15_key)
                _v1930_perf_log("m15_latest_completed", _t_stage, candle=candle_key)
                if not await asyncio.to_thread(_v1929_claim_m15_scan, candle_key):
                    await asyncio.sleep(10)
                    continue

                logger.info("M15_NEW_CLOSED_CANDLE | candle=%s | starting trade evaluation", candle_key)
                scan_t0=time.perf_counter()
                try:
                    result = await asyncio.to_thread(evaluate_signal)
                except Exception as exc:
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,False,exc,60.0)
                    logger.exception("M15 evaluation failed | candle=%s | latency_ms=%.1f",candle_key,(time.perf_counter()-scan_t0)*1000.0)
                    await asyncio.sleep(10); continue
                logger.info("M15_EVALUATION_LATENCY | candle=%s | latency_ms=%.1f",candle_key,(time.perf_counter()-scan_t0)*1000.0)

                # لا توجد أي بوابة جديدة تفرض صفقة؛ هذه هي بوابات الاستراتيجية الحالية نفسها.
                if not result.get("signal"):
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,True)
                    logger.info("M15 trade rejected | candle=%s score=%s direction=%s reasons=%s",
                                candle_key, result.get("score"), result.get("direction"),
                                result.get("rejection_reasons"))
                    await asyncio.sleep(10)
                    continue
                if result.get("liquidity_blocked"):
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,True)
                    logger.info("M15 trade blocked by existing liquidity retest gate | candle=%s", candle_key)
                    await asyncio.sleep(10)
                    continue
                if not result.get("trade"):
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,True)
                    logger.info("M15 signal has no executable trade | candle=%s score=%s direction=%s",
                                candle_key, result.get("score"), result.get("direction"))
                    await asyncio.sleep(10)
                    continue
                ...
                if not quota_tokens:
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,True)
                    logger.info("M15 trade skipped: no recipient has remaining quota")
                    await asyncio.sleep(10)
                    continue
                ...
                if not record:
                    for token in quota_tokens.values():
                        await asyncio.to_thread(_release_trade_quota, token)
                    logger.warning("M15 trade registration failed; quotas released")
                    await asyncio.sleep(10)
                    continue

                if not is_new:
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,True)
                    # الصفقة موجودة مسبقاً؛ لا نرسل إشعار NEW بسبب تغير الجودة فقط.
                    for token in quota_tokens.values():
                        await asyncio.to_thread(_release_trade_quota, token)
                    logger.info("M15 duplicate/update suppressed as new trade | candle=%s", candle_key)
                else:
                    await asyncio.gather(*[
                        _send_trade_notification(chat_id, result, record,
                                                 "🚨 إشارة ذهب — صفقة جديدة",
                                                 consume_quota=True, quota_token=token)
                        for chat_id, token in quota_tokens.items()
                    ], return_exceptions=True)
                    await asyncio.to_thread(_v1930_m15_scan_finish,candle_key,True)

            except Exception:
                logger.exception("M15 auto scan error")
            await asyncio.sleep(5)
    finally:
        with MAIN_AUTO_LOOP_LOCK:
            MAIN_AUTO_LOOP_RUNNING=False
        logger.info("MAIN_AUTO_LOOP_STOPPED")
# v19.30: reuse a short-lived institutional snapshot to reduce repeated external
# macro fetches during bursts. Missing values remain exactly as returned; no
# synthetic data or scoring changes are introduced.
_v1930_base_institutional_analysis = institutional_analysis
def institutional_analysis(df):
    now=time.time()
    with INSTITUTIONAL_SNAPSHOT_LOCK:
        cached=INSTITUTIONAL_SNAPSHOT.get("value")
        if cached is not None and now-float(INSTITUTIONAL_SNAPSHOT.get("ts",0)) < INSTITUTIONAL_SNAPSHOT_TTL:
            return cached.copy() if isinstance(cached,dict) else cached
    value=_v1930_base_institutional_analysis(df)
    with INSTITUTIONAL_SNAPSHOT_LOCK:
        INSTITUTIONAL_SNAPSHOT.update({"ts":time.time(),"value":value.copy() if isinstance(value,dict) else value})
    return value



# ============================================================
# v19.31 — POST-FIX AUDIT ONLY
# Scope: Alert Delivery + Scheduler + Telemetry / crash-restart / retry bounds.
# Trading logic, scoring, S/R decision rules, Entry/SL/TP, quota rules and
# subscription contracts are intentionally untouched.
# ============================================================
EVENT_ALERT_MAX_ATTEMPTS = max(1, int(os.getenv("EVENT_ALERT_MAX_ATTEMPTS", str(ALERT_MAX_ATTEMPTS))))


def _v1931_reconcile_alert_event_recipient(event_key, chat_id):
    idem=f"{event_key}:{int(chat_id)}"
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        row=conn.execute("SELECT status,attempts,max_attempts,last_error FROM alert_outbox WHERE idempotency_key=?",(idem,)).fetchone()
        if not row:
            return None
        state=str(row[0])
        attempts=int(row[1] or 0)
        outbox_max=int(row[2] or ALERT_MAX_ATTEMPTS)
        if state=='SENT': mapped='SENT'
        elif state=='FAILED' and attempts>=outbox_max: mapped='FAILED'
        else: mapped='QUEUED'
        conn.execute("UPDATE alert_event_state SET status=?,last_error=?,updated_at=? WHERE event_key=? AND chat_id=?",(mapped,row[3] if mapped=='FAILED' else None,time.time(),event_key,int(chat_id)))
        conn.commit()
        return mapped
    finally: conn.close()


def _v1931_claim_alert_event_recipient(event_key, chat_id):
    _init_alert_event_state_db()
    now=time.time(); cid=int(chat_id)
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        ...
        row=conn.execute("SELECT status,updated_at,attempts FROM alert_event_state WHERE event_key=? AND chat_id=?",(event_key,cid)).fetchone()
        if row:
            status=str(row[0]); age=now-float(row[1] or 0); attempts=int(row[2] or 0)
            if status=='SENT': conn.commit(); return False
            if attempts>=EVENT_ALERT_MAX_ATTEMPTS:
                if status!='FAILED':
                    conn.execute("UPDATE alert_event_state SET status='FAILED',last_error=?,updated_at=? WHERE event_key=? AND chat_id=?",("event retry cap reached",now,event_key,cid))
                conn.commit(); return False
            if status in ('SENDING','QUEUED') and age < 180:
                conn.commit(); return False
            conn.execute("UPDATE alert_event_state SET status='SENDING',attempts=attempts+1,updated_at=?,last_error=NULL WHERE event_key=? AND chat_id=?",(now,event_key,cid))
...
def _v1931_finish_alert_event_recipient(event_key,chat_id,status,error=None):
    conn=sqlite3.connect(ALERT_DB_PATH,timeout=10)
    try:
        conn.execute("PRAGMA busy_timeout=10000")
        normalized=str(status)
        if normalized not in {'SENT','FAILED','QUEUED','SENDING'}: normalized='FAILED'
        conn.execute("UPDATE alert_event_state SET status=?,last_error=?,updated_at=? WHERE event_key=? AND chat_id=?",(normalized,None if not error else str(error)[:500],time.time(),event_key,int(chat_id)))
        conn.commit()
    finally: conn.close()

# Final runtime bindings for the hardened state machine.
_reconcile_alert_event_recipient = _v1931_reconcile_alert_event_recipient
_claim_alert_event_recipient = _v1931_claim_alert_event_recipient
_finish_alert_event_recipient = _v1931_finish_alert_event_recipient


def _v1931_task_exists(name):
    return any(t.get_name()==name and not t.done() for t in asyncio.all_tasks())

async def _v1931_ensure_named_task(factory, name):
    for t in asyncio.all_tasks():
        if t.get_name()==name and not t.done():
            return t
    return asyncio.create_task(factory(), name=name)

# Audit guard for the account scheduler: a stale task reference must not create a duplicate.
_old_start_account_notification_scheduler_v1931 = _start_account_notification_scheduler
async def _start_account_notification_scheduler():
    global ACCOUNT_NOTIFICATION_TASK
    if ACCOUNT_NOTIFICATION_TASK and not ACCOUNT_NOTIFICATION_TASK.done():
        return ACCOUNT_NOTIFICATION_TASK
    if _v1931_task_exists('account-lifecycle-notifications'):
        return next(t for t in asyncio.all_tasks() if t.get_name()=='account-lifecycle-notifications' and not t.done())
    return await _old_start_account_notification_scheduler_v1931()


# Explicit post-fix self-tests (pure state/DB tests; no Telegram calls).
def _v1931_event_state_self_test(tmp_db):
    global ALERT_DB_PATH
    old=ALERT_DB_PATH; ALERT_DB_PATH=tmp_db
    try:
        _init_alert_event_state_db()
        key='TEST-EVENT-1931'; cid=1931
        assert _claim_alert_event_recipient(key,cid) is True
        _finish_alert_event_recipient(key,cid,'FAILED','x')
        assert _claim_alert_event_recipient(key,cid) is True
        _finish_alert_event_recipient(key,cid,'FAILED','x')
        assert _claim_alert_event_recipient(key,cid) is True
        _finish_alert_event_recipient(key,cid,'FAILED','x')
        assert _claim_alert_event_recipient(key,cid) is False
        row=sqlite3.connect(tmp_db).execute("SELECT status,attempts FROM alert_event_state WHERE event_key=? AND chat_id=?",(key,cid)).fetchone()
        assert row[0]=='FAILED' and row[1]==EVENT_ALERT_MAX_ATTEMPTS
        return {'ok':True,'attempts':row[1],'max':EVENT_ALERT_MAX_ATTEMPTS}
    finally:
        ALERT_DB_PATH=old
...
# ============================================================
# v19.33 — ALERT FOUR-BOUNDARY CLOSURE
# Scope strictly limited to Alert Delivery / Persistence / Scheduler.
# Guarantees: zero synchronous SQLite in affected async paths, atomic
# outbox+trade-link, deterministic trade idempotency, explicit at-least-once.
# ============================================================
VERSION = "v19.33_ALERT_DELIVERY_AT_LEAST_ONCE_ATOMIC_IDEMPOTENCY_HARDENED"
ALERT_DELIVERY_SEMANTICS = "AT_LEAST_ONCE"
ALERT_EXACTLY_ONCE_EXTERNAL_SIDE_EFFECT = False


# ============================================================
# v19.34 — STRICT ALERT DELIVERY / SCHEDULER / TELEMETRY CLOSURE
# ============================================================
# v19.34 strict Alert Delivery / Scheduler / Telemetry hardening
VERSION = "v19.36_GY_AUTO_RUNTIME_WIRING_FIXED"
ALERT_DELIVERY_SEMANTICS = "AT_LEAST_ONCE"
ALERT_EXACTLY_ONCE_EXTERNAL_SIDE_EFFECT = False


def _v1934_dispatcher_init(self, bot, db_path=ALERT_DB_PATH, workers=4):
    self.bot=bot; self.db_path=db_path; self.workers=max(1,min(int(workers),8))
    self._tasks=[]; self._running=False; self._wake=asyncio.Event(); self._db_lock=threading.Lock()
    self._waiters={}; self._waiters_lock=threading.RLock(); self._telemetry_lock=threading.Lock()
    self._telemetry={"enqueued":0,"claimed":0,"send_started":0,"sent":0,"retried":0,"failed":0}
PersistentAlertDispatcher.__init__=_v1934_dispatcher_init


def _v1934_record_part_delivery(self, alert_id, part_index, message_id=None):
    with self._db_lock:
        c=self._conn()
        try:
            row=c.execute("SELECT telegram_message_ids FROM alert_outbox WHERE alert_id=?",(alert_id,)).fetchone()
            ids=[]
            if row and row["telegram_message_ids"]:
                try:
                    ids=json.loads(row["telegram_message_ids"])
                    if not isinstance(ids,list): ids=[]
                except Exception: ids=[]
            if message_id is not None: ids.append(int(message_id))
            c.execute("UPDATE alert_outbox SET parts_sent=?,telegram_message_ids=? WHERE alert_id=? AND status='SENDING'",(int(part_index),json.dumps(ids,separators=(",",":")),alert_id))
            c.commit()
        finally: c.close()
PersistentAlertDispatcher._record_part_delivery=_v1934_record_part_delivery


async def _v1934_dispatcher_start(self):
    if self._running: return
    await asyncio.to_thread(self._init_db)
    self._running=True
    self._tasks=[asyncio.create_task(self._worker(i),name=f"persist-alert-{i}") for i in range(self.workers)]
    logger.info("PersistentAlertDispatcher started | workers=%s",self.workers)
PersistentAlertDispatcher.start=_v1934_dispatcher_start


async def _v1934_dispatcher_worker(self, idx):
    """Worker: Telegram I/O is async; every SQLite operation is offloaded."""
    while self._running:
        job=await asyncio.to_thread(self._claim)
        if not job:
            self._wake.clear()
        ...
        try:
            ...
            for part_index,part in enumerate(parts,1):
                ...
                await asyncio.to_thread(self._record_part_delivery,job["alert_id"],part_index,message_id)
            await asyncio.to_thread(self._result,job,True)
            with self._telemetry_lock: self._telemetry["sent"]+=1
            logger.info("ALERT_DELIVERED_E2E | id=%s kind=%s queue_plus_delivery_ms=%.1f",job["alert_id"],job["kind"],(time.time()-job["created_at"])*1000)
        except (Forbidden,BadRequest) as e:
            ...
            with self._telemetry_lock: self._telemetry["retried"]+=1
            logger.warning("TELEGRAM_RETRY | id=%s kind=%s error=%s",job["alert_id"],job["kind"],e)
            await asyncio.to_thread(self._result,job,False,e)
PersistentAlertDispatcher._worker=_v1934_dispatcher_worker


async def _v1934_dispatcher_health_async(self):
    return await asyncio.to_thread(self.health)
PersistentAlertDispatcher.health_async=_v1934_dispatcher_health_async


async def _v1934_sre_runtime_health_async():
    d=ALERT_DISPATCHER
    return {"alert_outbox":await d.health_async() if isinstance(d,PersistentAlertDispatcher) else {},"alert_running":bool(getattr(d,"_running",False)),"price_cache_age":max(0.0,time.time()-_LIVE_PRICE_CACHE["ts"]) if _LIVE_PRICE_CACHE["ts"] else None}
sre_runtime_health_async=_v1934_sre_runtime_health_async


def _v1934_admin_alert_snapshot_sync():
    out={"PENDING":0,"SENDING":0,"SENT":0,"FAILED":0,"oldest_pending_age":0.0}
    try:
        if os.path.exists(ALERT_DB_PATH):
            conn=sqlite3.connect(ALERT_DB_PATH,timeout=5); conn.row_factory=sqlite3.Row
    ...
    return out
async def _v1934_admin_alert_snapshot(): return await asyncio.to_thread(_v1934_admin_alert_snapshot_sync)
_admin_alert_snapshot=_v1934_admin_alert_snapshot


async def _v1934_admin_alerts(update,context):
    if not await _admin_guard(update): return
    a=await _admin_alert_snapshot(); tel=dict(ALERT_TELEMETRY)
    d=await ALERT_DISPATCHER.health_async() if isinstance(ALERT_DISPATCHER,PersistentAlertDispatcher) else {}
    text=("🔔 مراقبة التنبيهات\n\n"
          f"• PENDING: {a.get('PENDING',0)}\n• SENDING: {a.get('SENDING',0)}\n• SENT: {a.get('SENT',0)}\n• FAILED: {a.get('FAILED',0)}\n"
          f"• أقدم Pending: {a.get('oldest_pending_age',0):.1f} ثانية\n• Scheduler errors: {tel.get('scheduler_errors',0)}\n"
          f"• Dispatcher: {'🟢 يعمل' if ALERT_DISPATCHER and d else '🟡 غير مهيأ'}")
    await _admin_render(update,text,"alerts")
admin_alerts=_v1934_admin_alerts


async def _v1934_send_trade_notification(chat_id,result,record,title,consume_quota=False,quota_token=None):
    signature=_trade_notification_signature(record)
    if not signature: return False
    if await asyncio.to_thread(_notification_already_sent,chat_id,signature): return False
    if not await asyncio.to_thread(_claim_notification,chat_id,signature): return False
    quota_token=quota_token if consume_quota else None
    if consume_quota and not quota_token:
        await asyncio.to_thread(_cancel_notification_claim,chat_id,signature); return False
    aid=None
    try:
        payload=_format_trade_message(result,title,record); d=ALERT_DISPATCHER or await _start_alert_dispatcher()
        if not d: raise RuntimeError("Alert dispatcher unavailable")
        idem=_trade_alert_idempotency_key(chat_id,signature)
        aid=await asyncio.to_thread(d.enqueue_trade_with_link,chat_id,payload,signature,consume_quota,quota_token,kind="trade",priority=100,max_attempts=6,idempotency_key=idem)
        if not aid: raise RuntimeError("Atomic trade alert enqueue/link failed")
        delivered=await d.wait_delivery(aid,ALERT_DELIVERY_WAIT_SECONDS)
        link={"alert_id":aid,"chat_id":chat_id,"signature":signature,"consume_quota":1 if consume_quota else 0,"quota_token":quota_token}
        if delivered is True:
            await _resolve_alert_trade_link(link); return True
        if delivered is None:
            asyncio.create_task(_finalize_trade_delivery_after_timeout(aid,chat_id,signature,consume_quota,quota_token),name=f"trade-alert-finalize-{aid}")
            logger.warning("TRADE_ALERT_PENDING_AFTER_TIMEOUT | chat_id=%s alert_id=%s",chat_id,aid); return True
        await _resolve_alert_trade_link(link); return False
    except Exception:
        if aid is None:
            await asyncio.to_thread(_cancel_notification_claim,chat_id,signature)
            if consume_quota: await asyncio.to_thread(_release_trade_quota,quota_token)
            ...
            if status is None:
                ...
                if consume_quota: await asyncio.to_thread(_release_trade_quota,quota_token)
        logger.exception("Signal delivery failed for %s",chat_id); return False
_send_trade_notification=_v1934_send_trade_notification


_old_start_alert_dispatcher_v1934=_start_alert_dispatcher
async def _v1934_start_alert_dispatcher():
    d=await _old_start_alert_dispatcher_v1934()
    await asyncio.to_thread(_init_alert_trade_links)
    await _reconcile_alert_trade_links_once()
    if not any(t.get_name()=="alert-trade-reconciliation" and not t.done() for t in asyncio.all_tasks()):
        asyncio.create_task(_alert_trade_reconciliation_loop(),name="alert-trade-reconciliation")
    return d
_start_alert_dispatcher=_v1934_start_alert_dispatcher

ALERT_SQLITE_EVENT_LOOP_POLICY="ZERO_SYNC_SQLITE"
ALERT_WORKER_PART_CHECKPOINT="ASYNCIO_TO_THREAD"
ALERT_HEALTH_READ_POLICY="ASYNCIO_TO_THREAD"
ALERT_STARTUP_DB_POLICY="ASYNCIO_TO_THREAD"


# ============================================================
# v19.35 — G.Y LIFECYCLE + AUTOMATIC CONTRACT CLOSURE
# Scope: G.Y private trade lifecycle and its automatic generation contract only.
# The S/R decision rules, Entry/SL/TP geometry and scoring are untouched.
# ============================================================
VERSION = "v19.35_GY_LIFECYCLE_AUTO_CONTRACT_HARDENED"
GY_AUTO_CONTRACT = "ENABLED"
GY_LIFECYCLE_ACTIVE_STATUSES = ("OPEN", "TP1_HIT", "TP2_HIT")


def _v1935_gy_active_trade_conflict(conn, trade):
    """Block a new G.Y setup when another active setup occupies the same S/R zone.
    This is a duplicate/conflict guard, not a change to signal generation rules.
    """
    zone = _sr_num(trade.get("zone"))
    atr = _sr_num(trade.get("atr"))
    entry = _sr_num(trade.get("entry"))
    if zone is None:
        return None
    tolerance = max((atr or 0.0) * 0.35, abs(entry or zone) * 0.00035)
    rows = conn.execute(
        "SELECT setup_id,direction,status,snapshot_json FROM sr_trades "
        "WHERE status IN ('OPEN','TP1_HIT','TP2_HIT') ORDER BY created_at DESC"
    ).fetchall()
    for row in rows:
        try:
            snap = json.loads(row[3] or "{}")
            old_trade = snap.get("trade") or {}
            old_zone = _sr_num(old_trade.get("zone"))
            old_atr = _sr_num(old_trade.get("atr"))
            if old_zone is None:
                continue
            old_entry = _sr_num(old_trade.get("entry"))
            old_tol = max((old_atr or 0.0) * 0.35, abs(old_entry or old_zone) * 0.00035)
            if abs(old_zone - zone) <= max(tolerance, old_tol):
                return {
                    "setup_id": row[0],
                    "direction": row[1],
                    "status": row[2],
...
def _v1935_create_auto_gy_trade(trade, snapshot):
    """Create one automatic G.Y trade atomically with its private alert outbox.
    Recipient contract is gy_private_trade; the main trade-alert audience is not used.
    """
    fingerprint = _sr_gy_fingerprint(trade)
    setup_id = f"SR-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8].upper()}"
    conn = _sr_db_connect()
    try:
        ...
        existing = conn.execute(
            "SELECT setup_id FROM sr_trades WHERE signal_fingerprint=? "
            "AND status IN ('OPEN','TP1_HIT','TP2_HIT') LIMIT 1",
            (fingerprint,),
        ).fetchone()
        if existing:
            conn.rollback()
            SR_AUTO_HEALTH["duplicates_blocked"] = int(SR_AUTO_HEALTH.get("duplicates_blocked", 0)) + 1
            return None, False, "duplicate_fingerprint"

        conflict = _v1935_gy_active_trade_conflict(conn, trade)
        if conflict:
            conn.rollback()
            SR_AUTO_HEALTH["duplicates_blocked"] = int(SR_AUTO_HEALTH.get("duplicates_blocked", 0)) + 1
            logger.info(
                "G.Y_AUTO_CONFLICT_BLOCKED | new=%s existing=%s direction=%s existing_direction=%s",
                setup_id, conflict["setup_id"], trade.get("direction"), conflict["direction"],
            )
            return None, False, "active_zone_conflict"

        cur = conn.execute(
            """INSERT OR IGNORE INTO sr_trades
            (setup_id,created_at,direction,entry,sl,tp1,tp2,tp3,rr,score,trigger,status,
             snapshot_json,signal_fingerprint,alert_state)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,'OPEN',?,?, 'PENDING')""",
            (
                setup_id,
                now_damascus().isoformat(),
                trade["direction"], trade["entry"], trade["sl"], trade["tp1"],
                trade.get("tp2"), trade.get("tp3"), trade["rr"], trade["score"],
                trade["trigger"], json.dumps(snapshot, ensure_ascii=False, default=str), fingerprint,
            ),
        )
        if cur.rowcount != 1:
            conn.rollback()
            SR_AUTO_HEALTH["duplicates_blocked"] = int(SR_AUTO_HEALTH.get("duplicates_blocked", 0)) + 1
            return None, False, "insert_duplicate"

        conn.execute(
            "INSERT INTO sr_trade_history(setup_id,event_time,status,result,price,note) "
            "VALUES(?,?,?,?,?,?)",
            (setup_id, now_damascus().isoformat(), "OPEN", None, trade["entry"], "G.Y automatic setup created"),
        )

        try:
            recipients = set(_alert_recipients("gy_private_trade"))
        except Exception:
            recipients = set(ADMIN_IDS)

        payload_trade = dict(trade)
        payload_trade["_setup_id"] = setup_id
        payload = _sr_gy_alert_payload(payload_trade, snapshot)
        created_at = now_damascus().isoformat()
        for cid in recipients:
            try:
                conn.execute(
                    "INSERT OR IGNORE INTO sr_alert_outbox "
                    "(setup_id,chat_id,payload,status,attempts,created_at) "
                    "VALUES(?,?,?,'PENDING',0,?)",
                    (setup_id, int(cid), payload, created_at),
                )
            except Exception:
                logger.exception("G.Y automatic outbox insert failed chat_id=%s setup_id=%s", cid, setup_id)

        conn.commit()
        SR_AUTO_HEALTH["trades_created"] = int(SR_AUTO_HEALTH.get("trades_created", 0)) + 1
        return setup_id, True, "created"
    except Exception:
        conn.rollback()
        raise
    finally:
...
def _v1935_lifecycle_transition(row, price):
    """Pure G.Y state-machine decision. TP1/TP2 remain active until final close."""
    ...
    tp3 = _sr_num(row["tp3"])
    p = _sr_num(price)
    if p is None or sl is None or direction not in ("BUY", "SELL"):
        return None, None

    if direction == "BUY":
        if p <= sl:
            return "CLOSED", "SL"
        if tp3 is not None and p >= tp3:
            return "CLOSED", "TP3"
        if status == "OPEN" and tp2 is not None and p >= tp2:
            return "TP2_HIT", "TP2"
        if status == "OPEN" and tp1 is not None and p >= tp1:
            return "TP1_HIT", "TP1"
        if status == "TP1_HIT" and tp2 is not None and p >= tp2:
            return "TP2_HIT", "TP2"
    else:
        if p >= sl:
            return "CLOSED", "SL"
...
def _v1935_update_gy_lifecycle(price):
    """Advance all active G.Y states without losing TP1/TP2 positions."""
    p = _sr_num(price)
    if p is None:
        return []
    changed = []
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM sr_trades WHERE status IN ('OPEN','TP1_HIT','TP2_HIT') "
            "ORDER BY created_at ASC"
        ).fetchall()
        for r in rows:
            status, result = _v1935_lifecycle_transition(r, p)
            if not status:
                continue
            final_status = "CLOSED" if status == "CLOSED" else status
            ts = now_damascus().isoformat()
            conn.execute(
                "UPDATE sr_trades SET status=?, result=?, closed_at=? "
                "WHERE setup_id=? AND status IN ('OPEN','TP1_HIT','TP2_HIT')",
                (final_status, result, ts if final_status == "CLOSED" else None, r["setup_id"]),
            )
            conn.execute(
                "INSERT INTO sr_trade_history(setup_id,event_time,status,result,price,note) "
                "VALUES(?,?,?,?,?,?)",
                (r["setup_id"], ts, final_status, result, p, "G.Y lifecycle state transition"),
            )
            changed.append((dict(r), final_status, result, p))
        if changed:
            conn.execute(
                "DELETE FROM sr_trade_history WHERE id NOT IN "
                "(SELECT id FROM sr_trade_history ORDER BY id DESC LIMIT ?)",
                (SR_MAX_HISTORY,),
            )
        conn.commit()
        return changed
...
async def _v1935_gy_auto_loop():
    """Canonical automatic G.Y loop: lifecycle first, then new confirmed setup, then durable alerts."""
    global SR_AUTO_TASK
    SR_AUTO_HEALTH["running"] = True
    SR_AUTO_HEALTH["last_start"] = time.time()
    try:
        while True:
            cycle_started = time.time()
            _v1968_gy_cycle_id = uuid.uuid4().hex[:12]
            try:
                _V1968_GY_ACTIVE_CYCLE.set(_v1968_gy_cycle_id)
            except Exception:
                pass
            logger.info("GY_CYCLE_START | cycle=%s", _v1968_gy_cycle_id)
            try:
                SR_AUTO_HEALTH["cycles"] = int(SR_AUTO_HEALTH.get("cycles", 0)) + 1
                SR_AUTO_HEALTH["last_cycle"] = cycle_started

                # 1) Lifecycle is always evaluated first so active trades cannot be abandoned.
                p = await asyncio.to_thread(live_price)
                changed = await asyncio.to_thread(_v1935_update_gy_lifecycle, p)
                if changed:
                    recipients = await asyncio.to_thread(_alert_recipients, "gy_private_trade")
                    for row, status, result, price in changed:
                ...
                # 2) Automatic generation is explicitly enabled by the G.Y contract.
                if SR_AUTO_ENABLED and not market_closed_reason():
                    result = await asyncio.to_thread(sr_analyze)
                    ...
                    if signal:
                        df = result.get("df")
                        if df is None:
                            df = await asyncio.to_thread(get_bars, "15m", 120)
                        snapshot = _sr_snapshot(df, result.get("sr", {}), signal)
                        setup_id, is_new, reason = await asyncio.to_thread(
                            _v1935_create_auto_gy_trade, signal, snapshot
                        )
                        if is_new:
                            SR_AUTO_HEALTH["last_trade"] = setup_id
                            logger.info("G.Y_PRIVATE_TRADE_AUTO_CREATED | setup_id=%s", setup_id)
                        elif reason not in ("duplicate_fingerprint", "active_zone_conflict", "insert_duplicate"):
                            logger.info("G.Y_AUTO_NO_TRADE | reason=%s", reason)

                # 3) Drain the durable G.Y outbox after creation/lifecycle events.
                await _sr_gy_alert_outbox_once()
                SR_AUTO_HEALTH["last_success"] = time.time()
                SR_AUTO_HEALTH["last_error"] = None
                SR_AUTO_HEALTH["last_cycle_duration_ms"] = round((time.time() - cycle_started) * 1000.0, 2)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                SR_AUTO_HEALTH["last_error"] = f"{type(exc).__name__}: {exc}"
                SR_AUTO_HEALTH["last_cycle_duration_ms"] = round((time.time() - cycle_started) * 1000.0, 2)
                logger.exception("G.Y automatic lifecycle loop error")
            logger.info(
                "GY_CYCLE_END | cycle=%s duration_ms=%.1f last_error=%s",
                _v1968_gy_cycle_id,
                (time.time() - cycle_started) * 1000.0,
                SR_AUTO_HEALTH.get("last_error"),
            )
            await asyncio.sleep(SR_AUTO_SECONDS)
    finally:
        SR_AUTO_HEALTH["running"] = False
        if SR_AUTO_TASK is asyncio.current_task():
            SR_AUTO_TASK = None


# v19.39 — G.Y S/R COMPLETENESS CONTRACT
# G.Y requires at least one valid support and one valid resistance relative to price.
# S1+R1 is valid; all six levels are NOT required. MAIN remains untouched.
VERSION = "v19.39_GY_SR_COMPLETENESS_CONTRACT_FIXED"


# Final v19.35 bindings: these supersede the disabled lifecycle-only G.Y loop.
_sr_update_lifecycle = _v1935_update_gy_lifecycle
_sr_auto_loop = _v1935_gy_auto_loop
GY_AUTO_CONTRACT = "ENABLED"



# ============================================================
# v19.40 — PCS + G.Y INTEGRATION
# Scope: G.Y decision layer only.
# v19.39 remains the strategy/geometry reference implementation.
# MAIN trade engine, Entry/SL/TP geometry, R:R construction, quota,
# subscription contracts, M15 gate, persistence and alert delivery are
# not rewritten. PCS is a hard confluence gate above the existing G.Y engine.
# ============================================================
VERSION = "v19.42_W1_NON_BLOCKING_GY_PCS"
PCS_VERSION = "PCS-1.0"
PCS_MIN_SCORE = 80
PCS_CONTEXT_TTL_SECONDS = 15 * 60
PCS_HARD_GATES = True

_PCS_CONTEXT_LOCK = threading.RLock()
_PCS_CONTEXT_CACHE = {"key": None, "created_at": 0.0, "value": None}


def _v1940_m15_context_key(df):
    """Stable key tied to the latest completed M15 candle used by G.Y."""
    try:
        if df is not None and len(df) and "openTime" in df.columns:
            ts = pd.to_datetime(df.iloc[-1]["openTime"], utc=True, errors="coerce")
            if not pd.isna(ts):
                return ts.isoformat()
    except Exception:
        pass
    # Fallback is deliberately coarse and never used to manufacture market data.
    return str(int(time.time() // 900))


def _v1940_fetch_pcs_context(df):
    """Fetch W1/D1/H4 once per completed M15 candle and analyze them in parallel."""
    key = _v1940_m15_context_key(df)
    now = time.time()
    with _PCS_CONTEXT_LOCK:
        cached = _PCS_CONTEXT_CACHE.get("value")
        if cached is not None and _PCS_CONTEXT_CACHE.get("key") == key and (now - float(_PCS_CONTEXT_CACHE.get("created_at", 0.0))) < PCS_CONTEXT_TTL_SECONDS:
            return cached
    ...
    errors = {}
    try:
        with ThreadPoolExecutor(max_workers=3, thread_name_prefix="pcs-context") as pool:
            futures = {
                "W1": pool.submit(get_bars, "1w", 250),
                "D1": pool.submit(get_bars, "1d", 300),
                "H4": pool.submit(get_bars, "4h", 300),
            }
            raw = {}
            for name, future in futures.items():
                try:
                    raw[name] = future.result()
                    frames[name] = analyze(raw[name])
                except Exception as exc:
                    errors[name] = f"{type(exc).__name__}: {exc}"
                    frames[name] = {}
    except Exception as exc:
        errors["_context"] = f"{type(exc).__name__}: {exc}"
    ...
    with _PCS_CONTEXT_LOCK:
        _PCS_CONTEXT_CACHE.update({"key": key, "created_at": time.time(), "value": value})
    logger.info(
        "PCS_CONTEXT_BUILT | candle=%s latency_ms=%.2f errors=%s",
        key, value["latency_ms"], ",".join(sorted(errors)) if errors else "none",
    )
    return value


def _v1940_pcs_regime(h4, df, atr):
    """Deterministic market regime used only as a PCS confluence factor."""
    try:
        adx = float(h4.get("adx", 0) or 0)
    except Exception:
        adx = 0.0
    structure = h4.get("structure")
    try:
        a = float(atr or 0)
        ranges = []
        if df is not None and len(df) >= 5 and a > 0:
            for _, row in df.tail(5).iterrows():
                hi = float(row.get("high")); lo = float(row.get("low"))
                ranges.append(max(0.0, hi - lo) / a)
        last_ratio = ranges[-1] if ranges else 0.0
        impulsive_count = sum(1 for x in ranges if x >= 1.25)
        if last_ratio >= 1.80 or impulsive_count >= 3:
            return "HIGH_VOLATILITY", {"adx_h4": round(adx, 2), "last_range_atr": round(last_ratio, 2), "impulsive_count": impulsive_count}
    except Exception:
        pass
    if adx >= 25 and structure in ("صاعد", "هابط"):
        return "TRENDING", {"adx_h4": round(adx, 2), "structure": structure}
    if adx < 18:
        return "RANGING", {"adx_h4": round(adx, 2), "structure": structure}
    return "TRANSITION", {"adx_h4": round(adx, 2), "structure": structure}


def _v1940_pcs_evaluate(candidate, geometry, sr, df):
    """Evaluate PCS confluence without altering G.Y geometry or scenario math."""
    if not candidate or not geometry or not geometry.get("ok"):
        return {
            "approved": False, "score": 0, "confidence": "LOW", "state": "BLOCKED",
            "reason": "هندسة G.Y غير صالحة؛ PCS لا تنشئ صفقة من دون Geometry صالحة.",
            "hard_gates": {"geometry": False}, "version": PCS_VERSION,
        }

    context = _v1940_fetch_pcs_context(df)
    frames = context.get("frames", {})
    direction = candidate.get("direction")
    trigger = candidate.get("trigger")
    htf = {k: (frames.get(k) or {}) for k in ("W1", "D1", "H4")}
    dirs = {k: htf[k].get("direction") for k in htf}

    valid_context = all(dirs[k] in ("BUY", "SELL") for k in dirs)
    w1_ok = dirs["W1"] == direction
    d1_ok = dirs["D1"] == direction
    h4_ok = dirs["H4"] == direction
    atr = candidate.get("atr")
    regime, regime_meta = _v1940_pcs_regime(htf["H4"], df, atr)

        # Strong PCS anchor: W1 + D1 + H4 must agree with G.Y.
    # Any missing/neutral or opposite strategic frame fails the high-confidence gate.
    h4_conflict = h4_ok is False and dirs["H4"] in ("BUY", "SELL")

    # Scenario/regime compatibility is scored, not used to invent a trade.
    if regime == "TRENDING":
        regime_points = 15 if trigger in ("BREAKOUT_RETEST", "SR_FLIP") else 12
    elif regime == "RANGING":
        regime_points = 15 if trigger in ("REJECTION", "FALSE_BREAKOUT") else 8
    elif regime == "HIGH_VOLATILITY":
        regime_points = 5 if trigger in ("BREAKOUT_RETEST", "SR_FLIP") else 0
    else:  # TRANSITION
        regime_points = 8 if trigger in ("BREAKOUT_RETEST", "REJECTION", "FALSE_BREAKOUT", "SR_FLIP") else 0

    scenario_score = float(candidate.get("score", 0) or 0)
    scenario_points = min(25.0, max(0.0, scenario_score) * 0.25)

    confirmation_ok = bool(candidate.get("scenario", {}).get("confirmed"))
    confirmation_points = 15.0 if confirmation_ok else 0.0

    sr_contract = _gy_sr_completeness(sr, _sr_num(df["close"].iloc[-1])) if isinstance(sr, dict) else {"executable": False}
    sr_ok = bool(sr_contract.get("executable"))
    sr_points = 10.0 if sr_ok else 0.0

    rr = _sr_num(geometry.get("rr"))
    rr_ok = rr is not None and rr >= SR_MIN_RR
    rr_points = 15.0 if rr_ok else 0.0

    # Score is diagnostic; approval is governed by hard gates below.
    score = int(max(0, min(100, round(
        (15.0 if w1_ok else 0.0) +
        (15.0 if d1_ok else 0.0) +
        (10.0 if h4_ok else 0.0) +
        regime_points + scenario_points + confirmation_points + sr_points + rr_points
    ))))

    # v19.42: W1 is contextual evidence only.
    # A non-supportive W1 must never block an otherwise valid G.Y setup.
    # W1 remains visible in score/components and is explicitly classified as
    # supportive/non-supportive for diagnostics. Missing strategic context is
    # still protected by the existing context gate.
    hard_gates = {
        "context": valid_context,
        "w1_alignment": True,
        "d1_alignment": d1_ok,
        "h4_conflict": not h4_conflict,
        "sr_completeness": sr_ok,
        "scenario_confirmation": confirmation_ok,
        "geometry": True,
        "rr": rr_ok,
    }
    approved = all(hard_gates.values()) and score >= PCS_MIN_SCORE

    if not valid_context:
        reason = "تعذر بناء السياق W1/D1/H4 الكامل؛ تم إيقاف G.Y بدلاً من التخمين."
    elif not d1_ok:
        reason = f"D1 لا يدعم اتجاه G.Y ({direction}); لا توجد موافقة PCS."
    elif h4_conflict:
        reason = f"H4 يعارض اتجاه G.Y ({direction}); لا توجد موافقة PCS."
    elif not sr_ok:
        reason = "بنية S/R غير مكتملة؛ لا توجد موافقة PCS."
    elif not confirmation_ok:
        reason = "سيناريو G.Y غير مؤكد؛ لا توجد موافقة PCS."
    elif not rr_ok:
        reason = f"R:R أقل من الحد الأدنى {SR_MIN_RR:.2f}; لا توجد موافقة PCS."
    elif score < PCS_MIN_SCORE:
        reason = f"درجة التوافق PCS={score}/100 أقل من {PCS_MIN_SCORE}."
    else:
        reason = "توافق PCS كامل؛ G.Y مؤهلة للانتقال إلى Persistence/Outbox."

    confidence = "HIGH" if approved and score >= 90 else "MEDIUM" if score >= PCS_MIN_SCORE else "LOW"
    return {
        "approved": approved,
        "score": score,
        ...
        "htf": {k: {"direction": htf[k].get("direction"), "score": htf[k].get("score"), "structure": htf[k].get("structure"), "adx": htf[k].get("adx")} for k in htf},
        "w1_support": "داعم" if w1_ok else "غير داعم",
        "w1_supports_direction": bool(w1_ok),
        "hard_gates": hard_gates,
        "components": {
            "w1": 15 if w1_ok else 0,
            "d1": 15 if d1_ok else 0,
            "h4": 10 if h4_ok else 0,
            "regime": regime_points,
            "scenario": round(scenario_points, 2),
            "confirmation": confirmation_points,
            "sr": sr_points,
            "rr": rr_points,
        },
...
def _v1940_sr_snapshot(df, sr, trade):
    """Canonical G.Y snapshot now carries the PCS decision used for the trade."""
    snap = _sr_snapshot_v1940_base(df, sr, trade)
    snap["pcs"] = trade.get("pcs") if isinstance(trade, dict) else None
    snap["integration"] = {"engine": "PCS+G.Y", "version": VERSION}
    return snap


# Keep v19.39 snapshot semantics available to any legacy caller while ensuring
# automatic G.Y persistence uses the integration snapshot below.
_sr_snapshot_v1940_base = _sr_snapshot
_sr_snapshot = _v1940_sr_snapshot


# Replace only the G.Y decision function. MAIN/evaluate_signal remains untouched.
_sr_analyze_v1940_base = sr_analyze

def sr_analyze():
    """v19.40: v19.39 G.Y engine + PCS confluence gate."""
    result = _sr_analyze_v1940_base()
    ...
    sr = result.get("sr") or {}
    df = result.get("df")
    if not signal or not candidate or df is None:
        return result

    geometry = signal.get("geometry") if isinstance(signal, dict) else None
    if not geometry:
        geometry = _sr_geometry_diagnostics(candidate)
    pcs = _v1940_pcs_evaluate(candidate, geometry, sr, df)
    signal = dict(signal)
    signal["pcs"] = pcs
    result["pcs"] = pcs
    result["signal"] = signal if pcs.get("approved") else None
    result["trade"] = signal if pcs.get("approved") else None
    if not pcs.get("approved"):
        result["reason"] = pcs.get("reason", "لم تمر بوابات PCS.")
        result["execution_state"] = "PCS_BLOCKED"
        result["rejection_reasons"] = list(dict.fromkeys((result.get("rejection_reasons") or []) + [pcs.get("reason", "بوابة PCS لم تمر.")]))
    else:
        result["execution_state"] = "READY"
    return result


# Final persistence boundary: even if another caller supplies a G.Y trade,
# it cannot enter sr_trades unless the attached PCS snapshot explicitly passed.
_v1940_base_create_auto_gy_trade = _v1935_create_auto_gy_trade

def _v1935_create_auto_gy_trade(trade, snapshot):
    pcs = (snapshot or {}).get("pcs") if isinstance(snapshot, dict) else None
    if not isinstance(pcs, dict) or not pcs.get("approved"):
        logger.warning("PCS_GY_PERSISTENCE_BLOCKED | reason=%s", (pcs or {}).get("reason", "missing_pcs_approval"))
        return None, False, "pcs_blocked"
    return _v1940_base_create_auto_gy_trade(trade, snapshot)


# Manual G.Y/S-R persistence uses the same PCS approval boundary.
_v1940_base_sr_insert_trade = _sr_insert_trade

def _sr_insert_trade(trade, snapshot):
    pcs = (trade or {}).get("pcs") if isinstance(trade, dict) else None
    if not isinstance(pcs, dict) and isinstance(snapshot, dict):
        pcs = snapshot.get("pcs")
    if not isinstance(pcs, dict) or not pcs.get("approved"):
        logger.warning("PCS_GY_MANUAL_PERSISTENCE_BLOCKED | reason=%s", (pcs or {}).get("reason", "missing_pcs_approval"))
        return None, False
    return _v1940_base_sr_insert_trade(trade, snapshot)


# The canonical G.Y loop resolves this global at runtime, so rebinding the
# function above makes PCS part of M15 -> G.Y -> persistence -> outbox.
GY_PCS_CONTRACT = "ENFORCED"


def _v1940_pcs_self_tests():
    """Pure tests for PCS gates; no HTTP, Telegram, SQLite or live market I/O."""
    base = {
        "direction": "BUY", "trigger": "BREAKOUT_RETEST", "score": 92,
        "scenario": {"confirmed": True}, "atr": 10.0,
    }
    geom = {"ok": True, "rr": 2.0}
    sr = {
        "support1": 4250.0, "support2": 4240.0, "support3": 4230.0,
        "resistance1": 4300.0, "resistance2": 4350.0, "resistance3": 4400.0,
        "atr": 10.0,
    }
    df = pd.DataFrame([
        {"open": 4280.0, "high": 4290.0, "low": 4270.0, "close": 4285.0},
    ] * 30)
    fake_context = {
        "version": PCS_VERSION, "key": "SELFTEST", "errors": {},
        "frames": {
            "W1": {"direction": "BUY", "score": 80, "structure": "صاعد", "adx": 30},
            "D1": {"direction": "BUY", "score": 78, "structure": "صاعد", "adx": 28},
            "H4": {"direction": "BUY", "score": 75, "structure": "صاعد", "adx": 27},
        },
    }
    original = globals().get("_v1940_fetch_pcs_context")
    try:
        globals()["_v1940_fetch_pcs_context"] = lambda _df: fake_context
        ok = _v1940_pcs_evaluate(base, geom, sr, df)
        blocked = _v1940_pcs_evaluate({**base, "direction": "SELL"}, geom, sr, df)
        return {
            "approved_aligned_setup": bool(ok.get("approved")),
            "score_is_bounded": 0 <= int(ok.get("score", -1)) <= 100,
            "opposite_direction_blocked": not bool(blocked.get("approved")),
            "persistence_contract": GY_PCS_CONTRACT == "ENFORCED",
        }
    finally:
        globals()["_v1940_fetch_pcs_context"] = original


# v19.41 final runtime version marker. Diagnostic-only release; no strategy constant is changed.
VERSION = "v19.43_ALERT_LIFECYCLE_SUPERVISOR"
GEOMETRY_DIAGNOSTICS_VERSION = "1.0"


# ============================================================
# v19.43 — ALERT LIFECYCLE SUPERVISOR ONLY
# Scope: PersistentAlertDispatcher worker lifecycle / SQLite failure
# containment / Telegram worker recovery. No strategy, trade, quota,
# PCS, G.Y, Outbox schema, Claim/SENT semantics or idempotency changes.
# ============================================================
ALERT_LIFECYCLE_SUPERVISOR_SECONDS = max(
    0.5, float(os.getenv("ALERT_LIFECYCLE_SUPERVISOR_SECONDS", "2.0"))
)
ALERT_LIFECYCLE_CLAIM_RETRY_SECONDS = max(
    0.25, float(os.getenv("ALERT_LIFECYCLE_CLAIM_RETRY_SECONDS", "1.0"))
)
ALERT_LIFECYCLE_RESULT_RETRIES = max(
    1, int(os.getenv("ALERT_LIFECYCLE_RESULT_RETRIES", "5"))
)
ALERT_LIFECYCLE_RESULT_RETRY_SECONDS = max(
    0.25, float(os.getenv("ALERT_LIFECYCLE_RESULT_RETRY_SECONDS", "1.0"))
)

def _v1943_alive_alert_tasks(dispatcher):
    return [
        t for t in list(getattr(dispatcher, "_tasks", []) or [])
        if t is not None and not t.done()
    ]

def _v1943_spawn_alert_worker(self, idx):
    task = asyncio.create_task(
        self._worker(idx),
        name=f"persist-alert-{int(idx)}",
    )
    if not isinstance(getattr(self, "_tasks", None), list):
        self._tasks = []
    self._tasks.append(task)
    logger.info(
        "ALERT_WORKER_SPAWNED | idx=%s task=%s",
        int(idx), id(task),
    )
    return task

async def _v1943_safe_result(self, job, ok, error=None, retry_after=None):
    """Persist worker outcome without allowing SQLite failure to kill the worker."""
    last_error = None
    for attempt in range(1, ALERT_LIFECYCLE_RESULT_RETRIES + 1):
        try:
            await asyncio.to_thread(
                self._result, job, ok, error, retry_after
            )
            return True
        except asyncio.CancelledError:
            raise
            ...
            last_error = exc
            logger.exception(
                "ALERT_RESULT_PERSIST_RETRY | id=%s attempt=%s/%s error=%s",
                job.get("alert_id"), attempt,
                ALERT_LIFECYCLE_RESULT_RETRIES, exc,
            )
            if attempt < ALERT_LIFECYCLE_RESULT_RETRIES:
                await asyncio.sleep(
                    ALERT_LIFECYCLE_RESULT_RETRY_SECONDS * attempt
                )
    logger.error(
        "ALERT_RESULT_PERSIST_DEFERRED | id=%s error=%s",
        job.get("alert_id"), last_error,
    )
    return False

async def _v1943_dispatcher_worker(self, idx):
    """Lifecycle-hardened worker: claim/result failures are contained."""
    while self._running:
        try:
            job = await asyncio.to_thread(self._claim)
        except asyncio.CancelledError:
            ...
            logger.exception(
                "ALERT_CLAIM_FAILURE_CONTAINED | idx=%s error=%s",
                int(idx), exc,
            )
            await asyncio.sleep(ALERT_LIFECYCLE_CLAIM_RETRY_SECONDS)
            continue
        ...
        if not job:
            ...
            try:
                await asyncio.wait_for(
                    self._wake.wait(), timeout=ALERT_POLL_SECONDS
                )
            except asyncio.TimeoutError:
                pass
            except asyncio.CancelledError:
        ...
        with self._telemetry_lock:
            self._telemetry["send_started"] += 1
        logger.debug(
            "ALERT_SEND_START | id=%s kind=%s",
            job["alert_id"], job["kind"],
        )
        ...
        try:
            ...
            for part_index, part in enumerate(parts, 1):
                msg = await self.bot.send_message(
                    chat_id=job["chat_id"], text=localize_user_text(part)
                )
                message_id = getattr(msg, "message_id", None)
                await asyncio.to_thread(
                    self._record_part_delivery,
                    job["alert_id"], part_index, message_id,
                )

            persisted = await _v1943_safe_result(self, job, True)
            if persisted:
                with self._telemetry_lock:
                    self._telemetry["sent"] += 1
                logger.info(
                    "ALERT_DELIVERED_E2E | id=%s kind=%s queue_plus_delivery_ms=%.1f",
                    job["alert_id"], job["kind"],
                    (time.time() - job["created_at"]) * 1000,
                )
            else:
                logger.error(
                    "ALERT_DELIVERY_CHECKPOINT_DEFERRED | id=%s",
                    job["alert_id"],
                )
            ...
            await _v1943_safe_result(
                self, {**job, "max_attempts": 1}, False, exc
            )

        except RetryAfter as exc:
            with self._telemetry_lock:
                self._telemetry["retried"] += 1
            ...
            with self._telemetry_lock:
                self._telemetry["retried"] += 1
            logger.warning(
                "TELEGRAM_RETRY | id=%s kind=%s error=%s",
                job["alert_id"], job["kind"], exc,
            )
            await _v1943_safe_result(self, job, False, exc)

def _v1943_recover_stale_sending(self):
    """Reuse the existing crash-recovery rule for runtime-dead/stuck workers."""
    cutoff = time.time() - max(
        30.0,
        float(os.getenv("ALERT_LIFECYCLE_STALE_SENDING_SECONDS", "300"))
    )
    with self._db_lock:
        c = self._conn()
        try:
            rows = c.execute(
                "SELECT alert_id,parts_total,parts_sent FROM alert_outbox "
                "WHERE status='SENDING' AND COALESCE(send_started_at,claimed_at,last_attempt,0)<=?",
                (cutoff,),
            ).fetchall()
            recovered = 0
            for row in rows:
                complete = (
                    row["parts_total"] is not None
                    and int(row["parts_total"] or 0) > 0
                    and int(row["parts_sent"] or 0) >= int(row["parts_total"] or 0)
                )
                if complete:
                    c.execute(
                        "UPDATE alert_outbox SET status='SENT',"
                        "sent_at=COALESCE(sent_at,?),last_error=NULL "
                        "WHERE alert_id=? AND status='SENDING'",
                        (time.time(), row["alert_id"]),
                    )
                else:
                    c.execute(
                        ...
                        "WHERE alert_id=? AND status='SENDING'",
                        (time.time(), "runtime worker recovery", row["alert_id"]),
                    )
                recovered += 1
            c.commit()
            if recovered:
                logger.warning(
                    "ALERT_STALE_SENDING_RECOVERED | count=%s", recovered
                )
            return recovered
        finally:
            c.close()

def _v1943_reap_completed_worker(task):
    ...
    try:
        ...
        logger.exception("ALERT_WORKER_EXCEPTION_READ_FAILED | error=%s", exc)
        return
    if exc is not None:
        logger.error(
            "ALERT_WORKER_DIED | task=%s error=%s",
            id(task), exc,
        )

async def _v1943_worker_supervisor(self):
    """One permanent supervisor per dispatcher instance; never creates a second dispatcher."""
    while self._running:
        try:
            tasks = list(getattr(self, "_tasks", []) or [])

            # Reap completed workers so their exceptions are observed, then
            # keep only live tasks for the slot-reconciliation pass.
            live_tasks = []
            live_indices = set()
            for t in tasks:
                if t is None:
                    continue
                if t.done():
                    _v1943_reap_completed_worker(t)
                    continue
                name = t.get_name()
                if name.startswith("persist-alert-"):
                    try:
                        idx = int(name.rsplit("-", 1)[1])
                    except Exception:
                        idx = None
                    if idx is not None and idx in live_indices:
                        # Defensive duplicate-slot cleanup: never allow two
                        # workers to occupy one logical slot.
                        t.cancel()
                        continue
                    if idx is not None:
            ...
            # Recreate only missing/dead worker slots on this same dispatcher.

            for idx in range(self.workers):
                if idx not in live_indices:
                    if not self._running:
                        break
                    _v1943_spawn_alert_worker(self, idx)

            if not self._tasks and self._running:
                logger.warning(
                    "ALERT_WORKER_SUPERVISOR_ALL_WORKERS_RESTARTED | workers=%s",
                    self.workers,
                )

            try:
                await asyncio.to_thread(_v1943_recover_stale_sending, self)
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                logger.exception(
                    "ALERT_STALE_SENDING_RECOVERY_ERROR | error=%s",
                    exc,
                )

            await asyncio.sleep(ALERT_LIFECYCLE_SUPERVISOR_SECONDS)

        except asyncio.CancelledError:
            raise
        except Exception as exc:
            logger.exception(
                "ALERT_WORKER_SUPERVISOR_ERROR | error=%s",
                exc,
            )
            await asyncio.sleep(ALERT_LIFECYCLE_SUPERVISOR_SECONDS)

async def _v1943_dispatcher_start(self):
    # Serialize concurrent start calls so the singleton can never create a
    # second worker set merely because two callers raced at startup.
    lock = getattr(self, "_lifecycle_start_lock", None)
    if lock is None:
        lock = asyncio.Lock()
        self._lifecycle_start_lock = lock

    async with lock:
        if self._running:
            supervisor = getattr(self, "_supervisor_task", None)
            if supervisor is None or supervisor.done():
                self._supervisor_task = asyncio.create_task(
                    self._worker_supervisor(),
                    name="persist-alert-supervisor",
                )
                logger.info(
                    "ALERT_WORKER_SUPERVISOR_RESTARTED | task=%s",
                    id(self._supervisor_task),
                )
            return
        ...
        logger.info(
            "PersistentAlertDispatcher started | workers=%s supervisor=%s",
            self.workers, id(self._supervisor_task),
        )

async def _v1943_dispatcher_stop(self):
    lock = getattr(self, "_lifecycle_start_lock", None)
    if lock is None:
        lock = asyncio.Lock()
        self._lifecycle_start_lock = lock
...
async def _v1943_dispatcher_health_async(self):
    base = await asyncio.to_thread(self.health)
    tasks = list(getattr(self, "_tasks", []) or [])
    base["worker_count"] = len(
        [t for t in tasks if t is not None and not t.done()]
    )
    supervisor = getattr(self, "_supervisor_task", None)
    base["worker_supervisor_running"] = bool(
        supervisor is not None and not supervisor.done()
    )
    return base

# Final lifecycle-only bindings. The existing dispatcher instance, DB,
# outbox rows, idempotency keys and business contracts remain unchanged.
PersistentAlertDispatcher._worker = _v1943_dispatcher_worker
PersistentAlertDispatcher._worker_supervisor = _v1943_worker_supervisor
PersistentAlertDispatcher._safe_result = _v1943_safe_result
PersistentAlertDispatcher.start = _v1943_dispatcher_start
PersistentAlertDispatcher.stop = _v1943_dispatcher_stop
PersistentAlertDispatcher.health_async = _v1943_dispatcher_health_async

async def _v1943_start_alert_dispatcher():
    global ALERT_DISPATCHER
    if not APPLICATION:
        return None
    if not isinstance(ALERT_DISPATCHER, PersistentAlertDispatcher):
        if ALERT_DISPATCHER is not None:
            try:
                ...
                logger.exception("Previous dispatcher stop failed")
        ALERT_DISPATCHER = PersistentAlertDispatcher(
            APPLICATION.bot,
            workers=int(os.getenv("ALERT_WORKERS", "4")),
        )

    # Same singleton dispatcher: start() only repairs its worker lifecycle.
    await ALERT_DISPATCHER.start()

    # Preserve the existing durable trade-link reconciliation lifecycle.
    await asyncio.to_thread(_init_alert_trade_links)
    await _reconcile_alert_trade_links_once()
    if not any(
        t.get_name() == "alert-trade-reconciliation" and not t.done()
        for t in asyncio.all_tasks()
    ):
        asyncio.create_task(
            _alert_trade_reconciliation_loop(),
            name="alert-trade-reconciliation",
        )
    return ALERT_DISPATCHER

_start_alert_dispatcher = _v1943_start_alert_dispatcher

def _v1943_alert_lifecycle_static_contract_self_test():
    """Static contract: lifecycle-only names must exist; no trading symbol is rebound here."""
    required = (
        "_v1943_worker_supervisor",
        "_v1943_safe_result",
        "_v1943_dispatcher_worker",
        "_v1943_dispatcher_start",
        "_v1943_dispatcher_stop",
        "_v1943_recover_stale_sending",
    )
    return {
        "ok": all(callable(globals().get(name)) for name in required),
        "outbox_semantics": "PENDING/SENDING/SENT/FAILED unchanged",
        "idempotency": "unchanged",
        "scope": "ALERT_LIFECYCLE_ONLY",
    }
...
# v19.42 contract marker: W1 is supportive/non-supportive context, never a standalone G.Y blocker.
W1_GY_POLICY = "NON_BLOCKING_CONTEXT"

# ============================================================
# v19.44 — PAYMENT DATABASE CONTRACT v1
# Scope: payment persistence contract only.
# No Blockchain Monitor, no pricing recalculation, no trade logic changes.
# ============================================================
PAYMENT_DB_VERSION = "v1"
PAYMENT_DB_PATH = os.getenv("PAYMENT_DB_PATH", "payments.db")
PAYMENT_CHAIN_ID = 56
PAYMENT_NETWORK = "BSC"
PAYMENT_CURRENCY = "USDT"
PAYMENT_RECEIVER_ADDRESS = os.getenv(
    "PAYMENT_RECEIVER_ADDRESS",
    "0x1b1DB0D9d46Ef2a3E243beD0ED235da3a9646FF4",
).lower()
PAYMENT_USDT_CONTRACT = os.getenv("PAYMENT_USDT_CONTRACT", "0x55d398326f99059fF775485246999027B3197955").strip().lower()
PAYMENT_FINALITY_MODE = os.getenv("PAYMENT_FINALITY_MODE", "FINALIZED").upper()
PAYMENT_ORDER_MIN_DAYS = 1
PAYMENT_ORDER_MAX_DAYS = 365
PAYMENT_ORDER_TTL_SECONDS = max(300, int(os.getenv("PAYMENT_ORDER_TTL_SECONDS", "1800")))

PAYMENT_ORDER_STATES = frozenset({
    "CREATED", "WAITING_PAYMENT", "PAYMENT_DETECTED", "VERIFYING",
    "CONFIRMED", "ACTIVATING", "ACTIVATED", "EXPIRED", "UNDERPAID",
    "OVERPAID", "WRONG_NETWORK", "WRONG_TOKEN", "WRONG_RECEIVER",
    "FAILED", "MANUAL_REVIEW", "CANCELLED",
})
PAYMENT_TERMINAL_STATES = frozenset({
    "ACTIVATED", "EXPIRED", "UNDERPAID", "OVERPAID", "WRONG_NETWORK",
    "WRONG_TOKEN", "WRONG_RECEIVER", "FAILED", "CANCELLED",
})
PAYMENT_STATE_TRANSITIONS = {
    "CREATED": {"WAITING_PAYMENT", "CANCELLED"},
    "WAITING_PAYMENT": {"PAYMENT_DETECTED", "EXPIRED", "CANCELLED"},
    "PAYMENT_DETECTED": {"VERIFYING", "MANUAL_REVIEW", "FAILED"},
    "VERIFYING": {
        "VERIFYING", "CONFIRMED", "UNDERPAID", "OVERPAID",
        "WRONG_NETWORK", "WRONG_TOKEN", "WRONG_RECEIVER", "MANUAL_REVIEW", "FAILED",
    },
    "CONFIRMED": {"ACTIVATING", "MANUAL_REVIEW", "FAILED"},
    "ACTIVATING": {"ACTIVATED", "MANUAL_REVIEW", "FAILED"},
    "MANUAL_REVIEW": {"CONFIRMED", "ACTIVATING", "ACTIVATED", "FAILED", "CANCELLED"},
    "ACTIVATED": set(), "EXPIRED": {"MANUAL_REVIEW"}, "UNDERPAID": {"MANUAL_REVIEW"},
    "OVERPAID": {"MANUAL_REVIEW"}, "WRONG_NETWORK": {"MANUAL_REVIEW"},
    "WRONG_TOKEN": {"MANUAL_REVIEW"}, "WRONG_RECEIVER": {"MANUAL_REVIEW"},
    "FAILED": {"MANUAL_REVIEW"}, "CANCELLED": set(),
}
...
CREATE TABLE IF NOT EXISTS payment_orders (
    ...
    plan_code TEXT NOT NULL CHECK(plan_code IN ('BASIC','ELITE')),
    duration_days INTEGER NOT NULL CHECK(duration_days BETWEEN 1 AND 365),
    base_price_atomic INTEGER NOT NULL CHECK(base_price_atomic >= 0),
    discount_percent_bp INTEGER NOT NULL DEFAULT 0 CHECK(discount_percent_bp BETWEEN 0 AND 10000),
    discount_atomic INTEGER NOT NULL DEFAULT 0 CHECK(discount_atomic >= 0),
    final_price_atomic INTEGER NOT NULL CHECK(final_price_atomic > 0),
    currency TEXT NOT NULL CHECK(currency = 'USDT'),
    network TEXT NOT NULL CHECK(network = 'BSC'),
    chain_id INTEGER NOT NULL CHECK(chain_id = 56),
    token_contract TEXT NOT NULL,
    receiver_address TEXT NOT NULL,
    status TEXT NOT NULL CHECK(status IN (
        'CREATED','WAITING_PAYMENT','PAYMENT_DETECTED','VERIFYING','CONFIRMED','ACTIVATING','ACTIVATED',
        'EXPIRED','UNDERPAID','OVERPAID','WRONG_NETWORK','WRONG_TOKEN','WRONG_RECEIVER','FAILED','MANUAL_REVIEW','CANCELLED'
    )),
...
CREATE TABLE IF NOT EXISTS payment_transactions (
    ...
    finality_status TEXT NOT NULL DEFAULT 'PENDING' CHECK(finality_status IN ('PENDING','FINALIZED','UNKNOWN')),
    confirmations INTEGER NOT NULL DEFAULT 0 CHECK(confirmations >= 0),
    detected_at INTEGER NOT NULL,
    verified_at INTEGER,
    verification_status TEXT NOT NULL DEFAULT 'UNVERIFIED' CHECK(verification_status IN ('UNVERIFIED','VERIFYING','VERIFIED','REJECTED','UNKNOWN')),
...
CREATE INDEX IF NOT EXISTS idx_payment_tx_receiver_token_block ON payment_transactions(to_address, token_contract, block_number);
CREATE INDEX IF NOT EXISTS idx_payment_tx_verification ON payment_transactions(verification_status, finality_status);
CREATE INDEX IF NOT EXISTS idx_payment_tx_block ON payment_transactions(chain_id, block_number);
...
CREATE TABLE IF NOT EXISTS payment_verifications (
    ...
    check_type TEXT NOT NULL CHECK(check_type IN (
        'CHAIN','TOKEN_CONTRACT','TOKEN_DECIMALS','RECEIVER','AMOUNT','RECEIPT',
        'TRANSFER_EVENT','FINALITY','DUPLICATE','ORDER_MATCH'
    )),
    expected_value TEXT,
    actual_value TEXT,
...
CREATE INDEX IF NOT EXISTS idx_payment_verifications_order ON payment_verifications(payment_order_id, checked_at);
CREATE INDEX IF NOT EXISTS idx_payment_verifications_tx ON payment_verifications(payment_transaction_id, checked_at);
...
CREATE INDEX IF NOT EXISTS idx_payment_events_order_time ON payment_events(order_id, created_at);
...
CREATE TABLE IF NOT EXISTS payment_attempts (
    ...
    order_id INTEGER,
    transaction_id INTEGER,
    attempt_type TEXT NOT NULL CHECK(attempt_type IN ('BLOCK_SCAN','TX_LOOKUP','RECEIPT_LOOKUP','FINALITY_CHECK','TRANSFER_PARSE','VERIFICATION')),
    attempt_number INTEGER NOT NULL CHECK(attempt_number >= 1),
    status TEXT NOT NULL CHECK(status IN ('STARTED','SUCCESS','RETRY','FAILED','UNKNOWN')),
    rpc_endpoint TEXT,
...
CREATE INDEX IF NOT EXISTS idx_payment_attempts_order_time ON payment_attempts(order_id, started_at);
CREATE INDEX IF NOT EXISTS idx_payment_attempts_tx_time ON payment_attempts(transaction_id, started_at);

CREATE TABLE IF NOT EXISTS payment_outbox (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    event_id TEXT NOT NULL,
    recipient_type TEXT NOT NULL CHECK(recipient_type IN ('SUBSCRIBER','ADMIN')),
    ...
    sent_at INTEGER,
    updated_at INTEGER NOT NULL,
    UNIQUE(event_id, recipient_type, telegram_user_id)
);
CREATE INDEX IF NOT EXISTS idx_payment_outbox_ready ON payment_outbox(status, next_retry_at, created_at);
CREATE INDEX IF NOT EXISTS idx_payment_outbox_recipient ON payment_outbox(telegram_user_id, status);
...
CREATE TABLE IF NOT EXISTS payment_scanner_state (
    ...
    last_scanned_block_hash TEXT NOT NULL DEFAULT '',
    last_finalized_block INTEGER NOT NULL DEFAULT 0 CHECK(last_finalized_block >= 0),
...
CREATE TABLE IF NOT EXISTS payment_config (
    ...
    token_decimals INTEGER NOT NULL DEFAULT 0 CHECK(token_decimals BETWEEN 0 AND 36),
    receiver_address TEXT NOT NULL,
    finality_mode TEXT NOT NULL DEFAULT 'FINALIZED' CHECK(finality_mode IN ('FINALIZED','CONFIRMATIONS')),
    enabled INTEGER NOT NULL DEFAULT 0 CHECK(enabled IN (0,1)),
    updated_at INTEGER NOT NULL
);

CREATE TRIGGER IF NOT EXISTS trg_payment_events_no_update
...
class PaymentDatabase:
    """Durable SQLite implementation of Payment Database Contract v1.

    This class is persistence-only. It does not call RPC, Telegram, pricing,
    subscription activation, or any trading function.
    """
    def __init__(self, path=PAYMENT_DB_PATH):
        self.path = path
        self._lock = threading.RLock()
        self.initialize()
    ...
    def initialize(self):
        with self._lock, self.connect() as conn:
            conn.executescript(PAYMENT_DB_SCHEMA)
            now = int(time.time())
            conn.execute("""INSERT INTO payment_config
                (network,chain_id,token_symbol,token_contract,token_decimals,receiver_address,finality_mode,enabled,updated_at)
                VALUES(?,?,?,?,?,?,?,?,?)
                ...
                (PAYMENT_NETWORK, PAYMENT_CHAIN_ID, PAYMENT_CURRENCY,
                 PAYMENT_USDT_CONTRACT, PAYMENT_TOKEN_DECIMALS, PAYMENT_RECEIVER_ADDRESS,
                 PAYMENT_FINALITY_MODE, 1 if PAYMENT_USDT_CONTRACT else 0, now))
            conn.execute("""INSERT INTO payment_scanner_state(network,chain_id,updated_at)
                VALUES(?,?,?) ON CONFLICT(network) DO NOTHING""",
                (PAYMENT_NETWORK, PAYMENT_CHAIN_ID, now))
            conn.commit()

    @staticmethod
    def _json(value):
        return json.dumps(value if value is not None else {}, ensure_ascii=False, sort_keys=True, default=str, separators=(",", ":"))

    @staticmethod
    def _order_status_check(old, new):
        if old == new:
            return True
        return new in PAYMENT_STATE_TRANSITIONS.get(old, set())
    ...
    def create_order(self, *, order_id, telegram_user_id, plan_code, duration_days,
                     base_price_atomic, discount_percent_bp, discount_atomic,
                     final_price_atomic, token_contract=None, receiver_address=None,
                     created_by="user", now=None, expires_at=None):
        now = int(time.time()) if now is None else int(now)
        expires_at = now + PAYMENT_ORDER_TTL_SECONDS if expires_at is None else int(expires_at)
        token_contract = (token_contract or PAYMENT_USDT_CONTRACT).lower()
        receiver_address = (receiver_address or PAYMENT_RECEIVER_ADDRESS).lower()
        if not token_contract:
            raise ValueError("PAYMENT_USDT_CONTRACT is not configured; payment order cannot be created")
        if plan_code not in {"BASIC", "ELITE"}:
            raise ValueError("Invalid plan_code")
        if not (PAYMENT_ORDER_MIN_DAYS <= int(duration_days) <= PAYMENT_ORDER_MAX_DAYS):
            raise ValueError("duration_days must be between 1 and 365")
        if int(final_price_atomic) <= 0:
            raise ValueError("final_price_atomic must be positive")
        with self._transaction() as conn:
            cur = conn.execute("""INSERT INTO payment_orders
                (order_id,telegram_user_id,plan_code,duration_days,base_price_atomic,discount_percent_bp,
                 discount_atomic,final_price_atomic,currency,network,chain_id,token_contract,receiver_address,
                 status,created_at,expires_at,created_by,updated_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,'CREATED',?,?,?,?)""",
                (str(order_id), int(telegram_user_id), plan_code, int(duration_days), int(base_price_atomic),
                 int(discount_percent_bp), int(discount_atomic), int(final_price_atomic), PAYMENT_CURRENCY,
                 PAYMENT_NETWORK, PAYMENT_CHAIN_ID, token_contract, receiver_address, now, expires_at,
                 created_by, now))
            order_pk = int(cur.lastrowid)
            event_id = f"ORDER_CREATED:{order_id}"
            conn.execute("""INSERT INTO payment_events
                (event_id,order_id,event_type,old_status,new_status,actor_type,actor_id,payload_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?)""",
                (event_id, order_pk, "ORDER_CREATED", None, "CREATED", created_by,
                 str(telegram_user_id), self._json({"order_id": order_id}), now))
            conn.execute("""UPDATE payment_orders SET status='WAITING_PAYMENT',updated_at=? WHERE id=?""", (now, order_pk))
            conn.execute("""INSERT INTO payment_events
                (event_id,order_id,event_type,old_status,new_status,actor_type,actor_id,payload_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?)""",
                (f"ORDER_WAITING:{order_id}", order_pk, "ORDER_WAITING_PAYMENT", "CREATED", "WAITING_PAYMENT",
                 "system", "payment-db", "{}", now))
            return order_pk

    def get_order(self, order_id):
        with self.connect() as conn:
            row = conn.execute("SELECT * FROM payment_orders WHERE order_id=?", (str(order_id),)).fetchone()
            return dict(row) if row else None

    def transition_order(self, order_id, new_status, *, actor_type="system", actor_id="payment-db",
                         reason=None, event_type=None, payload=None, now=None):
        new_status = str(new_status).upper()
        if new_status not in PAYMENT_ORDER_STATES:
            raise ValueError(f"Unknown payment state: {new_status}")
        now = int(time.time()) if now is None else int(now)
        with self._transaction() as conn:
            row = conn.execute("SELECT * FROM payment_orders WHERE order_id=?", (str(order_id),)).fetchone()
            if not row:
            ...
            old = str(row["status"])
            if not self._order_status_check(old, new_status):
                raise ValueError(f"Invalid payment state transition {old} -> {new_status}")
            updates = ["status=?", "updated_at=?"]
            params = [new_status, now]
            if new_status == "CONFIRMED":
                updates.append("confirmed_at=COALESCE(confirmed_at,?)"); params.append(now)
            if new_status == "ACTIVATED":
                updates.append("activated_at=COALESCE(activated_at,?)"); params.append(now)
            if reason:
                updates.append("failure_reason=?"); params.append(str(reason)[:1000])
            params.append(str(order_id))
            conn.execute(f"UPDATE payment_orders SET {', '.join(updates)} WHERE order_id=?", params)
            eid = f"STATE:{order_id}:{new_status}:{now}:{uuid.uuid4().hex[:8]}"
            conn.execute("""INSERT INTO payment_events
                (event_id,order_id,event_type,old_status,new_status,actor_type,actor_id,payload_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?)""",
                (eid, row["id"], event_type or f"PAYMENT_{new_status}", old, new_status,
                 actor_type, str(actor_id), self._json(payload or ({"reason": reason} if reason else {})), now))
            return new_status

    def record_transaction(self, data):
        required = ("chain_id", "tx_hash", "token_contract", "token_symbol", "token_decimals",
                    "block_number", "block_hash", "from_address", "to_address", "amount_atomic", "detected_at")
        missing = [k for k in required if k not in data]
        if missing:
            raise ValueError(f"Missing transaction fields: {missing}")
        with self._transaction() as conn:
            try:
                cur = conn.execute("""INSERT INTO payment_transactions
                    (chain_id,tx_hash,token_contract,token_symbol,token_decimals,block_number,block_hash,
                     transaction_index,log_index,from_address,to_address,amount_atomic,receipt_status,
                     finality_status,confirmations,detected_at,verified_at,verification_status,payment_order_id,raw_reference)
                    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                    (int(data["chain_id"]), str(data["tx_hash"]).lower(), str(data["token_contract"]).lower(),
                     str(data["token_symbol"]), int(data["token_decimals"]), int(data["block_number"]),
                     str(data["block_hash"]), data.get("transaction_index"), data.get("log_index"),
                     str(data["from_address"]).lower(), str(data["to_address"]).lower(), int(data["amount_atomic"]),
                     data.get("receipt_status"), str(data.get("finality_status", "PENDING")).upper(),
                     int(data.get("confirmations", 0)), int(data["detected_at"]), data.get("verified_at"),
                     str(data.get("verification_status", "UNVERIFIED")).upper(), data.get("payment_order_id"),
                     data.get("raw_reference")))
                return int(cur.lastrowid), True
            except sqlite3.IntegrityError as exc:
                if "UNIQUE" in str(exc).upper():
                    row = conn.execute("SELECT id FROM payment_transactions WHERE chain_id=? AND tx_hash=? AND log_index IS ?",
                                       (int(data["chain_id"]), str(data["tx_hash"]).lower(), data.get("log_index"))).fetchone()
                    return (int(row[0]) if row else None), False
                raise

    def add_verification(self, *, payment_transaction_id=None, payment_order_id=None, check_type,
                         expected_value=None, actual_value=None, result, error_code=None,
                         rpc_endpoint=None, attempt_number=1, checked_at=None, duration_ms=None):
        ...
        with self._transaction() as conn:
            cur = conn.execute("""INSERT INTO payment_verifications
                (payment_transaction_id,payment_order_id,check_type,expected_value,actual_value,result,
                 error_code,rpc_endpoint,attempt_number,checked_at,duration_ms)
                VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                (payment_transaction_id, payment_order_id, check_type, None if expected_value is None else str(expected_value),
                 None if actual_value is None else str(actual_value), result, error_code, rpc_endpoint,
                 int(attempt_number), checked_at, duration_ms))
            return int(cur.lastrowid)

    def append_event(self, *, event_id, event_type, order_id=None, transaction_id=None,
                     old_status=None, new_status=None, actor_type="system", actor_id="payment-db",
                     payload=None, created_at=None):
        created_at = int(time.time()) if created_at is None else int(created_at)
        with self._transaction() as conn:
            conn.execute("""INSERT INTO payment_events
                (event_id,order_id,transaction_id,event_type,old_status,new_status,actor_type,actor_id,payload_json,created_at)
                VALUES(?,?,?,?,?,?,?,?,?,?)""",
                (str(event_id), order_id, transaction_id, str(event_type), old_status, new_status,
                 actor_type, str(actor_id), self._json(payload), created_at))

    def record_attempt(self, *, attempt_type, attempt_number, status, order_id=None, transaction_id=None,
                       rpc_endpoint=None, started_at=None, finished_at=None, duration_ms=None,
                       error_code=None, error_message=None):
        started_at = int(time.time()) if started_at is None else int(started_at)
        with self._transaction() as conn:
            cur = conn.execute("""INSERT INTO payment_attempts
                (order_id,transaction_id,attempt_type,attempt_number,status,rpc_endpoint,started_at,finished_at,
                 duration_ms,error_code,error_message)
                VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
                (order_id, transaction_id, attempt_type, int(attempt_number), status, rpc_endpoint, started_at,
                 finished_at, duration_ms, error_code, None if error_message is None else str(error_message)[:2000]))
            return int(cur.lastrowid)

    def enqueue_notification(self, *, event_id, recipient_type, telegram_user_id, notification_type,
                             payload, created_at=None):
        created_at = int(time.time()) if created_at is None else int(created_at)
        with self._transaction() as conn:
            conn.execute("""INSERT OR IGNORE INTO payment_outbox
                (event_id,recipient_type,telegram_user_id,notification_type,payload_json,status,attempts,created_at,updated_at)
                VALUES(?,?,?,?,?,'PENDING',0,?,?)""",
                (str(event_id), recipient_type, int(telegram_user_id) if telegram_user_id is not None else None,
                 notification_type, self._json(payload), created_at, created_at))
            row = conn.execute("""SELECT id FROM payment_outbox
                WHERE event_id=? AND recipient_type=? AND telegram_user_id IS ?""",
                (str(event_id), recipient_type, int(telegram_user_id) if telegram_user_id is not None else None)).fetchone()
            return int(row[0]) if row else None

    def scanner_state(self):
        with self.connect() as conn:
            row = conn.execute("SELECT * FROM payment_scanner_state WHERE network=?", (PAYMENT_NETWORK,)).fetchone()
            return dict(row) if row else None

    def update_scanner_state(self, *, last_scanned_block, last_scanned_block_hash="",
                             last_finalized_block=0, error=None, now=None):
        now = int(time.time()) if now is None else int(now)
        with self._transaction() as conn:
            conn.execute("""UPDATE payment_scanner_state SET last_scanned_block=?,last_scanned_block_hash=?,
                last_finalized_block=?,last_success_at=?,last_error_at=?,last_error=?,updated_at=? WHERE network=?""",
                (int(last_scanned_block), str(last_scanned_block_hash), int(last_finalized_block),
                 now if error is None else None, now if error is not None else None,
                 None if error is None else str(error)[:2000], now, PAYMENT_NETWORK))

    def expire_orders(self, now=None):
        now = int(time.time()) if now is None else int(now)
        changed = []
        with self._transaction() as conn:
            rows = conn.execute("""SELECT id,order_id,status FROM payment_orders
                WHERE status='WAITING_PAYMENT' AND expires_at<=? ORDER BY id""", (now,)).fetchall()
            for row in rows:
                conn.execute("UPDATE payment_orders SET status='EXPIRED',updated_at=? WHERE id=? AND status='WAITING_PAYMENT'",
                             (now, row["id"]))
                eid = f"EXPIRED:{row['order_id']}"
                conn.execute("""INSERT OR IGNORE INTO payment_events
                    (event_id,order_id,event_type,old_status,new_status,actor_type,actor_id,payload_json,created_at)
                    VALUES(?,?,?,?,?,?,?,?,?)""",
                    (eid, row["id"], "PAYMENT_EXPIRED", "WAITING_PAYMENT", "EXPIRED", "system", "expiry-worker", "{}", now))
                changed.append(row["order_id"])
        return changed


def _payment_database_contract_self_test():
    import tempfile
    failures = []
    with tempfile.TemporaryDirectory(prefix="xau_payment_db_") as td:
        path = os.path.join(td, "payments.db")
        db = PaymentDatabase(path)
        now = int(time.time())
        order_pk = db.create_order(
            order_id="TEST-ORDER-1", telegram_user_id=12345, plan_code="BASIC", duration_days=30,
            base_price_atomic=30000000, discount_percent_bp=0, discount_atomic=0,
            final_price_atomic=30000000, token_contract=(PAYMENT_USDT_CONTRACT or "0xtest"),
            now=now, expires_at=now + 1800,
        )
        order = db.get_order("TEST-ORDER-1")
        if not order or order["status"] != "WAITING_PAYMENT": failures.append("order_creation")
        db.transition_order("TEST-ORDER-1", "PAYMENT_DETECTED")
        db.transition_order("TEST-ORDER-1", "VERIFYING")
        db.transition_order("TEST-ORDER-1", "CONFIRMED")
        db.transition_order("TEST-ORDER-1", "ACTIVATING")
        db.transition_order("TEST-ORDER-1", "ACTIVATED")
        final = db.get_order("TEST-ORDER-1")
        if final["status"] != "ACTIVATED": failures.append("state_machine")
        try:
            db.transition_order("TEST-ORDER-1", "WAITING_PAYMENT")
            failures.append("illegal_transition_not_blocked")
        except ValueError:
            pass
        db.enqueue_notification(event_id="SUBSCRIPTION_ACTIVATED:TEST-ORDER-1", recipient_type="SUBSCRIBER",
                                telegram_user_id=12345, notification_type="SUBSCRIPTION_ACTIVATED", payload={"order_id":"TEST-ORDER-1"})
        db.enqueue_notification(event_id="SUBSCRIPTION_ACTIVATED:TEST-ORDER-1", recipient_type="SUBSCRIBER",
                                telegram_user_id=12345, notification_type="SUBSCRIPTION_ACTIVATED", payload={"order_id":"TEST-ORDER-1"})
        with db.connect() as conn:
            outbox_count = conn.execute("SELECT COUNT(*) FROM payment_outbox").fetchone()[0]
            event_count = conn.execute("SELECT COUNT(*) FROM payment_events WHERE order_id=?", (order_pk,)).fetchone()[0]
        if outbox_count != 1: failures.append("outbox_idempotency")
        if event_count < 5: failures.append("event_audit")
        return {"ok": not failures, "version": PAYMENT_DB_VERSION, "failures": failures}


def _init_payment_database_contract():
    global PAYMENT_DB
    PAYMENT_DB = PaymentDatabase(PAYMENT_DB_PATH)
    logger.info("PAYMENT_DB_READY | version=%s path=%s chain_id=%s receiver=%s contract_configured=%s",
                PAYMENT_DB_VERSION, PAYMENT_DB_PATH, PAYMENT_CHAIN_ID, PAYMENT_RECEIVER_ADDRESS,
                bool(PAYMENT_USDT_CONTRACT))
    return PAYMENT_DB

PAYMENT_DB = None

# Initialize persistence during startup only; no RPC/Telegram/trading work is performed here.
_prev_integrated_start_bot_payment_db = integrated_start_bot
async def integrated_start_bot():
    await asyncio.to_thread(_init_payment_database_contract)
    return await _prev_integrated_start_bot_payment_db()

VERSION = "v19.44_PAYMENT_DATABASE_CONTRACT"


# ============================================================
# v19.45 — PAYMENT TELEGRAM INTEGRATION / PHASE 1
# Scope: Telegram UI -> Payment Order -> Payment DB -> User Payment Status
# No Blockchain Monitor in this version.
# No changes to trading strategy, Entry/SL/TP, Score/R:R, M15, S/R, G.Y,
# trade alerts, trade monitor, quota or Trade Engine.
# ============================================================

from decimal import Decimal, ROUND_HALF_UP
import secrets

PAYMENT_PHASE = "TELEGRAM_UI_ORDER_DB_STATUS"
PAYMENT_PRICE_BASIC_30D_USDT = Decimal(os.getenv("PAYMENT_PRICE_BASIC_30D_USDT", "30"))
PAYMENT_PRICE_ELITE_30D_USDT = Decimal(os.getenv("PAYMENT_PRICE_ELITE_30D_USDT", "60"))
PAYMENT_TOKEN_DECIMALS = int(os.getenv("PAYMENT_TOKEN_DECIMALS", "18"))
# SQLite INTEGER is signed 64-bit; 18-decimal token units exceed that for normal USDT prices.
# Keep exact user-facing USDT pricing at 2 decimals while storing order money in 6-decimal DB units.
PAYMENT_DB_ATOMIC_DECIMALS = int(os.getenv("PAYMENT_DB_ATOMIC_DECIMALS", "6"))
PAYMENT_CURRENCY_DISPLAY = "USDT"

# Discount schedule is payment-only and configurable. No trading/subscription entitlement logic is changed here.
PAYMENT_DISCOUNT_BP_31_90 = int(os.getenv("PAYMENT_DISCOUNT_BP_31_90", "500"))
PAYMENT_DISCOUNT_BP_91_180 = int(os.getenv("PAYMENT_DISCOUNT_BP_91_180", "1000"))
PAYMENT_DISCOUNT_BP_181_365 = int(os.getenv("PAYMENT_DISCOUNT_BP_181_365", "1500"))


def _payment_price_usdt(plan_code, duration_days):
    plan_code = str(plan_code).upper()
    days = int(duration_days)
    if plan_code == "BASIC":
        monthly = PAYMENT_PRICE_BASIC_30D_USDT
    elif plan_code == "ELITE":
        monthly = PAYMENT_PRICE_ELITE_30D_USDT
    else:
        raise ValueError("Unsupported paid plan")
    if days < 1 or days > 365:
        raise ValueError("duration_days must be between 1 and 365")
    raw = monthly * Decimal(days) / Decimal(30)
    if days <= 30:
        bp = 0
    elif days <= 90:
        bp = PAYMENT_DISCOUNT_BP_31_90
    elif days <= 180:
        bp = PAYMENT_DISCOUNT_BP_91_180
    else:
        bp = PAYMENT_DISCOUNT_BP_181_365
    discount = (raw * Decimal(bp) / Decimal(10000)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    final = (raw - discount).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    return {"base": raw, "discount_bp": bp, "discount_percent": Decimal(bp) / Decimal(100), "discount": discount, "final": final}


def _payment_atomic(usdt_amount):
    q = Decimal(10) ** PAYMENT_DB_ATOMIC_DECIMALS
    value = (Decimal(usdt_amount) * q).quantize(Decimal("1"), rounding=ROUND_HALF_UP)
    return int(value)


def _payment_order_id():
    return f"XAU-{datetime.now(timezone.utc).strftime('%Y%m%d')}-{secrets.token_hex(4).upper()}"


def _payment_money(value):
    return f"{Decimal(value):.2f}"


def _payment_duration_label(days):
    days = int(days)
    return f"{days} يومًا"


def _payment_order_text(order):
    discount = Decimal(int(order["discount_percent_bp"] or 0)) / Decimal(100)
    lines = [
        "💳 طلب الدفع",
        "",
        f"رقم الطلب:",
        ...
        "",
        f"الباقة:",
        f"{'🟢 BASIC' if order['plan_code'] == 'BASIC' else '🟣 ELITE'}",
        "",
        f"المدة:",
        _payment_duration_label(order["duration_days"]),
        "",
        f"المبلغ المطلوب:",
        f"{_payment_money(Decimal(order['final_price_atomic']) / (Decimal(10) ** PAYMENT_DB_ATOMIC_DECIMALS))} USDT",
        "",
        f"الشبكة:",
        "BNB Smart Chain",
        "BEP-20",
        ...
        PAYMENT_RECEIVER_ADDRESS,
        "",
        "⚠️ أرسل USDT فقط عبر شبكة BEP-20.",
        f"💡 الخصم: {_payment_money(discount)}%" if discount > 0 else "",
        "",
        "ℹ️ بعد الدفع استخدم رقم الطلب مع بيانات المعاملة عند توفرها.",
    ]
    return "\n".join(x for x in lines if x != "")


def _payment_token_ready():
    return bool(str(PAYMENT_USDT_CONTRACT or "").strip())


async def _payment_plans_ui(update, context):
    await asyncio.to_thread(_ensure_user, update)
    text = (
        "💳 باقات الدفع\n\n"
        "🟢 BASIC — 30 USDT / 30 يومًا\n"
        "🎯 حتى 5 صفقات يوميًا\n\n"
        "🟣 ELITE — 60 USDT / 30 يومًا\n"
        "🎯 جميع الصفقات دون حد يومي\n\n"
        "اختر الباقة ثم المدة من 1 إلى 365 يومًا.\n"
        "كلما زادت المدة بعد 30 يومًا يطبق خصم المدة تلقائيًا."
    )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("🟢 BASIC — 30 USDT / 30 يومًا", callback_data="pay_plan:BASIC")],
        [InlineKeyboardButton("🟣 ELITE — 60 USDT / 30 يومًا", callback_data="pay_plan:ELITE")],
        [InlineKeyboardButton("👤 حالة الاشتراك", callback_data="payment_status")],
    ])
    q = getattr(update, "callback_query", None)
    if q:
        await q.edit_message_text(with_market_header(text), reply_markup=markup)
...
async def _payment_duration_ui(update, context, plan_code):
    plan_code = str(plan_code).upper()
    if plan_code not in {"BASIC", "ELITE"}:
        await update.callback_query.edit_message_text(with_market_header("❌ الباقة غير صالحة."))
        return
    context.user_data["payment_plan_code"] = plan_code
    prices = [1, 7, 14, 30, 60, 90, 180, 365]
    rows = []
    for i in range(0, len(prices), 2):
        row = []
        for days in prices[i:i+2]:
            p = _payment_price_usdt(plan_code, days)
            row.append(InlineKeyboardButton(f"{days} يوم — {_payment_money(p['final'])} USDT", callback_data=f"pay_duration:{plan_code}:{days}"))
        rows.append(row)
    rows.append([InlineKeyboardButton("🔙 العودة للباقات", callback_data="payment_plans")])
    name = "🟢 BASIC" if plan_code == "BASIC" else "🟣 ELITE"
    text = f"💳 اختيار مدة الاشتراك\n\nالباقة: {name}\n\nاختر المدة المطلوبة:"
    await update.callback_query.edit_message_text(with_market_header(text), reply_markup=InlineKeyboardMarkup(rows))


async def _payment_create_order_ui(update, context, plan_code, duration_days):
    q = update.callback_query
    chat_id = int(update.effective_chat.id)
    if not _payment_token_ready():
        await q.edit_message_text(with_market_header(
            "⚠️ الدفع غير مفعّل حاليًا.\n\n"
            "لم يتم ضبط عقد USDT المعتمد على BSC في إعدادات الخادم بعد."
        ), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للباقات", callback_data="payment_plans")]]))
        logger.error("PAYMENT_UI_ORDER_BLOCKED_TOKEN_CONTRACT_NOT_CONFIGURED")
        return
    days = int(duration_days)
    calc = _payment_price_usdt(plan_code, days)
    amount_atomic = _payment_atomic(calc["final"])
    order_id = _payment_order_id()
    now = int(time.time())
    db = globals().get("PAYMENT_DB")
    if db is None:
        await q.edit_message_text(with_market_header("❌ قاعدة بيانات الدفع غير جاهزة. حاول مرة أخرى."))
        return
    try:
        db.create_order(
            order_id=order_id,
            ...
            plan_code=plan_code,
            duration_days=days,
            base_price_atomic=_payment_atomic(calc["base"]),
            discount_percent_bp=int(calc["discount_bp"]),
            discount_atomic=_payment_atomic(calc["discount"]),
            final_price_atomic=amount_atomic,
            token_contract=PAYMENT_USDT_CONTRACT,
            receiver_address=PAYMENT_RECEIVER_ADDRESS,
            created_by="user",
            now=now,
        )
        order = db.get_order(order_id)
        logger.info("PAYMENT_ORDER_CREATED | order_id=%s | user=%s | plan=%s | days=%s", order_id, chat_id, plan_code, days)
    except Exception:
        logger.exception("PAYMENT_ORDER_CREATE_FAILED | user=%s | plan=%s | days=%s", chat_id, plan_code, days)
        await q.edit_message_text(with_market_header("❌ تعذر إنشاء طلب الدفع. حاول مرة أخرى."), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للباقات", callback_data="payment_plans")]]))
        return
    context.user_data.pop("payment_plan_code", None)
    await q.edit_message_text(
        with_market_header(_payment_order_text(order)),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 حالة الطلب", callback_data=f"payment_order:{order_id}")],
            [InlineKeyboardButton("💳 باقات الدفع", callback_data="payment_plans")],
            [InlineKeyboardButton("👤 حالة الاشتراك", callback_data="payment_status")],
        ]),
    )


async def _payment_order_status_ui(update, context, order_id):
    q = update.callback_query
    chat_id = int(update.effective_chat.id)
    db = globals().get("PAYMENT_DB")
    order = db.get_order(order_id) if db else None
    if not order or int(order["telegram_user_id"]) != chat_id:
        await q.edit_message_text(with_market_header("❌ طلب الدفع غير موجود أو لا يخص حسابك."))
        return
    status_map = {
        "WAITING_PAYMENT": "🟡 بانتظار الدفع",
        "PAYMENT_DETECTED": "🔵 تم رصد دفعة — بانتظار التحقق",
        "VERIFYING": "🔵 جارٍ التحقق",
        "CONFIRMED": "🟢 تم تأكيد الدفع",
        "ACTIVATING": "🟢 جارٍ تفعيل الاشتراك",
        "ACTIVATED": "🟢 تم تفعيل الاشتراك",
        "EXPIRED": "⚪ انتهى الطلب",
        "MANUAL_REVIEW": "🟠 مراجعة يدوية",
    }
    amount = Decimal(int(order["final_price_atomic"])) / (Decimal(10) ** PAYMENT_DB_ATOMIC_DECIMALS)
    text = (
        "💳 حالة طلب الدفع\n\n"
        f"رقم الطلب: #{order['order_id']}\n"
        f"الباقة: {order['plan_code']}\n"
        f"المدة: {order['duration_days']} يومًا\n"
        f"المبلغ: {_payment_money(amount)} USDT\n"
        f"الحالة: {status_map.get(order['status'], order['status'])}"
    )
    await q.edit_message_text(with_market_header(text), reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 تحديث", callback_data=f"payment_order:{order_id}")],
        [InlineKeyboardButton("👤 حالة الاشتراك", callback_data="payment_status")],
        [InlineKeyboardButton("💳 الباقات", callback_data="payment_plans")],
    ]))


async def _payment_status_ui(update, context):
    chat_id = int(update.effective_chat.id)
    await asyncio.to_thread(_ensure_user, update)
    # Keep the existing entitlement engine authoritative. Payment DB is read-only here.
    status = await asyncio.to_thread(plan_status_text, chat_id)
    db = globals().get("PAYMENT_DB")
    pending = []
    if db is not None:
        try:
            with db.connect() as conn:
                pending = conn.execute(
                    "SELECT order_id,plan_code,duration_days,final_price_atomic,status FROM payment_orders "
                    "WHERE telegram_user_id=? AND status IN ('WAITING_PAYMENT','PAYMENT_DETECTED','VERIFYING','CONFIRMED','ACTIVATING','MANUAL_REVIEW') "
                    "ORDER BY id DESC LIMIT 3", (chat_id,)
                ).fetchall()
        except Exception:
            logger.exception("PAYMENT_STATUS_DB_READ_FAILED | user=%s", chat_id)
    lines = ["💳 اشتراكي", "", "الباقة الحالية:", status.split("\n", 1)[0], ""]
    # Prefer the existing subscription expiry as the source of truth for entitlement display.
    raw_status = status
    expiry = None
    for line in raw_status.splitlines():
        if line.startswith("📅 الانتهاء:"):
            expiry = line.split(":", 1)[1].strip()
            break
    if expiry and not expiry.startswith("لا يوجد"):
        lines += ["الحالة:", "🟢 فعال", "", "تاريخ الانتهاء:", expiry]
    else:
        lines += ["الحالة:", "🟡 مجانية", "", "تاريخ الانتهاء:", "لا يوجد انتهاء"]
    if pending:
        lines += ["", "🧾 طلبات دفع قائمة:"]
        for row in pending:
            lines.append(f"#{row['order_id']} — {row['plan_code']} — {row['duration_days']} يومًا — {row['status']}")
    markup_rows = [[InlineKeyboardButton("💳 تمديد / شراء اشتراك", callback_data="payment_plans")]]
    for row in pending:
        markup_rows.append([InlineKeyboardButton(f"🔄 طلب #{row['order_id']}", callback_data=f"payment_order:{row['order_id']}")])
    q = getattr(update, "callback_query", None)
...
# ---------- Telegram callback routing: payment-only routes ----------
_prev_callback_router_v19_45_payment = callback_router
async def callback_router(update, context):
    q = getattr(update, "callback_query", None)
    data = str(getattr(q, "data", "") or "") if q else ""
    if data == "payment_plans":
        await q.answer()
        await _payment_plans_ui(update, context)
        return
    if data == "payment_status":
        await q.answer()
        await _payment_status_ui(update, context)
        return
    if data.startswith("pay_plan:"):
        await q.answer()
        await _payment_duration_ui(update, context, data.split(":", 1)[1])
        return
    if data.startswith("pay_duration:"):
        await q.answer()
        _, plan_code, days = data.split(":", 2)
        await _payment_create_order_ui(update, context, plan_code, int(days))
        return
    if data.startswith("payment_order:"):
        await q.answer()
        await _payment_order_status_ui(update, context, data.split(":", 1)[1])
        return
    return await _prev_callback_router_v19_45_payment(update, context)


# ---------- Text router: payment entry points ----------
_prev_router_v19_45_payment = router
async def router(update, context):
    text = (getattr(getattr(update, "effective_message", None), "text", "") or "").strip()
    if text == "💳 الباقات":
        await _payment_plans_ui(update, context)
    ...
    if text == "👤 الاشتراك":
        await _payment_status_ui(update, context)
        return
    return await _prev_router_v19_45_payment(update, context)


# ---------- Subscription UI override: status only, no legacy ID/support/packages buttons ----------
_prev_my_subscription_v19_45_payment = my_subscription
async def my_subscription(update, context):
    return await _payment_status_ui(update, context)


# ---------- Startup: ensure payment DB exists before Telegram starts ----------
_prev_integrated_start_bot_v19_45_payment = integrated_start_bot
async def integrated_start_bot():
    if globals().get("PAYMENT_DB") is None:
        await asyncio.to_thread(_init_payment_database_contract)
    return await _prev_integrated_start_bot_v19_45_payment()


VERSION = "v19.45_PAYMENT_TELEGRAM_INTEGRATION"


def run_v19_45_payment_integration_self_tests():
    failures = []
    try:
        assert PAYMENT_PHASE == "TELEGRAM_UI_ORDER_DB_STATUS"
        assert PAYMENT_PRICE_BASIC_30D_USDT == Decimal("30")
        assert PAYMENT_PRICE_ELITE_30D_USDT == Decimal("60")
        assert _payment_price_usdt("BASIC", 30)["final"] == Decimal("30.00")
        assert _payment_price_usdt("ELITE", 30)["final"] == Decimal("60.00")
        assert _payment_price_usdt("BASIC", 365)["final"] > Decimal("0")
        assert _payment_atomic(Decimal("30.00")) == 30000000
        assert _payment_atomic(Decimal("109.60")) == 109600000
        assert _payment_order_id().startswith("XAU-")
    except Exception as exc:
        failures.append(f"self_test:{type(exc).__name__}:{exc}")
    return {"ok": not failures, "version": VERSION, "failures": failures}

# v19.47: defer the executable entrypoint until ALL append-only override layers
# below this point have been defined. The previous v19.45 entrypoint executed
# main_v19_13() before the v19.46 payment UI overrides were bound.
# (No runtime trading logic is changed here.)
W1_GY_POLICY = "NON_BLOCKING_CONTEXT"


# ============================================================
# v19.46 — PAYMENT UI CONTRACT CORRECTION
# ============================================================
# الهدف:
# 1) استبدال قائمة مدد الدفع الثابتة بإدخال حر من 1 إلى 365 يومًا.
# 2) الحفاظ على BASIC=30 وELITE=60 لكل 30 يومًا مع خصم المدد الطويلة.
# 3) جعل شاشة طلب الدفع مطابقة لعقد الواجهة المعتمد.
# 4) جعل «💳 اشتراكي» يعرض الباقة/الحالة/المتبقي/الانتهاء فقط.
# 5) لا تغيير في Trade Engine أو شروط التداول أو S/R أو G.Y.
# ============================================================

PAYMENT_PHASE = "TELEGRAM_UI_ORDER_DB_STATUS_FREE_DURATION"


def _payment_normalize_digits(value):
    """Normalize Arabic/Persian digits so duration input works naturally in Arabic Telegram UI."""
    table = str.maketrans("٠١٢٣٤٥٦٧٨٩۰۱۲۳۴۵۶۷۸۹", "01234567890123456789")
    return str(value or "").translate(table).strip()


def _payment_duration_prompt_text(plan_code):
    name = "🟢 BASIC" if str(plan_code).upper() == "BASIC" else "🟣 ELITE"
    return (
        "💳 اختيار مدة الاشتراك\n\n"
        f"الباقة: {name}\n"
        "أرسل الآن عدد أيام الاشتراك المطلوب.\n"
        "المدة المسموحة: من 1 إلى 365 يومًا.\n"
        "مثال: 137"
    )


async def _payment_duration_ui_v19_46(update, context, plan_code):
    plan_code = str(plan_code or "").upper()
    if plan_code not in {"BASIC", "ELITE"}:
        await update.callback_query.edit_message_text(
            with_market_header("❌ الباقة غير صالحة."),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 العودة للباقات", callback_data="payment_plans")]
            ]),
        )
        return
    # Persist the selected plan in both user_data and chat_data.
    # This keeps the payment input state stable across callback -> message updates.
    context.user_data["payment_plan_code"] = plan_code
    try:
        context.chat_data["payment_plan_code"] = plan_code
    except Exception:
        pass
    await update.callback_query.edit_message_text(
        with_market_header(_payment_duration_prompt_text(plan_code)),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 العودة للباقات", callback_data="payment_plans")]
        ]),
    )
...
def _payment_order_from_db(order_id):
    db = globals().get("PAYMENT_DB")
    return db.get_order(order_id) if db else None


async def _payment_create_order_from_duration_message(update, context, duration_days):
    chat_id = int(update.effective_chat.id)
    plan_code = str(context.user_data.get("payment_plan_code") or "").upper()
    if plan_code not in {"BASIC", "ELITE"}:
        await reply(update, "❌ لم يتم تحديد الباقة. افتح 💳 الباقات واختر BASIC أو ELITE أولًا.")
        context.user_data.pop("payment_plan_code", None)
        return
    ...
    days = int(duration_days)
    if days < PAYMENT_ORDER_MIN_DAYS or days > PAYMENT_ORDER_MAX_DAYS:
        await reply(update, "❌ المدة غير صالحة. أرسل عددًا صحيحًا من 1 إلى 365 يومًا.")
        return
    ...
    db = globals().get("PAYMENT_DB")
    if db is None:
        await reply(update, "❌ قاعدة بيانات الدفع غير جاهزة. حاول مرة أخرى.")
        return
    ...
    try:
        ...
        order = db.get_order(order_id)
        logger.info(
            "PAYMENT_ORDER_CREATED_V19_46 | order_id=%s | user=%s | plan=%s | days=%s",
            order_id, chat_id, plan_code, days,
        )
    except Exception:
        logger.exception(
            "PAYMENT_ORDER_CREATE_FAILED_V19_46 | user=%s | plan=%s | days=%s",
            chat_id, plan_code, days,
        )
        await reply(update, "❌ تعذر إنشاء طلب الدفع. حاول مرة أخرى.")
        return

    context.user_data.pop("payment_plan_code", None)
    await reply(
        update,
        _payment_order_text(order),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 حالة الطلب", callback_data=f"payment_order:{order_id}")],
            [InlineKeyboardButton("💳 الباقات", callback_data="payment_plans")],
            [InlineKeyboardButton("👤 اشتراكي", callback_data="payment_status")],
        ]),
    )


def _payment_order_text_v19_46(order):
    """Clean payment-order contract: one fact per line, no commercial noise."""
    amount = Decimal(int(order["final_price_atomic"])) / (Decimal(10) ** PAYMENT_DB_ATOMIC_DECIMALS)
    plan_label = "🟢 BASIC" if order["plan_code"] == "BASIC" else "🟣 ELITE"
    return "\n".join([
        "💳 طلب الدفع",
        "",
        "رقم الطلب:",
...
async def _payment_status_ui_v19_46(update, context):
    chat_id = int(update.effective_chat.id)
    await asyncio.to_thread(_ensure_user, update)
    snap = await asyncio.to_thread(_account_status_snapshot, chat_id)
    plan = str(snap.get("plan") or "FREE").upper()
    if plan == "BASIC":
        plan_label = "🟦 BASIC"
    elif plan == "ELITE":
        plan_label = "🟨 ELITE"
    ...
    status_value = str(snap.get("status") or "active").lower()
    status_label = "🟢 فعال" if status_value == "active" else snap.get("status_ar", "ℹ️ غير معروف")
    expiry = snap.get("expiry")
    remaining = snap.get("remaining")

    if expiry is not None:
        try:
            expiry_text = expiry.astimezone(DAMASCUS).strftime("%d/%m/%Y") if expiry.tzinfo else expiry.strftime("%d/%m/%Y")
        except Exception:
            expiry_text = str(expiry)[:10]
        remaining_days = max(0, int((int(remaining or 0) + 86399) // 86400))
        remaining_text = f"{remaining_days} يومًا"
    else:
        expiry_text = "لا يوجد انتهاء"
        remaining_text = "غير محدد"
        ...
        try:
            with db.connect() as conn:
                pending = conn.execute(
                    ...
                    "('WAITING_PAYMENT','PAYMENT_DETECTED','VERIFYING','CONFIRMED','ACTIVATING','MANUAL_REVIEW') "
                    "ORDER BY id DESC LIMIT 3",
                    (chat_id,),
                ).fetchall()
        except Exception:
            logger.exception("PAYMENT_STATUS_PENDING_READ_FAILED_V19_46 | user=%s", chat_id)

    rows = [[InlineKeyboardButton("💳 شراء / تمديد اشتراك", callback_data="payment_plans")]]
    for row in pending:
        rows.append([InlineKeyboardButton(f"🔄 طلب #{row['order_id']}", callback_data=f"payment_order:{row['order_id']}")])
    q = getattr(update, "callback_query", None)
...
# Preserve the database/order contract, override only Telegram presentation/input.
_payment_order_text_v19_45 = _payment_order_text
_payment_order_text = _payment_order_text_v19_46
_payment_duration_ui_v19_45 = _payment_duration_ui
_payment_duration_ui = _payment_duration_ui_v19_46
_payment_status_ui_v19_45 = _payment_status_ui
_payment_status_ui = _payment_status_ui_v19_46

_prev_callback_router_v19_46_payment = callback_router
async def callback_router(update, context):
    q = getattr(update, "callback_query", None)
    data = str(getattr(q, "data", "") or "") if q else ""
    if data.startswith("pay_plan:"):
        await q.answer()
        await _payment_duration_ui_v19_46(update, context, data.split(":", 1)[1])
        return
    return await _prev_callback_router_v19_46_payment(update, context)

_prev_router_v19_46_payment = router
async def router(update, context):
    text = (getattr(getattr(update, "effective_message", None), "text", "") or "").strip()
    normalized = _payment_normalize_digits(text)
    if context.user_data.get("payment_plan_code") and normalized.isdigit():
        days = int(normalized)
        await _payment_create_order_from_duration_message(update, context, days)
        return
    if text == "💳 الباقات":
        await _payment_plans_ui(update, context)
        return
    if text == "👤 الاشتراك":
        await _payment_status_ui_v19_46(update, context)
        return
    return await _prev_router_v19_46_payment(update, context)



# ============================================================
# v19.51 — PAYMENT DURATION DIRECT HANDLER FIX
# ============================================================
async def _payment_duration_message_direct_handler(update, context):
    # Read the selected plan from both stores; chat_data is the durable
    # callback->message bridge for this payment flow.
    user_data = getattr(context, "user_data", {}) or {}
    chat_data = getattr(context, "chat_data", {}) or {}
    plan_code = str(
        user_data.get("payment_plan_code")
        or chat_data.get("payment_plan_code")
        or ""
    ).upper()
    if plan_code not in {"BASIC", "ELITE"}:
        return
    ...
    days = int(normalized)
    # Rehydrate user_data if the message context only retained chat_data.
    user_data["payment_plan_code"] = plan_code
    await _payment_create_order_from_duration_message(update, context, days)
    # Stop the generic text router so the numeric duration can never fall
    # through to the main menu/legacy router.
    raise ApplicationHandlerStop

VERSION = "v19.52_PAYMENT_DURATION_STATE_HARDENED"


def run_v19_47_payment_ui_self_tests():
    failures = []
    try:
        assert PAYMENT_PHASE == "TELEGRAM_UI_ORDER_DB_STATUS_FREE_DURATION"
        assert _payment_price_usdt("BASIC", 30)["final"] == Decimal("30.00")
        assert _payment_price_usdt("ELITE", 30)["final"] == Decimal("60.00")
        assert _payment_price_usdt("BASIC", 60)["final"] == Decimal("57.00")
        assert _payment_price_usdt("BASIC", 90)["final"] == Decimal("85.50")
        assert _payment_price_usdt("BASIC", 180)["final"] == Decimal("162.00")
        assert _payment_price_usdt("BASIC", 365)["final"] == Decimal("310.25")
        assert _payment_normalize_digits("١٣٧") == "137"
        assert "1 إلى 365" in _payment_duration_prompt_text("BASIC")
        # Payment duration must not depend on a single context store.
        class _Ctx:
            user_data = {}
            chat_data = {"payment_plan_code": "BASIC"}
        assert str(_Ctx.chat_data["payment_plan_code"]).upper() == "BASIC"
    except Exception as exc:
        failures.append(f"self_test:{type(exc).__name__}:{exc}")
    return {"ok": not failures, "version": VERSION, "failures": failures}
...
# ============================================================
# v19.47 — FINAL EXECUTABLE ENTRYPOINT
# Must be physically after every payment UI override.
# ============================================================
# v19.52 legacy executable block intentionally disabled; final v19.53 entrypoint is below.


# ============================================================
# v19.53 — RUNTIME LIFECYCLE SUPERVISOR HARDENING
# Scope: reliability only.
# - Supervises the canonical M15 trading loop, G.Y loop and alert scheduler.
# - Recreates a task only when the previous task is actually absent/done.
# - Never starts duplicate named tasks.
# - Does not change strategy thresholds, Entry/SL/TP, score, quota or DB contracts.
# ============================================================
V19_53_VERSION = "v19.53_RUNTIME_SUPERVISOR_HARDENED"
RUNTIME_SUPERVISOR_TASK = None
RUNTIME_SUPERVISOR_INTERVAL = max(5.0, float(os.getenv("RUNTIME_SUPERVISOR_INTERVAL", "15")))


def _v1953_live_task(name):
...
async def _v1953_ensure_runtime_task(factory, name):
    task = _v1953_live_task(name)
    if task is not None:
        return task
    try:
        task = asyncio.create_task(factory(), name=name)
        logger.warning("RUNTIME_TASK_RECREATED_V19_53 | task=%s", name)
        return task
    except Exception:
        logger.exception("RUNTIME_TASK_CREATE_FAILED_V19_53 | task=%s", name)
        return None


async def _v1953_runtime_supervisor_loop():
    """Keep canonical long-lived runtime tasks alive without touching trade logic."""
    global RUNTIME_SUPERVISOR_TASK
    logger.info("RUNTIME_SUPERVISOR_STARTED_V19_53")
    try:
        while True:
            try:
                # Do not create runtime workers before Telegram Application startup
                # has completed enough to expose APPLICATION.
                if globals().get("APPLICATION") is None:
                    await asyncio.sleep(1.0)
                    continue

                # Alert timing scheduler: recreate only if the task has died.
                await _ensure_alert_scheduler_task()

                # Canonical M15 trading engine.
                await _v1953_ensure_runtime_task(auto_loop, "main-trading-loop")

                # Canonical G.Y private-trade lifecycle engine.
                if callable(globals().get("_v1935_gy_auto_loop")):
                    await _v1953_ensure_runtime_task(_v1935_gy_auto_loop, "gy-private-trade-auto")

                # Account lifecycle notification scheduler, when configured.
                if callable(globals().get("_start_account_notification_scheduler")):
                    try:
                        await _start_account_notification_scheduler()
                    except asyncio.CancelledError:
                        raise
                    except Exception:
                        logger.exception("ACCOUNT_NOTIFICATION_SUPERVISOR_CHECK_FAILED_V19_53")
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("RUNTIME_SUPERVISOR_ERROR_V19_53")
            await asyncio.sleep(RUNTIME_SUPERVISOR_INTERVAL)
    finally:
        if RUNTIME_SUPERVISOR_TASK is asyncio.current_task():
            RUNTIME_SUPERVISOR_TASK = None
        logger.info("RUNTIME_SUPERVISOR_STOPPED_V19_53")


async def _v1953_start_runtime_supervisor():
    global RUNTIME_SUPERVISOR_TASK
    task = _v1953_live_task("runtime-lifecycle-supervisor")
    if task is not None:
        RUNTIME_SUPERVISOR_TASK = task
        return task
    task = asyncio.create_task(_v1953_runtime_supervisor_loop(), name="runtime-lifecycle-supervisor")
    RUNTIME_SUPERVISOR_TASK = task
    return task


_prev_integrated_start_bot_v1953 = integrated_start_bot


async def integrated_start_bot():
    # The canonical startup function intentionally contains a long-running wait.
    # Run it as the startup task so the lifecycle supervisor is installed after
    # initialization begins, without waiting for the perpetual startup loop to return.
    startup_task = asyncio.create_task(_prev_integrated_start_bot_v1953(), name="canonical-integrated-startup")
    await asyncio.sleep(0)
    await _v1953_start_runtime_supervisor()
    return await startup_task



def run_v19_53_runtime_hardening_self_tests():
    failures = []
    try:
        assert V19_53_VERSION == "v19.53_RUNTIME_SUPERVISOR_HARDENED"
        assert callable(_v1953_live_task)
        assert callable(_v1953_ensure_runtime_task)
        assert callable(_v1953_runtime_supervisor_loop)
        assert callable(_v1953_start_runtime_supervisor)
        assert callable(integrated_start_bot)
        assert callable(auto_loop)
        assert callable(_ensure_alert_scheduler_task)
    except Exception as exc:
        failures.append(f"runtime_self_test:{type(exc).__name__}:{exc}")
    return {"ok": not failures, "version": V19_53_VERSION, "failures": failures}


# ============================================================
# v19.55 — EXECUTION ACCURACY HARDENING
# Scope: MAIN trade execution gate only.
# Evidence-driven rules from M1 2024 replay:
#   1) A trade must have an actually CONFIRMED liquidity retest.
#   2) The confirming BOS must displace at least 0.50 ATR from the
#      pre-sweep structural level (causal, completed candles only).
#   3) Existing Entry / SL / TP / R:R construction remains unchanged.
#   4) Subscription, quota, S/R-GY, alerts, DB contracts and news policy
#      are not modified by this block.
# ============================================================
V19_54_VERSION = "v19.55_EXECUTION_ACCURACY_HARDENED"
V19_54_MIN_BOS_DISPLACEMENT_ATR = 0.50

_prev_liquidity_analysis_v1954 = liquidity_analysis

def _v1954_liquidity_analysis_with_bos_metric(df, lookback=80):
    result = _prev_liquidity_analysis_v1954(df, lookback)
    try:
        retest = result.get("retest", {}) if isinstance(result, dict) else {}
        state = str(retest.get("state") or "NONE").upper()
        key = retest.get("key")
        if not key or not isinstance(df, pd.DataFrame) or len(df) < 30:
            return result
        st = LIQUIDITY_STATE.get(key)
        if not isinstance(st, dict):
            return result
        bos_idx = st.get("bos_bar_index")
        sweep_idx = st.get("sweep_bar_index")
        bos_time = st.get("bos_time")
        if bos_idx is None or sweep_idx is None or bos_time is None:
            return result

        completed = df.iloc[:-1].copy() if len(df) > 2 else df.copy()
        bos_idx = int(bos_idx)
        sweep_idx = int(sweep_idx)
        if bos_idx < 0 or bos_idx >= len(completed) or sweep_idx < 0 or sweep_idx >= len(completed):
            return result

        pre = completed.iloc[max(0, sweep_idx - 20):sweep_idx]
        if len(pre) < 4:
            return result
        pre_high = float(pre["high"].max())
        pre_low = float(pre["low"].min())
        bos_close = float(completed["close"].iloc[bos_idx])
        side = str(st.get("sweep") or "")
        bos_level = pre_high if side == "SELL_SIDE_SWEEP" else pre_low if side == "BUY_SIDE_SWEEP" else None
        if bos_level is None:
            return result

        atr_series = ATR(completed)
        bos_atr = float(atr_series.iloc[bos_idx]) if bos_idx < len(atr_series) else 0.0
        displacement = abs(bos_close - bos_level)
        displacement_atr = displacement / bos_atr if bos_atr > 0 else 0.0

        # Persist the metric in the lifecycle state for deterministic logging.
        st["bos_level"] = float(bos_level)
        st["bos_close"] = float(bos_close)
        st["bos_displacement"] = float(displacement)
        st["bos_atr"] = float(bos_atr)
        st["bos_displacement_atr"] = float(displacement_atr)

        retest["bos_level"] = float(bos_level)
        retest["bos_close"] = float(bos_close)
        retest["bos_displacement"] = float(displacement)
        retest["bos_atr"] = float(bos_atr)
        retest["bos_displacement_atr"] = float(displacement_atr)
        retest["bos_strength_ok"] = bool(displacement_atr >= V19_54_MIN_BOS_DISPLACEMENT_ATR)
        retest["valid_retest"] = bool(state == "CONFIRMED")
        result["retest"] = retest
    except Exception:
        logger.exception("V19_54_BOS_METRIC_ERROR")
    return result

liquidity_analysis = _v1954_liquidity_analysis_with_bos_metric

_prev_evaluate_signal_v1954 = evaluate_signal

def evaluate_signal():
    """Final MAIN execution gate: confirmed retest + sufficiently strong BOS."""
    global LAST_ANALYSIS
    result = _prev_evaluate_signal_v1954()
    try:
        direction = str(result.get("direction") or "WAIT").upper()
        mtf = result.get("mtf", {}) if isinstance(result.get("mtf"), dict) else {}
        m15 = mtf.get("m15", {}) if isinstance(mtf.get("m15"), dict) else {}
        liq = m15.get("liquidity", {}) if isinstance(m15.get("liquidity"), dict) else {}
        retest = liq.get("retest", {}) if isinstance(liq.get("retest"), dict) else {}
        state = str(retest.get("state") or "NONE").upper()
        bos_strength_ok = bool(retest.get("bos_strength_ok"))
        bos_disp_atr = float(retest.get("bos_displacement_atr", 0.0) or 0.0)

        gate_reasons = []
        if direction not in ("BUY", "SELL"):
            gate_reasons.append("لا يوجد اتجاه تنفيذي مؤكد")
        if state != "CONFIRMED":
            gate_reasons.append("إعادة الاختبار ليست مؤكدة")
        if not bos_strength_ok:
            gate_reasons.append(
                f"كسر BOS أضعف من الحد الأدنى {V19_54_MIN_BOS_DISPLACEMENT_ATR:.2f} ATR "
                f"(الحالي {bos_disp_atr:.2f} ATR)"
            )

        if gate_reasons:
            result["signal"] = False
            result["trade"] = None
            result["execution_state"] = "WAIT"
            result.setdefault("setup_reasons", []).extend(gate_reasons)
            result["accuracy_gate"] = {
                "state": "BLOCKED",
                "min_bos_displacement_atr": V19_54_MIN_BOS_DISPLACEMENT_ATR,
                "bos_displacement_atr": bos_disp_atr,
                "retest_state": state,
        ...
        result["execution_accuracy_version"] = V19_54_VERSION
        result["rejection_reasons"] = _signal_rejection_reasons(result)
        LAST_ANALYSIS = result
        logger.info(
            "EXECUTION_ACCURACY_GATE_V19_54 | direction=%s state=%s bos_disp_atr=%.3f gate=%s",
            direction, state, bos_disp_atr, result["accuracy_gate"]["state"]
        )
        return result
    except Exception:
        logger.exception("V19_54_EXECUTION_GATE_ERROR")
        # Fail closed: an accuracy-gate failure must never create a live trade.
        result["signal"] = False
        result["trade"] = None
        result["execution_state"] = "WAIT"
        result["accuracy_gate"] = {"state": "ERROR"}
        return result


def run_v19_54_accuracy_self_tests():
    failures = []
    try:
        assert V19_54_VERSION == "v19.54_EXECUTION_ACCURACY_HARDENED"
        assert abs(V19_54_MIN_BOS_DISPLACEMENT_ATR - 0.50) < 1e-9
        assert callable(_v1954_liquidity_analysis_with_bos_metric)
        assert callable(evaluate_signal)
        assert callable(build_trade)
    except Exception as exc:
        failures.append(f"accuracy_self_test:{type(exc).__name__}:{exc}")
    return {"ok": not failures, "version": V19_54_VERSION, "failures": failures}


VERSION = V19_54_VERSION

# ============================================================
# v19.56 — G.Y EXECUTION-QUALITY HARD GATE
# Evidence-driven from the isolated G.Y M1-2024 replay.
# Candidate frozen before production modification:
#   FALSE_BREAKOUT + BUY + TRANSITION/TRENDING
# Scope is G.Y only. MAIN, PCS, geometry, SL/TP, R:R, quotas,
# subscriptions, alerts and lifecycle persistence are unchanged.
# ============================================================
GY_V1956_VERSION = "v19.56_GY_ACCURACY_HARDENED"
GY_V1956_ALLOWED_TRIGGER = "FALSE_BREAKOUT"
GY_V1956_ALLOWED_DIRECTION = "BUY"
GY_V1956_ALLOWED_REGIMES = frozenset(("TRANSITION", "TRENDING"))

_v1955_sr_analyze_reference = sr_analyze

def sr_analyze():
    """Final G.Y decision gate: keep PCS, then require the validated scenario filter."""
    result = _v1955_sr_analyze_reference()
    if not isinstance(result, dict):
        return result
    signal = result.get("signal")
    if not signal:
    ...
    try:
        ...
        pcs = signal.get("pcs") if isinstance(signal.get("pcs"), dict) else result.get("pcs")
        regime = str((pcs or {}).get("regime") or "")

        reasons = []
        if trigger != GY_V1956_ALLOWED_TRIGGER:
            reasons.append(f"G.Y trigger غير معتمد: {trigger or 'NONE'}")
        if direction != GY_V1956_ALLOWED_DIRECTION:
            reasons.append(f"G.Y direction غير معتمد: {direction or 'NONE'}")
        if regime not in GY_V1956_ALLOWED_REGIMES:
            reasons.append(f"حالة السوق غير معتمدة لـ G.Y: {regime or 'NONE'}")

        gate = {
            "version": GY_V1956_VERSION,
            "state": "PASS" if not reasons else "BLOCKED",
            "trigger": trigger,
            "direction": direction,
            "regime": regime,
            "allowed_trigger": GY_V1956_ALLOWED_TRIGGER,
            "allowed_direction": GY_V1956_ALLOWED_DIRECTION,
            "allowed_regimes": sorted(GY_V1956_ALLOWED_REGIMES),
            "reasons": reasons,
        }
        result["gy_accuracy_gate"] = gate

        if reasons:
            result["signal"] = None
            result["trade"] = None
            result["execution_state"] = "GY_ACCURACY_BLOCKED"
            result["reason"] = "تم حجب G.Y: السيناريو/الاتجاه/حالة السوق لم تطابق المرشح المثبت في اختبار M1."
            result["rejection_reasons"] = list(dict.fromkeys(
                (result.get("rejection_reasons") or []) + reasons
            ))
        else:
            result["execution_state"] = "READY"

        logger.info(
            "GY_ACCURACY_GATE_V19_56 | trigger=%s direction=%s regime=%s gate=%s",
            trigger, direction, regime, gate["state"]
        )
        return result
    except Exception as exc:
        logger.exception("GY_ACCURACY_GATE_V19_56_ERROR")
        result["signal"] = None
        result["trade"] = None
        result["execution_state"] = "GY_ACCURACY_BLOCKED"
        result["gy_accuracy_gate"] = {
            "version": GY_V1956_VERSION,
            "state": "ERROR",
...
def run_v19_56_gy_accuracy_self_tests():
    checks = []
    def check(name, ok):
        checks.append({"test": name, "status": "PASS" if ok else "FAIL"})
    check("version", GY_V1956_VERSION == "v19.56_GY_ACCURACY_HARDENED")
    check("trigger_contract", GY_V1956_ALLOWED_TRIGGER == "FALSE_BREAKOUT")
    check("direction_contract", GY_V1956_ALLOWED_DIRECTION == "BUY")
    check("regime_contract", GY_V1956_ALLOWED_REGIMES == frozenset(("TRANSITION", "TRENDING")))
    check("sr_analyze_wrapped", callable(_v1955_sr_analyze_reference) and callable(sr_analyze))
    check("main_evaluate_untouched_reference", callable(evaluate_signal))
    return {"ok": all(x["status"] == "PASS" for x in checks), "version": GY_V1956_VERSION, "results": checks}


# ============================================================
# v19.57 runtime marker — G.Y trigger-role contract is the active patch.
# v19.56 remains preserved as the historical G.Y accuracy gate.
# ============================================================
VERSION = V19_57_GY_TRIGGER_ROLE_VERSION
GY_RUNTIME_PATCH_VERSION = V19_57_GY_TRIGGER_ROLE_VERSION


# ============================================================
# v19.58 — MAIN TRADE AVAILABILITY RECOVERY
# Root cause fixed:
#   v19.54 converted the research-grade BOS/retest rule into a
#   universal hard gate for MAIN execution. This could suppress
#   every MAIN trade when no live CONFIRMED retest state existed.
#
# v19.58 policy:
#   1) MAIN returns to the immediately preceding validated execution
#      behavior (pre-v19.54).
#   2) v19.54 BOS/retest metrics remain diagnostic/advisory only.
#   3) No direction inversion is introduced.
#   4) G.Y v19.56 accuracy gate and v19.57 trigger-role contract
#      remain active and are NOT bypassed.
#   5) Entry / SL / TP / R:R / score formulas are untouched.
# ============================================================
V19_58_VERSION = "v19.58_MAIN_TRADE_RECOVERY_GY_PRESERVED"
V19_58_MAIN_ACCURACY_POLICY = "ADVISORY_NOT_BLOCKING"

# v19.54 saved the real pre-hard-gate MAIN evaluator here.
# Reuse that exact callable rather than reconstructing strategy logic.
_v1958_main_base_evaluate_signal = _prev_evaluate_signal_v1954

def evaluate_signal():
    """Restore MAIN trade availability while retaining v19.54 diagnostics."""
    global LAST_ANALYSIS
    result = _v1958_main_base_evaluate_signal()

    try:
        mtf = result.get("mtf", {}) if isinstance(result, dict) else {}
        m15 = mtf.get("m15", {}) if isinstance(mtf.get("m15"), dict) else {}
        liq = m15.get("liquidity", {}) if isinstance(m15.get("liquidity"), dict) else {}
        retest = liq.get("retest", {}) if isinstance(liq.get("retest"), dict) else {}
        ...
        advisory_reasons = []
        if state != "CONFIRMED":
            advisory_reasons.append("إعادة الاختبار ليست مؤكدة")
        if not bos_strength_ok:
            advisory_reasons.append(
                f"BOS أقل من {V19_54_MIN_BOS_DISPLACEMENT_ATR:.2f} ATR "
                f"(الحالي {bos_disp_atr:.2f} ATR)"
            )

        result["execution_accuracy_version"] = V19_58_VERSION
        result["accuracy_gate"] = {
            "state": "ADVISORY_PASS" if not advisory_reasons else "ADVISORY",
            "policy": V19_58_MAIN_ACCURACY_POLICY,
            "min_bos_displacement_atr": V19_54_MIN_BOS_DISPLACEMENT_ATR,
            "bos_displacement_atr": bos_disp_atr,
            "retest_state": state,
        ...
        # IMPORTANT:
        # Do not modify result["signal"] or result["trade"] here.
        # MAIN availability must remain controlled only by the original
        # pre-v19.54 execution logic plus its existing safeguards.
        result["rejection_reasons"] = _signal_rejection_reasons(result)
        LAST_ANALYSIS = result

        logger.info(
            "MAIN_TRADE_RECOVERY_V19_58 | direction=%s signal=%s "
            "state=%s bos_disp_atr=%.3f advisory=%s",
            str(result.get("direction") or "WAIT"),
            bool(result.get("signal")),
            state,
            bos_disp_atr,
            result["accuracy_gate"]["state"],
        )
        return result

    except Exception:
        logger.exception("V19_58_MAIN_TRADE_RECOVERY_DIAGNOSTIC_ERROR")
        # Diagnostics must never become an accidental execution blocker.
        result["execution_accuracy_version"] = V19_58_VERSION
        result["accuracy_gate"] = {
            "state": "ADVISORY_ERROR",
            "policy": V19_58_MAIN_ACCURACY_POLICY,
        }
        LAST_ANALYSIS = result
        return result
...
def run_v19_58_trade_recovery_self_tests():
    checks = []
    ...
    check(
        "pre_v1954_main_reference",
        callable(_v1958_main_base_evaluate_signal),
    )
    check(
        "main_evaluate_signal_callable",
        callable(evaluate_signal),
    )
    check(
        "gy_accuracy_gate_callable",
        callable(sr_analyze),
    )
    check(
        "gy_trigger_role_contract",
        V19_57_GY_TRIGGER_ROLE_CONTRACT == "ENFORCED",
    )
    check(
        "gy_runtime_version",
        str(GY_RUNTIME_PATCH_VERSION).startswith("v19.57"),
    )

    # Static source-contract checks: the v19.58 wrapper must not contain
    # assignments that turn a valid MAIN result into signal=False/trade=None.
    try:
        import inspect as _inspect
        _src = _inspect.getsource(evaluate_signal)
        check(
            "main_recovery_no_signal_false_assignment",
            'result["signal"] = False' not in _src,
        )
        check(
            "main_recovery_no_trade_none_assignment",
            'result["trade"] = None' not in _src,
        )
    except Exception as exc:
        check("main_recovery_source_contract", False, str(exc))

    return {
        "ok": all(x["status"] == "PASS" for x in checks),
        "version": V19_58_VERSION,
        "results": checks,
    }


# v19.58 is the active runtime marker.
VERSION = V19_58_VERSION
# ============================================================
# v19.62 — I/O / LOCK / G.Y EXECUTION HARDENING VERIFIED
# Scope: runtime blocking/I-O isolation + G.Y gate correction only.
# MAIN strategy, score, Entry/SL/TP/R:R and M15 closed-candle contract
# are NOT changed.
# ============================================================
V19_62_VERSION = "v19.62_IO_GY_HARDENED_VERIFIED"
V19_62_GY_POLICY = "V19_56_ADVISORY_PCS_ENFORCED"

# ------------------------------------------------------------------
# 1) PRICE FETCH: never hold the coordination lock during network I/O.
# ------------------------------------------------------------------
_V1959_PRICE_COORD_LOCK = threading.Lock()
_V1959_PRICE_FETCH_DONE = threading.Event()
_V1959_PRICE_FETCH_DONE.set()
_V1959_PRICE_FETCH_INFLIGHT = False


def live_price():
    """Single-flight synchronous price fetch with zero network I/O under a lock.

    Callers that need an async-safe boundary must continue using asyncio.to_thread;
    this function itself never performs HTTP while holding _PRICE_FETCH_LOCK.
    """
    global _V1959_PRICE_FETCH_INFLIGHT

    while True:
        now = time.time()
        with _PRICE_CACHE_LOCK:
            cached = _LIVE_PRICE_CACHE.get("value")
            cached_ts = float(_LIVE_PRICE_CACHE.get("ts") or 0.0)
            if cached is not None and now - cached_ts < 2.0:
                return dict(cached)

        with _V1959_PRICE_COORD_LOCK:
            if not _V1959_PRICE_FETCH_INFLIGHT:
                _V1959_PRICE_FETCH_INFLIGHT = True
                fetch_done = _V1959_PRICE_FETCH_DONE
                fetch_done.clear()
                leader = True
            else:
                fetch_done = _V1959_PRICE_FETCH_DONE
        ...
        if not leader:
            # Wait without holding any lock. After the leader completes, re-check
            # the cache and only one waiter will become the next leader if needed.
            fetch_done.wait(timeout=18.0)
            continue

        errors = []
        try:
            for url, params, parser, name in [
                (LIVE_URL, {"allowStale": "false"},
                 lambda d: sf(d.get("mid"), None) if isinstance(d, dict) else None,
                 "Biquote"),
                ("https://xaus.com/api/v1/spot", None,
                 lambda d: sf(d.get("spot_usd_oz"), None) if isinstance(d, dict) else None,
                 "XAUS"),
            ]:
                try:
                    t0 = time.perf_counter()
                    ...
                    if r.ok:
                        payload = r.json()
                        pvalue = parser(payload)
                        if pvalue and pvalue > 0:
                            if name == "Biquote":
                                age = payload.get("quoteAgeSeconds") if isinstance(payload, dict) else None
                            else:
                                age = ((payload.get("data_state") or {}).get("age_seconds")
                                       if isinstance(payload, dict) else None)
                            value = {"price": pvalue, "source": name, "age": age}
                            with _PRICE_CACHE_LOCK:
                                _LIVE_PRICE_CACHE.update({"ts": time.time(), "value": value})
                            logger.debug(
                                "PRICE_API_LATENCY | source=%s ms=%.1f",
...
# ------------------------------------------------------------------
# 2) TRADE DB / TRADE_LOCK: keep I/O outside TRADE_LOCK.
# ------------------------------------------------------------------

def register_trade(result):
    """Register/update a trade without performing SQLite I/O under TRADE_LOCK."""
    if not result.get("signal") or not result.get("trade"):
        return None, False
    ...
    with TRADE_LOCK:
        price = float(result.get("price", t["entry"]))
        atr = max(float(result.get("mtf", {}).get("m15", {}).get("atr", 0) or 0), 0.50)
        entry_tol = max(atr * 0.30, price * 0.00025, 0.50)
        sl_tol = max(atr * 0.45, price * 0.00035, 0.75)
        tp_tol = max(atr * 0.55, price * 0.00045, 1.00)

        for record in reversed(TRADE_HISTORY):
            if record.get("status") not in ("ACTIVE", "TP1", "TP2"):
                continue
            if record.get("direction") != result["direction"]:
                continue
            if abs(float(record.get("entry", 0)) - float(t["entry"])) > entry_tol:
                continue
            if abs(float(record.get("sl", 0)) - float(t["sl"])) > sl_tol:
                continue
            if abs(float(record.get("tp1", 0)) - float(t["tp1"])) > tp_tol:
                continue
            if abs(float(record.get("tp2", 0)) - float(t["tp2"])) > tp_tol * 1.5:
                continue
            if abs(float(record.get("tp3", 0)) - float(t["tp3"])) > tp_tol * 2:
                continue

            record.update({
                "score": result["score"],
                "quality": result["quality"],
                "last_update": now_damascus().isoformat(),
            })
        ...
        if persisted_record is None:
            ...
            new_record = {
                ...
                "status": "ACTIVE",
                "result": "OPEN",
                "liquidity_state": result.get("mtf", {}).get("m15", {}).get("liquidity", {}).get("retest", {}).get("state"),
                "retest_status": liquidity_retest_summary(result.get("mtf", {}).get("m15", {}).get("liquidity", {})),
                "last_update": now,
                "last_price": price,
                "tp1_time": None,
                "tp2_time": None,
            ...
            persisted_record = dict(new_record)
            is_new = True

    # SQLite is deliberately outside TRADE_LOCK.
    _save_trade_locked(persisted_record)
    if is_new:
        _auditor_register_trade(persisted_record, result)
    return persisted_record, is_new


def update_trade_results():
    """Update open trades while keeping market I/O and DB writes outside TRADE_LOCK."""
    with TRADE_LOCK:
        if not any(t.get("status") in ("ACTIVE", "TP1", "TP2") for t in TRADE_HISTORY):
            return []

    # Network I/O happens before TRADE_LOCK is acquired for state mutation.
    bars = None
    try:
        bars = get_bars("1m", 100)
        if bars is not None and not bars.empty and "openTime" in bars.columns:
        ...
        logger.exception("تعذر جلب M1 لتحديث الصفقات")
    ...
    with TRADE_LOCK:
        ...
        for trade in active:
            changed = False
            if bars is not None and {"high", "low"}.issubset(bars.columns):
                changed = _update_from_m1(trade, bars)

            if not changed and current_price is not None:
                p = current_price
                state = trade.get("status", "ACTIVE")
                if trade.get("direction") == "BUY":
                    sl_hit, t1, t2, t3 = (
                        p <= trade["sl"], p >= trade["tp1"], p >= trade["tp2"], p >= trade["tp3"]
                    )
                else:
                    sl_hit, t1, t2, t3 = (
                        p >= trade["sl"], p <= trade["tp1"], p <= trade["tp2"], p <= trade["tp3"]
                ...
                if sl_hit:
                    _mark_trade_state(trade, "CLOSED", "LOSS", now, {"close_time": now, "close_reason": "SL hit"})
                    changed = True
                elif t3:
                    trade["tp1_time"] = trade.get("tp1_time") or now
                    trade["tp2_time"] = trade.get("tp2_time") or now
                    trade["tp3_time"] = now
                    _mark_trade_state(trade, "CLOSED", "TP3 / WIN", now, {"close_time": now})
                    changed = True
                elif t2 and state in ("ACTIVE", "TP1"):
                    trade["tp1_time"] = trade.get("tp1_time") or now
                    trade["tp2_time"] = now
                    _mark_trade_state(trade, "TP2", "TP2", now)
                    changed = True
                elif t1 and state == "ACTIVE":
                    trade["tp1_time"] = trade.get("tp1_time") or now
                    _mark_trade_state(trade, "TP1", "TP1", now)
                    changed = True
            ...
            if changed or current_price is not None:
                snapshot = dict(trade)
                records_to_persist.append(snapshot)
            if changed:
                changed_records.append(dict(trade))

    # SQLite writes happen only after TRADE_LOCK is released.
    for snapshot in records_to_persist:
        _save_trade_locked(snapshot)

    return changed_records


# ------------------------------------------------------------------
# 3) G.Y: v19.56 becomes advisory; PCS remains the execution gate.
#    The research result was useful diagnostically but not stable enough
#    to remain a universal hard production filter.
# ------------------------------------------------------------------
_v1959_gy_pcs_sr_analyze = _v1955_sr_analyze_reference
GY_V1962_VERSION = "v19.62_GY_PCS_EXECUTION_ADVISORY"


def sr_analyze():
    """G.Y execution: retain PCS/trigger-role validation; downgrade v19.56 to advisory."""
    result = _v1959_gy_pcs_sr_analyze()
    if not isinstance(result, dict):
        return result

    signal = result.get("signal")
    if not signal:
        # This is normally a real PCS/context rejection, not the old v19.56 filter.
        return result
    ...
    try:
        ...
        legacy_reasons = []
        if trigger != GY_V1956_ALLOWED_TRIGGER:
            legacy_reasons.append(f"v19.56 advisory trigger: {trigger or 'NONE'}")
        if direction != GY_V1956_ALLOWED_DIRECTION:
            legacy_reasons.append(f"v19.56 advisory direction: {direction or 'NONE'}")
        if regime not in GY_V1956_ALLOWED_REGIMES:
            legacy_reasons.append(f"v19.56 advisory regime: {regime or 'NONE'}")

        result["gy_accuracy_gate"] = {
            "version": GY_V1962_VERSION,
            "state": "ADVISORY" if legacy_reasons else "ADVISORY_PASS",
            "policy": V19_62_GY_POLICY,
            "trigger": trigger,
            "direction": direction,
            "regime": regime,
            "legacy_v1956_filter": {
        ...
        result["execution_state"] = "READY"
        logger.info(
            "GY_EXECUTION_ADVISORY_V19_59 | trigger=%s direction=%s regime=%s advisories=%s",
            trigger, direction, regime, legacy_reasons,
        )
        return result
    except Exception:
        logger.exception("GY_V19_59_ADVISORY_ERROR")
        # Preserve the already-approved PCS result; diagnostics cannot block execution.
        return result


# ------------------------------------------------------------------
# 4) Self-tests focused on the exact regression risks fixed here.
# ------------------------------------------------------------------
def run_v19_62_hardening_self_tests():
    import inspect
    checks = []
    ...
    # MAIN recovery contract must remain untouched.
    src_eval = inspect.getsource(evaluate_signal)
    check("main_signal_false_not_in_v1958_wrapper", 'result["signal"] = False' not in src_eval)
    check("main_trade_none_not_in_v1958_wrapper", 'result["trade"] = None' not in src_eval)

    # Price HTTP cannot be lexically nested inside the coordination lock.
    src_price = inspect.getsource(live_price)
    check("price_fetch_lock_not_used_for_http", "with _PRICE_FETCH_LOCK" not in src_price)
    check("price_coord_lock_exists", "with _V1959_PRICE_COORD_LOCK" in src_price)
    check("price_http_outside_coord_lock", src_price.find("requests.get") > src_price.find("fetch_done = _V1959_PRICE_FETCH_DONE"))

    # Runtime trade I/O isolation.
    for fname in ("register_trade", "update_trade_results"):
        source = inspect.getsource(globals()[fname])
        check(f"{fname}_has_trade_lock", "with TRADE_LOCK" in source)
        check(f"{fname}_db_write_after_lock", source.rfind("_save_trade_locked") > source.rfind("    # SQLite"))
        lock_block_start = source.find("with TRADE_LOCK")
        lock_block_end = source.find("\n    # SQLite", lock_block_start)
        block = source[lock_block_start:lock_block_end] if lock_block_start >= 0 and lock_block_end >= 0 else source
        check(f"{fname}_no_market_io_inside_lock", "get_bars(" not in block and "live_price(" not in block)

    # Final async trade alert path must push DB claim/rollback work to a thread.
    src_send = inspect.getsource(_send_trade_notification)
    check("async_alert_claim_to_thread", "asyncio.to_thread(_claim_notification" in src_send)
    check("async_alert_cancel_to_thread", "asyncio.to_thread(_cancel_notification_claim" in src_send)
    check("async_alert_complete_to_thread_via_resolver", "_resolve_alert_trade_link" in src_send)

    # G.Y: legacy v19.56 is not an execution blocker in the new wrapper.
    src_gy = inspect.getsource(sr_analyze)
    check("gy_uses_pcs_reference", "_v1959_gy_pcs_sr_analyze" in src_gy)
    check("gy_no_legacy_signal_none", 'result["signal"] = None' not in src_gy)
    check("gy_no_legacy_trade_none", 'result["trade"] = None' not in src_gy)
    check("gy_marks_legacy_as_advisory", "ADVISORY" in src_gy)

    # Synthetic G.Y pass-through: a SELL setup that PCS already approved must
    # not be rejected merely because it is outside the old v19.56 BUY contract.
    original = globals()["_v1959_gy_pcs_sr_analyze"]
    try:
        globals()["_v1959_gy_pcs_sr_analyze"] = lambda: {
            "signal": {"trigger": "REJECTION", "direction": "SELL", "pcs": {"regime": "RANGING"}},
            "pcs": {"regime": "RANGING"},
            "execution_state": "READY",
        }
        sample = sr_analyze()
        check("gy_sell_rejection_not_blocked_by_legacy_gate", bool(sample.get("signal")) and sample.get("execution_state") == "READY")
        check("gy_legacy_gate_is_advisory", sample.get("gy_accuracy_gate", {}).get("state") == "ADVISORY")
    finally:
        globals()["_v1959_gy_pcs_sr_analyze"] = original

    return {"ok": all(x["status"] == "PASS" for x in checks), "version": V19_62_VERSION, "results": checks}



# ============================================================
# v19.64 — ALERT CONSISTENCY / CLAIM TOKEN / G.Y OUTBOX RACE HARDENING
# Scope: Alert delivery lifecycle only.
# MAIN strategy, Entry/SL/TP/R:R, scoring, subscriptions and quotas unchanged.
# ============================================================
V19_65_VERSION = "v19.65_ALERT_CONSISTENCY_HARDENED"
V19_64_VERSION = V19_65_VERSION
ALERT_DELIVERY_SEMANTICS = "AT_LEAST_ONCE"
ALERT_CLAIM_TOKEN_VERSION = "v1"
ALERT_STALE_SENDING_SECONDS = max(30.0, float(os.getenv("ALERT_STALE_SENDING_SECONDS", "300")))
GY_ALERT_STALE_CLAIM_SECONDS = max(30.0, float(os.getenv("GY_ALERT_STALE_CLAIM_SECONDS", "120")))
GY_ALERT_RECONCILE_SECONDS = max(2.0, float(os.getenv("GY_ALERT_RECONCILE_SECONDS", "10")))

# ------------------------------------------------------------------
# A) Persistent dispatcher: every delivery attempt gets a durable claim token.
#    Old workers can therefore never overwrite the state of a newer retry.
# ------------------------------------------------------------------
_v1964_base_init_db = PersistentAlertDispatcher._init_db


def _v1964_init_db(self):
    _v1964_base_init_db(self)
    with self._db_lock:
        c = self._conn()
        try:
            cols = {r[1] for r in c.execute("PRAGMA table_info(alert_outbox)").fetchall()}
            if "claim_token" not in cols:
                c.execute("ALTER TABLE alert_outbox ADD COLUMN claim_token TEXT")
            if "parts_total" not in cols:
                c.execute("ALTER TABLE alert_outbox ADD COLUMN parts_total INTEGER")
            if "parts_sent" not in cols:
                c.execute("ALTER TABLE alert_outbox ADD COLUMN parts_sent INTEGER DEFAULT 0")
            if "telegram_message_ids" not in cols:
                c.execute("ALTER TABLE alert_outbox ADD COLUMN telegram_message_ids TEXT")
            c.execute(
                "CREATE INDEX IF NOT EXISTS idx_alert_claim_token "
                "ON alert_outbox(claim_token) WHERE claim_token IS NOT NULL"
            )
            # Legacy SENDING rows predate claim fencing; never let an un-fenced
            # worker compete with the v19.64+ attempt state machine.
            c.execute(
                "UPDATE alert_outbox SET status='PENDING', next_retry=?, claim_token=NULL "
...
def _v1964_claim(self):
    ...
    with self._db_lock:
        ...
        try:
            ...
            updated = c.execute(
                "UPDATE alert_outbox SET status='SENDING',last_attempt=?,"
                "claimed_at=?,send_started_at=?,claim_token=? "
                "WHERE alert_id=? AND status='PENDING'",
                (now, now, now, claim_token, alert_id),
            )
            if updated.rowcount != 1:
                c.rollback()
                return None
            fresh = c.execute(
                "SELECT * FROM alert_outbox WHERE alert_id=? AND status='SENDING'",
                (alert_id,),
            ).fetchone()
            c.commit()
            if not fresh:
                return None
            with self._telemetry_lock:
                self._telemetry["claimed"] += 1
            logger.debug("ALERT_CLAIMED | id=%s token=%s", alert_id, claim_token[:10])
            return dict(fresh)
        except Exception:
            c.rollback()
            raise
...
def _v1964_record_part_delivery(self, alert_id, part_index, message_id=None, claim_token=None):
    if not claim_token:
        return False
    with self._db_lock:
        c = self._conn()
        try:
            row = c.execute(
                "SELECT telegram_message_ids FROM alert_outbox "
                "WHERE alert_id=? AND status='SENDING' AND claim_token=?",
            ...
            cur = c.execute(
                "UPDATE alert_outbox SET parts_sent=?,telegram_message_ids=? "
                "WHERE alert_id=? AND status='SENDING' AND claim_token=?",
                (
                    int(part_index),
...
def _v1964_result(self, job, ok, error=None, retry_after=None):
    now = time.time()
    attempts = int(job.get("attempts") or 0) + (0 if ok else 1)
    alert_id = str(job["alert_id"])
    ...
    with self._db_lock:
        ...
        try:
            if ok:
                cur = c.execute(
                    "UPDATE alert_outbox SET status='SENT',sent_at=?,delivery_latency_ms=?,"
                    "last_error=NULL,claim_token=NULL "
                    "WHERE alert_id=? AND status='SENDING' AND claim_token=?",
                    (now, (now - float(job["created_at"])) * 1000.0, alert_id, claim_token),
                )
                if cur.rowcount != 1:
                    c.rollback()
                    return False
                c.commit()
                self._resolve_waiter(alert_id, True)
                return True

            if attempts >= int(job["max_attempts"]):
                cur = c.execute(
                    "UPDATE alert_outbox SET status='FAILED',attempts=?,last_error=?,claim_token=NULL "
                    "WHERE alert_id=? AND status='SENDING' AND claim_token=?",
                    (attempts, str(error)[:500], alert_id, claim_token),
                )
                if cur.rowcount != 1:
                    c.rollback()
                    return False
                c.commit()
                self._resolve_waiter(alert_id, False)
                return True

            delay = (
                float(retry_after)
                if retry_after is not None
                else min(60.0, 2 ** min(attempts, 6)) + random.uniform(0, 0.5)
            )
            cur = c.execute(
                "UPDATE alert_outbox SET status='PENDING',attempts=?,next_retry=?,last_error=?,claim_token=NULL "
                "WHERE alert_id=? AND status='SENDING' AND claim_token=?",
                (attempts, now + delay, str(error)[:500], alert_id, claim_token),
            )
            if cur.rowcount != 1:
                c.rollback()
                return False
...
async def _v1964_dispatcher_worker(self, idx):
    """Worker with attempt fencing: stale workers cannot overwrite newer retries."""
    while self._running:
        try:
            job = await asyncio.to_thread(self._claim)
        except asyncio.CancelledError:
            ...
            logger.exception("ALERT_CLAIM_FAILURE_CONTAINED_V19_64 | idx=%s error=%s", idx, exc)
            await asyncio.sleep(ALERT_LIFECYCLE_CLAIM_RETRY_SECONDS)
            continue
        ...
        if not job:
            self._wake.clear()
            try:
                await asyncio.wait_for(self._wake.wait(), timeout=ALERT_POLL_SECONDS)
            except asyncio.TimeoutError:
                pass
            except asyncio.CancelledError:
                raise
        ...
        try:
            ...
            sent_parts = max(0, int(job.get("parts_sent") or 0))
            if sent_parts >= parts_total:
                persisted = await _v1943_safe_result(self, job, True)
                if persisted:
                    with self._telemetry_lock:
                        self._telemetry["sent"] += 1
            ...
            for part_index in range(sent_parts + 1, parts_total + 1):
                part = parts[part_index - 1]
                msg = await self.bot.send_message(
                    chat_id=job["chat_id"],
                    text=localize_user_text(part),
                )
                message_id = getattr(msg, "message_id", None)
                ...
                if not checkpointed:
                    raise RuntimeError("alert delivery checkpoint lost claim token")
            ...
            if persisted:
                ...
                logger.info(
                    "ALERT_DELIVERED_E2E | id=%s kind=%s queue_plus_delivery_ms=%.1f",
                    job["alert_id"], job["kind"],
                    (time.time() - job["created_at"]) * 1000.0,
                )
        except (Forbidden, BadRequest) as exc:
            with self._telemetry_lock:
                self._telemetry["failed"] += 1
            await _v1943_safe_result(self, {**job, "max_attempts": 1}, False, exc)
        except RetryAfter as exc:
            with self._telemetry_lock:
                self._telemetry["retried"] += 1
            ra = getattr(exc, "retry_after", 1)
            ra = float(ra.total_seconds() if hasattr(ra, "total_seconds") else ra)
            await _v1943_safe_result(self, job, False, exc, max(1.0, ra))
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            with self._telemetry_lock:
                self._telemetry["retried"] += 1
            logger.warning("TELEGRAM_RETRY_V19_64 | id=%s kind=%s error=%s", job["alert_id"], job["kind"], exc)
            await _v1943_safe_result(self, job, False, exc)


def _v1964_recover_stale_sending(self):
    cutoff = time.time() - ALERT_STALE_SENDING_SECONDS
    with self._db_lock:
        c = self._conn()
        try:
            rows = c.execute(
                "SELECT alert_id,parts_total,parts_sent,claim_token "
                "FROM alert_outbox WHERE status='SENDING' "
                "AND COALESCE(send_started_at,claimed_at,last_attempt,0)<=?",
                (cutoff,),
            ).fetchall()
            recovered = 0
            for row in rows:
                ...
                if complete:
                    c.execute(
                        "UPDATE alert_outbox SET status='SENT',sent_at=COALESCE(sent_at,?),"
                        "last_error=NULL,claim_token=NULL WHERE alert_id=? AND status='SENDING' AND claim_token=?",
                        (time.time(), row["alert_id"], row["claim_token"]),
                    )
                else:
                    c.execute(
                        "UPDATE alert_outbox SET status='PENDING',next_retry=?,"
                        "last_error=?,claim_token=NULL WHERE alert_id=? AND status='SENDING' AND claim_token=?",
                        (time.time(), "runtime stale-worker recovery", row["alert_id"], row["claim_token"]),
                    )
                if c.total_changes:
                    recovered += 1
            c.commit()
            if recovered:
                logger.warning("ALERT_STALE_SENDING_RECOVERED_V19_64 | count=%s", recovered)
            return recovered
        finally:
            c.close()

PersistentAlertDispatcher._init_db = _v1964_init_db
PersistentAlertDispatcher._claim = _v1964_claim
PersistentAlertDispatcher._record_part_delivery = _v1964_record_part_delivery
PersistentAlertDispatcher._result = _v1964_result
PersistentAlertDispatcher._worker = _v1964_dispatcher_worker
_v1943_recover_stale_sending = _v1964_recover_stale_sending

# Thread-safe completion for asyncio Futures: _result() executes in a worker thread.
def _v1964_resolve_waiter(self, aid, ok):
    with self._waiters_lock:
        fut = self._waiters.pop(aid, None)
    ...
    try:
        ...
        loop.call_soon_threadsafe(
            lambda: (not fut.done()) and fut.set_result(bool(ok))
        )
    except Exception:
        logger.exception("ALERT_WAITER_RESOLVE_FAILED | id=%s", aid)

PersistentAlertDispatcher._resolve_waiter = _v1964_resolve_waiter

# ------------------------------------------------------------------
# B) G.Y creation-alert outbox: atomic claiming + deterministic idempotency.
#    Two auto loops/restarts can no longer enqueue the same recipient twice.
# ------------------------------------------------------------------
def _v1964_gy_prepare_db():
    conn = _sr_db_connect()
    try:
        cols = {r[1] for r in conn.execute("PRAGMA table_info(sr_alert_outbox)").fetchall()}
        if "claimed_at" not in cols:
            conn.execute("ALTER TABLE sr_alert_outbox ADD COLUMN claimed_at REAL")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_sr_alert_outbox_claimed ON sr_alert_outbox(status,claimed_at,created_at)")
        conn.commit()
    finally:
        conn.close()


def _v1964_gy_requeue_orphans():
    now = time.time()
    cutoff = now - GY_ALERT_STALE_CLAIM_SECONDS
    conn = _sr_db_connect()
    try:
        conn.execute("BEGIN IMMEDIATE")
        conn.execute(
            "UPDATE sr_alert_outbox SET status='PENDING',claimed_at=NULL,alert_id=NULL,last_error=? "
            "WHERE status='SENDING' AND alert_id IS NULL AND COALESCE(claimed_at,0)<=?",
            ("stale G.Y claim recovered", cutoff),
        )
        conn.commit()
    except Exception:
        conn.rollback()
...
def _v1964_gy_claim_pending(limit=100):
    now = time.time()
    rows = []
    conn = _sr_db_connect()
    try:
        conn.execute("BEGIN IMMEDIATE")
        pending = conn.execute(
            "SELECT setup_id,chat_id,payload,status,attempts,alert_id,claimed_at "
            "FROM sr_alert_outbox WHERE status='PENDING' ORDER BY created_at ASC LIMIT ?",
            (int(limit),),
        ).fetchall()
        for row in pending:
            changed = conn.execute(
                "UPDATE sr_alert_outbox SET status='SENDING',attempts=attempts+1,"
                "claimed_at=?,last_error=NULL WHERE setup_id=? AND chat_id=? AND status='PENDING'",
                (now, row["setup_id"], int(row["chat_id"])),
            )
...
def _v1964_gy_attach_alert_id(setup_id, chat_id, alert_id):
    conn = _sr_db_connect()
    try:
        conn.execute(
            "UPDATE sr_alert_outbox SET alert_id=?,status='SENDING',claimed_at=? "
            "WHERE setup_id=? AND chat_id=? AND status='SENDING'",
            (str(alert_id), time.time(), setup_id, int(chat_id)),
        )
        conn.commit()
    finally:
        conn.close()
...
def _v1964_gy_requeue(setup_id, chat_id, error):
    conn = _sr_db_connect()
    try:
        conn.execute(
            "UPDATE sr_alert_outbox SET status='PENDING',alert_id=NULL,claimed_at=NULL,last_error=? "
            "WHERE setup_id=? AND chat_id=?",
            (str(error)[:500], setup_id, int(chat_id)),
        )
        conn.commit()
    finally:
        conn.close()
...
async def _v1964_sr_gy_alert_outbox_once():
    await asyncio.to_thread(_v1964_gy_prepare_db)
    try:
        await asyncio.to_thread(_v1964_gy_requeue_orphans)
    except Exception:
        logger.exception("G.Y stale alert claim recovery failed")
    ...
    # First reconcile already-claimed durable rows.
    for row in rows:
        if row.get("status") != "SENDING" or not row.get("alert_id"):
            continue
        try:
            status = await asyncio.to_thread(d.delivery_status, row["alert_id"])
        except Exception:
            status = None
        if status == "SENT":
            await asyncio.to_thread(_sr_gy_mark_alert_sent, row["setup_id"], row["chat_id"])
        elif status == "FAILED":
            await asyncio.to_thread(_sr_gy_mark_alert_retry, row["setup_id"], row["chat_id"], "dispatcher failed")

    claimed = await asyncio.to_thread(_v1964_gy_claim_pending, 100)
    for row in claimed:
        setup_id = row["setup_id"]
        chat_id = int(row["chat_id"])
        idem = f"GY:{setup_id}:{chat_id}:CREATE"
        try:
            aid = await asyncio.to_thread(
                d.enqueue,
                chat_id,
            ...
            if aid:
                await asyncio.to_thread(_v1964_gy_attach_alert_id, setup_id, chat_id, str(aid))
            else:
                await asyncio.to_thread(_v1964_gy_requeue, setup_id, chat_id, "dispatcher enqueue rejected")
        except Exception as exc:
            await asyncio.to_thread(_v1964_gy_requeue, setup_id, chat_id, str(exc)[:500])


_sr_gy_alert_outbox_once = _v1964_sr_gy_alert_outbox_once

# ------------------------------------------------------------------
# C) Lifecycle G.Y alerts: deterministic event keys + durable history replay.
# ------------------------------------------------------------------
def _v1964_gy_lifecycle_event_key(setup_id, event_time, status, result):
    # event_time is intentionally ignored: the same lifecycle transition can be
    # produced by the live path (no history timestamp available) and by the
    # durable history replay path. setup + status + result are the stable identity.
    raw = f"{setup_id}|{status}|{result or ''}"
    return "GY-LIFECYCLE:" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:32]


def _v1964_load_gy_lifecycle_events(limit=250):
    conn = _sr_db_connect()
    try:
        rows = conn.execute(
            "SELECT id,setup_id,event_time,status,result,price,note "
            "FROM sr_trade_history WHERE note='G.Y lifecycle state transition' "
...
async def _v1964_gy_lifecycle_alert_reconciler():
    while True:
        try:
            recipients = await asyncio.to_thread(_alert_recipients, "gy_private_trade")
            if recipients:
                events = await asyncio.to_thread(_v1964_load_gy_lifecycle_events, 250)
                for event in events:
                    status = str(event.get("status") or "")
                    result = str(event.get("result") or "")
                    setup_id = str(event.get("setup_id") or "")
                    event_key = _v1964_gy_lifecycle_event_key(
                        setup_id, event.get("event_time"), status, result
                    )
                    text = with_market_header(
                        f"🎯 تحديث {SR_GY_LABEL}\n"
                        f"🆔 {setup_id}\n"
                        f"💰 السعر: {event.get('price')}\n"
                        f"📌 النتيجة: {result}\n"
                        f"📊 الحالة: {status}"
                    )
                    await _dispatch_event_broadcast(
                        recipients,
                        text,
                        kind="sr",
                        event_key=event_key,
            ...
            logger.exception("G.Y lifecycle alert reconciliation error")
        await asyncio.sleep(GY_ALERT_RECONCILE_SECONDS)


async def _v1964_dispatch_broadcast(recipients, text, *, kind="generic", max_attempts=ALERT_MAX_ATTEMPTS, event_key=None):
    if event_key is None and str(kind) == "sr" and str(text).startswith("🎯 تحديث"):
        import re as _re
        m = _re.search(r"🆔\s*([^\n]+).*?📌\s*([^\n]+).*?📊\s*([^\n]+)", str(text), _re.S)
        if m:
            event_key = _v1964_gy_lifecycle_event_key(
                m.group(1).strip(), "", m.group(3).strip(), m.group(2).strip()
            )
    return await _v1964_prev_dispatch_broadcast(
        recipients, text, kind=kind, max_attempts=max_attempts, event_key=event_key
    )

_dispatch_broadcast = _v1964_dispatch_broadcast

# Start the durable lifecycle replay worker once per running event loop.
_v1964_prev_start_alert_dispatcher = _start_alert_dispatcher


async def _v1964_start_alert_dispatcher():
    d = await _v1964_prev_start_alert_dispatcher()
    live = [
        t for t in asyncio.all_tasks()
        if t.get_name() == "gy-lifecycle-alert-reconciler" and not t.done()
    ]
    if not live:
        asyncio.create_task(
            _v1964_gy_lifecycle_alert_reconciler(),
            name="gy-lifecycle-alert-reconciler",
        )
    return d

_start_alert_dispatcher = _v1964_start_alert_dispatcher


# ------------------------------------------------------------------
# D) Deterministic self-tests for the newly hardened contracts.
# ------------------------------------------------------------------
def run_v19_64_alert_hardening_self_tests():
    import inspect as _inspect
    checks = []
    ...
    src_claim = _inspect.getsource(_v1964_claim)
    src_result = _inspect.getsource(_v1964_result)
    src_worker = _inspect.getsource(_v1964_dispatcher_worker)
    check("claim_token_created", "claim_token = uuid.uuid4().hex" in src_claim)
    check("result_is_fenced", "claim_token=?" in src_result and "status='SENDING'" in src_result)
    check("worker_resumes_after_checkpoint", "sent_parts + 1" in src_worker)
    check("worker_checkpoint_fenced", "job.get(\"claim_token\")" in src_worker)
    check("waiter_thread_safe", "call_soon_threadsafe" in _inspect.getsource(_v1964_resolve_waiter))
    check("gy_atomic_claim", "BEGIN IMMEDIATE" in _inspect.getsource(_v1964_gy_claim_pending))
    check("gy_deterministic_idempotency", "GY:{setup_id}:{chat_id}:CREATE" in _inspect.getsource(_v1964_sr_gy_alert_outbox_once))
    check("gy_lifecycle_replay", callable(_v1964_gy_lifecycle_alert_reconciler))
    check("alert_semantics_stated", ALERT_DELIVERY_SEMANTICS == "AT_LEAST_ONCE")
    return {"ok": all(x["status"] == "PASS" for x in checks), "version": V19_65_VERSION, "results": checks}


VERSION = V19_64_VERSION

# v19.67: final executable entrypoint is moved to the physical end of the file.

# ============================================================
# v19.63 — LIVE MONITOR SOURCE-OF-TRUTH / SELF-HEALING UI — APPEND-ONLY
# Scope: monitoring/reporting only.
# - MAIN trade monitor reads canonical trades.db directly.
# - G.Y monitor reads canonical xau_sr_trades.db directly.
# - A monitor refresh performs a bounded lifecycle refresh before reporting.
# - Auditor DB is no longer a prerequisite for the Telegram monitor.
# - MAIN strategy, G.Y strategy, quotas, subscriptions and alert contracts unchanged.
# ============================================================
V19_63_VERSION = "v19.63_LIVE_MONITOR_SOURCE_OF_TRUTH"
V19_63_MONITOR_REFRESH_TIMEOUT = 12.0


def _v1963_damascus_cutoff():
    """Current-day cutoff in Damascus, returned as UTC for DB timestamp comparison."""
    try:
...
def _v1963_parse_any_dt(value):
    ...
    try:
        ...
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=ZoneInfo("Asia/Damascus"))
        return dt.astimezone(timezone.utc)
    except Exception:
        return None


def _v1963_is_today(value, cutoff):
    dt = _v1963_parse_any_dt(value)
    return bool(dt is not None and dt >= cutoff)


def _v1963_load_main_source():
    """Load canonical MAIN trades directly from trades.db, never from Auditor."""
    result = {"ok": True, "rows": [], "error": None}
    conn = None
    try:
        conn = _trade_db_connect()
        raw = conn.execute("SELECT payload FROM trades ORDER BY updated_at ASC").fetchall()
        for (payload,) in raw:
            try:
                item = json.loads(payload)
...
def _v1963_load_gy_source():
    """Load canonical G.Y trades directly from sr_trades."""
    result = {"ok": True, "rows": [], "error": None}
    conn = None
    try:
        conn = _sr_db_connect()
        result["rows"] = [dict(r) for r in conn.execute(
            "SELECT setup_id, created_at, direction, entry, sl, tp1, tp2, tp3, rr, score, trigger, status, result, closed_at "
            "FROM sr_trades ORDER BY created_at ASC"
        ).fetchall()]
    except Exception as exc:
        result["ok"] = False
        result["error"] = f"{type(exc).__name__}: {exc}"
...
def _v1963_classify_main(rows, cutoff):
    total = len(rows)
    active_rows = [r for r in rows if str(r.get("status") or "").upper() in {"ACTIVE", "TP1", "TP2"}]
    closed_rows = [r for r in rows if str(r.get("status") or "").upper() == "CLOSED"]
    today_rows = [r for r in rows if _v1963_is_today(r.get("time") or r.get("created_at"), cutoff)]
    today_closed = [r for r in closed_rows if _v1963_is_today(r.get("close_time") or r.get("last_update"), cutoff)]

    full = sum(str(r.get("result") or "").upper() in {"TP3", "TP3 / WIN", "FULL_SUCCESS"} for r in closed_rows)
    partial = sum(str(r.get("result") or "").upper() in {"TP1", "TP2", "PARTIAL_SUCCESS"} for r in closed_rows)
    failed = sum(str(r.get("result") or "").upper() in {"LOSS", "SL", "FAILED"} for r in closed_rows)
    expired = sum(str(r.get("result") or "").upper() in {"EXPIRED", "EXPIRY"} for r in closed_rows)
    ambiguous = sum(str(r.get("result") or "").upper() == "AMBIGUOUS" for r in closed_rows)

    return {
        "total": total,
        "today_created": len(today_rows),
...
def _v1963_classify_gy(rows, cutoff):
    total = len(rows)
    active_states = {"OPEN", "TP1_HIT", "TP2_HIT"}
    active_rows = [r for r in rows if str(r.get("status") or "").upper() in active_states]
    closed_rows = [r for r in rows if str(r.get("status") or "").upper() == "CLOSED"]
    today_rows = [r for r in rows if _v1963_is_today(r.get("created_at"), cutoff)]
    today_closed = [r for r in closed_rows if _v1963_is_today(r.get("closed_at"), cutoff)]

    full = sum(str(r.get("result") or "").upper() in {"TP3", "FULL_SUCCESS"} for r in closed_rows)
    partial = sum(
        str(r.get("status") or "").upper() in {"TP1_HIT", "TP2_HIT"}
        or str(r.get("result") or "").upper() in {"TP1", "TP2", "PARTIAL_SUCCESS"}
        for r in closed_rows
    )
    failed = sum(str(r.get("result") or "").upper() in {"SL", "FAILED", "LOSS"} for r in closed_rows)
    expired = sum(str(r.get("result") or "").upper() in {"EXPIRED", "EXPIRY"} for r in closed_rows)
    ambiguous = sum(str(r.get("result") or "").upper() == "AMBIGUOUS" for r in closed_rows)
...
def _v1963_refresh_monitor_state():
    """Refresh live lifecycle state without making the monitor depend on it succeeding."""
    errors = []

    try:
        update_trade_results()
    except Exception as exc:
        errors.append(f"MAIN lifecycle: {type(exc).__name__}: {exc}")

    try:
        price = live_price()
        _v1935_update_gy_lifecycle(price.get("price"))
    except Exception as exc:
        errors.append(f"G.Y lifecycle: {type(exc).__name__}: {exc}")

    # Keep Auditor synchronized when available, but never make its availability
    # a requirement for the monitor itself.
    if PERFORMANCE_AUDITOR is not None:
        try:
            reconcile = _auditor_reconcile_from_main_history()
            PERFORMANCE_AUDITOR._last_trade_reconcile_stats = dict(reconcile)
        except Exception as exc:
            errors.append(f"Auditor mirror: {type(exc).__name__}: {exc}")

    return {"ok": not errors, "errors": errors}


def _v1963_live_monitor_report():
    """Canonical read-through monitor report from the two production DBs."""
    cutoff = _v1963_damascus_cutoff()
    refresh = _v1963_refresh_monitor_state()
    main_src = _v1963_load_main_source()
    gy_src = _v1963_load_gy_source()

    main = _v1963_classify_main(main_src["rows"], cutoff) if main_src["ok"] else {
        "total": 0, "today_created": 0, "today_closed": 0, "active": 0, "closed": 0,
        "full_success": 0, "partial_success": 0, "failed": 0, "expired": 0, "ambiguous": 0,
    }
    gy = _v1963_classify_gy(gy_src["rows"], cutoff) if gy_src["ok"] else {
        "total": 0, "today_created": 0, "today_closed": 0, "active": 0, "closed": 0,
        "full_success": 0, "partial_success": 0, "failed": 0, "expired": 0, "ambiguous": 0,
    }
    ...
    if not main_src["ok"]:
        errors.append(f"MAIN DB: {main_src['error']}")
    if not gy_src["ok"]:
        errors.append(f"G.Y DB: {gy_src['error']}")

    return {
        "version": V19_63_VERSION,
        "generated_at": now_damascus().strftime("%Y-%m-%d %H:%M:%S"),
        "main": main,
        "gy": gy,
        "health": {
            "main_db": main_src["ok"],
            ...
            "errors": errors[:8],
            "source_main": TRADE_DB_PATH,
            "source_gy": SR_DB_PATH,
        },
    }


async def trade_performance_monitor(update, context):
    """Live monitor: canonical DB read-through + lifecycle refresh + partial failure isolation."""
    user_id = update.effective_user.id if update.effective_user else update.effective_chat.id
    chat_id = update.effective_chat.id if update.effective_chat else user_id
    if not (is_admin_chat(user_id) or is_admin_chat(chat_id)):
        await reply(update, "⛔ مراقب الصفقات مخصص للإدارة فقط.")
        return

    try:
        report = await asyncio.to_thread(_v1963_live_monitor_report)
        main = report["main"]
        gy = report["gy"]
        health = report["health"]

        lines = [
            "🛡️ XAU SMART TRADER — المراقب الحي",
            "",
            "🟢 الحالة الحالية",
            f"• الصفقات العامة تحت المراقبة: {main['active']}",
            f"• الصفقات الخاصة G.Y تحت المراقبة: {gy['active']}",
            f"• صفقات اليوم المنشأة: {main['today_created'] + gy['today_created']}",
            f"• العامة اليوم: {main['today_created']}",
            f"• G.Y اليوم: {gy['today_created']}",
            "",
            "🎯 الصفقات العامة",
            f"• الإجمالي المسجل: {main['total']}",
            f"• المغلقة: {main['closed']}",
            f"• المفتوحة: {main['active']}",
            f"• أُنشئت اليوم: {main['today_created']}",
            f"• أُغلقت اليوم: {main['today_closed']}",
            f"• نجاح كامل: {main['full_success']}",
            f"• نجاح جزئي: {main['partial_success']}",
            f"• خسائر: {main['failed']}",
            "",
            "🎯 الصفقات الخاصة G.Y",
            f"• الإجمالي المسجل: {gy['total']}",
            f"• المغلقة: {gy['closed']}",
            f"• المفتوحة: {gy['active']}",
            f"• أُنشئت اليوم: {gy['today_created']}",
            f"• أُغلقت اليوم: {gy['today_closed']}",
            f"• نجاح كامل: {gy['full_success']}",
            f"• نجاح جزئي: {gy['partial_success']}",
            f"• خسائر: {gy['failed']}",
            "",
            "🔐 مصدر الحقيقة",
            f"• MAIN: {'✅ trades.db' if health['main_db'] else '❌ غير متاح'}",
            f"• G.Y: {'✅ xau_sr_trades.db' if health['gy_db'] else '❌ غير متاح'}",
            f"• آخر تحديث: {report['generated_at']} دمشق",
        ]

        if health["errors"]:
            lines += ["", "⚠️ تنبيه المراقب", f"• {health['errors'][0]}"]
        else:
            lines += ["", "🟢 المراقب يعمل والبيانات مقروءة من قواعد التشغيل مباشرة."]

        buttons = [
            [InlineKeyboardButton("🔄 تحديث المراقب الآن", callback_data="menu_trade_performance")],
            [InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")],
        ]
        await update.effective_message.reply_text(
            with_market_header("\n".join(lines)), reply_markup=InlineKeyboardMarkup(buttons)
        ...
        logger.exception("V19.63 live trade monitor error")
        await reply(update, f"🛡️ تعذر إنشاء المراقب الحي.\nالسبب: {exc}")


def run_v19_63_monitor_self_tests():
    checks = []
    def check(name, ok, detail=""):
        checks.append({"test": name, "status": "PASS" if ok else "FAIL", "detail": detail})

    cutoff = _v1963_damascus_cutoff()
    local_now = datetime.now(ZoneInfo("Asia/Damascus"))
    sample_main = [
        {"time": local_now.isoformat(), "status": "ACTIVE", "result": "OPEN"},
        {"time": (local_now - timedelta(days=1)).isoformat(), "status": "CLOSED", "result": "LOSS"},
    ]
    sample_gy = [
        {"created_at": local_now.isoformat(), "status": "OPEN", "result": None},
        {"created_at": (local_now - timedelta(days=1)).isoformat(), "status": "CLOSED", "result": "TP3"},
    ]
    main = _v1963_classify_main(sample_main, cutoff)
    gy = _v1963_classify_gy(sample_gy, cutoff)
    check("main_today_created", main["today_created"] == 1, str(main))
    check("main_active", main["active"] == 1, str(main))
    check("gy_today_created", gy["today_created"] == 1, str(gy))
    check("gy_active", gy["active"] == 1, str(gy))
    check("gy_full_success_history", gy["full_success"] == 1, str(gy))
    check("monitor_version", V19_63_VERSION == "v19.63_LIVE_MONITOR_SOURCE_OF_TRUTH")
    check("monitor_not_auditor_only", "_v1963_load_main_source" in inspect.getsource(_v1963_live_monitor_report))
    return {"ok": all(x["status"] == "PASS" for x in checks), "version": V19_63_VERSION, "results": checks}

VERSION = V19_63_VERSION

# ============================================================
# v19.66 — END-TO-END ALERT SOURCE-OF-TRUTH CLOSURE
# Scope: Alert delivery/persistence/scheduler/recovery only.
# Trading logic, MAIN scoring, G.Y strategy gates, Entry/SL/TP,
# subscriptions and quota rules are unchanged.
#
# Guarantees introduced here:
# 1) Durable Outbox + deterministic Idempotency is the alert source of truth.
# 2) trade_notifications is a derived/mirror state, never a pre-enqueue gate.
# 3) Account notifications use the same PersistentAlertDispatcher.
# 4) Auditor alerts use the same PersistentAlertDispatcher.
# 5) G.Y lifecycle replay uses a durable SQLite watermark/cursor.
# 6) ALERT_WORKERS has one canonical bounded runtime value.
# 7) G.Y creation outbox receives claim-token fencing for stale workers.
# ============================================================
V19_66_VERSION = "v19.66_ALERT_END_TO_END_HARDENED"
VERSION = V19_66_VERSION
ALERT_DELIVERY_SEMANTICS = "AT_LEAST_ONCE"
ALERT_WORKERS = max(1, min(int(os.getenv("ALERT_WORKERS", "8")), 8))
GY_LIFECYCLE_REPLAY_BATCH = max(25, int(os.getenv("GY_LIFECYCLE_REPLAY_BATCH", "250")))
GY_LIFECYCLE_WATERMARK_KEY = "gy_lifecycle_v19_66"
GY_LIFECYCLE_REPLAY_LOG_INTERVAL = max(1, int(os.getenv("GY_LIFECYCLE_REPLAY_LOG_INTERVAL", "100")))

# ------------------------------------------------------------------
# A) MAIN trade alert: Outbox first, business mirror second.
# ------------------------------------------------------------------
def _v1966_ensure_trade_notification_mirror(chat_id, signature):
    """Persist derived SENDING state after the durable Outbox already exists.

    This is intentionally NOT a delivery gate. The Outbox/idempotency record is
    authoritative; this table is only a compatibility/status mirror.
    """
    if not signature:
        return False
    key, status, result = signature
    ...
    try:
        ...
        conn.execute("BEGIN IMMEDIATE")
        row = conn.execute(
            "SELECT state FROM trade_notifications WHERE trade_key=? AND chat_id=? AND status=? AND result=?",
            (key, int(chat_id), status, result),
        ).fetchone()
        now = now_damascus().isoformat()
        if row is None:
            conn.execute(
                "INSERT INTO trade_notifications(trade_key,chat_id,status,result,sent_at,state) VALUES(?,?,?,?,?,?)",
                (key, int(chat_id), status, result, now, "SENDING"),
            )
        elif str(row[0]) != "SENT":
            conn.execute(
                "UPDATE trade_notifications SET state='SENDING',sent_at=? WHERE trade_key=? AND chat_id=? AND status=? AND result=?",
                (now, key, int(chat_id), status, result),
            )
        conn.commit()
        return True
    except Exception:
        if conn:
            try:
                ...
                pass
        logger.exception("TRADE_NOTIFICATION_MIRROR_ERROR | chat_id=%s", chat_id)
        return False
    finally:
        if conn:
            conn.close()
...
def _v1966_get_alert_trade_link(aid):
    conn = None
    try:
        conn = _alert_trade_link_conn()
        row = conn.execute(
            "SELECT * FROM alert_trade_links WHERE alert_id=? LIMIT 1",
            (str(aid),),
        ).fetchone()
        return dict(row) if row else None
    finally:
...
async def _v1966_send_trade_notification(
    chat_id, result, record, title, consume_quota=False, quota_token=None
):
    """Durable-first trade alert path.

    Critical ordering:
      1. Build deterministic Outbox/idempotency record atomically.
      2. Only after durable persistence, mirror the legacy trade-notification state.
      3. Telegram delivery is performed solely by PersistentAlertDispatcher.

    Therefore a crash after step 1 cannot produce `claim exists + no Outbox`.
    """
    signature = _trade_notification_signature(record)
    if not signature:
        return False

    # Fast in-memory duplicate suppression remains only an optimization.
    if _notification_already_sent(chat_id, signature):
        # The caller may have already reserved a fresh quota token before reaching
        # this path. Release only that fresh reservation; no new alert is created.
        if consume_quota and quota_token:
            await asyncio.to_thread(_release_trade_quota, quota_token)
        return False
    ...
    aid = None
    try:
        # Durable source of truth first. This transaction atomically persists the
        # Outbox row and alert_trade_links business mapping.
        aid = await asyncio.to_thread(
            d.enqueue_trade_with_link,
            chat_id,
            payload,
            signature,
            consume_quota,
        ...
        if not aid:
            if consume_quota:
                await asyncio.to_thread(_release_trade_quota, quota_token)
            logger.error("TRADE_ALERT_ENQUEUE_REJECTED | chat_id=%s idem=%s", chat_id, idem)
            return False

        # Compatibility mirror is strictly after durable Outbox persistence.
        await asyncio.to_thread(_v1966_ensure_trade_notification_mirror, chat_id, signature)

        link = await asyncio.to_thread(_v1966_get_alert_trade_link, aid)
        if link:
            # If this call supplied a fresh quota token but the deterministic alert
            # already existed from an earlier attempt, the new reservation must be
            # released. The durable link owns the original token.
            stored_token = link.get("quota_token")
            stored_aid = str(link.get("alert_id"))
            if consume_quota and quota_token and stored_aid == str(aid):
                if stored_token and str(stored_token) != str(quota_token):
                    await asyncio.to_thread(_release_trade_quota, quota_token)
                    quota_token = None
                elif not stored_token:
                    # Existing admin/no-quota link: a newly reserved user quota must
                    # not be consumed by a duplicate delivery attempt.
                    await asyncio.to_thread(_release_trade_quota, quota_token)
                    quota_token = None
        else:
            # Extremely defensive: Outbox exists but link is missing. The dispatcher
            # migration/reconciliation will repair it, and the current fresh quota
            # reservation should not remain consumed without ownership.
            if consume_quota and quota_token:
                await asyncio.to_thread(_release_trade_quota, quota_token)
                quota_token = None
        ...
        resolve_link = {
            ...
            "chat_id": int(chat_id),
            "signature": signature,
            "consume_quota": 1 if (link or {}).get("consume_quota") else 0,
            "quota_token": (link or {}).get("quota_token"),
        }

        if delivered is True:
            await _resolve_alert_trade_link(resolve_link)
            return True

        if delivered is None:
            asyncio.create_task(
                _finalize_trade_delivery_after_timeout(
                    aid,
                    int(chat_id),
                    signature,
                    bool(resolve_link.get("consume_quota")),
                    resolve_link.get("quota_token"),
                ),
                name=f"trade-alert-finalize-{aid}",
            )
            logger.warning(
                "TRADE_ALERT_PENDING_AFTER_TIMEOUT_V1966 | chat_id=%s alert_id=%s",
                chat_id,
                aid,
            )
            return True
        ...
        # The durable Outbox remains authoritative once aid exists. Do not delete
        # business state merely because the caller-side wait/enqueue wrapper failed.
        if aid is None:
            if consume_quota and quota_token:
                await asyncio.to_thread(_release_trade_quota, quota_token)
        else:
            logger.exception(
                "TRADE_ALERT_POST_ENQUEUE_ERROR_V1966 | chat_id=%s alert_id=%s",
                chat_id,
                aid,
            )
        return False
...
_send_trade_notification = _v1966_send_trade_notification

# ------------------------------------------------------------------
# B) Account notifications: same durable dispatcher, no direct Telegram call.
# ------------------------------------------------------------------
async def _v1966_send_account_notification(chat_id, event_key, text):
    if not APPLICATION or not text or is_admin_chat(chat_id):
        return False
    cid = int(chat_id)
    idem = f"ACCOUNT:{cid}:{str(event_key)}"
    payload = with_market_header(text)
    d = ALERT_DISPATCHER or await _start_alert_dispatcher()
    if not d:
        return False
    try:
        aid = await asyncio.to_thread(
            ...
            kind="system",
            priority=100,
            max_attempts=EVENT_ALERT_MAX_ATTEMPTS,
            idempotency_key=idem,
        )
        if not aid:
            logger.error("ACCOUNT_ALERT_ENQUEUE_REJECTED | chat_id=%s event=%s", cid, event_key)
            return False
        # Compatibility/audit mirror only. It never decides whether the alert is
        # allowed to enter or remain in the durable Outbox.
        try:
            await asyncio.to_thread(_claim_account_notification, cid, event_key)
        except Exception:
            logger.exception("ACCOUNT_ALERT_MIRROR_ERROR | chat_id=%s event=%s", cid, event_key)
        logger.info("ACCOUNT_ALERT_QUEUED | chat_id=%s event=%s alert_id=%s", cid, event_key, aid)
        return True
    except Exception:
        logger.exception("ACCOUNT_ALERT_ENQUEUE_ERROR | chat_id=%s event=%s", cid, event_key)
        return False


_send_account_notification = _v1966_send_account_notification

# ------------------------------------------------------------------
# C) Auditor alerts: route to the same durable dispatcher.
# ------------------------------------------------------------------
def _v1966_auditor_alert_callback(level, reason, snapshot):
    """Thread-safe bridge: Auditor never calls Telegram directly."""
    try:
        loop = BOT_LOOP
        if loop is None or APPLICATION is None or not ADMIN_IDS:
            return
        reason_text = str(reason or "unknown")[:300]
        level_text = str(level or "INFO")[:64]
        # Five-minute bucket: repeated identical observer state does not spam admins,
        # while a persistent condition can produce a fresh durable event periodically.
        bucket = int(time.time() // 300)
        raw = f"{level_text}|{reason_text}|{bucket}"
        event_hash = hashlib.sha256(raw.encode("utf-8")).hexdigest()[:24]
        text = (
            f"🛡️ تنبيه مراقب الأداء\n\n"
            f"المستوى: {level_text}\n"
            f"المشكلة: {reason_text}\n"
            f"CPU: {float(snapshot.get('cpu_pct', 0) or 0):.1f}% | "
            f"RAM: {float(snapshot.get('rss_mb', 0) or 0):.1f} MB\n"
            f"Webhook P95: {float(snapshot.get('request_p95_ms', 0) or 0):.1f} ms\n"
            f"Handler P95: {float(snapshot.get('handler_p95_ms', 0) or 0):.1f} ms\n"
            f"Event Loop P95: {float(snapshot.get('event_loop_lag_ms', 0) or 0):.1f} ms"
        )
        for cid in tuple(int(x) for x in ADMIN_IDS):
            idem = f"AUDITOR:{cid}:{event_hash}"
            fut = asyncio.run_coroutine_threadsafe(
                _dispatch_alert(
                    cid,
                    localize_user_text(text),
                    kind="system",
                    max_attempts=ALERT_MAX_ATTEMPTS,
                    wait_delivery=False,
                    idempotency_key=idem,
            ...
            def _audit_done(done_fut, chat=cid, key=idem):
                try:
                    ok = done_fut.result()
                    if not ok:
                        logger.error("AUDITOR_ALERT_ENQUEUE_FAILED | chat_id=%s idem=%s", chat, key)
                except Exception:
                    logger.exception("AUDITOR_ALERT_ENQUEUE_FUTURE_FAILED | chat_id=%s idem=%s", chat, key)
            fut.add_done_callback(_audit_done)
    except Exception:
        logger.exception("AUDITOR_ALERT_OUTBOX_BRIDGE_ERROR")


_auditor_alert_callback = _v1966_auditor_alert_callback

# ------------------------------------------------------------------
# D) G.Y lifecycle outbox: claim-token fencing + durable watermark.
# ------------------------------------------------------------------
def _v1966_gy_prepare_db():
    _v1964_gy_prepare_db()
    conn = _sr_db_connect()
    try:
        cols = {r[1] for r in conn.execute("PRAGMA table_info(sr_alert_outbox)").fetchall()}
        if "claim_token" not in cols:
            conn.execute("ALTER TABLE sr_alert_outbox ADD COLUMN claim_token TEXT")
        if "claimed_at" not in cols:
            conn.execute("ALTER TABLE sr_alert_outbox ADD COLUMN claimed_at REAL")
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_sr_alert_outbox_claim_token "
            "ON sr_alert_outbox(claim_token) WHERE claim_token IS NOT NULL"
        )
        conn.execute(
            """CREATE TABLE IF NOT EXISTS sr_alert_replay_state(
        ...
        row = conn.execute(
            "SELECT last_history_id FROM sr_alert_replay_state WHERE state_key=?",
            (GY_LIFECYCLE_WATERMARK_KEY,),
        ).fetchone()
        if row is None:
            max_id = conn.execute(
                "SELECT COALESCE(MAX(id),0) FROM sr_trade_history "
                "WHERE note='G.Y lifecycle state transition'"
            ).fetchone()[0]
            now_iso = now_damascus().isoformat()
            conn.execute(
                "INSERT INTO sr_alert_replay_state(state_key,last_history_id,initialized_at,updated_at) VALUES(?,?,?,?)",
                (GY_LIFECYCLE_WATERMARK_KEY, int(max_id or 0), now_iso, now_iso),
            )
            logger.info(
                "GY_LIFECYCLE_WATERMARK_INITIALIZED | last_history_id=%s",
                int(max_id or 0),
            )
        conn.commit()
    finally:
...
def _v1966_gy_requeue_orphans():
    ...
    try:
        conn.execute("BEGIN IMMEDIATE")
        cur = conn.execute(
            "UPDATE sr_alert_outbox SET status='PENDING',claimed_at=NULL,claim_token=NULL,"
            "alert_id=NULL,last_error=? "
            "WHERE status='SENDING' AND alert_id IS NULL AND COALESCE(claimed_at,0)<=?",
            ("stale G.Y claim recovered", cutoff),
        )
        conn.commit()
...
def _v1966_gy_claim_pending(limit=100):
    now = time.time()
    rows = []
    conn = _sr_db_connect()
    try:
        conn.execute("BEGIN IMMEDIATE")
        pending = conn.execute(
            "SELECT setup_id,chat_id,payload,status,attempts,alert_id,claimed_at,claim_token "
            "FROM sr_alert_outbox WHERE status='PENDING' ORDER BY created_at ASC LIMIT ?",
            (int(limit),),
        ).fetchall()
        for row in pending:
            ...
            changed = conn.execute(
                "UPDATE sr_alert_outbox SET status='SENDING',attempts=attempts+1,"
                "claimed_at=?,claim_token=?,last_error=NULL "
                "WHERE setup_id=? AND chat_id=? AND status='PENDING'",
                (now, token, row["setup_id"], int(row["chat_id"])),
            )
            if changed.rowcount == 1:
                item = dict(row)
...
def _v1966_gy_attach_alert_id(setup_id, chat_id, alert_id, claim_token):
    conn = _sr_db_connect()
    try:
        cur = conn.execute(
            "UPDATE sr_alert_outbox SET alert_id=?,claimed_at=? "
            "WHERE setup_id=? AND chat_id=? AND status='SENDING' AND claim_token=?",
            (str(alert_id), time.time(), setup_id, int(chat_id), str(claim_token)),
        )
        conn.commit()
        return cur.rowcount == 1
    finally:
...
def _v1966_gy_mark_alert_sent(setup_id, chat_id, claim_token=None):
    conn = _sr_db_connect()
    try:
        if claim_token:
            cur = conn.execute(
                "UPDATE sr_alert_outbox SET status='SENT',sent_at=?,last_error=NULL,"
                "claim_token=NULL,claimed_at=NULL WHERE setup_id=? AND chat_id=? "
                "AND status='SENDING' AND claim_token=?",
                (now_damascus().isoformat(), setup_id, int(chat_id), str(claim_token)),
            )
        else:
            cur = conn.execute(
                "UPDATE sr_alert_outbox SET status='SENT',sent_at=?,last_error=NULL,"
                "claim_token=NULL,claimed_at=NULL WHERE setup_id=? AND chat_id=? "
                "AND status='SENDING' AND (claim_token IS NULL OR claim_token='')",
                (now_damascus().isoformat(), setup_id, int(chat_id)),
            )
        conn.commit()
        return cur.rowcount == 1
    finally:
...
def _v1966_gy_mark_alert_retry(setup_id, chat_id, error, claim_token=None):
    conn = _sr_db_connect()
    try:
        if claim_token:
            cur = conn.execute(
                "UPDATE sr_alert_outbox SET status='PENDING',last_error=?,alert_id=NULL,"
                "claim_token=NULL,claimed_at=NULL WHERE setup_id=? AND chat_id=? "
                "AND status='SENDING' AND claim_token=?",
                (str(error)[:500], setup_id, int(chat_id), str(claim_token)),
            )
        else:
            cur = conn.execute(
                "UPDATE sr_alert_outbox SET status='PENDING',last_error=?,alert_id=NULL,"
...
def _v1966_sr_gy_pending_alert_rows():
    conn = _sr_db_connect()
    try:
        return [
            dict(r)
            for r in conn.execute(
                "SELECT setup_id,chat_id,payload,status,attempts,alert_id,claimed_at,claim_token "
                "FROM sr_alert_outbox WHERE status IN ('PENDING','SENDING') "
...
async def _v1966_sr_gy_alert_outbox_once():
    await asyncio.to_thread(_v1966_gy_prepare_db)
    try:
        recovered = await asyncio.to_thread(_v1966_gy_requeue_orphans)
        if recovered:
            logger.warning("GY_ALERT_STALE_CLAIMS_RECOVERED_V1966 | count=%s", recovered)
    except Exception:
        logger.exception("G.Y stale alert claim recovery failed")
    ...
    rows = await asyncio.to_thread(_v1966_sr_gy_pending_alert_rows)
    # Reconcile already-attached dispatcher alerts first.
    for row in rows:
        if row.get("status") != "SENDING" or not row.get("alert_id"):
            continue
        try:
        ...
        if status == "SENT":
            await asyncio.to_thread(
                _v1966_gy_mark_alert_sent,
                row["setup_id"], row["chat_id"], row.get("claim_token")
            )
        elif status == "FAILED":
            await asyncio.to_thread(
                _v1966_gy_mark_alert_retry,
                row["setup_id"], row["chat_id"],
                "dispatcher failed",
                row.get("claim_token"),
            )

    claimed = await asyncio.to_thread(_v1966_gy_claim_pending, 100)
    for row in claimed:
        setup_id = row["setup_id"]
        chat_id = int(row["chat_id"])
        ...
        try:
            ...
            if not aid:
                await asyncio.to_thread(
                    _v1966_gy_mark_alert_retry,
                    setup_id, chat_id, "dispatcher enqueue rejected", claim_token
                )
                continue
            attached = await asyncio.to_thread(
                _v1966_gy_attach_alert_id, setup_id, chat_id, str(aid), claim_token
            )
            if not attached:
                logger.warning(
                    "GY_ALERT_CLAIM_LOST_V1966 | setup_id=%s chat_id=%s",
                    setup_id, chat_id,
                )
                continue
            # Fast path: if idempotency resolved to an already-sent Alert, finalize now.
            status = await asyncio.to_thread(d.delivery_status, aid)
            if status == "SENT":
                await asyncio.to_thread(
                    _v1966_gy_mark_alert_sent, setup_id, chat_id, claim_token
                )
            elif status == "FAILED":
                await asyncio.to_thread(
                    _v1966_gy_mark_alert_retry,
                    setup_id, chat_id, "dispatcher failed", claim_token
                )
        except Exception as exc:
            await asyncio.to_thread(
                _v1966_gy_mark_alert_retry,
                setup_id, chat_id, str(exc)[:500], claim_token
            )


_sr_gy_alert_outbox_once = _v1966_sr_gy_alert_outbox_once

# ------------------------------------------------------------------
# E) G.Y lifecycle replay: permanent SQLite cursor/watermark.
# ------------------------------------------------------------------
def _v1966_gy_get_watermark():
    conn = _sr_db_connect()
    try:
        row = conn.execute(
            "SELECT last_history_id FROM sr_alert_replay_state WHERE state_key=?",
...
def _v1966_gy_load_lifecycle_batch(after_id, limit=None):
    batch_limit = int(limit or GY_LIFECYCLE_REPLAY_BATCH)
    conn = _sr_db_connect()
    try:
        rows = conn.execute(
            "SELECT id,setup_id,event_time,status,result,price,note "
            "FROM sr_trade_history "
            "WHERE note='G.Y lifecycle state transition' AND id>? "
            "ORDER BY id ASC LIMIT ?",
            (int(after_id), batch_limit),
        ).fetchall()
...
def _v1966_gy_set_watermark(history_id):
    now_iso = now_damascus().isoformat()
    conn = _sr_db_connect()
    try:
        cur = conn.execute(
            "UPDATE sr_alert_replay_state SET last_history_id=?,updated_at=? WHERE state_key=?",
            (int(history_id), now_iso, GY_LIFECYCLE_WATERMARK_KEY),
        )
        if cur.rowcount != 1:
            conn.execute(
                "INSERT OR REPLACE INTO sr_alert_replay_state(state_key,last_history_id,initialized_at,updated_at) VALUES(?,?,?,?)",
                (GY_LIFECYCLE_WATERMARK_KEY, int(history_id), now_iso, now_iso),
            )
        conn.commit()
    finally:
        conn.close()
...
def _v1966_gy_lifecycle_text(event):
    status = str(event.get("status") or "")
    result = str(event.get("result") or "")
    setup_id = str(event.get("setup_id") or "")
    return with_market_header(
        f"🎯 تحديث {SR_GY_LABEL}\n"
        f"🆔 {setup_id}\n"
        f"💰 السعر: {event.get('price')}\n"
        f"📌 النتيجة: {result}\n"
...
async def _v1966_gy_lifecycle_alert_reconciler():
    await asyncio.to_thread(_v1966_gy_prepare_db)
    cycles_without_events = 0
    while True:
        try:
            recipients = await asyncio.to_thread(_alert_recipients, "gy_private_trade")
            watermark = await asyncio.to_thread(_v1966_gy_get_watermark)
            events = await asyncio.to_thread(
                _v1966_gy_load_lifecycle_batch,
                watermark,
                GY_LIFECYCLE_REPLAY_BATCH,
            )
            if not events:
                ...
                for event in events:
                    ...
                    if recipients:
                        results = await _dispatch_event_broadcast(
                            recipients,
                            _v1966_gy_lifecycle_text(event),
                            kind="sr",
                            event_key=event_key,
                        )
                        if not all(bool(x) for x in results):
                            raise RuntimeError(
                                f"G.Y lifecycle event enqueue incomplete at history_id={event['id']}"
                            )
                    # We advance only after every recipient has durable Outbox state.
                    # A crash before this update merely replays the same deterministic
                    # event key; the Outbox idempotency layer absorbs duplicates.
                    await asyncio.to_thread(_v1966_gy_set_watermark, int(event["id"]))

                if len(events) >= GY_LIFECYCLE_REPLAY_BATCH:
                    logger.info(
                        "GY_LIFECYCLE_WATERMARK_CATCHUP | processed=%s last_id=%s",
                        len(events), events[-1]["id"],
                    )

        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("G.Y lifecycle watermark reconciliation error")

        await asyncio.sleep(GY_ALERT_RECONCILE_SECONDS)


# Keep the dispatcher-start wrapper but make the function it launches the
# watermark/cursor implementation above.
_v1964_gy_lifecycle_alert_reconciler = _v1966_gy_lifecycle_alert_reconciler

# ------------------------------------------------------------------
# F) One canonical worker-count configuration across all constructors.
# ------------------------------------------------------------------
_alert_dispatcher_config = lambda: (int(os.getenv("ALERT_QUEUE_SIZE", "5000")), ALERT_WORKERS)

_v1966_base_dispatcher_init = PersistentAlertDispatcher.__init__
def _v1966_dispatcher_init(self, bot, db_path=ALERT_DB_PATH, workers=None):
    # Ignore drifting call-site defaults (4/8/etc.); one bounded runtime value rules all starts.
    return _v1966_base_dispatcher_init(
        self,
        bot,
        db_path=db_path,
        workers=ALERT_WORKERS,
    )

PersistentAlertDispatcher.__init__ = _v1966_dispatcher_init

# ------------------------------------------------------------------
# G) Final alert-dispatcher start wrapper.
# ------------------------------------------------------------------
_v1966_prev_start_alert_dispatcher = _start_alert_dispatcher

async def _v1966_start_alert_dispatcher():
    d = await _v1966_prev_start_alert_dispatcher()
    if d is None:
        return None

    # Durable trade-link recovery remains active.
    await asyncio.to_thread(_init_alert_trade_links)
    await _reconcile_alert_trade_links_once()

    # G.Y watermark DB is durable and initialized exactly once.
    await asyncio.to_thread(_v1966_gy_prepare_db)
    ...
    if not any(
        t.get_name() == "gy-lifecycle-alert-reconciler" and not t.done()
        for t in asyncio.all_tasks()
    ):
        asyncio.create_task(
            _v1966_gy_lifecycle_alert_reconciler(),
            name="gy-lifecycle-alert-reconciler",
        )
    return d

_start_alert_dispatcher = _v1966_start_alert_dispatcher

# ------------------------------------------------------------------
# H) Static/runtime contract self-test definitions.
# ------------------------------------------------------------------
def run_v19_66_alert_contract_self_tests():
    import inspect as _inspect
    checks = []
    ...
    send_src = _inspect.getsource(_send_trade_notification)
    account_src = _inspect.getsource(_send_account_notification)
    auditor_src = _inspect.getsource(_auditor_alert_callback)
    gy_src = _inspect.getsource(_v1966_gy_lifecycle_alert_reconciler)

    check("version", V19_66_VERSION == "v19.66_ALERT_END_TO_END_HARDENED")
    check("trade_outbox_before_mirror", send_src.find("d.enqueue_trade_with_link") < send_src.find("_v1966_ensure_trade_notification_mirror"))
    check("trade_mirror_not_a_gate", "if not claimed" not in send_src)
    check("trade_sqlite_via_thread", "asyncio.to_thread(_v1966_ensure_trade_notification_mirror" in send_src)
    check("account_uses_dispatcher", "d.enqueue" in account_src and "APPLICATION.bot.send_message" not in account_src)
    check("auditor_uses_dispatcher", "_dispatch_alert" in auditor_src and "APPLICATION.bot.send_message" not in auditor_src)
    check("gy_watermark_table", "sr_alert_replay_state" in _inspect.getsource(_v1966_gy_prepare_db))
    check("gy_cursor_query", "id>?" in _inspect.getsource(_v1966_gy_load_lifecycle_batch))
    check("gy_claim_token", "claim_token" in _inspect.getsource(_v1966_gy_claim_pending))
    check("canonical_workers", isinstance(ALERT_WORKERS, int) and 1 <= ALERT_WORKERS <= 8)
    check("dispatcher_init_uses_constant", "workers=ALERT_WORKERS" in _inspect.getsource(PersistentAlertDispatcher.__init__))
    check("semantic", ALERT_DELIVERY_SEMANTICS == "AT_LEAST_ONCE")
    check("replay_advances_after_enqueue", "_v1966_gy_set_watermark" in gy_src)
    return {
        "ok": all(x["status"] == "PASS" for x in checks),
        "version": V19_66_VERSION,
        "workers": ALERT_WORKERS,
...
# Preserve version contract for external health/status callers.
VERSION = V19_66_VERSION

# ============================================================
# v19.67 — RUNTIME / PERFORMANCE / G.Y / AUDITOR HARDENING
# Scope: runtime ordering, startup readiness, data-fetch stampede control,
# institutional refresh single-flight, G.Y schema migration, supervisor liveness.
# Trading strategy, scoring, thresholds, Entry/SL/TP, R:R, quota/subscriptions
# and G.Y trigger/accuracy rules are intentionally untouched.
# ============================================================
V19_67_VERSION = "v19.67_RUNTIME_PERFORMANCE_GY_AUDITOR_HARDENED"
VERSION = V19_67_VERSION

_V1967_DATA_LOCKS = {}
_V1967_DATA_LOCKS_GUARD = threading.RLock()
_V1967_DATA_LOCK_KEYS_MAX = 64

def _v1967_get_data_lock(interval, limit):
    key = (str(interval).lower(), int(limit))
    with _V1967_DATA_LOCKS_GUARD:
        lock = _V1967_DATA_LOCKS.get(key)
        if lock is None:
            lock = threading.RLock()
            _V1967_DATA_LOCKS[key] = lock
            # Known production call sites use a small fixed set of (interval,limit) keys.
            # Keep locks stable for correctness; do not evict a lock that another thread may use.
        return lock

_v1967_base_get_bars = get_bars

def _v1967_get_bars(interval, limit=300):
    lock = _v1967_get_data_lock(interval, limit)
    with lock:
        return _v1967_base_get_bars(interval, limit)

get_bars = _v1967_get_bars

_V1967_INSTITUTIONAL_REFRESH_LOCK = threading.Lock()
_v1967_base_institutional_analysis = institutional_analysis

def _v1967_institutional_analysis(df):
    now = time.time()
    with INSTITUTIONAL_SNAPSHOT_LOCK:
        cached = INSTITUTIONAL_SNAPSHOT.get("value")
        cached_ts = float(INSTITUTIONAL_SNAPSHOT.get("ts", 0.0) or 0.0)
        if cached is not None and now - cached_ts < INSTITUTIONAL_SNAPSHOT_TTL:
            return cached.copy() if isinstance(cached, dict) else cached
    with _V1967_INSTITUTIONAL_REFRESH_LOCK:
        now = time.time()
        with INSTITUTIONAL_SNAPSHOT_LOCK:
            cached = INSTITUTIONAL_SNAPSHOT.get("value")
            cached_ts = float(INSTITUTIONAL_SNAPSHOT.get("ts", 0.0) or 0.0)
            if cached is not None and now - cached_ts < INSTITUTIONAL_SNAPSHOT_TTL:
                return cached.copy() if isinstance(cached, dict) else cached
        started = time.perf_counter()
        value = _v1967_base_institutional_analysis(df)
        with INSTITUTIONAL_SNAPSHOT_LOCK:
            INSTITUTIONAL_SNAPSHOT.update({"ts": time.time(), "value": value.copy() if isinstance(value, dict) else value})
        logger.info("PERF_STAGE | stage=institutional_refresh latency_ms=%.1f single_flight=1", (time.perf_counter() - started) * 1000.0)
        return value

institutional_analysis = _v1967_institutional_analysis

_V1967_PREFETCH_EXECUTOR = ThreadPoolExecutor(max_workers=1, thread_name_prefix="institutional-prefetch")
_v1967_base_evaluate_signal = evaluate_signal

def evaluate_signal():
    try:
        _V1967_PREFETCH_EXECUTOR.submit(institutional_analysis, None)
    except Exception:
        logger.exception("V19_67_INSTITUTIONAL_PREFETCH_SUBMIT_ERROR")
    return _v1967_base_evaluate_signal()

_V1967_GY_SCHEMA_LOCK = threading.RLock()

def _v1967_gy_prepare_db():
    with _V1967_GY_SCHEMA_LOCK:
        conn = _sr_db_connect()
        try:
            conn.execute("PRAGMA busy_timeout=10000")
            conn.execute("BEGIN IMMEDIATE")
            cols = {r[1] for r in conn.execute("PRAGMA table_info(sr_alert_outbox)").fetchall()}
            for name, typ in (("claim_token", "TEXT"), ("claimed_at", "REAL")):
                if name not in cols:
                    try:
                        conn.execute(f"ALTER TABLE sr_alert_outbox ADD COLUMN {name} {typ}")
                    except sqlite3.OperationalError as exc:
                        if "duplicate column name" not in str(exc).lower():
                            raise
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sr_alert_outbox_claimed ON sr_alert_outbox(status,claimed_at,created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sr_alert_outbox_claim_token ON sr_alert_outbox(claim_token) WHERE claim_token IS NOT NULL")
            conn.execute("""CREATE TABLE IF NOT EXISTS sr_alert_replay_state(
                state_key TEXT PRIMARY KEY,
                last_history_id INTEGER NOT NULL DEFAULT 0,
                initialized_at TEXT NOT NULL,
                updated_at TEXT NOT NULL
            )""")
            row = conn.execute("SELECT last_history_id FROM sr_alert_replay_state WHERE state_key=?", (GY_LIFECYCLE_WATERMARK_KEY,)).fetchone()
            if row is None:
                max_id = conn.execute("SELECT COALESCE(MAX(id),0) FROM sr_trade_history WHERE note='G.Y lifecycle state transition'").fetchone()[0]
                now_iso = now_damascus().isoformat()
                conn.execute("INSERT INTO sr_alert_replay_state(state_key,last_history_id,initialized_at,updated_at) VALUES(?,?,?,?)", (GY_LIFECYCLE_WATERMARK_KEY, int(max_id or 0), now_iso, now_iso))
                logger.info("GY_LIFECYCLE_WATERMARK_INITIALIZED | last_history_id=%s", int(max_id or 0))
            conn.commit()
        except Exception:
            conn.rollback()
            raise
...
_v1966_gy_prepare_db = _v1967_gy_prepare_db

_V1967_RUNTIME_BOOT_GRACE = max(1.0, float(os.getenv("RUNTIME_BOOT_GRACE_SECONDS", "5")))

async def _v1967_runtime_supervisor_loop():
    global RUNTIME_SUPERVISOR_TASK
    logger.info("RUNTIME_SUPERVISOR_STARTED_V19_67")
        ...
        while True:
            try:
                ...
                if app is None:
                    await asyncio.sleep(1.0)
                    continue
                app_running = getattr(app, "running", True)
                if app_running is False:
                    await asyncio.sleep(0.5)
                    continue
                in_boot_grace = (time.monotonic() - loop_started) < _V1967_RUNTIME_BOOT_GRACE
                await _ensure_alert_scheduler_task()
                main_alive = bool(globals().get("MAIN_AUTO_LOOP_RUNNING", False))
                if not main_alive and not in_boot_grace:
                    await _v1953_ensure_runtime_task(auto_loop, "main-trading-loop")
                gy_health = globals().get("SR_AUTO_HEALTH") or {}
                gy_alive = bool(gy_health.get("running"))
                if callable(globals().get("_v1935_gy_auto_loop")) and not gy_alive and not in_boot_grace:
                    await _v1953_ensure_runtime_task(_v1935_gy_auto_loop, "gy-private-trade-auto")
                if callable(globals().get("_start_account_notification_scheduler")):
                    try:
                        await _start_account_notification_scheduler()
                        ...
                        logger.exception("ACCOUNT_NOTIFICATION_SUPERVISOR_CHECK_FAILED_V19_67")
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("RUNTIME_SUPERVISOR_ERROR_V19_67")
            await asyncio.sleep(RUNTIME_SUPERVISOR_INTERVAL)
    finally:
        if RUNTIME_SUPERVISOR_TASK is asyncio.current_task():
            RUNTIME_SUPERVISOR_TASK = None
        logger.info("RUNTIME_SUPERVISOR_STOPPED_V19_67")

_v1953_runtime_supervisor_loop = _v1967_runtime_supervisor_loop

def run_v19_67_runtime_hardening_self_tests():
    import inspect
    checks = []
    def check(name, ok, detail=""):
        checks.append({"test": name, "status": "PASS" if bool(ok) else "FAIL", "detail": str(detail)})
    source = Path(__file__).read_text(encoding="utf-8")
    real_entrypoints = [line for line in source.splitlines() if line.strip() == 'if __name__ == "__main__":']
    check("single_physical_entrypoint", len(real_entrypoints) == 1, f"count={len(real_entrypoints)}")
    check("version", V19_67_VERSION == "v19.67_RUNTIME_PERFORMANCE_GY_AUDITOR_HARDENED")
    check("auditor_reconcile_constant", "AUDITOR_TRADE_RECONCILE_SECONDS = max" in source)
    check("webhook_application_running_guard", "getattr(APPLICATION, \"running\", None)" in source)
    check("gy_schema_lock", callable(_v1967_gy_prepare_db))
    check("main_liveness_guard", "MAIN_AUTO_LOOP_RUNNING" in _v1967_runtime_supervisor_loop.__code__.co_names)
    check("gy_liveness_guard", "SR_AUTO_HEALTH" in _v1967_runtime_supervisor_loop.__code__.co_names)
    check("strategy_reference_preserved", callable(_v1967_base_evaluate_signal))
    check("reply_markup_supported", "reply_markup" in inspect.signature(reply).parameters)
    return {"ok": all(x["status"] == "PASS" for x in checks), "version": V19_67_VERSION, "results": checks}


# ============================================================
# v19.68 — RUNTIME DEEP TELEMETRY ONLY
# Scope: diagnostics/observability only.
# No strategy, score, threshold, Entry/SL/TP, R:R, quota, subscription,
# persistence, alert semantics, or execution gate is changed by this layer.
# ============================================================
V19_68_VERSION = "v19.68_RUNTIME_DEEP_TELEMETRY_GY_INSTITUTIONAL"
VERSION = V19_68_VERSION

_V1968_GY_ACTIVE_CYCLE = ContextVar("v1968_gy_active_cycle", default=None)
_V1968_ACTIVE_INSTITUTIONAL_CYCLE = None
_V1968_INSTITUTIONAL_SEQ = 0
_V1968_GY_TELEMETRY_LOCK = threading.RLock()
_V1968_IMF_TLS = threading.local()


def _v1968_next_institutional_cycle():
    global _V1968_INSTITUTIONAL_SEQ
    with _V1968_GY_TELEMETRY_LOCK:
        _V1968_INSTITUTIONAL_SEQ += 1
        return f"I{int(time.time() * 1000):x}-{_V1968_INSTITUTIONAL_SEQ:04d}"


def _v1968_log_stage(event, cycle=None, **fields):
    payload = []
    if cycle:
        payload.append(f"cycle={cycle}")
    for key, value in fields.items():
        if value is None:
            continue
        text = str(value).replace("\n", " ").replace("\r", " ")
        payload.append(f"{key}={text}")
    logger.info("%s | %s", event, " ".join(payload))


def _v1968_gy_cycle_id():
    return _V1968_GY_ACTIVE_CYCLE.get()


# ---------- Institutional source wrappers ----------
_V1968_ORIG_MACRO_REAL = _macro_real
_V1968_ORIG_COT_REAL = _cot_real
_V1968_ORIG_GLD_REAL = _gld_real
_V1968_ORIG_CB_REAL = _cb_real
_V1968_ORIG_IMF_DISCOVER = _imf_discover_structure
_V1968_ORIG_IMF_MONTHLY = _imf_irfcl_monthly
_V1968_ORIG_IMF_PARSE_CSV = _imf_parse_csv
_V1968_ORIG_IMF_PARSE_JSON = _imf_parse_json
_V1968_ORIG_INST_HTTP = _inst_http
_V1968_ORIG_INST_ANALYSIS = _v1967_base_institutional_analysis


def _v1968_source_timed(name, func, *args, **kwargs):
    cycle = _V1968_ACTIVE_INSTITUTIONAL_CYCLE
    started = time.perf_counter()
    _v1968_log_stage(f"{name}_START", cycle, thread=threading.current_thread().name)
    try:
        ...
        _v1968_log_stage(
            f"{name}_END", cycle,
            status="ERROR", latency_ms=f"{(time.perf_counter() - started) * 1000.0:.1f}",
            error_type=type(exc).__name__, error=str(exc)[:500],
        )
        raise
    finally:
        # END is emitted only once on success; the error path above emits it already.
        if False:
            logger.info("%s", name)


def _v1968_macro_real():
    run = uuid.uuid4().hex[:10]
    started = time.perf_counter()
    _v1968_log_stage("MACRO_START", run=run, thread=threading.current_thread().name)
    try:
        value = _V1968_ORIG_MACRO_REAL()
        _v1968_log_stage("MACRO_END", run=run, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        return value
    except Exception as exc:
        _v1968_log_stage("MACRO_END", run=run, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise
...
def _v1968_cot_real():
    run = uuid.uuid4().hex[:10]
    started = time.perf_counter()
    _v1968_log_stage("COT_START", run=run, thread=threading.current_thread().name)
    try:
        value = _V1968_ORIG_COT_REAL()
        _v1968_log_stage("COT_END", run=run, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        return value
    except Exception as exc:
        _v1968_log_stage("COT_END", run=run, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise
...
def _v1968_gld_real():
    run = uuid.uuid4().hex[:10]
    started = time.perf_counter()
    _v1968_log_stage("GLD_START", run=run, thread=threading.current_thread().name)
    try:
        value = _V1968_ORIG_GLD_REAL()
        _v1968_log_stage("GLD_END", run=run, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        return value
    except Exception as exc:
        _v1968_log_stage("GLD_END", run=run, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise
...
def _v1968_cb_real():
    ...
    started = time.perf_counter()
    tl = _V1968_IMF_TLS
    cb_run = uuid.uuid4().hex[:10]
    tl.cb_run = cb_run
    tl.imf_request_seq = 0
    tl.wgc_open = bool(_inst_cache_get("cb") is None)
    tl.wgc_started = started if tl.wgc_open else None
    if tl.wgc_open:
        _v1968_log_stage("WGC_START", run=cb_run, thread=threading.current_thread().name)
    _v1968_log_stage("CB_START", run=cb_run, thread=threading.current_thread().name)
    try:
        value = _V1968_ORIG_CB_REAL()
        _v1968_log_stage("CB_END", run=cb_run, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        if tl.wgc_open:
            _v1968_log_stage("WGC_END", run=cb_run, status="OK", latency_ms=f"{(time.perf_counter()-tl.wgc_started)*1000.0:.1f}")
            tl.wgc_open = False
        return value
    except Exception as exc:
        _v1968_log_stage("CB_END", run=cb_run, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        if tl.wgc_open:
            _v1968_log_stage("WGC_END", run=cb_run, status="ERROR", latency_ms=f"{(time.perf_counter()-tl.wgc_started)*1000.0:.1f}", error_type=type(exc).__name__)
            tl.wgc_open = False
        raise
    finally:
        tl.cb_run = None
...
def _v1968_imf_discover_structure():
    run = getattr(_V1968_IMF_TLS, "cb_run", None)
    _V1968_IMF_TLS.imf_request_seq = 0
    if run and getattr(_V1968_IMF_TLS, "wgc_open", False):
        _v1968_log_stage("WGC_END", run=run, status="OK", latency_ms=f"{(time.perf_counter()-float(_V1968_IMF_TLS.wgc_started))*1000.0:.1f}")
        _V1968_IMF_TLS.wgc_open = False
    started = time.perf_counter()
    _v1968_log_stage("IMF_DISCOVERY_START", run=run)
    try:
        value = _V1968_ORIG_IMF_DISCOVER()
        _v1968_log_stage("IMF_DISCOVERY_END", run=run, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        return value
    except Exception as exc:
        _v1968_log_stage("IMF_DISCOVERY_END", run=run, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise


def _v1968_imf_irfcl_monthly(indicator):
    run = getattr(_V1968_IMF_TLS, "cb_run", None)
    started = time.perf_counter()
    _v1968_log_stage("IMF_DATA_START", run=run, indicator=indicator)
    try:
        value = _V1968_ORIG_IMF_MONTHLY(indicator)
        _v1968_log_stage("IMF_DATA_END", run=run, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", countries=len(value) if isinstance(value, dict) else None)
        return value
    except Exception as exc:
        _v1968_log_stage("IMF_DATA_END", run=run, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise


def _v1968_imf_parse_wrapper(parser_name, func, *args, **kwargs):
    run = getattr(_V1968_IMF_TLS, "cb_run", None)
    started = time.perf_counter()
    _v1968_log_stage("IMF_PARSE_START", run=run, parser=parser_name)
    try:
        value = func(*args, **kwargs)
        _v1968_log_stage("IMF_PARSE", run=run, parser=parser_name, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", countries=len(value) if isinstance(value, dict) else None)
        return value
    except Exception as exc:
        _v1968_log_stage("IMF_PARSE", run=run, parser=parser_name, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise


def _v1968_imf_parse_csv(text):
    return _v1968_imf_parse_wrapper("CSV", _V1968_ORIG_IMF_PARSE_CSV, text)


def _v1968_imf_parse_json(payload):
    return _v1968_imf_parse_wrapper("JSON", _V1968_ORIG_IMF_PARSE_JSON, payload)


def _v1968_inst_http(url, params=None, timeout=None, headers=None):
    run = getattr(_V1968_IMF_TLS, "cb_run", None)
    is_imf = bool(run and "imf.org" in str(url).lower())
    if not is_imf:
        return _V1968_ORIG_INST_HTTP(url, params=params, timeout=timeout, headers=headers)
    with _V1968_GY_TELEMETRY_LOCK:
        n = int(getattr(_V1968_IMF_TLS, "request_seq", 0) or 0) + 1
        _V1968_IMF_TLS.request_seq = n
    started = time.perf_counter()
    _v1968_log_stage("IMF_REQUEST_START", run=run, request=f"#{n}", timeout=timeout, url=url)
    try:
        response = _V1968_ORIG_INST_HTTP(url, params=params, timeout=timeout, headers=headers)
        _v1968_log_stage(
            "IMF_REQUEST_END", run=run, request=f"#{n}", status="OK",
            latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}",
        ...
        _v1968_log_stage(
            "IMF_REQUEST_END", run=run, request=f"#{n}", status="ERROR",
            latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}",
            error_type=type(exc).__name__, error=str(exc)[:500],
        )
        raise
...
# Rebind only diagnostic wrappers; underlying behavior remains the original code.
_macro_real = _v1968_macro_real
_cot_real = _v1968_cot_real
_gld_real = _v1968_gld_real
_cb_real = _v1968_cb_real
_imf_discover_structure = _v1968_imf_discover_structure
_imf_irfcl_monthly = _v1968_imf_irfcl_monthly
_imf_parse_csv = _v1968_imf_parse_csv
_imf_parse_json = _v1968_imf_parse_json
_inst_http = _v1968_inst_http

_V1968_ORIG_V1967_INSTITUTIONAL = _v1967_institutional_analysis


def _v1968_institutional_analysis(df):
    global _V1968_ACTIVE_INSTITUTIONAL_CYCLE
    cycle = _v1968_next_institutional_cycle()
    started = time.perf_counter()
    _v1968_log_stage("INSTITUTIONAL_START", cycle, thread=threading.current_thread().name)
    _V1968_ACTIVE_INSTITUTIONAL_CYCLE = cycle
    try:
        value = _V1968_ORIG_V1967_INSTITUTIONAL(df)
        _v1968_log_stage("INSTITUTIONAL_END", cycle, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        return value
    except Exception as exc:
        _v1968_log_stage("INSTITUTIONAL_END", cycle, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise
    finally:
        _V1968_ACTIVE_INSTITUTIONAL_CYCLE = None

# v19.67 already owns the snapshot/single-flight boundary. This wrapper is
# telemetry-only and delegates to that exact implementation.
institutional_analysis = _v1968_institutional_analysis


# ---------- G.Y telemetry wrappers ----------
_V1968_ORIG_LIVE_PRICE = live_price
_V1968_ORIG_GET_BARS = get_bars
_V1968_ORIG_SR_ANALYZE = sr_analyze
_V1968_ORIG_CREATE_GY = _v1935_create_auto_gy_trade


def _v1968_live_price():
    cycle = _v1968_gy_cycle_id()
    if not cycle:
        return _V1968_ORIG_LIVE_PRICE()
    started = time.perf_counter()
    _v1968_log_stage("GY_LIVE_PRICE_START", cycle)
    try:
        value = _V1968_ORIG_LIVE_PRICE()
        _v1968_log_stage("GY_LIVE_PRICE", cycle, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", price=(value or {}).get("price") if isinstance(value, dict) else None, source=(value or {}).get("source") if isinstance(value, dict) else None)
        return value
    except Exception as exc:
        _v1968_log_stage("GY_LIVE_PRICE", cycle, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise


def _v1968_get_bars(interval, limit=300):
    cycle = _v1968_gy_cycle_id()
    if not cycle:
        return _V1968_ORIG_GET_BARS(interval, limit)
    started = time.perf_counter()
    is_m15 = str(interval).lower() in {"15m", "m15"}
    if is_m15:
        _v1968_log_stage("GY_M15_FETCH_START", cycle, interval=interval, limit=limit)
    try:
        value = _V1968_ORIG_GET_BARS(interval, limit)
        if is_m15:
            latest = None
            state = None
            try:
                if value is not None and len(value):
                    col = "openTime" if "openTime" in value.columns else None
                    if col:
                        latest = pd.to_datetime(value.iloc[-1][col], utc=True, errors="coerce")
                    if latest is not None and not pd.isna(latest):
                        floor = pd.Timestamp.now(tz="UTC").floor("15min")
                        state = "FORMING" if latest >= floor else "CLOSED"
            except Exception:
                pass
            _v1968_log_stage("GY_M15_FETCH", cycle, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", rows=len(value) if value is not None else 0)
            _v1968_log_stage("GY_LATEST_CANDLE", cycle, candle=latest.isoformat() if latest is not None and not pd.isna(latest) else None, state=state, rows=len(value) if value is not None else 0)
        return value
    except Exception as exc:
        if is_m15:
            _v1968_log_stage("GY_M15_FETCH", cycle, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise


def _v1968_sr_analyze():
    cycle = _v1968_gy_cycle_id()
    if not cycle:
        return _V1968_ORIG_SR_ANALYZE()
    started = time.perf_counter()
    _v1968_log_stage("GY_S/R_START", cycle)
    try:
        result = _V1968_ORIG_SR_ANALYZE()
        sr = result.get("sr") if isinstance(result, dict) else None
        candidate = result.get("candidate") if isinstance(result, dict) else None
        signal = result.get("signal") if isinstance(result, dict) else None
        if isinstance(sr, dict):
            best = sr.get("best")
            _v1968_log_stage("GY_S/R", cycle, status="OK", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", best=best, signal=bool(signal), candidate=bool(candidate))
        else:
            _v1968_log_stage("GY_S/R", cycle, status="NO_DATA", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", signal=bool(signal), candidate=bool(candidate))

        if isinstance(candidate, dict):
            direction = str(candidate.get("direction") or "").upper()
            if direction == "BUY":
                _v1968_log_stage("GY_CANDIDATE_BUY", cycle, trigger=candidate.get("trigger"), score=candidate.get("score"))
            elif direction == "SELL":
                _v1968_log_stage("GY_CANDIDATE_SELL", cycle, trigger=candidate.get("trigger"), score=candidate.get("score"))

        pcs = result.get("pcs") if isinstance(result, dict) else None
        if isinstance(pcs, dict):
            _v1968_log_stage("GY_PCS", cycle, approved=pcs.get("approved"), score=pcs.get("score"), state=pcs.get("state"), reason=pcs.get("reason"))

        if not signal:
            execution_state = result.get("execution_state") if isinstance(result, dict) else None
            reason = result.get("reason") if isinstance(result, dict) else None
            if isinstance(pcs, dict) and pcs.get("approved") is False:
                _v1968_log_stage("GY_BLOCKED", cycle, stage="PCS", execution_state=execution_state, reason=reason or pcs.get("reason"))
            elif execution_state and "BLOCKED" in str(execution_state).upper():
                _v1968_log_stage("GY_BLOCKED", cycle, stage=execution_state, reason=reason)
            else:
                _v1968_log_stage("GY_NO_SIGNAL", cycle, execution_state=execution_state, reason=reason)
        return result
    except Exception as exc:
        _v1968_log_stage("GY_S/R", cycle, status="ERROR", latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}", error_type=type(exc).__name__, error=str(exc)[:500])
        raise


def _v1968_create_auto_gy_trade(trade, snapshot):
    cycle = _v1968_gy_cycle_id()
    if not cycle:
        return _V1968_ORIG_CREATE_GY(trade, snapshot)
    started = time.perf_counter()
    try:
        result = _V1968_ORIG_CREATE_GY(trade, snapshot)
        setup_id = result[0] if isinstance(result, tuple) and len(result) > 0 else None
        is_new = bool(result[1]) if isinstance(result, tuple) and len(result) > 1 else False
        reason = result[2] if isinstance(result, tuple) and len(result) > 2 else None
        if is_new:
            _v1968_log_stage("GY_CREATED", cycle, setup_id=setup_id, direction=(trade or {}).get("direction"), latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        else:
            _v1968_log_stage("GY_BLOCKED", cycle, stage="PERSISTENCE", reason=reason, latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        return result
    except Exception as exc:
        _v1968_log_stage("GY_BLOCKED", cycle, stage="PERSISTENCE_ERROR", error_type=type(exc).__name__, error=str(exc)[:500], latency_ms=f"{(time.perf_counter()-started)*1000.0:.1f}")
        raise


live_price = _v1968_live_price
get_bars = _v1968_get_bars
sr_analyze = _v1968_sr_analyze
_v1935_create_auto_gy_trade = _v1968_create_auto_gy_trade


def run_v19_68_runtime_telemetry_self_tests():
    """Static/runtime contract test for the telemetry layer; no market I/O required."""
    checks = []
    def check(name, ok, detail=""):
        checks.append({"test": name, "status": "PASS" if bool(ok) else "FAIL", "detail": str(detail)})
    ...
    source = Path(__file__).read_text(encoding="utf-8")
    required = [
        "INSTITUTIONAL_START", "MACRO_START", "MACRO_END", "COT_START", "COT_END",
        "GLD_START", "GLD_END", "CB_START", "CB_END", "WGC_START", "WGC_END",
        "IMF_DISCOVERY_START", "IMF_DISCOVERY_END", "IMF_REQUEST_START", "IMF_REQUEST_END",
        "IMF_PARSE_START", "IMF_PARSE", "INSTITUTIONAL_END", "GY_CYCLE_START",
        "GY_LIVE_PRICE", "GY_M15_FETCH", "GY_LATEST_CANDLE", "GY_S/R",
        "GY_CANDIDATE_BUY", "GY_CANDIDATE_SELL", "GY_PCS", "GY_NO_SIGNAL",
        "GY_BLOCKED", "GY_CREATED", "GY_CYCLE_END",
    ]
    for marker in required:
        check(f"telemetry_marker:{marker}", marker in source)

    check("version", V19_68_VERSION == "v19.68_RUNTIME_DEEP_TELEMETRY_GY_INSTITUTIONAL")
    check("institutional_original_preserved", callable(_V1968_ORIG_V1967_INSTITUTIONAL))
    check("strategy_reference_preserved", callable(_v1967_base_evaluate_signal))
    check("gy_original_preserved", callable(_V1968_ORIG_SR_ANALYZE) and callable(_V1968_ORIG_CREATE_GY))
    check("main_evaluate_not_replaced_by_strategy_copy", _V1968_ORIG_V1967_INSTITUTIONAL is _v1967_institutional_analysis)
    check("single_entrypoint", sum(1 for line in source.splitlines() if line.strip() == 'if __name__ == "__main__":') == 1)
    check("no_strategy_threshold_text_added", "MIN_TRADE_SCORE = 50" in source)
    return {"ok": all(x["status"] == "PASS" for x in checks), "version": V19_68_VERSION, "results": checks}


if __name__ == "__main__":
    if "--v19-68-runtime-telemetry-self-test" in sys.argv:
        print(json.dumps(run_v19_68_runtime_telemetry_self_tests(), ensure_ascii=False, indent=2))
    elif "--v19-67-runtime-self-test" in sys.argv:
        print(json.dumps(run_v19_67_runtime_hardening_self_tests(), ensure_ascii=False, indent=2))
    elif "--v19-66-alert-self-test" in sys.argv:
        print(json.dumps(run_v19_66_alert_contract_self_tests(), ensure_ascii=False, indent=2))
    elif "--v19-58-trade-recovery-self-test" in sys.argv:
        print(json.dumps(run_v19_58_trade_recovery_self_tests(), ensure_ascii=False, indent=2))
    elif "--v19-53-runtime-self-test" in sys.argv:
        print(json.dumps(run_v19_53_runtime_hardening_self_tests(), ensure_ascii=False, indent=2))
    elif "--payment-db-self-test" in sys.argv:
        print(json.dumps(_payment_database_contract_self_test(), ensure_ascii=False, indent=2))
    elif "--payment-ui-self-test" in sys.argv:
        print(json.dumps(run_v19_47_payment_ui_self_tests(), ensure_ascii=False, indent=2))
    else:
        main_v19_13()
