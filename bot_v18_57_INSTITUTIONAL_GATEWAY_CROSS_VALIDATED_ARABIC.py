...
VERSION = "v18.57-INSTITUTIONAL-GATEWAY-CROSS-VALIDATED-SPDR-IMF-WGC-FEDFUNDS-ARABIC-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
def _cb_real():
    ...
    if wgc is not None and imf is not None:
        ...
        cv={'status':'PASS' if pct<=5.0 else 'DEGRADED','method':'مقارنة WGC مع IMF IRFCL','difference_tonnes':round(diff,4),'difference_pct':round(pct,2),'validated':pct<=5.0,'wgc_value':a,'imf_value':b,'tolerance_pct':5.0}
...
def run_institutional_self_tests():
    ...
    check('version', VERSION.startswith('v18.57-'), VERSION)
