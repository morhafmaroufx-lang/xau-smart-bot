...
async def _payment_duration_message_direct_handler(update, context):
    plan_code = str(getattr(context, "user_data", {}).get("payment_plan_code") or "").upper()
    if plan_code not in {"BASIC", "ELITE"}:
        return
    text = (getattr(getattr(update, "effective_message", None), "text", "") or "").strip()
    normalized = _payment_normalize_digits(text)
    if not normalized.isdigit():
        return
    await _payment_create_order_from_duration_message(update, context, int(normalized))
    raise ApplicationHandlerStop

VERSION = "v19.51_PAYMENT_DURATION_DIRECT_HANDLER_FIX"
