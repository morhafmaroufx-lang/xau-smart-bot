...
# Telegram transport. Polling is the reliable default for this single-process
# Render deployment; it removes the Flask↔asyncio bridge from the critical
# command path. Webhook remains available only when explicitly requested.
BOT_TRANSPORT = os.environ.get("BOT_TRANSPORT", "polling").strip().lower()
if BOT_TRANSPORT not in {"polling", "webhook"}:
...
async def shutdown_bot():
    ...
    global APPLICATION, AUTO_TASK, TELEGRAM_WATCHDOG_TASK
    SHUTDOWN_EVENT.set()
    if TELEGRAM_WATCHDOG_TASK is not None:
        TELEGRAM_WATCHDOG_TASK.cancel()
        try:
            await TELEGRAM_WATCHDOG_TASK
    ...
    if APPLICATION is not None:
        try:
            ...
            logger.exception("TELEGRAM_UPDATER_STOP_ERROR")
        try:
...
def _polling_error_callback(exc):
    """Never let a Telegram polling exception silently stop update intake."""
    try:
        logger.error("TELEGRAM_POLLING_ERROR: %s", exc, exc_info=exc if isinstance(exc, BaseException) else None)
    except Exception:
        logging.getLogger(__name__).exception("TELEGRAM_POLLING_ERROR_LOGGING_FAILED")


async def _telegram_polling_watchdog():
    """Keep Telegram intake alive if the updater task exits unexpectedly."""
    while not SHUTDOWN_EVENT.is_set():
        try:
            await asyncio.sleep(5)
            if SHUTDOWN_EVENT.is_set() or APPLICATION is None or BOT_TRANSPORT != "polling":
                continue
            updater = getattr(APPLICATION, "updater", None)
            if updater is None:
                continue
            if not updater.running:
                logger.error("TELEGRAM_POLLING_DOWN; attempting controlled restart")
                ...
                logger.info("TELEGRAM_POLLING_RESTARTED")
        except asyncio.CancelledError:
            raise
        except Exception:
            logger.exception("TELEGRAM_POLLING_WATCHDOG_ERROR")
...
async def start_bot():
    ...
    # Process Telegram updates concurrently. The previous single-update execution
    # model allowed one slow analysis/UI callback to hold every later command in
    # the update queue. That exactly matches the observed symptom: old commands
    # arrive in a burst, then the bot appears silent while one handler is busy.
    # Handlers that perform blocking work already use asyncio.to_thread().
    APPLICATION = (
        Application.builder()
        .token(TOKEN)
    ...
    if BOT_TRANSPORT == "polling":
        # Remove any previous webhook before polling. Otherwise Telegram can
        # return 409 Conflict and no commands will reach the handlers.
        ...
        logger.info("TELEGRAM_TRANSPORT=polling READY")
    else:
        webhook_kwargs = {
            "url": WEBHOOK_URL,
            "allowed_updates": allowed_updates,
    ...
    logger.info("Telegram transport: %s", BOT_TRANSPORT)
    TELEGRAM_WATCHDOG_TASK = asyncio.create_task(_telegram_polling_watchdog(), name="telegram-polling-watchdog")
