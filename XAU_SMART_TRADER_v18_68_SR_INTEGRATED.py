...
def _sr_execution_decision(levels, price, direction, atr):
    """محرك S/R التنفيذي المتوافق مع عقد v18_68. لا يغيّر مستويات Baseline."""
    result={"state":"UNAVAILABLE","score":0,"ready":False,"direction":direction if direction in ("BUY","SELL") else "WAIT","side":None,"distance_atr":None,"strength":0,"touches":0,"level_price":None,"opposite_distance_atr":None,"reasons":[]}
    try:
        p=float(price); a=float(atr or 0)
        if not math.isfinite(p) or p<=0 or not math.isfinite(a) or a<=0:
            result["reasons"].append("بيانات S/R أو ATR غير صالحة"); return result
        if direction not in ("BUY","SELL"):
            result["state"]="WAIT"; result["reasons"].append("لا يوجد اتجاه تنفيذي صالح"); return result
        fav="support" if direction=="BUY" else "resistance"; opp="resistance" if direction=="BUY" else "support"
        favorable=[]; opposite=[]
        for i in (1,2,3):
            fz=levels.get(f"{fav}{i}") if isinstance(levels,dict) else None; oz=levels.get(f"{opp}{i}") if isinstance(levels,dict) else None
            fp=_zone_price(fz); op=_zone_price(oz)
            if fp is not None and ((direction=="BUY" and fp<p) or (direction=="SELL" and fp>p)): favorable.append((abs(p-fp),fp,fz))
            if op is not None and ((direction=="BUY" and op>p) or (direction=="SELL" and op<p)): opposite.append((abs(p-op),op,oz))
        if not favorable:
            result["reasons"].append("لا يوجد مستوى S/R ملائم حسب الاتجاه"); return result
        favorable.sort(key=lambda x:x[0]); dist,lp,zone=favorable[0]; datr=dist/a
        strength=0; touches=0
        if isinstance(zone,dict):
            try: strength=int(round(float(zone.get("strength",0) or 0)))
            except (TypeError,ValueError): pass
            try: touches=int(zone.get("touches",0) or 0)
            except (TypeError,ValueError): pass
        score=10 if datr<=0.35 else 7 if datr<=0.75 else 4 if datr<=1.20 else 0
        if strength>=80: score+=4
        elif strength>=60: score+=3
        elif strength>=40: score+=2
        if touches>=4: score+=3
        elif touches>=3: score+=2
        elif touches>=2: score+=1
        odatr=None
        if opposite:
            opposite.sort(key=lambda x:x[0]); odatr=opposite[0][0]/a
            if odatr<=0.50: score-=8; result["reasons"].append("المستوى المقابل قريب جداً من السعر")
            elif odatr<=0.80: score-=5; result["reasons"].append("المستوى المقابل قريب ويضغط على مساحة الحركة")
            elif odatr<=1.20: score-=3; result["reasons"].append("المستوى المقابل يقلل مساحة الحركة")
        score=int(max(0,min(20,score))); ready=datr<=1.20 and score>=8
        result.update({"state":"READY" if ready else "TOO_FAR" if datr>1.20 else "WEAK","score":score,"ready":ready,"side":fav,"distance_atr":round(datr,3),"strength":strength,"touches":touches,"level_price":round(lp,2),"opposite_distance_atr":round(odatr,3) if odatr is not None else None})
        side_ar="شراء" if direction=="BUY" else "بيع"
        result["reasons"].append(f"S/R داعم لل{side_ar}: {lp:.2f}" if ready else "S/R غير مؤهل كبوابة تنفيذ")
        return result
    except Exception as exc:
        result["state"]="ERROR"; result["reasons"].append(f"خطأ في محرك S/R: {exc}"); return result
...
def _daily_execution_core():
    ...
    sr_decision = _sr_execution_decision(levels, price, direction, float(m15.get("atr", 0) or 0))
    setup_reasons.extend(sr_decision.get("reasons", []))
    ...
    ready = bool(direction in ("BUY","SELL") and setup_score >= MIN_TRADE_SCORE and alignment and m15_adx >= 15 and not liquidity_blocked and not news_blocked and rr_ok and sr_decision.get("ready"))
    ...
    return {"signal":ready,"execution_state":state,"direction":direction,"score":setup_score,"scenario_score":int(scenario_score),"setup_score":setup_score,"quality":quality,"quality_icon":quality_icon,"price":price,"levels":levels,"sr_decision":sr_decision,"liquidity_blocked":liquidity_blocked,"liquidity_state":liquidity_state,"liquidity":liquidity,"factors":factors,"setup_reasons":setup_reasons,"trade":candidate if ready else None,"news_blocked":news_blocked,"news":news_text,"daily":{"h1":h1,"m15":m15,"m5":m5,"buy_score":buy_score,"sell_score":sell_score,"buy_factors":buy_factors,"sell_factors":sell_factors}}
...
def build_analysis(include_trade=True):
    ...
    _sr=result.get('sr_decision') or {}
    L=[f'🤖 XAU SMART TRADER {VERSION}','📊 التحليل الهيكلي والكمّي والمؤسسي','','🎯 القرار',direction,f"🧠 قوة السيناريو: {result.get('scenario_score',result['score'])}/100",f"🧩 الدرجة الفنية: {result.get('technical_score',result['score'])}/100",f"🏦 تأثير المؤسسي: {result.get('institutional_adjustment',0):+d} نقطة",f"🎯 الإعداد التنفيذي: {result.get('setup_score',result['score'])}/100",f"📍 بوابة S/R: {_sr.get('score',0)}/20 — {_sr.get('state','UNAVAILABLE')}",f"🚦 حالة التنفيذ: {result.get('execution_state','WATCH')}",'','🔎 سبب القرار']
