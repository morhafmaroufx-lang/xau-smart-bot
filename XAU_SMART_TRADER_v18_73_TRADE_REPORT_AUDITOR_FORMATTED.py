...
async def audit_report(update, context):
    """لوحة مراقب الصفقات — للإدارة فقط، قراءة فقط."""
    ...
    if not (is_admin_chat(user_id) or is_admin_chat(update.effective_chat.id)):
        ...
        return
    if PERFORMANCE_AUDITOR is None:
        await reply(update, "🛡️ مراقب الصفقات\n\n🔴 المحرك غير متاح حالياً.")
        return
    try:
        ...
        win = float(t.get("win_rate_pct", 0) or 0)
        state = "🟢 أداء جيد" if total and win >= 60 else ("🟡 أداء متوسط" if total and win >= 40 else ("🔴 يحتاج مراجعة" if total else "⚪ لا توجد صفقات"))
        lines = [
            "🛡️ XAU SMART TRADER",
            "📈 مراقب أداء الصفقات",
            "━━━━━━━━━━━━━━━━━━━━",
            ...
            f"المفتوحة       │ {t.get('open', 0)}",
            f"المغلقة        │ {t.get('closed', 0)}",
            "",
            "🎯 نتائج الصفقات",
            f"نجاح كامل      │ {t.get('full_success', 0)} ✅",
            f"نجاح جزئي      │ {t.get('partial_success', 0)} 🟡",
            f"فشل            │ {t.get('failed', 0)} ❌",
            f"منتهية         │ {t.get('expired', 0)} ⏱️",
            "",
            "📈 مؤشرات الأداء",
            f"Win Rate       │ {t.get('win_rate_pct', 0):.2f}%",
            f"Full Target    │ {t.get('full_target_rate_pct', 0):.2f}%",
            f"متوسط الجودة   │ {t.get('avg_score', 0):.2f} نقطة",
            ...
            f"Win Rate       │ {sr.get('win_rate_pct', 0):.2f}%",
            f"متوسط الجودة   │ {sr.get('avg_score', 0):.2f} نقطة",
            f"متوسط R:R      │ {sr.get('avg_rr', 0):.2f}",
            "━━━━━━━━━━━━━━━━━━━━",
            "📋 التقارير",
            "اختر التقرير المطلوب من الأزرار أدناه.",
            "",
            "🔒 مراقب الصفقات والتقارير فقط"
        ]
        buttons = [
            [InlineKeyboardButton("📅 التقرير اليومي", callback_data="audit_daily"), InlineKeyboardButton("📆 التقرير الأسبوعي", callback_data="audit_weekly")],
            [InlineKeyboardButton("🔄 تحديث", callback_data="menu_audit"), InlineKeyboardButton("🏠 الرئيسية", callback_data="nav_home")],
        ]
