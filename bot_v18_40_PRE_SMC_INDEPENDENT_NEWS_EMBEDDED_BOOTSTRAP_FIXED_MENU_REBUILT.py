...
VERSION = "v18.40-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIX-MENU"
...
async def liquidity_menu(update, context):
    ...
    try:
        ...
        def lines(items, icon, prefix):
            out=[]
            for i,item in enumerate(items[:3],1):
                if isinstance(item,dict):
                    price=item.get("price",item.get("level")); extra=[]
                    if item.get("strength") is not None: extra.append(f"قوة {item.get('strength')}")
                    if item.get("touches") is not None: extra.append(f"{item.get('touches')} لمسات")
                    out.append(f"{icon} {prefix}{i}: {fmt(price)}"+(" | "+" | ".join(extra) if extra else ""))
                else: out.append(f"{icon} {prefix}{i}: {fmt(item)}")
            return out or ["⚪ لا توجد مستويات متاحة حالياً"]
        body=("💧 قراءة السيولة\n"
              ...
              "📍 الدعم والمقاومة\n"+"\n".join(lines(supports,"🟢","S"))+"\n"+"\n".join(lines(resistances,"🔴","R")))
