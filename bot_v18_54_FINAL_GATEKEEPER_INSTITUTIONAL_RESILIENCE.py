...
VERSION = "v18.54-FINAL-GATEKEEPER-INSTITUTIONAL-RESILIENCE-GATEWAY-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
WGC_CENTRAL_BANK_CHANGES_URL = ""  # discovered dynamically from the official WGC page; no monthly hard-code
...
def _read_xlsx_without_openpyxl(content):
    ...
    with zipfile.ZipFile(io.BytesIO(content)) as z:
        ...
        for name,target in sheets:
            ...
            if rows:
                header=rows[0]; width=max(map(len,rows)); header=[str(x).strip() for x in header]
                data=[r+[None]*(width-len(r)) for r in rows[1:]]
                if any(header): tables.append((name,pd.DataFrame(data,columns=header)))
...
def _nyfed_rates():
    """Official New York Fed EFFR page: extracts latest EFFR and target range without API credentials."""
    r=_inst_http('https://www.newyorkfed.org/markets/reference-rates/effr',timeout=20)
    tables=pd.read_html(io.StringIO(r.text))
    for d in tables:
        cols={str(c).strip().lower():c for c in d.columns}
        dc=next((c for k,c in cols.items() if k=='date' or k.startswith('date ')),None)
        rc=next((c for k,c in cols.items() if k.startswith('rate')),None)
        tc=next((c for k,c in cols.items() if 'target rate' in k),None)
        if dc is None or rc is None: continue
        x=d.copy(); x['_date']=pd.to_datetime(x[dc],errors='coerce'); x['_effr']=pd.to_numeric(x[rc],errors='coerce'); x=x.dropna(subset=['_date','_effr']).sort_values('_date')
        if x.empty: continue
        a=x.iloc[-1]; lo=hi=None
        if tc is not None:
            m=re.search(r'([0-9.]+)\s*-\s*([0-9.]+)',str(a[tc]))
            if m: lo,hi=float(m.group(1)),float(m.group(2))
        return float(a['_effr']),a['_date'],lo,hi
    raise RuntimeError('NY Fed EFFR table/schema unavailable')
...
def _gld_real():
    ...
    for href in hrefs:
        low=href.lower()
        if '.xlsx' in low and ('historical' in low or 'archive' in low): urls.append(requests.compat.urljoin(page_r.url,href))
    if SPDR_GLD_HISTORY_URL: urls.append(SPDR_GLD_HISTORY_URL)
    last_exc=None
    for url in dict.fromkeys(urls):
        try:
            r=_inst_http(url,timeout=25)
            if not r.content.startswith(b'PK\x03\x04'): raise RuntimeError('GLD response is not an XLSX archive')
            try:
                xl=pd.ExcelFile(io.BytesIO(r.content)); sheets=[(sh,pd.read_excel(xl,sheet_name=sh)) for sh in xl.sheet_names]
            except Exception as exc:
                if 'openpyxl' not in str(exc).lower(): raise
                sheets=_read_xlsx_without_openpyxl(r.content)
            best=None
            for sh,d in sheets:
                cols={str(x).strip().lower():x for x in d.columns}
                dc=next((cols[k] for k in cols if 'date' in k or 'as of' in k),None)
                oc=next((cols[k] for k in cols if any(t in k for t in ('gold holdings','gold ounces','ounces of gold','gold oz','ounces')) and 'share' not in k),None)
                sc=next((cols[k] for k in cols if any(t in k for t in ('shares outstanding','total shares'))),None)
                if dc is not None and oc is not None:
                    x=d.copy(); x['_date']=pd.to_datetime(x[dc],errors='coerce'); x=x.dropna(subset=['_date']).sort_values('_date')
                    if len(x)>1: best=(x,oc,sc); break
            if best is None: raise RuntimeError('SPDR GLD XLSX schema unavailable')
            d,oc,sc=best; a,b=d.iloc[-1],d.iloc[-2]
            ao=pd.to_numeric(a[oc],errors='coerce'); bo=pd.to_numeric(b[oc],errors='coerce')
            ...
            out={'source':'SPDR Gold Shares official historical archive','date':a['_date'].strftime('%Y-%m-%d'),'gold_oz':float(ao),'gold_oz_delta':float(ao-bo) if pd.notna(bo) else None,'shares_delta':float(ass-bss) if pd.notna(ass) and pd.notna(bss) else None,'shares_direction':shares_dir,'flow_proxy':direction,'source_url':url}
            out.update(_inst_status(a['_date'],5,'SPDR GLD'))
            return _inst_cache_put('gld',out)
        except Exception as exc: last_exc=exc
    raise RuntimeError(f'SPDR GLD data unavailable: {last_exc}')
...
def _cb_real():
    ...
    try:
        page_r=_inst_http('https://www.gold.org/goldhub/data/gold-reserves-by-country',timeout=20)
        page=page_r.text
        hrefs=re.findall(r'(?:href|data-href)=[\"\']([^\"\']+)',page,flags=re.I)
        found=[]
        for href in hrefs:
            if 'Changes' in href and '.xlsx' in href.lower():
                found.append(requests.compat.urljoin(page_r.url,href))
        urls=(found+urls) if found else urls
    except Exception:
        pass
    last_exc=None
    for url in dict.fromkeys(urls):
        try:
            r=_inst_http(url,timeout=25)
            try:
                dset=pd.ExcelFile(io.BytesIO(r.content)); tables=[(sh,pd.read_excel(dset,sheet_name=sh)) for sh in dset.sheet_names]
            except Exception as exc:
                if 'openpyxl' not in str(exc).lower(): raise
                tables=_read_xlsx_without_openpyxl(r.content)
            for sh,d in tables:
                if d.empty:continue
                cols=list(d.columns)
...
def _macro_real():
    ...
    if c is not None:return c
    errors=[]
    # Independent official sources: FRED is optional fallback, never a single failure domain.
    f=pd.DataFrame(columns=['DATE']); lo=hi=effr=vix=broad_usd=None; lo_date=hi_date=effr_date=vix_date=broad_usd_date=None
    try:
        effr,effr_date,lo,hi=_nyfed_rates(); lo_date=hi_date=effr_date
    except Exception as exc: errors.append('NY Fed rates: '+str(exc))
    try:
    ...
    if lo is not None and hi is not None:
        # Current target range itself plus change from previous observations.
        x=f[['DATE','DFEDTARL','DFEDTARU']].dropna() if 'DFEDTARL' in f.columns and 'DFEDTARU' in f.columns else pd.DataFrame()
        if len(x)>=2:
            delta=((float(x.iloc[-1].DFEDTARL)+float(x.iloc[-1].DFEDTARU))-(float(x.iloc[-2].DFEDTARL)+float(x.iloc[-2].DFEDTARU)))/2
            pol='HOLD' if abs(delta)<1e-9 else ('HAWKISH' if delta>0 else 'DOVISH')
        else: pol='HOLD'
    # DXY is not substituted with a third-party proxy. Exact ICE DXY requires licensed access.
    ...
    exp='غير متاحة'; fedwatch_status={'status':'UNAVAILABLE','fresh':False,'age_days':None,'asof':None,'source':'CME FedWatch API'}
        ...
        try:
            ...
            if isinstance(probs,list) and probs:
                ...
                for item in probs:
                    ...
                    label=str(item.get('label') or item.get('outcome') or '').lower()
                    prob=item.get('probability') or item.get('prob')
                    try: vals.append((label,float(prob)))
                    except Exception: pass
                if vals:
                    best=max(vals,key=lambda x:x[1]); exp=best[0] or 'متاحة من CME FedWatch'
            asof=pd.to_datetime(payload.get('asOf') or payload.get('timestamp') or p.get('asOf') if isinstance(p,dict) else None,errors='coerce')
            if not pd.isna(asof): fedwatch_status=_inst_status(asof,1,'CME FedWatch API')
            else: fedwatch_status={'status':'FRESH','fresh':True,'age_days':0,'asof':None,'source':'CME FedWatch API'}
    ...
    out={'source':'Official NY Fed + Federal Reserve H.10 + U.S. Treasury' ,'policy':pol,'fed_lower':lo,'fed_upper':hi,'effr':effr,
         'rate_expectation':exp,'rate_expectation_source':'CME FedWatch API' if exp!='غير متاحة' else 'غير متاح',
    ...
    out['series_status']={
        'fed_lower':_inst_status(lo_date,3,'FRED DFEDTARL'), 'fed_upper':_inst_status(hi_date,3,'FRED DFEDTARU'),
        'effr':_inst_status(effr_date,3,'New York Fed EFFR'), 'us10y':_inst_status(us10y_date,3,'U.S. Treasury 10Y'),
        'real10y':_inst_status(real10y_date,3,'U.S. Treasury Real 10Y'), 'vix':_inst_status(vix_date,3,'FRED VIXCLS'),
        'usd_broad':_inst_status(broad_usd_date,3,'Federal Reserve H.10 Broad USD')}
    out['dxy_status']=_inst_status(dxy_asof,3,'ICE DXY licensed') if dxy_asof is not None else {'status':'UNAVAILABLE','fresh':False,'age_days':None,'asof':None,'source':'ICE DXY licensed'}
...
def final_gatekeeper(technical_score, direction, alignment, structure_ok, liquidity_state, news_blocked, institutional_gate, rr_ok, m15_adx):
    """بوابة القرار المركزية: لا يمكن إصدار صفقة إلا بعد مرور جميع البوابات الإلزامية."""
    gates = {}
    gates['technical'] = direction in ('BUY','SELL') and float(technical_score or 0) >= MIN_TRADE_SCORE
    gates['scenario'] = bool(alignment and structure_ok and float(m15_adx or 0) >= 15)
    gates['liquidity'] = liquidity_state == 'CONFIRMED'
    gates['retest'] = liquidity_state == 'CONFIRMED'
    gates['news'] = not bool(news_blocked)
    inst_state = (institutional_gate or {}).get('state', 'WAIT')
    gates['institutional'] = inst_state == 'ALIGNED'
    gates['risk_reward'] = bool(rr_ok)
    failures = []
    labels = {
        'technical':'الدرجة الفنية غير مؤهلة',
        'scenario':'الاتجاه/الهيكل/الزخم غير مكتمل',
        'liquidity':'سحب السيولة غير مؤكد',
        'retest':'إعادة الاختبار غير مؤكدة',
        'news':'بوابة الأخبار تمنع التنفيذ',
        'institutional':'الأهلية المؤسسية غير مكتملة',
        'risk_reward':'نسبة العائد إلى المخاطرة غير صالحة',
    }
    for key, passed in gates.items():
        if not passed:
            failures.append(labels[key])
    if inst_state == 'CONFLICT':
        return {'state':'BLOCKED','ready':False,'gates':gates,'reasons':failures or ['تعارض مؤسسي قوي']}
    if all(gates.values()):
        return {'state':'READY','ready':True,'gates':gates,'reasons':[]}
    return {'state':'WAIT','ready':False,'gates':gates,'reasons':failures}
...
def _daily_execution_core():
    ...
    # هذا محرك فني أولي فقط؛ READY النهائي يصدر حصراً من FINAL GATEKEEPER بعد إدخال السيولة/إعادة الاختبار والمؤسسي.
    ready = False
    state = "CANDIDATE" if direction in ("BUY","SELL") and setup_score >= MIN_TRADE_SCORE else "WATCH"
...
def evaluate_signal():
    ...
    # FINAL GATEKEEPER: المسار الوحيد المخوّل بإصدار READY أو Trade Signal.
    expected_structure = 'صاعد' if technical_direction == 'BUY' else 'هابط' if technical_direction == 'SELL' else None
    structure_ok = bool(expected_structure and h1.get('structure') == expected_structure and m15.get('structure') == expected_structure)
    final_gate = final_gatekeeper(
        technical_score=result.get('score', 0),
        direction=result.get('direction'),
        alignment=bool(core.get('alignment')),
        structure_ok=structure_ok,
        liquidity_state=core.get('liquidity_state'),
        news_blocked=result.get('news_blocked'),
        institutional_gate=gate,
        rr_ok=bool(core.get('trade') or build_trade(technical_direction, h1, m15, core.get('levels', {}))),
        m15_adx=float(m15.get('adx', 0) or 0),
    )
    result['final_gate'] = final_gate
    result['gate_status'] = final_gate['state']
    if final_gate['ready']:
        candidate = build_trade(technical_direction, h1, m15, core.get('levels', {}))
        result['signal'] = True
        result['execution_state'] = 'READY'
        result['trade'] = candidate
    else:
        result['signal'] = False
        result['trade'] = None
        result['execution_state'] = 'BLOCKED' if final_gate['state'] == 'BLOCKED' else ('CANDIDATE' if result.get('direction') in ('BUY','SELL') else 'WATCH')
        result.setdefault('setup_reasons', []).extend(final_gate.get('reasons', []))
