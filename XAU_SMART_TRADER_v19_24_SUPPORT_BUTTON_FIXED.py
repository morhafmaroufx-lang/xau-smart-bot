...
def _support_contact_url(contact):
    """تحويل بيانات ADMIN_CONTACT الموجودة إلى رابط Telegram عند الإمكان."""
    value = str(contact or "").strip()
    if not value:
        return ""
    if value.startswith("https://t.me/") or value.startswith("http://t.me/"):
        return value
    if value.startswith("https://telegram.me/") or value.startswith("http://telegram.me/"):
        return value
    if value.startswith("@"):
        username = value[1:].strip()
    else:
        username = value.strip()
    # رقم حساب Telegram يمكن فتحه مباشرة عبر رابط tg:// عند استخدام ADMIN_CONTACT كـ Chat ID رقمي.
...
async def support_button(update, context):
    ...
    if is_admin_chat(chat_id):
        # حماية إضافية: حتى لو وصل النص يدوياً، الإدارة لا تحصل على قسم الدعم.
    ...
    if contact_url:
        ...
        keyboard = None
        contact_line = f"📩 وسيلة التواصل: {contact}"
    else:
        keyboard = None
        contact_line = "📩 بيانات وسيلة التواصل غير مهيأة حالياً."

    # لا نستخدم reply() هنا لأن reply() في هذا الإصدار لا يستقبل reply_markup.
    # الإرسال المباشر يضمن ظهور زر التواصل الفعلي للمشترك.
    message = getattr(update, "effective_message", None)
    if message is None:
        return
    support_text = (
    ...
    try:
        await message.reply_text(
            with_market_header(support_text),
            reply_markup=keyboard,
        )
    except Exception:
        logger.exception("SUPPORT_BUTTON_SEND_ERROR")
        await message.reply_text(with_market_header(support_text))
