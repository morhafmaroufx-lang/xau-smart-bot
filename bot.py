...
async def weekly_report(update, context):
    ...
    # ---------------------------------------------------------
    # تحديد سيناريو الأسبوع
    # ---------------------------------------------------------
    weekly_trend = str(w.get("trend", "غير متوفر"))

    if "صاعد" in weekly_trend:
        scenario = (
            "الأولوية للبحث عن فرص شراء من مناطق الدعم والسيولة، "
            "مع تجنب مطاردة السعر بعد الارتفاع."
        )
    elif "هابط" in weekly_trend:
        scenario = (
            "الأولوية للبحث عن فرص بيع من مناطق المقاومة والسيولة، "
            "مع تجنب البيع بعد الهبوط الحاد."
        )
    else:
        scenario = (
            "السوق يحتاج إلى تأكيد قبل اتخاذ اتجاه واضح، "
            "والأفضل التعامل مع مناطق الدعم والمقاومة وانتظار الاختراق أو الارتداد."
        )
    ...
    # ---------------------------------------------------------
    return (
        "📅 التقرير الاستراتيجي الأسبوعي — الذهب XAU/USD\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"

        f"🕐 توقيت دمشق: "
        f"{datetime.now(DAMASCUS).strftime('%Y-%m-%d %H:%M')}\n"
        f"💰 السعر الحالي: {a['price']:.2f}\n\n"

        "🧠 الرؤية الاستراتيجية\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"🌍 الاتجاه W1: {w.get('trend', 'غير متوفر')}\n"
        f"📊 الاتجاه D1: {d.get('trend', 'غير متوفر')}\n"
        f"⏱ الاتجاه H4: {h.get('trend', 'غير متوفر')}\n"
        f"🏗 الهيكل H4: {h.get('structure', 'غير متوفر')}\n"
        f"💪 قوة التوافق: {score}%\n"
        f"🎯 التحيز العام: {bias}\n\n"

        "💧 خريطة السيولة والمناطق\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"🟢 دعم رئيسي 1: {format_zone(lv, 'support1')}\n"
        f"🟢 دعم رئيسي 2: {format_zone(lv, 'support2')}\n"
        f"🔴 مقاومة رئيسية 1: {format_zone(lv, 'resistance1')}\n"
        f"🔴 مقاومة رئيسية 2: {format_zone(lv, 'resistance2')}\n\n"
        ...
        "📌 خطة التعامل\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "• لا يتم اعتماد الاتجاه من مؤشر واحد.\n"
        "• الأولوية لتوافق الاتجاه + الهيكل + مناطق السيولة.\n"
        "• انتظار تأكيد سعري قبل الدخول.\n"
        "• عند تعارض الفريمات، يتم تخفيض الثقة وتجنب الدخول العشوائي.\n\n"

        "📝 الخلاصة\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"{narrative(a)}\n\n"

        "⚠️ التقرير استراتيجي وتحليلي فقط، "
        "ولا يضمن الربح أو يمنع الخسارة."
        )
