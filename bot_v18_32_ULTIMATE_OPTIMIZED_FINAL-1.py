# ============================================================
# XAU SMART TRADER v18.32 — ULTIMATE OPTIMIZED | SECURITY & PERFORMANCE HARDENED
        ...
        with self.db.transaction() as conn:
            ...
            for row in rows:
                ...
                if hit_sl:
                    ...
                    changed.append(trade["trade_id"]); continue
                if hit_targets:
                    # Record only the first newly reached target; all earlier targets are implied only if already recorded.
                    ...
                    for label, target in hit_targets:
                        if label not in events:
                            self._event(conn, trade, label, point, distance=abs(point.price-target), metadata={"source": point.source})
...
if not TELEGRAM_WEBHOOK_SECRET and TOKEN:
    TELEGRAM_WEBHOOK_SECRET = hashlib.sha256(TOKEN.encode("utf-8")).hexdigest()
...
SUBSCRIPTION_DB_INIT_LOCK = threading.Lock()
SUBSCRIPTION_DB_INITIALIZED = False
TRADE_DB_INIT_LOCK = threading.Lock()
TRADE_DB_INITIALIZED = False
...
# Unified synchronization for shared in-memory state.
...
def _init_subscription_db():
    global SUBSCRIPTION_DB_INITIALIZED
    if SUBSCRIPTION_DB_INITIALIZED:
        return
    with SUBSCRIPTION_DB_INIT_LOCK:
        if SUBSCRIPTION_DB_INITIALIZED:
            return
        ...
        try:
            ...
            conn.commit()
            SUBSCRIPTION_DB_INITIALIZED = True
        finally:
            conn.close()


def _db():
    _init_subscription_db()
    conn = sqlite3.connect(SUBSCRIPTION_DB_PATH, timeout=10)
    conn.execute("PRAGMA busy_timeout=10000")
    conn.row_factory = sqlite3.Row
    return conn
...
def _completed_bars(df):
    """Return only completed OHLC candles; never expose the live/unfinished bar."""
    if df is None or getattr(df, "empty", True):
        return df
    work = df.copy()
    if "openTime" in work.columns and len(work) > 1:
        work = work.sort_values("openTime")
...
def institutional_analysis(df):
    """استدلال سياق مؤسسي من بيانات مكتملة فقط؛ لا يدّعي بيانات بنوك/ETF مباشرة."""
    df = _completed_bars(df)
    if df is None or len(df) < 30:
        raise ValueError("بيانات غير كافية للسياق المؤسسي")
    ...
    if vol_ratio >= 1.20 and len(recent) >= 2:
...
def analyze(df):
    ...
    work = _completed_bars(df)
    if work is None or len(work) < 30:
        raise ValueError("بيانات مكتملة غير كافية للتحليل")
...
def get_news_events(force=False):
    ...
    with NEWS_CACHE_LOCK:
        if not force and NEWS_CACHE.get("time", 0) and now_ts - NEWS_CACHE.get("time", 0) < NEWS_CACHE_SECONDS:
            return list(NEWS_CACHE.get("events", []))
...
def _init_trade_db():
    global TRADE_DB_INITIALIZED
    if TRADE_DB_INITIALIZED:
        return
    with TRADE_DB_INIT_LOCK:
        if TRADE_DB_INITIALIZED:
            return
        ...
        try:
            ...
            conn.execute("""CREATE TABLE IF NOT EXISTS trade_notifications (
                ...
                result TEXT NOT NULL, sent_at TEXT NOT NULL, state TEXT NOT NULL DEFAULT 'SENT',
...
def _auditor_worker():
    ...
    while not AUDITOR_WORKER_STOP.is_set() or not AUDITOR_QUEUE.empty():
        try:
            task = AUDITOR_QUEUE.get(timeout=0.25)
        except queue.Empty:
            continue
        try:
            kind = task[0]
...
def _stop_auditor_bridge_worker(timeout=5.0):
    """Stop the bridge and drain already-queued audit tasks best-effort."""
    global AUDITOR_WORKER_THREAD
    AUDITOR_WORKER_STOP.set()
    worker = AUDITOR_WORKER_THREAD
...
async def reply(update, text):
    try:
        ...
        if getattr(update, "message", None):
            ...
            await update.callback_query.message.reply_text(payload)
    except Exception:
...
async def start(update, context):
    ...
    await update.message.reply_text(with_market_header(text), reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True))
...
async def quick_analysis(update, context):
    ...
    try:
        ...
        if trade:
            target = float(trade.get("tp1", price))
            stop = float(trade.get("sl", price))
...
def _quick_analysis_data():
    # Single Source of Truth: evaluate_signal() builds the exact executable trade.
    result = evaluate_signal()
    df = _completed_bars(get_bars("15m", 220))
    m15 = result.get("daily", {}).get("m15") or analyze(df)
...
async def analyses_menu(update, context):
    if not await feature_guard("full_analysis")(update, context): return
    buttons = [
        [InlineKeyboardButton("📊 التحليل الكامل", callback_data="menu_full_analysis")],
        [InlineKeyboardButton("📝 التقرير التوضيحي اليومي", callback_data="menu_daily_report")],
        [InlineKeyboardButton("📅 التقرير التوضيحي الأسبوعي", callback_data="menu_weekly_report")],
    ]
    await reply(update, "📊 التحليلات\n\nاختر نوع التحليل:",)
    await update.message.reply_text(with_market_header("اختر نوع التحليل:"), reply_markup=InlineKeyboardMarkup(buttons))


async def trades_menu(update, context):
    buttons = [
        [InlineKeyboardButton("🎯 صفقة الآن", callback_data="menu_trade_now")],
        [InlineKeyboardButton("📜 سجل الصفقات", callback_data="menu_trade_history")],
        [InlineKeyboardButton("🔔 حالة تنبيهات الصفقات", callback_data="menu_trade_alerts")],
    ]
    await update.message.reply_text(with_market_header("🎯 الصفقات\n\nاختر العملية:"), reply_markup=InlineKeyboardMarkup(buttons))


async def market_news_menu(update, context):
    if not await feature_guard("markets")(update, context): return
    buttons = [
        [InlineKeyboardButton("📰 الأخبار", callback_data="menu_news")],
        [InlineKeyboardButton("🌍 الجلسات والأسواق", callback_data="menu_markets")],
        [InlineKeyboardButton("💰 سعر الذهب", callback_data="menu_gold_price")],
    ]
    await update.message.reply_text(with_market_header("🌍 السوق والأخبار\n\nاختر القسم:"), reply_markup=InlineKeyboardMarkup(buttons))
...
async def institutional_menu(update, context):
    ...
    try:
        ...
        await reply(update, "🏦 التحليل المؤسسي\n━━━━━━━━━━━━━━━━━━\n🧠 Institutional Context\n" + str(context_text) + "\n\n📡 Institutional Data\n⚪ البيانات المؤسسية المباشرة تعتمد على توفر مصدر مباشر؛ لا يتم اختلاق بيانات غير متاحة.")
    except Exception as e:
        await reply(update, f"🏦 التحليل المؤسسي\n\n❌ تعذر إنشاء القسم: {e}")
...
async def liquidity_menu(update, context):
    ...
    try:
        ...
        await reply(update, f"🎯 مناطق السيولة\n\n❌ تعذر تحديث السيولة: {e}")


async def help_menu(update, context):
    await reply(update, "ℹ️ المساعدة\n\nاختر من القائمة الرئيسية للوصول إلى التحليل، الصفقات، السوق والأخبار، التحليل المؤسسي ومناطق السيولة.\n\n⚠️ جميع الإشارات تحليلية وليست ضماناً للربح.")
...
async def router(update, context):
    ...
    routes = {
        ...
        # أوامر داخلية قديمة تبقى مدعومة دون عرضها في القائمة.
        "🎯 صفقة الآن": trade_now,
...
def webhook():
    ...
    if (not TELEGRAM_WEBHOOK_SECRET or not supplied_secret or
            not hmac.compare_digest(supplied_secret, TELEGRAM_WEBHOOK_SECRET)):
    ...
    try:
        ...
        # Do not claim successful delivery if scheduling on the bot loop failed.
...
async def auto_loop():
    ...
    while not SHUTDOWN_EVENT.is_set():
        ...
        try:
            await asyncio.wait_for(asyncio.to_thread(SHUTDOWN_EVENT.wait, AUTO_SCAN_SECONDS), timeout=AUTO_SCAN_SECONDS + 0.1)
...
async def shutdown_bot():
    """Coordinated shutdown for Telegram, auto loop and Auditor workers."""
    global AUTO_TASK, APPLICATION
    SHUTDOWN_EVENT.set()
    if AUTO_TASK and not AUTO_TASK.done():
        ...
        try:
            ...
            pass
    AUTO_TASK = None
    _stop_auditor_bridge_worker(timeout=5.0)
    if PERFORMANCE_AUDITOR is not None:
        try:
            PERFORMANCE_AUDITOR.stop()
            thread = getattr(PERFORMANCE_AUDITOR, "_thread", None)
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.exception("MAIN_SHUTDOWN_ERROR")
    finally:
