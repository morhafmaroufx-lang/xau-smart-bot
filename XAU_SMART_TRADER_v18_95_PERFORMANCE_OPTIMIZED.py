# v18.93 ARCHITECTURE OWNERSHIP + LIFECYCLE + ALERT ACTIVATION HARDENING
# نطاق الإصلاح: ترتيب التشغيل، دورة حياة S/R، منع تكرار/تعارض S/R،
# وربط الإصدار النهائي لطبقة التنبيهات قبل نقطة التشغيل.
V18_93_ARCHITECTURE_FIX = True
V18_95_PERFORMANCE_OPTIMIZED = True
V18_94_FULL_ANALYSIS_REFACTOR = True
    ...
    def report(self):
        ...
        def before_cutoff(row,key):
            try:
                ...
                return False
        final_trades=[t for t in all_trades if t.get("status")=="CLOSED" and t.get("final_result") in FINAL_RESULTS and before_cutoff(t,"close_time")]
        monitoring_trades=[t for t in all_trades if t not in final_trades]
...
class PerformanceAuditor:
    ...
    def __init__(self,db_path=AUDITOR_DB_PATH,price_engine=None):
        ...
        self._stop=threading.Event(); self._thread=None; self._async_task=None; self._alert_callback=None; self._heal_callback=None; self._alert_state={}; self._last_persist=0.0; self._last_cleanup=0.0
    @staticmethod
    ...
    def metrics_report(self):
        r=self.metrics.report(); r['system']=self.metrics.system_snapshot(queue_depth=_safe_queue_depth(),queue_capacity=_safe_queue_capacity(),queue_drops=_safe_queue_drops()); r['handler_root_trace']=self.metrics.handler_trace_report(); return r
...
VERSION = "v18.92-TRADE-MONITOR-SR-GY"
...
def _institutional_analysis_live(df):
...
# ============================================================
# v18.95 INSTITUTIONAL SNAPSHOT / BACKGROUND WARMER
# لا يغير أي قاعدة scoring أو قرار؛ يمنع طلب Telegram من انتظار
# المصادر المؤسسية عند وجود snapshot صالح، ويمنع stampede عند البداية.
# ============================================================
INSTITUTIONAL_SNAPSHOT_TTL = max(60, int(os.getenv("INSTITUTIONAL_SNAPSHOT_TTL", "600")))
INSTITUTIONAL_WARM_INTERVAL = max(120, int(os.getenv("INSTITUTIONAL_WARM_INTERVAL", "300")))
INSTITUTIONAL_WARM_START_DELAY = max(0, int(os.getenv("INSTITUTIONAL_WARM_START_DELAY", "5")))
_INSTITUTIONAL_SNAPSHOT_LOCK = threading.RLock()
_INSTITUTIONAL_REFRESH_LOCK = threading.Lock()
_INSTITUTIONAL_SNAPSHOT = {"ts": 0.0, "value": None}
_INSTITUTIONAL_WARMER_STARTED = False
_INSTITUTIONAL_WARMER_STOP = threading.Event()


def _institutional_snapshot_get(max_age=None):
    age_limit = INSTITUTIONAL_SNAPSHOT_TTL if max_age is None else max(0, float(max_age))
    with _INSTITUTIONAL_SNAPSHOT_LOCK:
        item = dict(_INSTITUTIONAL_SNAPSHOT)
    value = item.get("value")
    ts = float(item.get("ts") or 0.0)
    if value is not None and (time.time() - ts) <= age_limit:
        return value
    return None


def _institutional_snapshot_put(value):
    with _INSTITUTIONAL_SNAPSHOT_LOCK:
        _INSTITUTIONAL_SNAPSHOT["ts"] = time.time()
        _INSTITUTIONAL_SNAPSHOT["value"] = value
    return value


def institutional_analysis(df=None, *, force_refresh=False):
    """واجهة مؤسسية سريعة: snapshot أولاً، وجلب حي فقط عند الحاجة."""
    if not force_refresh:
        cached = _institutional_snapshot_get()
        if cached is not None:
            out = dict(cached)
            meta = dict(out.get("_snapshot_meta") or {})
            meta.update({"served_from_snapshot": True, "snapshot_age_ms": round((time.time() - float(_INSTITUTIONAL_SNAPSHOT.get("ts") or time.time())) * 1000.0, 1)})
            out["_snapshot_meta"] = meta
            return out
    # Single-flight: one refresh only. A concurrent request uses the last
    # snapshot if available rather than starting another set of HTTP calls.
    acquired = _INSTITUTIONAL_REFRESH_LOCK.acquire(blocking=False)
    if not acquired:
        fallback = _institutional_snapshot_get(max_age=INSTITUTIONAL_CACHE_SECONDS)
        if fallback is not None:
            out = dict(fallback)
            out["_snapshot_meta"] = {**dict(out.get("_snapshot_meta") or {}), "served_from_snapshot": True, "refresh_in_progress": True}
            return out
        # No snapshot exists yet; this caller owns the unavoidable first-load wait.
        _INSTITUTIONAL_REFRESH_LOCK.acquire()
        acquired = True
    try:
        if not force_refresh:
            cached = _institutional_snapshot_get()
            if cached is not None:
                return cached
        started = time.perf_counter()
        with handler_trace_stage("full_analysis:institutional_live"):
            value = _institutional_analysis_live(df)
        value = dict(value or {})
        value["_snapshot_meta"] = {
            "served_from_snapshot": False,
            "refresh_elapsed_ms": round((time.perf_counter() - started) * 1000.0, 2),
            "refreshed_at": datetime.now(timezone.utc).isoformat(),
        }
        return _institutional_snapshot_put(value)
    finally:
        _INSTITUTIONAL_REFRESH_LOCK.release()


def _institutional_warmer_loop():
    if INSTITUTIONAL_WARM_START_DELAY:
        _INSTITUTIONAL_WARMER_STOP.wait(INSTITUTIONAL_WARM_START_DELAY)
    while not _INSTITUTIONAL_WARMER_STOP.is_set():
        try:
            institutional_analysis(None, force_refresh=True)
            logger.info("INSTITUTIONAL_SNAPSHOT_REFRESHED")
        except Exception:
            logger.exception("INSTITUTIONAL_SNAPSHOT_REFRESH_FAILED")
        _INSTITUTIONAL_WARMER_STOP.wait(INSTITUTIONAL_WARM_INTERVAL)


def start_institutional_warmer():
    global _INSTITUTIONAL_WARMER_STARTED
    if _INSTITUTIONAL_WARMER_STARTED:
        return
    _INSTITUTIONAL_WARMER_STARTED = True
    threading.Thread(target=_institutional_warmer_loop, name="institutional-snapshot-warmer", daemon=True).start()


def stop_institutional_warmer():
    _INSTITUTIONAL_WARMER_STOP.set()
...
def get_risk(now=None):
    """News risk is informational only: never blocks trading execution."""
    now = now or _now_utc()
    try:
        events = get_events()
        upcoming = []
        ...
        if upcoming:
            ...
            return {
                ...
                "message": "⚠️ خبر عالي التأثير ضمن نافذة التقلب. هذا تنبيه معلوماتي ولا يحجب التداول.\n" + "\n".join(details),
                "events": [e for e, _ in upcoming],
            }
        return {
            "state": "CLEAR", "blocked": False,
            "source": events[0].get("source", "local"),
            "message": "🟢 لا يوجد حالياً خبر عالي التأثير ضمن نافذة التنبيه.",
            "events": [],
        }
    except Exception as exc:
        return {
            "state": "UNKNOWN", "blocked": False, "source": "none", "events": [],
            "message": f"⚠️ حالة الأخبار غير متاحة حالياً. هذا لا يوقف التداول.\nالسبب: {exc}",
        }
def _fetch_te():
    if not TE_API_KEY:
...
def news_filter():
    """Informational news status only; preserved API compatibility without any trading block."""
    risk = get_risk()
...
def _daily_execution_core():
    ...
    try:
        ...
        news_blocked, news_text = False, f"⚠️ حالة الأخبار غير متاحة: {exc}"
...
# ============================================================
# v18.94 FULL ANALYSIS REFACTOR
# فصل دورة التحليل الكامل إلى orchestration / decision / rendering.
# لا يغيّر شروط الإشارة أو الدرجات أو Entry/SL/TP أو S/R.
# ============================================================

def _full_analysis_prepare(include_trade=True):
    """تشغيل محرك القرار مرة واحدة وإرجاع snapshot موحد للعرض."""
    started = time.perf_counter()
    with handler_trace_stage("full_analysis:decision"):
        result = evaluate_signal()
    elapsed_ms = round((time.perf_counter() - started) * 1000.0, 2)
    if not isinstance(result, dict):
        raise RuntimeError("محرك التحليل أعاد نتيجة غير صالحة.")
    required = ("direction", "score", "price", "levels", "mtf", "institutional")
    missing = [key for key in required if key not in result]
    if missing:
        raise RuntimeError("نتيجة التحليل ناقصة: " + ", ".join(missing))
    result["_full_analysis_meta"] = {"version": "v18.95", "include_trade": bool(include_trade), "decision_elapsed_ms": elapsed_ms}
    return result

def build_analysis(include_trade=True):
    """منسق التحليل الكامل: قرار موحد → snapshot → عرض."""
    with handler_trace_stage("full_analysis:orchestrator"):
        result = _full_analysis_prepare(include_trade=include_trade)
        with handler_trace_stage("full_analysis:render"):
            return _render_full_analysis_report(result, include_trade=include_trade)


def _render_full_analysis_report(result, include_trade=True):
    """طبقة العرض فقط: تحول نتيجة التحليل الموحدة إلى تقرير Telegram."""
    mtf=result['mtf']; levels=result['levels']; inst=result.get('institutional') or {}; src=inst.get('sources') or {}; m=src.get('macro') or {}; c=src.get('cot') or {}; g=src.get('gld') or {}; cb=src.get('central_banks') or {}
...
async def full_analysis(update, context):
    """واجهة Telegram رفيعة: صلاحية → quota → تحليل بالخلفية → عرض."""
    ...
    try:
        ...
        chat_id = update.effective_chat.id
        with handler_trace_stage("full_analysis:quota"):
            can_trade = await asyncio.to_thread(can_receive_trade, chat_id)
        analysis_text = await asyncio.to_thread(build_analysis, can_trade)
        with handler_trace_stage("full_analysis:reply"):
...
async def quick_analysis(update, context):
    ...
    try:
        ...
        ema9 = float(m15.get("ema9", m15.get("ema50", price)))
        ema21 = float(m15.get("ema21", m15.get("ema50", price)))
...
async def _performance_report_for_admin(update):
    """حارس مشترك لمراقبي الأداء — قراءة فقط ولا يغيّر أي قرار تداول."""
    user_id = update.effective_user.id if update.effective_user else update.effective_chat.id
    chat_id = update.effective_chat.id if update.effective_chat else user_id
    if not (is_admin_chat(user_id) or is_admin_chat(chat_id)):
        await reply(update, "⛔ مراقبا الأداء مخصصان للإدارة فقط.")
        return None
    if PERFORMANCE_AUDITOR is None:
...
def webhook():
    ...
    try:
        ...
        if PERFORMANCE_AUDITOR is not None:
            try:
                PERFORMANCE_AUDITOR.metrics.record_request((time.perf_counter() - _started) * 1000.0)
            except Exception:
                pass
...
# ============================================================
# v18.89 NEWS ALERT DEDUP + NEWS NON-BLOCKING FIX
# Scope: News Alert Delivery/Scheduler state only. Trading strategy/scoring untouched.
# ============================================================
def _news_alert_conn():
    # ALERT_DB_PATH is defined later in the legacy Alert Delivery section.
    # This startup-safe lookup avoids a NameError during early initialization
    # while preserving the same database path used by PersistentAlertDispatcher.
    db_path = globals().get("ALERT_DB_PATH") or os.getenv("ALERT_DB_PATH", "xau_alert_queue.db")
    c = sqlite3.connect(db_path, timeout=5, check_same_thread=False)
    c.execute("PRAGMA journal_mode=WAL")
    c.execute("PRAGMA busy_timeout=5000")
    return c

def _init_news_alert_dedup():
    c = _news_alert_conn()
    try:
        c.execute("""CREATE TABLE IF NOT EXISTS news_alert_dedup(
        ...
        c.execute("CREATE INDEX IF NOT EXISTS idx_news_alert_dedup_event ON news_alert_dedup(event_key, phase)")
        c.commit()
    finally:
        c.close()

def _claim_news_alert_once(alert_key, event_key, phase, event_time):
    c = _news_alert_conn()
    try:
        c.execute("BEGIN IMMEDIATE")
        cur = c.execute("INSERT OR IGNORE INTO news_alert_dedup(alert_key,event_key,phase,event_time,status,created_at) VALUES(?,?,?,?,?,?)",
                        (alert_key,event_key,phase,event_time,"CLAIMED",time.time()))
        c.commit()
        return cur.rowcount == 1
    finally:
        c.close()

def _mark_news_alert_sent(alert_key):
    c = _news_alert_conn()
    try:
        c.execute("UPDATE news_alert_dedup SET status='SENT', sent_at=? WHERE alert_key=?", (time.time(),alert_key))
        c.commit()
    finally:
        c.close()

def _release_news_alert_claim(alert_key):
    c = _news_alert_conn()
    try:
        c.execute("DELETE FROM news_alert_dedup WHERE alert_key=? AND status='CLAIMED'", (alert_key,))
        c.commit()
    finally:
        c.close()

_init_news_alert_dedup()

async def send_news_alerts():
    """Send each HIGH-impact news event once per alert phase; news never blocks trading."""
    ...
    for event in events:
        ...
        if not isinstance(dt, datetime):
            continue
        delta = (dt - now).total_seconds() / 60.0
        if not (-1 <= delta <= NEWS_BEFORE_MIN):
            continue

        # Stable identity: do NOT include rounded remaining minutes, otherwise every
        # scheduler tick creates a new key and spams the same event.
        event_key = "NEWS|{}|{}|{}".format(
            str(event.get("currency", "")).strip().upper(),
            str(event.get("event", "")).strip(),
            dt.astimezone(DAMASCUS).strftime("%Y-%m-%dT%H:%M"),
        )
        phase = "PRE_EVENT"
        key = f"{event_key}|{phase}"
        if NEWS_ALERT_STATE.get(key):
            logger.info("NEWS_ALERT_DEDUP_MEMORY_HIT | key=%s", key)
            continue

        claimed = await asyncio.to_thread(_claim_news_alert_once, key, event_key, phase, dt.timestamp())
        if not claimed:
            NEWS_ALERT_STATE[key] = True
            logger.info("NEWS_ALERT_DEDUP_PERSISTENT_HIT | key=%s", key)
            continue

        phase_text = "🚨 خلال/قريب من الخبر" if delta <= 1 else f"⏳ قبل الخبر بنحو {max(1, round(delta))} دقيقة"
        text = (f"📰 تنبيه خبر عالي التأثير\n\n"
                f"{phase_text}\n💱 {event.get('currency','')}\n"
                f"📌 {event.get('event','خبر اقتصادي')}\n"
                f"🕐 {dt.strftime('%Y-%m-%d %H:%M')} دمشق\n"
                "ℹ️ تنبيه معلوماتي: قد ترتفع التقلبات، لكن الأخبار لا تحجب التداول تلقائياً.")
        try:
            results = await _dispatch_broadcast(recipients, text, kind="news")
            if any(x is True for x in results):
                NEWS_ALERT_STATE[key] = True
                await asyncio.to_thread(_mark_news_alert_sent, key)
                logger.info("NEWS_ALERT_SENT_ONCE | key=%s", key)
            else:
                # Release claim if nothing was accepted so a later scheduler tick can retry.
                await asyncio.to_thread(_release_news_alert_claim, key)
        except Exception:
            await asyncio.to_thread(_release_news_alert_claim, key)
            raise
...
def run_institutional_self_tests():
    ...
    check('version', VERSION.startswith('v18.92-'), VERSION)
...
# حماية التكرار/التعارض لا تغيّر شروط الاستراتيجية؛ تطبق فقط على الصفقات النشطة المسجلة.
SR_DUP_WINDOW_MINUTES = max(1.0, float(os.getenv("SR_DUP_WINDOW_MINUTES", "30")))
SR_DUP_ENTRY_ATR = max(0.01, float(os.getenv("SR_DUP_ENTRY_ATR", "0.35")))
...
def _sr_init_db():
    ...
    try:
        ...
        """)
        # ترقية آمنة لقواعد البيانات القديمة: إضافة البصمة دون إسقاط أو إعادة كتابة الصفقات.
        columns = {row[1] for row in conn.execute("PRAGMA table_info(sr_trades)").fetchall()}
        if "setup_signature" not in columns:
            conn.execute("ALTER TABLE sr_trades ADD COLUMN setup_signature TEXT")
        ...
        """)
        conn.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS uq_sr_setup_signature
            ON sr_trades(setup_signature)
...
def _sr_setup_signature(trade):
    """بصمة حتمية للصفقة S/R لمنع إعادة تسجيل نفس الإعداد."""
    direction = str(trade.get("direction") or "").upper()
    trigger = str(trade.get("trigger") or "").upper()
    values = [direction, trigger]
    for key in ("entry", "sl", "tp1", "tp2", "tp3"):
        value = _sr_num(trade.get(key))
        values.append("" if value is None else f"{value:.4f}")
    return "SR|" + "|".join(values)


def _sr_trade_created_epoch(value):
    try:
...
def _sr_conflict_exists(conn, trade):
    """حماية تسجيل فقط: تمنع التكرار والتعارض القريب، ولا تغيّر مولد الإشارة."""
    direction = str(trade.get("direction") or "").upper()
    trigger = str(trade.get("trigger") or "").upper()
    ...
    now_ts = time.time()
    cutoff = now_ts - SR_DUP_WINDOW_MINUTES * 60.0
    rows = conn.execute(
        "SELECT * FROM sr_trades WHERE status IN ('OPEN','TP1_HIT','TP2_HIT') ORDER BY created_at DESC"
    ).fetchall()
    atr = _sr_num(trade.get("atr"))
    if atr is None:
        try:
            snap_trade = (json.loads(trade.get("snapshot_json") or "{}") or {}).get("trade", {})
            atr = _sr_num(snap_trade.get("atr"))
        except Exception:
            atr = None
    tolerance = max(0.01, (atr or 0.0) * SR_DUP_ENTRY_ATR)

    for row in rows:
        created_ts = _sr_trade_created_epoch(row["created_at"])
        if created_ts is None or created_ts < cutoff:
            continue
        old_entry = _sr_num(row["entry"])
        if old_entry is None or abs(old_entry - entry) > tolerance:
            continue
        old_direction = str(row["direction"] or "").upper()
        old_trigger = str(row["trigger"] or "").upper()
        if old_direction == direction:
            return {"type": "DUPLICATE", "setup_id": row["setup_id"]}
        # التعارض يعالج فقط عندما يكون السيناريو نفسه أو عندما تكون البصمة البنيوية متطابقة.
        if old_trigger == trigger and trigger:
            return {"type": "OPPOSITE_CONFLICT", "setup_id": row["setup_id"]}
    return None


def _sr_insert_trade(trade, snapshot):
    setup_id = f"SR-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:8].upper()}"
    signature = _sr_setup_signature(trade)
    conn = _sr_db_connect()
    try:
        conn.execute("BEGIN IMMEDIATE")
        # تأكد من أن البصمة متاحة أيضاً داخل لقطة الصفقة الحالية لأغراض التدقيق.
        if isinstance(snapshot, dict):
            snapshot = dict(snapshot)
            snapshot["setup_signature"] = signature
        trade_for_conflict = dict(trade)
        if isinstance(snapshot, dict):
            levels = snapshot.get("levels") or {}
            trade_for_conflict["atr"] = _sr_num(levels.get("atr"))
        conflict = _sr_conflict_exists(conn, trade_for_conflict)
        if conflict:
            conn.rollback()
            logger.info("SR_SETUP_SUPPRESSED | type=%s existing=%s direction=%s entry=%s trigger=%s",
                        conflict["type"], conflict["setup_id"], trade.get("direction"), trade.get("entry"), trade.get("trigger"))
            return conflict["setup_id"], False
        cur = conn.execute("""
            INSERT OR IGNORE INTO sr_trades
            (setup_id, setup_signature, created_at, direction, entry, sl, tp1, tp2, tp3, rr, score, trigger, status, snapshot_json)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'OPEN', ?)
        """, (setup_id, signature, now_damascus().isoformat(), trade["direction"], trade["entry"], trade["sl"],
        ...
        conn.rollback()
        # السباق بين عاملين: البصمة الفريدة هي الحكم النهائي.
        row = conn.execute("SELECT setup_id FROM sr_trades WHERE setup_signature=?", (signature,)).fetchone()
        return (row["setup_id"] if row else None), False
    finally:
        conn.close()
...
def _sr_update_lifecycle(price):
    """المالك الوحيد لدورة حياة S/R؛ يراقب جميع الحالات غير النهائية."""
    price = _sr_num(price)
    if price is None:
        return []
    changed=[]
    conn=_sr_db_connect()
    try:
        rows=conn.execute("SELECT * FROM sr_trades WHERE status IN ('OPEN','TP1_HIT','TP2_HIT') ORDER BY created_at").fetchall()
        for r in rows:
            direction=str(r["direction"] or "").upper()
            current_status=str(r["status"] or "OPEN")
            status=None; result=None
            if direction == "BUY":
                if price <= r["sl"]:
                    status,result="CLOSED","SL"
                elif r["tp3"] is not None and price >= r["tp3"]:
                    status,result="CLOSED","TP3"
                elif r["tp2"] is not None and price >= r["tp2"] and current_status in {"OPEN","TP1_HIT"}:
                    status,result="TP2_HIT","TP2"
                elif price >= r["tp1"] and current_status == "OPEN":
                    status,result="TP1_HIT","TP1"
            else:
                if price >= r["sl"]:
                    status,result="CLOSED","SL"
                elif r["tp3"] is not None and price <= r["tp3"]:
            ...
            if status:
                ...
                cur = conn.execute(
                    "UPDATE sr_trades SET status=?, result=?, closed_at=? WHERE setup_id=? AND status IN ('OPEN','TP1_HIT','TP2_HIT')",
...
def _sr_install_telegram_handler():
    ...
    APPLICATION.add_handler(
        MessageHandler(filters.TEXT & filters.Regex(r"^📐 استراتيجية S/R$"), _trace_handler(_sr_button_guarded, "message:sr_button")),
        group=0,
    )
...
def main_v18_69_sr():
    ...
    if "--self-test" in sys.argv:
        print(json.dumps(run_institutional_self_tests(), ensure_ascii=False, indent=2))
        return
    if "--architecture-self-test" in sys.argv:
        print(json.dumps(run_v18_93_architecture_self_tests(), ensure_ascii=False, indent=2))
        return
    start_institutional_warmer()
    global BOT_LOOP
    server=threading.Thread(target=run_flask, daemon=True)
    server.start()
    loop=asyncio.new_event_loop()
...
class PersistentAlertDispatcher:
    ...
    def __init__(self, bot, db_path=ALERT_DB_PATH, workers=4):
        self.bot=bot; self.db_path=db_path; self.workers=max(1,min(int(workers),8))
        self._tasks=[]; self._running=False; self._wake=asyncio.Event(); self._db_lock=threading.Lock()
        self._waiters={}; self._waiters_lock=threading.RLock(); self._init_db()
    def _conn(self):
        c=sqlite3.connect(self.db_path, timeout=5, check_same_thread=False); c.row_factory=sqlite3.Row
        c.execute('PRAGMA journal_mode=WAL'); c.execute('PRAGMA synchronous=NORMAL'); c.execute('PRAGMA busy_timeout=5000')
        return c
    ...
    def enqueue(self, chat_id, text, *, kind='generic', priority=50, max_attempts=ALERT_MAX_ATTEMPTS):
        ...
        self._wake.set(); logger.info('ALERT_ENQUEUED | id=%s kind=%s',aid,kind); return aid
    ...
    async def _worker(self,idx):
        while self._running:
            ...
            except (Forbidden,BadRequest) as e: await asyncio.to_thread(self._result,{**job,'max_attempts':1},False,e)
            except RetryAfter as e:
                ra=getattr(e,'retry_after',1);ra=float(ra.total_seconds() if hasattr(ra,'total_seconds') else ra)
                await asyncio.to_thread(self._result,job,False,e,max(1.0,ra))
            except Exception as e:
...
# نقطة التشغيل تؤجل إلى نهاية الملف بعد اكتمال جميع طبقات الإصلاح النهائية.
...
# ============================================================
# v18.93 ARCHITECTURE SELF-TEST
# فحص ساكن لا يحتاج شبكة أو Telegram أو قاعدة بيانات إنتاجية.
# ============================================================
def run_v18_93_architecture_self_tests():
    import ast as _ast
    source = Path(__file__).read_text(encoding="utf-8")
    tree = _ast.parse(source)
    checks = []
    ...
    guards = [n for n in tree.body if isinstance(n, _ast.If) and "__name__" in _ast.unparse(n.test) and "__main__" in _ast.unparse(n.test)]
    check("single final runtime guard", len(guards) == 1, str([g.lineno for g in guards]))
    check("runtime guard is final top-level statement", bool(tree.body) and tree.body[-1] is guards[0] if guards else False,
          str(getattr(tree.body[-1], "lineno", None)))

    defs = {}
    for node in tree.body:
        if isinstance(node, (_ast.FunctionDef, _ast.AsyncFunctionDef, _ast.ClassDef)):
            defs.setdefault(node.name, []).append(node.lineno)
    check("main defined before final guard", "main_v18_69_sr" in defs and defs["main_v18_69_sr"][-1] < guards[0].lineno if guards else False,
          str(defs.get("main_v18_69_sr")))
    check("S/R lifecycle monitors nonterminal states", "status IN ('OPEN','TP1_HIT','TP2_HIT')" in source)
    check("S/R deterministic setup signature", "def _sr_setup_signature(trade):" in source and "uq_sr_setup_signature" in source)
    check("S/R conflict window", "SR_DUP_WINDOW_MINUTES" in source and "SR_DUP_ENTRY_ATR" in source)
    check("durable alert-trade link", "_create_alert_trade_link" in source and "_resolve_alert_trade_link" in source)
    check("final dispatcher is last definition before runtime", defs.get("_start_alert_dispatcher", [0])[-1] < guards[0].lineno if guards else False,
          str(defs.get("_start_alert_dispatcher")))
    check("final trade notification uses durable link", defs.get("_send_trade_notification", [0])[-1] < guards[0].lineno and "_create_alert_trade_link" in source,
          str(defs.get("_send_trade_notification")))
    check("strategy isolation preserved", "evaluate_signal" not in _ast.unparse(next((n for n in tree.body if isinstance(n, (_ast.FunctionDef, _ast.AsyncFunctionDef)) and n.name == "sr_analyze"), _ast.Pass())))

    check("full analysis renderer is pure", "def _render_full_analysis_report(result, include_trade=True):" in source and "result=evaluate_signal()" not in source.split("def _render_full_analysis_report",1)[1].split("def ",1)[0])
    check("institutional snapshot layer", "def institutional_analysis(df=None, *, force_refresh=False):" in source and "INSTITUTIONAL_SNAPSHOT_TTL" in source)
    check("institutional single-flight refresh", "_INSTITUTIONAL_REFRESH_LOCK" in source and "acquire(blocking=False)" in source)
    check("background institutional warmer", "def start_institutional_warmer()" in source and "institutional-snapshot-warmer" in source)
    passed = sum(x["status"] == "PASS" for x in checks)
    return {"passed": passed, "total": len(checks), "ok": passed == len(checks), "results": checks}


# ============================================================
# v18.93 FINAL RUNTIME ENTRYPOINT
# يجب أن تكون هذه آخر نقطة تشغيل في الملف لضمان تفعيل آخر تعريفات
# التنبيهات والجدولة والتتبع قبل بدء الحلقة طويلة العمر.
# ============================================================
