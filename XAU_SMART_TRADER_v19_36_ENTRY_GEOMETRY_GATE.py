...
def _entry_geometry_gate(df, direction, atr, levels=None, trigger_state="NONE", liquidity_state=None):
    """v19.36 — Entry Geometry only.

    This gate does NOT alter direction, score, score weights, alignment, R:R,
    SL/TP construction, or liquidity scoring. It decides only whether the
    current completed M15 close is executable now, should WAIT, or is invalid
    because the geometry/data is unavailable.
    """
    direction = str(direction or "").upper().strip()
    if direction not in ("BUY", "SELL"):
        return {"state":"NO_TRADE", "reason":"لا يوجد اتجاه تنفيذي صالح", "pass":False}
    try:
        import math as _eg_math
        atr_v = float(atr)
        if not _eg_math.isfinite(atr_v) or atr_v <= 0:
            return {"state":"NO_TRADE", "reason":"ATR غير صالح", "pass":False}
        if df is None or len(df) < 6:
            return {"state":"NO_TRADE", "reason":"بيانات M15 غير كافية لهندسة Entry", "pass":False}
        cols = {str(c).lower() for c in getattr(df, "columns", [])}
        if not {"open","high","low","close"}.issubset(cols):
            return {"state":"NO_TRADE", "reason":"بيانات OHLC غير مكتملة", "pass":False}

        # Use the same completed-candle convention as analyze().
        work = df.iloc[:-1].copy() if len(df) >= 31 else df.copy()
        if len(work) < 6:
            return {"state":"NO_TRADE", "reason":"بيانات M15 المكتملة غير كافية", "pass":False}

        def _v(row, key):
            value = float(row[key])
            return value if _eg_math.isfinite(value) else None

        rows = work.iloc[-5:]
        last = work.iloc[-1]
        o = _v(last, "open"); h = _v(last, "high"); l = _v(last, "low"); c = _v(last, "close")
        if None in (o,h,l,c) or h <= l:
            return {"state":"NO_TRADE", "reason":"آخر شمعة مكتملة غير صالحة", "pass":False}
        ...
        # Impulse extension: net displacement, consecutive directional candles,
        # and expansion are intentionally separate from Score.
        closes = [float(x["close"]) for _,x in rows.iterrows()]
        opens = [float(x["open"]) for _,x in rows.iterrows()]
        valid_series = all(_eg_math.isfinite(x) for x in closes+opens)
        if not valid_series:
            return {"state":"NO_TRADE", "reason":"سلسلة الشموع غير صالحة", "pass":False}
        net_move_atr = abs(closes[-1]-closes[0])/atr_v
        consecutive = 0
        for oo,cc in zip(reversed(opens), reversed(closes)):
            if (cc > oo) if direction == "BUY" else (cc < oo):
                consecutive += 1
            else:
                break

        # Nearest executable anchor/obstacle from the existing S/R engine.
        levels = levels if isinstance(levels, dict) else {}
        def _level(name):
            item = levels.get(name)
            if isinstance(item, dict): item = item.get("price")
            try:
                value=float(item)
                return value if _eg_math.isfinite(value) else None
            except Exception:
                return None
        supports=sorted([x for x in (_level("support1"),_level("support2"),_level("support3")) if x is not None and x < c], reverse=True)
        resistances=sorted([x for x in (_level("resistance1"),_level("resistance2"),_level("resistance3")) if x is not None and x > c])
        anchor = supports[0] if direction == "BUY" and supports else resistances[0] if direction == "SELL" and resistances else None
        obstacle = resistances[0] if direction == "BUY" and resistances else supports[0] if direction == "SELL" and supports else None
        anchor_distance_atr = abs(c-anchor)/atr_v if anchor is not None else None
        obstacle_distance_atr = abs(obstacle-c)/atr_v if obstacle is not None else None

        confirmed = str(liquidity_state or trigger_state or "").upper() == "CONFIRMED" or str(trigger_state or "").upper() == "RETEST"
        # A completed liquidity reversal/retest is allowed to validate a strong
        # move; it does not reverse the direction and does not change Score.
        if confirmed and directional:
            return {"state":"PASS", "pass":True, "reason":"إعادة اختبار/انعكاس سيولة مكتمل — هندسة الدخول مؤكدة",
                    "range_atr":range_atr,"body_ratio":body_ratio,"close_pos":close_pos,
                    "net_move_atr":net_move_atr,"consecutive":consecutive,
                    "anchor_distance_atr":anchor_distance_atr,"obstacle_distance_atr":obstacle_distance_atr,
                    "trigger_state":trigger_state,"liquidity_state":liquidity_state}

        reasons=[]
        if not directional:
            reasons.append("إغلاق الشمعة لا يؤكد اتجاه التنفيذ")
        if range_atr >= 2.0:
            reasons.append("مدى الشمعة ≥ 2 ATR — لا دخول فوري")
        elif range_atr >= 1.35:
            reasons.append("مدى الشمعة 1.35–2 ATR — يلزم تأكيد/إعادة اختبار")
        if net_move_atr >= 2.5:
            reasons.append("امتداد اندفاعي مرتفع خلال 3–5 شموع")
        elif net_move_atr >= 1.8:
            reasons.append("امتداد اندفاعي يحتاج انتظاراً")
        if obstacle_distance_atr is not None and obstacle_distance_atr < 0.70:
            reasons.append("السعر قريب من مقاومة/دعم معاكس حساس")
        if anchor_distance_atr is not None and anchor_distance_atr < 0.25:
            # Continuation entries sitting directly on the anchor are lower quality;
            # reversal entries can use the anchor when confirmed above.
            reasons.append("السعر ملاصق لمنطقة الارتكاز")

        # Immediate execution is intentionally conservative only for geometry.
        # Normal candles with clean directional close and usable room pass.
        severe = range_atr >= 2.0 or net_move_atr >= 2.5
        elevated = range_atr >= 1.35 or net_move_atr >= 1.8
        obstacle_block = obstacle_distance_atr is not None and obstacle_distance_atr < 0.70
        bad_anchor = anchor_distance_atr is not None and anchor_distance_atr < 0.25
        if severe or elevated or obstacle_block or bad_anchor or not directional:
            return {"state":"WAIT", "pass":False, "reason":"؛ ".join(dict.fromkeys(reasons)) or "هندسة الدخول تحتاج تأكيداً",
                    "range_atr":range_atr,"body_ratio":body_ratio,"close_pos":close_pos,
                    "net_move_atr":net_move_atr,"consecutive":consecutive,
                    "anchor_distance_atr":anchor_distance_atr,"obstacle_distance_atr":obstacle_distance_atr,
                    "trigger_state":trigger_state,"liquidity_state":liquidity_state}
        return {"state":"PASS", "pass":True, "reason":"هندسة Entry طبيعية — لا يوجد امتداد أو عائق قريب",
                "range_atr":range_atr,"body_ratio":body_ratio,"close_pos":close_pos,
                "net_move_atr":net_move_atr,"consecutive":consecutive,
                "anchor_distance_atr":anchor_distance_atr,"obstacle_distance_atr":obstacle_distance_atr,
                "trigger_state":trigger_state,"liquidity_state":liquidity_state}
    except Exception as exc:
        return {"state":"NO_TRADE", "reason":f"فشل تقييم هندسة Entry: {type(exc).__name__}", "pass":False}
...
def _daily_execution_core():
    ...
    entry_geometry = _entry_geometry_gate(
        m15_df, direction, float(m15.get("atr",0) or 0), levels,
        trigger_state=trigger_state, liquidity_state=liquidity_state
    ) if direction in ("BUY","SELL") else {"state":"NO_TRADE","pass":False,"reason":"لا يوجد اتجاه تنفيذي صالح"}
    ready = bool(direction in ("BUY","SELL") and setup_score >= MIN_TRADE_SCORE and alignment and m15_adx >= 15 and not liquidity_blocked and rr_ok and entry_geometry.get("pass") is True)
    ...
    if entry_geometry.get("state") != "PASS": setup_reasons.append(f"هندسة Entry: {entry_geometry.get('reason','غير صالحة')}")
...
VERSION = "v19.34_ALERT_DELIVERY_SCHEDULER_TELEMETRY_STRICT_SQLITE_ISOLATION"
ALERT_DELIVERY_SEMANTICS = "AT_LEAST_ONCE"
ALERT_EXACTLY_ONCE_EXTERNAL_SIDE_EFFECT = False
...
# ============================================================
VERSION = "v19.36_ENTRY_GEOMETRY_GATE"
