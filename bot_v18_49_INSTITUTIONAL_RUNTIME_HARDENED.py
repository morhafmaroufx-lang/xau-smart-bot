...
VERSION = "v18.49-INSTITUTIONAL-RUNTIME-HARDENED-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIX-MENU-MARKETS-RUNTIME-CORRECTED"
...
def _inst_http(url,params=None,timeout=None,headers=None):
    h={
        "User-Agent":"Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/124 Safari/537.36 XAU-Smart-Trader/18.49",
        "Accept":"*/*",
    }
...
def _macro_real():
    ...
    effr,effr_date=_latest_series_value(f,'EFFR'); us10y,us10y_date=_latest_series_value(f,'DGS10'); real10y,real10y_date=_latest_series_value(f,'DFII10'); vix,vix_date=_latest_series_value(f,'VIXCLS'); broad_usd,broad_usd_date=_latest_series_value(f,'DTWEXBGS')
    lo_prev=None; hi_prev=None
    if lo is not None:
        x=f[['DATE','DFEDTARL']].dropna(); lo_prev=float(x.iloc[-2].DFEDTARL) if len(x)>1 else lo
    else: lo_prev=lo
    if hi is not None:
        x=f[['DATE','DFEDTARU']].dropna(); hi_prev=float(x.iloc[-2].DFEDTARU) if len(x)>1 else hi
    else: hi_prev=hi
    plo,phi=lo_prev,hi_prev
    ...
    out={'source':'FRED + secondary market feed','policy':pol,'fed_lower':lo,'fed_upper':hi,'effr':effr,
    ...
    out['series_status']={
        ...
        'effr':_inst_status(effr_date,3,'FRED EFFR'), 'us10y':_inst_status(us10y_date,3,'FRED DGS10'),
        'real10y':_inst_status(real10y_date,3,'FRED DFII10'), 'vix':_inst_status(vix_date,3,'FRED VIXCLS'),
...
def institutional_analysis(df):
    ...
    macro_fresh=m.get('status')=='FRESH'
    ...
    real=m.get('real10y')
    if macro_fresh and real is not None:
    ...
    if macro_fresh and (vix is not None or risk in ('RISK-ON','RISK-OFF')):
