...
# v19.38 — Durable MAIN trade storage.
# The path is explicit/configurable and is resolved to an absolute path so the
# persistence target is never accidentally tied to the process working directory.
TRADE_DB_DIR = os.environ.get("TRADE_DB_DIR")
if not TRADE_DB_DIR:
    TRADE_DB_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
TRADE_DB_DIR = os.path.abspath(TRADE_DB_DIR)
os.makedirs(TRADE_DB_DIR, exist_ok=True)
TRADE_DB_PATH = os.path.abspath(os.environ.get("TRADE_DB_PATH") or os.path.join(TRADE_DB_DIR, "trades.db"))
os.makedirs(os.path.dirname(TRADE_DB_PATH), exist_ok=True)
TRADE_DB_WRITE_RETRIES = max(1, int(os.environ.get("TRADE_DB_WRITE_RETRIES", "4")))
TRADE_DB_RETRY_BASE_SECONDS = max(0.05, float(os.environ.get("TRADE_DB_RETRY_BASE_SECONDS", "0.20")))


def _trade_db_connect():
    conn = sqlite3.connect(TRADE_DB_PATH, timeout=10)
    conn.execute("PRAGMA busy_timeout=10000")
    # FULL synchronous mode makes a successful COMMIT durable at the SQLite
    # level instead of relying on the OS/process buffers.
...
def _save_trade_locked(record):
    """Durably persist one MAIN trade with retry + post-commit verification.

    This function changes only storage reliability. It does not alter any
    trading condition, level, score, R:R, or lifecycle decision.
    """
    key = _trade_key(record)
    if not key:
        logger.error("MAIN_TRADE_PERSISTENCE_INVALID_KEY")
        return False
    payload = json.dumps(record, ensure_ascii=False, default=str)
    updated_at = str(record.get("last_update", ""))
    last_exc = None
    for attempt in range(1, TRADE_DB_WRITE_RETRIES + 1):
        conn = None
        try:
            conn = _trade_db_connect()
            conn.execute("BEGIN IMMEDIATE")
            conn.execute(
                "INSERT INTO trades(trade_key,payload,updated_at) VALUES(?,?,?) "
                "ON CONFLICT(trade_key) DO UPDATE SET payload=excluded.payload, updated_at=excluded.updated_at",
                (key, payload, updated_at),
            )
            conn.commit()
            # Verify the exact committed payload before reporting success.
            row = conn.execute("SELECT payload, updated_at FROM trades WHERE trade_key=?", (key,)).fetchone()
            if not row or row[0] != payload or str(row[1]) != updated_at:
                raise sqlite3.DatabaseError("post-commit verification failed")
            logger.debug("MAIN_TRADE_DURABLE_WRITE_OK key=%s attempt=%d", key, attempt)
            return True
        except Exception as exc:
            last_exc = exc
            if conn is not None:
            ...
            logger.exception("MAIN_TRADE_DURABLE_WRITE_FAILED key=%s attempt=%d/%d", key, attempt, TRADE_DB_WRITE_RETRIES)
            if attempt < TRADE_DB_WRITE_RETRIES:
                time.sleep(TRADE_DB_RETRY_BASE_SECONDS * (2 ** (attempt - 1)))
        finally:
            if conn is not None:
                try: conn.close()
                except Exception: pass
    logger.error("MAIN_TRADE_PERSISTENCE_EXHAUSTED key=%s error=%s", key, last_exc)
    return False
...
def register_trade(result):
    ...
    with TRADE_LOCK:
        ...
        for record in reversed(TRADE_HISTORY):
            ...
            record.update({"score":result["score"],"quality":result["quality"],"last_update":now_damascus().isoformat()})
            if not _save_trade_locked(record):
                raise RuntimeError("MAIN trade durable update failed after retries")
            return record,False
        now=now_damascus().isoformat()
        ...
        # Durable-first: never expose a newly created MAIN trade as persisted
        # in memory unless its canonical SQLite row was committed and verified.
        if not _save_trade_locked(new_record):
            raise RuntimeError("MAIN trade durable persistence failed after retries")
...
TRADE_STORAGE_VERSION = "v19.38_DURABLE_WRITE_RETRY_VERIFY"
...
# v19.36 — G.Y AUTO RUNTIME WIRING FIX
# Startup invokes the canonical automatic G.Y engine directly.
VERSION = "v19.36_GY_AUTO_RUNTIME_WIRING_FIXED"
