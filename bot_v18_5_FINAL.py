...
def register_trade(result):
    ...
    for record in reversed(TRADE_HISTORY):
        if record["status"] not in ("OPEN", "TP1"): continue
        ...
        return record, False
    record = {"time": now_damascus().isoformat(), "direction": result["direction"], "score": result["score"], "quality": result["quality"], "entry": t["entry"], "sl": t["sl"], "tp1": t["tp1"], "tp2": t["tp2"], "rr": t["rr"], "status": "OPEN", "result": "OPEN", "last_update": now_damascus().isoformat()}
...
async def trade_now(update, context):
    ...
    try:
        ...
        if not result["signal"]:
            ...
            return
        trade = result["trade"]
        _, is_new_trade = register_trade(result)
        if is_new_trade and not consume_trade(update.effective_chat.id):
...
async def trade_history(update, context):
    ...
    for i, trade in enumerate(reversed(TRADE_HISTORY[-15:]), 1):
        ...
        lines += [
            ...
            f"🎯 TP1: {trade['tp1']:.2f} | TP2: {trade['tp2']:.2f} | R:R 1:{trade['rr']:.2f}",
            f"📌 الحالة: {trade['status']} — {trade['result']}",
            f"🕐 {trade['time']}"
        ]
    await reply(update, "\n".join(lines))
                ...
                if SUBSCRIBERS:
                    ...
                    if result.get("signal") and not result.get("news_blocked") and result.get("trade"):
                        ...
                        for chat_id in list(SUBSCRIBERS):
                            if is_new_trade and not can_receive_trade(chat_id):
                                continue
                            try:
                                title = "🚨 إشارة ذهب — صفقة جديدة" if is_new_trade else "🔄 تحديث الصفقة — الفحص الدوري"
                                await APPLICATION.bot.send_message(chat_id=chat_id, text=_format_trade_message(result, title))
                                if is_new_trade:
