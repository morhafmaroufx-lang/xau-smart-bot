...
def _cancel_admin_free_grant(chat_id, cancelled_by):
    """Cancel the active administrative free grant and restore the exact prior user state."""
    cid = int(chat_id)
    admin_id = int(cancelled_by)
...
async def admin_user_details(update, context, chat_id):
    ...
    rows = [
        ...
        [("❌ إلغاء المنحة المجانية", "admin_cancel_free_grant")],
    ...
    if data == "admin_cancel_free_grant":
        ...
        try:
            result = await asyncio.to_thread(
                _cancel_admin_free_grant, cid, update.effective_user.id
            )
            if not result.get("cancelled"):
                await reply(update, "ℹ️ لا توجد منحة مجانية إدارية نشطة لهذا الحساب.")
                return
            await reply(update, with_market_header(
                "✅ تم إلغاء المنحة المجانية بنجاح\n\n"
                f"• Chat ID: {cid}\n"
                f"• الباقة الملغاة: {result['grant_plan']}\n"
                f"• الحالة المستعادة: {result['previous_plan']}"
            ))
        except Exception as exc:
            await reply(update, f"❌ تعذر إلغاء المنحة: {exc}")
        return
