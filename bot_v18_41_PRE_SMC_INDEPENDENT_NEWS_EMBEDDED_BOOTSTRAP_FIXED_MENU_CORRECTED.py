...
VERSION = "v18.41-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED-BOOTSTRAP-FIX-MENU-CORRECTED"
...
async def markets(update, context):
    """عرض جلسات الأسواق بتوقيت دمشق، مع تحويل حي يحترم التوقيت الصيفي."""
    if not await feature_guard("markets")(update, context): return
    now = now_damascus()

    def damascus_time(zone_name, hour, minute=0):
        local = now.astimezone(ZoneInfo(zone_name)).replace(hour=hour, minute=minute, second=0, microsecond=0)
        t = local.astimezone(DAMASCUS_TZ).strftime('%I:%M %p').lstrip('0')
    ...
    lines = ["🌍 الجلسات والأسواق", "", f"🕐 توقيت دمشق الآن: {now.strftime('%I:%M %p').lstrip('0').replace('AM', 'ص').replace('PM', 'م')}", "", "🟢 أوقات الافتتاح بتوقيت دمشق"]
    for label, zone, hour in sessions:
        lines.append(f"• {label}: {damascus_time(zone, hour)}")
    state, code = market_state(now)
    lines += ["", "📊 حالة السوق", f"• {state}", "", "ℹ️ الأوقات تُحسب تلقائياً وفق التوقيت الصيفي للدول المعنية."]
    await reply(update, "\n".join(lines))
