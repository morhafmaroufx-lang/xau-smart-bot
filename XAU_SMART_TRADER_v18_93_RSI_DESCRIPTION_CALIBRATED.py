...
def _data_quality_score(frames_map, price, levels):
    ...
    for name in expected:
        ...
        if not bars or float(bars)>=60: sample_ok+=1
        age=x.get('age_seconds',x.get('staleness_seconds'))
        if age is None or float(age)<=freshness_limits[name]: freshness_ok+=1
...
def _scenario_score(frames_data, frames, direction, levels, price, atr, horizon=None):
    ...
    trend=20 if xs and support==len(xs) else 15 if xs and support/len(xs)>=.66 else 8 if support else 0
    ...
    ss=sum(1 for x in xs if x.get('structure')==expected)
    structure=20 if xs and ss==len(xs) else 15 if ss>=2 else 8 if ss==1 else 0
    if structure: factors.append(f'الهيكل {expected} داعم ({ss}/{len(xs)})')
    if horizon is None: horizon='weekly' if any(n=='W1' for n,_ in frames) else 'daily'
    liq_name='H1' if horizon=='daily' else 'D1'
    liq=(frames_data.get(liq_name,{}) or {}).get('liquidity',{}) or {}
    wanted='SELL_SIDE_SWEEP' if direction=='BUY' else 'BUY_SIDE_SWEEP'
    maturity,state=_trigger_maturity(frames_data,direction,horizon)
    liquidity=0
    if liq.get('sweep')==wanted: liquidity=min(20, 6 + int(maturity*0.14)); factors.append(f'دورة السيولة داعمة ({state})')
    elif state=='INVALIDATED': factors.append('حالة السيولة ملغاة')
    ...
    trigger=int(round(min(100,maturity)*0.15))
    if trigger: factors.append(f'نضج المحفز ({state}) — ({trigger}/15)')
    scenario_type=_scenario_type(direction,frames_data,levels,price)
    if direction=='BUY': obstacle=nearest_resistance(levels,price); anchor=nearest_support(levels,price)
    else: obstacle=nearest_support(levels,price); anchor=nearest_resistance(levels,price)
    ...
    # قرب anchor إيجابي في الانعكاس، ومحايد/سلبي في الاستمرار حسب العائق.
    ...
    if location: factors.append(f'جودة موقع الدخول ({location}/10) — {scenario_type}')
