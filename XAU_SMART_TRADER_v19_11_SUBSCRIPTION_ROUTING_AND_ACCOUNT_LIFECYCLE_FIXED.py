...
# ============================================================
# v19.10 — ADMIN ACCOUNT STATUS / FREE GRANT CONTROL
# Built directly on the known-working v19.04 administration path.
# Scope: administration only. Trading/SR/alerts/scheduler/webhook untouched.
# ============================================================
V19_10_VERSION = "v19.10_ADMIN_ACCOUNT_STATUS_CONTROL"
V19_11_VERSION = "v19.11_SUBSCRIPTION_ROUTING_AND_ACCOUNT_LIFECYCLE_FIXED"
ADMIN_FREE_GRANT_TABLE = "admin_free_grants"


def _v19_10_ensure_grant_table():
    conn = _db()
    try:
        conn.execute(f"""
            CREATE TABLE IF NOT EXISTS {ADMIN_FREE_GRANT_TABLE} (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                chat_id INTEGER NOT NULL,
                plan TEXT NOT NULL CHECK(plan IN ('BASIC','ELITE')),
                granted_at TEXT NOT NULL,
                expiry_date TEXT NOT NULL,
                granted_by INTEGER NOT NULL,
                previous_plan TEXT NOT NULL,
        ...
        """)
        conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{ADMIN_FREE_GRANT_TABLE}_active ON {ADMIN_FREE_GRANT_TABLE}(chat_id,active,expiry_date)")
        conn.commit()
    finally:
        conn.close()


def _v19_10_active_grant(chat_id):
    try:
        cid = int(chat_id)
    except Exception:
        return None
    _v19_10_ensure_grant_table()
    now = now_damascus()
    conn = _db()
    try:
        row = conn.execute(
...
def _v19_10_reconcile_grant(chat_id):
    try:
        cid = int(chat_id)
    except Exception:
        return
    ...
    try:
        ...
        for g in rows:
            ...
            conn.execute(
                ...
                (
                    g["previous_plan"] if g["previous_plan"] in PLANS else "FREE",
                    g["previous_status"] or "active",
                    g["previous_start_date"],
                    g["previous_expiry_date"],
                    now.isoformat(), cid,
                ),
            )
            conn.execute(f"UPDATE {ADMIN_FREE_GRANT_TABLE} SET active=0 WHERE id=?", (g["id"],))
        conn.commit()
    finally:
        conn.close()


def _v19_10_grant_label(grant):
    if not grant:
        return None
    plan = str(grant.get("plan", "")).upper()
    if plan == "BASIC":
        return "FREE+1"
    if plan == "ELITE":
        return "FREE+2"
...
# Effective feature access: free grants are temporary overlays on the normal account.
_V19_10_LEGACY_HAS_FEATURE = has_feature

def has_feature(chat_id, feature):
    if is_admin_chat(chat_id):
        return True
    row = get_member(chat_id)
    if row and str(row["status"]).lower() == "blocked":
        return False
    _v19_10_reconcile_grant(chat_id)
    grant = _v19_10_active_grant(chat_id)
    if grant:
        plan = str(grant["plan"]).upper()
        return _canonical_feature(feature) in PLANS[plan]["features"]
    return _V19_10_LEGACY_HAS_FEATURE(chat_id, feature)


_V19_10_LEGACY_PLAN_STATUS = plan_status_text

def plan_status_text(chat_id):
    if is_admin_chat(chat_id):
        return _V19_10_LEGACY_PLAN_STATUS(chat_id)
    row = get_member(chat_id)
    if row and str(row["status"]).lower() == "blocked":
    ...
    if grant:
        ...
        hours = rem // 3600
        label = _v19_10_grant_label(grant)
        return (
            f"🆓 {label} — 🎁 منحة {PLANS[plan]['name']} مجانية\n"
            f"⏳ المدة المتبقية: {days} يوم و{hours} ساعة\n"
            f"📅 تنتهي: {expiry.strftime('%Y-%m-%d %H:%M')} دمشق\n"
            f"🎯 الصفقات: {TRADE_LIMIT_TEXT[plan]}"
        )
    return _V19_10_LEGACY_PLAN_STATUS(chat_id)


def _v19_10_grant_free_plan(chat_id, days, granted_by, grant_plan):
    cid, duration, admin_id = int(chat_id), int(days), int(granted_by)
    grant_plan = str(grant_plan).upper().strip()
    if duration <= 0:
        raise ValueError("عدد الأيام يجب أن يكون أكبر من صفر")
    if grant_plan not in {"BASIC", "ELITE"}:
        raise ValueError("الباقة يجب أن تكون BASIC أو ELITE")
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    now = now_damascus(); _v19_10_reconcile_grant(cid)
    expiry = now + timedelta(days=duration)
    _v19_10_ensure_grant_table()
    conn = _db()
    try:
        user = conn.execute("SELECT chat_id,plan,status,start_date,expiry_date FROM users WHERE chat_id=?", (cid,)).fetchone()
        if not user:
            conn.execute(
                "INSERT INTO users(chat_id,plan,status,referral_code,created_at,updated_at) VALUES(?,?,?,?,?,?)",
                (cid, "FREE", "active", f"ref_{cid}", now.isoformat(), now.isoformat()),
            ...
            user = conn.execute("SELECT chat_id,plan,status,start_date,expiry_date FROM users WHERE chat_id=?", (cid,)).fetchone()
        active = conn.execute(f"SELECT * FROM {ADMIN_FREE_GRANT_TABLE} WHERE chat_id=? AND active=1 ORDER BY id DESC LIMIT 1", (cid,)).fetchone()
        if active:
            base = datetime.fromisoformat(str(active["expiry_date"]))
            new_expiry = max(base, now) + timedelta(days=duration)
            conn.execute(f"UPDATE {ADMIN_FREE_GRANT_TABLE} SET plan=?,expiry_date=?,granted_by=? WHERE id=?", (grant_plan,new_expiry.isoformat(),admin_id,active["id"]))
            conn.execute("UPDATE users SET plan=?,status='active',expiry_date=?,updated_at=? WHERE chat_id=?", (grant_plan,new_expiry.isoformat(),now.isoformat(),cid))
            conn.commit()
            return {"ok":True,"expiry":new_expiry,"extended":True,"grant_plan":grant_plan}
        conn.execute(
            f"""INSERT INTO {ADMIN_FREE_GRANT_TABLE}
            (chat_id,plan,granted_at,expiry_date,granted_by,previous_plan,previous_status,previous_start_date,previous_expiry_date,active)
            VALUES(?,?,?,?,?,?,?,?,?,1)""",
            (cid,grant_plan,now.isoformat(),expiry.isoformat(),admin_id,
             user["plan"] if user["plan"] in PLANS else "FREE",user["status"] or "active",user["start_date"],user["expiry_date"]),
        )
        conn.execute("UPDATE users SET plan=?,status='active',start_date=?,expiry_date=?,updated_at=? WHERE chat_id=?", (grant_plan,now.isoformat(),expiry.isoformat(),now.isoformat(),cid))
        conn.commit()
        return {"ok":True,"expiry":expiry,"extended":False,"grant_plan":grant_plan}
    finally:
        conn.close()


def _v19_10_cancel_free_grant(chat_id, admin_id):
    cid = int(chat_id)
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    _v19_10_ensure_grant_table()
    conn = _db()
    try:
        grant = conn.execute(f"SELECT * FROM {ADMIN_FREE_GRANT_TABLE} WHERE chat_id=? AND active=1 ORDER BY id DESC LIMIT 1", (cid,)).fetchone()
        if not grant:
            return {"ok":False}
        conn.execute(
            "UPDATE users SET plan=?,status=?,start_date=?,expiry_date=?,updated_at=? WHERE chat_id=?",
            (grant["previous_plan"] if grant["previous_plan"] in PLANS else "FREE",grant["previous_status"] or "active",grant["previous_start_date"],grant["previous_expiry_date"],now_damascus().isoformat(),cid),
        )
        conn.execute(f"UPDATE {ADMIN_FREE_GRANT_TABLE} SET active=0 WHERE id=?", (grant["id"],))
        conn.commit()
        return {"ok":True,"restored_plan":grant["previous_plan"],"restored_status":grant["previous_status"]}
    finally:
        conn.close()


def _v19_10_cancel_subscription(chat_id, admin_id):
    cid = int(chat_id)
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    conn = _db()
    try:
        row = conn.execute("SELECT plan,status FROM users WHERE chat_id=?", (cid,)).fetchone()
        if not row:
            return False
        conn.execute("UPDATE users SET plan='FREE',status='active',start_date=NULL,expiry_date=NULL,updated_at=? WHERE chat_id=?", (now_damascus().isoformat(),cid))
        conn.commit()
        return True
    finally:
        conn.close()
...
def _v19_10_set_blocked(chat_id, blocked, admin_id):
    cid = int(chat_id)
    if not is_admin_chat(admin_id):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    ...
    try:
        ...
        if not row:
            return False
        conn.execute("UPDATE users SET status=?,updated_at=? WHERE chat_id=?", ("blocked" if blocked else "active",now_damascus().isoformat(),cid))
        conn.commit()
        return True
    finally:
        conn.close()
...
async def admin_user_grant_menu(update, context):
    ...
    cid = context.user_data.get("admin_selected_user")
    if not cid:
        await reply(update,"⛔ لم يتم تحديد حساب."); return
    row = await asyncio.to_thread(_admin_user_lookup,cid)
    if not row:
        await reply(update,"🔎 لم يتم العثور على هذا الحساب."); return
    context.user_data["admin_mode"]="grant_free_plan"
    text=("🎁 منح اشتراك مجاني\n\n"
          f"• Chat ID: {row['chat_id']}\n"
          f"• الحالة الحالية: {row['plan']} / {row['status']}\n\n"
          "اختر نوع المنحة:\n"
          "🆓 FREE+1 = منحة BASIC مجانية\n"
          "🆓 FREE+2 = منحة ELITE مجانية\n\n"
          "بعد الاختيار أرسل عدد الأيام المطلوب.")
    rows=[[('🥉 منح BASIC مجانًا','admin_grant_free_basic')],
          [('💎 منح ELITE مجانًا','admin_grant_free_elite')],
          [('🔙 بيانات المستخدم','admin_user_back')]]
    await _admin_render(update,text,rows)
...
async def admin_user_details(update, context, chat_id):
    ...
    context.user_data["admin_selected_user"]=int(chat_id)
    context.user_data.pop("admin_mode",None)
    grant=await asyncio.to_thread(_v19_10_active_grant,chat_id)
    label=_v19_10_grant_label(grant)
    effective=(f"🆓 {label}" if label else row["plan"])
    lines=["👥 بيانات المستخدم","",f"• Chat ID: {row['chat_id']}",
           f"• الاسم: {row['first_name'] or '—'}",
           f"• Username: @{row['username']}" if row['username'] else "• Username: —",
           f"• الباقة الفعلية: {effective}",
           f"• الحالة: {row['status']}",
           f"• البداية: {row['start_date'] or '—'}",
           f"• الانتهاء: {row['expiry_date'] or '—'}"]
    if grant:
        lines += [f"• نوع المنحة: {'BASIC مجانية' if grant['plan']=='BASIC' else 'ELITE مجانية'}",
                  f"• انتهاء المنحة: {grant['expiry_date']}"]
    rows=[[('🎁 منح اشتراك مجاني','admin_grant_free_menu')],
          [('❌ إلغاء الباقة المجانية','admin_cancel_free_grant')],
          [('🚫 إلغاء اشتراك الحساب','admin_cancel_subscription')],
          [('⛔ حظر الحساب','admin_block_user')] if str(row['status']).lower()!='blocked' else [('🟢 فك حظر الحساب','admin_unblock_user')],
          [('🔙 إدارة المستخدمين','admin_users'),('👑 لوحة الإدارة','admin_home')]]
    await _admin_render(update,"\n".join(lines),rows)
...
async def admin_users(update, context):
    ...
    try:
        grant_rows=conn.execute(
            f"""SELECT plan,status,COUNT(*) n FROM users u
            JOIN {ADMIN_FREE_GRANT_TABLE} g ON g.chat_id=u.chat_id
    ...
    if grant_rows:
        ...
        for r in grant_rows:
            marker="FREE+1" if r["plan"]=="BASIC" else "FREE+2"
            lines.append(f"• {marker} / {r['status']}: {r['n']}")
    context.user_data["admin_mode"]="search_user"
    lines += ["","🔎 أرسل Chat ID للبحث عن حساب محدد."]
    await _admin_render(update,"\n".join(lines),"users")
...
# v19.11 direct user subscription cancellation.
async def user_cancel_subscription_v19_11(update, context):
    cid = update.effective_chat.id
    try:
        result = await asyncio.to_thread(_v19_10_cancel_subscription, cid, cid)
        if result:
            msg = "✅ تم إلغاء الاشتراك.\n\n🔄 عاد حسابك الآن إلى 🆓 FREE.\n🎯 يمكنك الاشتراك مجدداً في أي وقت."
        else:
            msg = "ℹ️ الحساب غير موجود أو هو بالفعل على 🆓 FREE."
        await reply(update, with_market_header(msg))
    except Exception as exc:
        await reply(update, with_market_header(
            f"❌ تعذر إلغاء الاشتراك.\nالسبب: {type(exc).__name__}: {exc}"
        ))

# Final administrative callback layer, replacing only the v19.04 callback object.
_V19_10_LEGACY_CALLBACK = callback_router
async def callback_router(update, context):
    q=getattr(update,"callback_query",None)
    if not q: return
    data=(q.data or "").strip()

    # v19.11: deterministic subscription callback dispatch.
    # These routes are handled before the historical callback chain.
    if data in {"open_subscription", "menu_subscription", "my_subscription"}:
        await q.answer()
        await my_subscription(update, context)
        return
    if data == "user_cancel_subscription":
        await q.answer()
        await user_cancel_subscription_v19_11(update, context)
        return
    if data in {"open_plans", "menu_plans"}:
        await q.answer()
        await plans(update, context)
        return
    ...
    admin_map={
        ...
        "admin_database":admin_database,
        "admin_scheduler":admin_scheduler,
        "admin_grant_free_menu":admin_user_grant_menu,
    }
    if data=="admin_home":
        await q.answer(); await admin_dashboard(update,context); return
    if data in {"admin_grant_free_basic","admin_grant_free_elite"}:
        await q.answer()
        context.user_data["admin_grant_plan"]="BASIC" if data.endswith("basic") else "ELITE"
        context.user_data["admin_mode"]="grant_free_plan"
        await reply(update,f"{'🥉 BASIC' if context.user_data['admin_grant_plan']=='BASIC' else '💎 ELITE'} — اخترت منحة مجانية.\n\n✏️ أرسل عدد الأيام المطلوب.")
        return
    if data=="admin_user_back":
        await q.answer()
        cid=context.user_data.get("admin_selected_user")
        await (admin_user_details(update,context,cid) if cid else admin_users(update,context)); return
    if data in {"admin_cancel_free_grant","admin_cancel_subscription","admin_block_user","admin_unblock_user"}:
        await q.answer()
        if not await _admin_guard(update): return
        cid=context.user_data.get("admin_selected_user")
        if not cid:
        ...
        try:
            if data=="admin_cancel_free_grant":
                result=await asyncio.to_thread(_v19_10_cancel_free_grant,cid,update.effective_user.id)
                msg="ℹ️ لا توجد باقة مجانية نشطة لهذا الحساب." if not result["ok"] else "✅ تم إلغاء الباقة المجانية واستعادة الحالة السابقة."
            elif data=="admin_cancel_subscription":
                ok=await asyncio.to_thread(_v19_10_cancel_subscription,cid,update.effective_user.id)
                msg="✅ تم إلغاء اشتراك الحساب وإعادته إلى FREE." if ok else "🔎 لم يتم العثور على الحساب."
            elif data=="admin_block_user":
                ok=await asyncio.to_thread(_v19_10_set_blocked,cid,True,update.effective_user.id)
                msg="⛔ تم حظر الحساب." if ok else "🔎 لم يتم العثور على الحساب."
            else:
                ok=await asyncio.to_thread(_v19_10_set_blocked,cid,False,update.effective_user.id)
                msg="🟢 تم فك حظر الحساب." if ok else "🔎 لم يتم العثور على الحساب."
        ...
        return
    if data.startswith("plan_") or data.startswith("request_") or data=="back_to_plans":
        await _V19_10_LEGACY_CALLBACK(update,context); return
    await _V19_10_LEGACY_CALLBACK(update,context)


_V19_10_LEGACY_ROUTER=router
async def router(update,context):
    text=(update.effective_message.text or "").strip() if getattr(update,"effective_message",None) else ""
    # v19.11: deterministic direct routing for subscription buttons.
    # The production file contains multiple historical router layers; the
    # subscription action must never depend on legacy fall-through.
    direct_subscription_routes = {
        "👤 الاشتراك": my_subscription,
        "💳 الاشتراك": my_subscription,
    ...
    direct_fn = direct_subscription_routes.get(text)
    if direct_fn is not None:
        await direct_fn(update, context)
        return
    mode=context.user_data.get("admin_mode") if getattr(context,"user_data",None) is not None else None
    if mode=="grant_free_plan":
        if not _admin_ok(update):
        ...
        cid=context.user_data.get("admin_selected_user"); grant_plan=context.user_data.get("admin_grant_plan")
        if not cid or grant_plan not in {"BASIC","ELITE"}:
            context.user_data.pop("admin_mode",None); context.user_data.pop("admin_grant_plan",None)
            await reply(update,"⛔ لم يتم تحديد الحساب أو نوع المنحة."); return
        context.user_data.pop("admin_mode",None); context.user_data.pop("admin_grant_plan",None)
        try:
            result=await asyncio.to_thread(_v19_10_grant_free_plan,cid,int(text),update.effective_user.id,grant_plan)
            marker="FREE+1" if grant_plan=="BASIC" else "FREE+2"
            await reply(update,with_market_header(
                f"✅ تم منح {marker} بنجاح\n\n• الحساب: {cid}\n• المنحة: {'BASIC' if grant_plan=='BASIC' else 'ELITE'} مجانية\n• المدة: {int(text)} يوم\n• الانتهاء: {result['expiry'].strftime('%Y-%m-%d %H:%M')} دمشق"
            ))
        except Exception as exc:
            await reply(update,f"❌ تعذر تنفيذ المنحة.\nالسبب: {type(exc).__name__}: {exc}")
        return
    if text=="👑 الإدارة":
        await admin_dashboard(update,context); return
    await _V19_10_LEGACY_ROUTER(update,context)


# Replace the startup function with the v19.04 startup body while resolving the
# callback/router names at runtime, after every v19.10 definition above exists.
_V19_10_LEGACY_START=integrated_start_bot
async def integrated_start_bot():
    global APPLICATION, SR_AUTO_TASK
    if not TOKEN: raise RuntimeError("TELEGRAM_TOKEN غير موجود في Render.")
    APPLICATION=Application.builder().token(TOKEN).build()
    ...
    APPLICATION.add_handler(CommandHandler("plans",_trace_handler(plans,"command:/plans")))
    APPLICATION.add_handler(CommandHandler("subscription",_trace_handler(my_subscription,"command:/subscription")))
    APPLICATION.add_handler(CommandHandler("unsubscribe",_trace_handler(user_cancel_subscription,"command:/unsubscribe")))
    APPLICATION.add_handler(CommandHandler("referral",_trace_handler(referral,"command:/referral")))
    APPLICATION.add_handler(CommandHandler("admin",_trace_handler(admin_command,"command:/admin")))
    APPLICATION.add_handler(CommandHandler("id",_trace_handler(user_id_command,"command:/id")))
    APPLICATION.add_handler(CommandHandler("audit",_trace_handler(audit_report,"command:/audit")))
    ...
    await APPLICATION.initialize(); await APPLICATION.start(); await _start_alert_dispatcher()
    await APPLICATION.bot.set_webhook(url=WEBHOOK_URL,allowed_updates=["message","callback_query"],drop_pending_updates=True)
    logger.info("XAU SMART TRADER %s started | subscription routing fixed",V19_11_VERSION)
    asyncio.create_task(auto_loop(),name="main-trading-loop")
    SR_AUTO_TASK=asyncio.create_task(_sr_auto_loop(),name="gy-private-lifecycle-monitor")
    if PERFORMANCE_AUDITOR is not None:
        try:
...
def run_v19_10_self_tests():
    failures=[]
    def check(name,cond):
        if not cond: failures.append(name)
    check("version",V19_10_VERSION=="v19.10_ADMIN_ACCOUNT_STATUS_CONTROL")
    check("plans",set(PLANS)=={"FREE","BASIC","ELITE"})
    check("free_plus_1",_v19_10_grant_label({"plan":"BASIC"})=="FREE+1")
    check("free_plus_2",_v19_10_grant_label({"plan":"ELITE"})=="FREE+2")
    check("admin_users",callable(admin_users))
    check("grant_menu",callable(admin_user_grant_menu))
    check("details_controls",all(x in admin_user_details.__code__.co_consts for x in ("🎁 منح اشتراك مجاني","❌ إلغاء الباقة المجانية","🚫 إلغاء اشتراك الحساب","⛔ حظر الحساب","🟢 فك حظر الحساب")))
    check("callback_admin_users", "admin_users" in callback_router.__code__.co_consts)
    check("callback_cancel", "admin_cancel_subscription" in callback_router.__code__.co_consts)
    check("callback_block", "admin_block_user" in callback_router.__code__.co_consts)
    check("startup_callback", "callback:router" in integrated_start_bot.__code__.co_consts)
    return {"ok":not failures,"passed":10-len(failures),"total":10,"failures":failures}



def run_v19_11_self_tests():
    failures=[]
    def check(name, cond):
        if not cond: failures.append(name)
    check("version", V19_11_VERSION=="v19.11_SUBSCRIPTION_ROUTING_AND_ACCOUNT_LIFECYCLE_FIXED")
    check("direct_subscription_text_route", "👤 الاشتراك" in router.__code__.co_consts)
    check("direct_subscription_callback_route", "open_subscription" in callback_router.__code__.co_consts)
    check("direct_plans_route", "💳 الباقات" in router.__code__.co_consts)
    check("cancel_callback_route", "user_cancel_subscription" in callback_router.__code__.co_consts)
    check("unsubscribe_handler", 'CommandHandler("unsubscribe"' in integrated_start_bot.__code__.co_consts)
    check("grant_basic", _v19_10_grant_label({"plan":"BASIC"})=="FREE+1")
    check("grant_elite", _v19_10_grant_label({"plan":"ELITE"})=="FREE+2")
    check("admin_cancel", callable(_v19_10_cancel_subscription))
    check("admin_block", callable(_v19_10_set_blocked))
    return {"ok":not failures,"passed":10-len(failures),"total":10,"failures":failures}

def main_v19_11():
    if "--v19-11-self-test" in sys.argv:
        print(json.dumps(run_v19_11_self_tests(),ensure_ascii=False,indent=2)); return
    global BOT_LOOP
    server=threading.Thread(target=run_flask,daemon=True); server.start()
    loop=asyncio.new_event_loop(); BOT_LOOP=loop; asyncio.set_event_loop(loop)
    try: loop.run_until_complete(integrated_start_bot())
