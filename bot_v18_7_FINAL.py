...
def build_trade(direction, h1, m15, levels):
    ...
    if direction == "BUY":
        ...
        if r2 and tp1 < r2["price"] <= entry + max_tp2:
            ...
            tp2 = natural_tp2
        tp3 = max(tp2 + atr * 0.80, entry + max(atr * 2.60, risk * 2.20))
        if r2 and r2["price"] > tp2 and r2["price"] <= entry + max_tp3:
            tp3 = max(tp3, r2["price"])
    elif direction == "SELL":
        entry = price
        ...
        if s2 and tp1 > s2["price"] >= entry - max_tp2:
            ...
            tp2 = natural_tp2
        tp3 = min(tp2 - atr * 0.80, entry - max(atr * 2.60, risk * 2.20))
        if s2 and s2["price"] < tp2 and s2["price"] >= entry - max_tp3:
            tp3 = min(tp3, s2["price"])
    else:
        return None

    risk = abs(entry - sl)
    # ضمان ترتيب الأهداف وعدم تقاطعها مع بعضها.
    if direction == "BUY":
        tp2 = max(tp2, tp1 + atr * 0.25)
        tp3 = max(tp3, tp2 + atr * 0.50)
    else:
        tp2 = min(tp2, tp1 - atr * 0.25)
        tp3 = min(tp3, tp2 - atr * 0.50)
...
def register_trade(result):
    ...
    t = result["trade"]
    atr = max(float(result.get("mtf", {}).get("m15", {}).get("atr", 0.0) or 0.0), 0.50)
    entry_tol = max(atr * 0.30, result["price"] * 0.00025, 0.50)
    sl_tol = max(atr * 0.45, result["price"] * 0.00035, 0.75)
    tp_tol = max(atr * 0.55, result["price"] * 0.00045, 1.00)
    for record in reversed(TRADE_HISTORY):
        if record["status"] not in ("ACTIVE", "TP1", "TP2"): continue
        if record["direction"] != result["direction"]: continue
        if abs(record["entry"] - t["entry"]) > entry_tol: continue
        if abs(record["sl"] - t["sl"]) > sl_tol: continue
        if abs(record["tp1"] - t["tp1"]) > tp_tol: continue
        if abs(record["tp2"] - t["tp2"]) > tp_tol * 1.5: continue
        if abs(record["tp3"] - t["tp3"]) > tp_tol * 2.0: continue
        record.update({"score": result["score"], "quality": result["quality"], "last_update": now_damascus().isoformat()})
        return record, False
    record = {
        "time": now_damascus().isoformat(), "direction": result["direction"],
        "score": result["score"], "quality": result["quality"],
        "entry": t["entry"], "sl": t["sl"], "tp1": t["tp1"],
        "tp2": t["tp2"], "tp3": t["tp3"], "rr": t["rr"],
        "status": "ACTIVE", "result": "OPEN",
        "last_update": now_damascus().isoformat(), "tp1_time": None, "tp2_time": None, "close_time": None
    }
...
def update_trade_results():
    ...
    for trade in TRADE_HISTORY:
        if trade["status"] not in ("ACTIVE", "TP1", "TP2"): continue
        if trade["direction"] == "BUY":
            if price <= trade["sl"]:
                trade["status"] = "CLOSED"; trade["result"] = "LOSS"; trade["close_time"] = now
            elif price >= trade["tp3"]:
                trade["status"] = "CLOSED"; trade["result"] = "TP3 / WIN"; trade["close_time"] = now
                trade["tp3_time"] = now
            elif price >= trade["tp2"]:
                if trade["status"] != "TP2": trade["tp2_time"] = now
                trade["status"] = "TP2"; trade["result"] = "TP2"
            elif price >= trade["tp1"]:
                if trade["status"] == "ACTIVE": trade["tp1_time"] = now
                trade["status"] = "TP1"; trade["result"] = "TP1"
        else:
            if price >= trade["sl"]:
                trade["status"] = "CLOSED"; trade["result"] = "LOSS"; trade["close_time"] = now
            elif price <= trade["tp3"]:
                ...
                if trade["status"] == "ACTIVE": trade["tp1_time"] = now
                trade["status"] = "TP1"; trade["result"] = "TP1"
        trade["last_price"] = price; trade["last_update"] = now
...
def _make_scenarios(frames_data, levels, price, atr, horizon):
    ...
    if direction == "BUY":
        ...
        primary = {"title": "استمرار الاتجاه الهابط / البيع من المقاومة", "quality": sell_score,
                   ...
                   "trigger": _confirmation_text(confirm, r1, "SELL", "تأكيد الثبات تحت المقاومة") if r1 else "تأكيد هابط من منطقة مقاومة واضحة.",
            ...
            if AUTO_ENABLED:
                ...
                if SUBSCRIBERS:
                    ...
                    if result.get("signal") and not result.get("news_blocked") and result.get("trade"):
                        eligible_new = [c for c in list(SUBSCRIBERS) if can_receive_trade(c)]
                        existing_record, already_exists = register_trade(result)
                        if already_exists or eligible_new:
                            for chat_id in list(SUBSCRIBERS):
                                if not already_exists and not can_receive_trade(chat_id):
                                    continue
                                try:
                                    title = "🚨 إشارة ذهب — صفقة جديدة" if not already_exists else "🔄 تحديث الصفقة — الفحص الدوري"
                                    await APPLICATION.bot.send_message(chat_id=chat_id, text=_format_trade_message(result, title, existing_record))
                                    if not already_exists:
                                        consume_trade(chat_id)
                                    LAST_SIGNAL[chat_id] = (result["direction"], round(result["trade"]["entry"], 1), round(result["trade"]["tp1"], 1), round(result["trade"]["sl"], 1))
                                except Exception:
