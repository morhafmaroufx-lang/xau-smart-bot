...
def _db():
    ...
    # Backward-compatible migration: older bot databases may already contain the
    # users table without the newer subscription columns. CREATE TABLE IF NOT EXISTS
    # does NOT migrate an existing SQLite table, which previously caused feature_guard()
    # to fail before every normal menu (while /audit, which bypasses feature_guard,
    # continued to work). Migrate only missing nullable columns; never drop/overwrite data.
    ...
    for column, sql_type in required_user_columns.items():
        if column not in existing_user_columns:
            # All migration columns are nullable, so SQLite permits ADD COLUMN safely.
            conn.execute(f"ALTER TABLE users ADD COLUMN {column} {sql_type}")

    required_usage_columns = {
        "chat_id": "INTEGER", "feature": "TEXT", "used_at": "TEXT"
    }
    existing_usage_columns = {row[1] for row in conn.execute("PRAGMA table_info(usage)").fetchall()}
    for column, sql_type in required_usage_columns.items():
        if column not in existing_usage_columns:
            conn.execute(f"ALTER TABLE usage ADD COLUMN {column} {sql_type}")
...
def webhook():
    ...
    try:
        ...
        logger.exception("Webhook scheduling error")
