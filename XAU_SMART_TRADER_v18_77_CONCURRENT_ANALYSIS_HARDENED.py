# ============================================================
# XAU SMART TRADER v18.77
# v18.77: Concurrent Analysis + Request/Handler Metrics Hardening
# تحسينات أداء معزولة: منع تكرار جلب البيانات، توحيد التحليل المتزامن، وفصل قياس Webhook عن Handler.
# لا تغيير في استراتيجية التداول أو S/R أو Score أو Thresholds.
...
class Metrics:
    def __init__(self,db):
        ...
        # Webhook/request latency and handler latency are separate signals.
        # The previous implementation mixed both into one ring, making a slow
        # handler appear as a slow HTTP request. That produced misleading P95 alerts.
    ...
    def record_request(self,latency_ms):
        # True webhook/HTTP admission latency only.
        with self._lock:
            self.latencies.append(max(0.0,float(latency_ms)))
            self.request_count+=1
            self.window_requests+=1
    def record_handler(self,latency_ms):
        # Handler execution latency is intentionally independent from HTTP latency.
        with self._lock:
            self.handler_latencies.append(max(0.0,float(latency_ms)))
    ...
    def system_snapshot(self,queue_depth=0,queue_capacity=0,queue_drops=0):
        ...
        return {"uptime_s":now-self.started_mono,"cpu_pct":cpu_pct,"rss_mb":rss,"event_loop_lag_ms":_percentile(loops,95),"event_loop_lag_max_ms":max(loops) if loops else 0.0,"request_avg_ms":sum(req)/len(req) if req else 0.0,"request_p95_ms":_percentile(req,95),"request_max_ms":max(req) if req else 0.0,"handler_avg_ms":sum(handlers)/len(handlers) if handlers else 0.0,"handler_p95_ms":_percentile(handlers,95),"handler_max_ms":max(handlers) if handlers else 0.0,"request_count":req_count,"handler_count":handler_count,"throughput_per_min":throughput,"price_latency_ms":sum(pls)/len(pls) if pls else self.last_price_latency_ms,"db_write_latency_ms":sum(dbs)/len(dbs) if dbs else 0.0,"price_age_s":price_age,"queue_depth":queue_depth,"queue_capacity":queue_capacity,"queue_drops":queue_drops,"error_count":errors,"alert_count":alerts,"self_heal_count":heals,"auditor_heartbeat":hb}
    @staticmethod
    def _rss_mb():
        try:
    ...
    def report(self):
        with self.db.connect() as c:
            total=c.execute("SELECT COUNT(*) FROM Live_Trades").fetchone()[0]
            rows=c.execute("SELECT final_result FROM Live_Trades").fetchall(); analyses=c.execute("SELECT * FROM Market_Analysis WHERE status='CLOSED'").fetchall()
            trades=[dict(r) for r in c.execute("SELECT * FROM Live_Trades").fetchall()]
        full=sum(r[0]=='FULL_SUCCESS' for r in rows); failed=sum(r[0]=='FAILED' for r in rows); partial=sum(r[0]=='PARTIAL_SUCCESS' for r in rows); expired=sum(r[0]=='EXPIRED' for r in rows); ambiguous=sum(r[0]=='AMBIGUOUS' for r in rows); closed=full+failed+partial+expired+ambiguous
        def avg(field):
            vals=[float(x[field]) for x in trades if x[field] is not None]; return sum(vals)/len(vals) if vals else 0.0
        def ast(items):
            items=[dict(a) for a in items]; n=len(items); matched=sum(a['result']=='MATCHED' for a in items)
            return {'total':n,'matched':matched,'not_matched':sum(a['result']=='NOT_MATCHED' for a in items),'accuracy_pct':100*matched/n if n else 0.0,'direction_accuracy_avg':sum(a['direction_accuracy'] or 0 for a in items)/n if n else 0.0,'range_accuracy_avg':sum(a['range_accuracy'] or 0 for a in items)/n if n else 0.0,'target_accuracy_avg':sum(a['target_accuracy'] or 0 for a in items)/n if n else 0.0}
        daily=ast([a for a in analyses if a['analysis_type']=='DAILY']); weekly=ast([a for a in analyses if a['analysis_type']=='WEEKLY']); score=defaultdict(lambda:{'total':0,'full':0,'failed':0,'partial':0})
        for t in trades:
        ...
        return {'trades':{'total':total,'closed':closed,'full_success':full,'partial_success':partial,'failed':failed,'expired':expired,'ambiguous':ambiguous,'win_rate_pct':100*full/closed if closed else 0.0,'full_target_rate_pct':100*full/total if total else 0.0,'avg_rr':avg('risk_reward'),'avg_drawdown':avg('max_drawdown'),'avg_mae':avg('max_adverse_excursion'),'avg_mfe':avg('max_favorable_excursion')},'daily':daily,'weekly':weekly,'score_performance':dict(score)}
...
class PerformanceAuditor:
    ...
    def _evaluate_health(self,s):
        ...
        if s['request_p95_ms']>=AUDITOR_LATENCY_CRIT_MS: checks.append(('CRITICAL','WEBHOOK_REQUEST_LATENCY'))
        elif s['request_p95_ms']>=AUDITOR_LATENCY_WARN_MS: checks.append(('WARNING','WEBHOOK_REQUEST_LATENCY'))
...
VERSION = "v18.76-FULL-ANALYSIS-PERFORMANCE-OPTIMIZED"
...
# Per-key fetch locks prevent a burst of identical commands from launching
# duplicate network requests for the same timeframe/limit. Locks are acquired
# only around the cache-miss fetch path; cached reads remain lock-light.
_DATA_FETCH_LOCKS = defaultdict(threading.Lock)
_DATA_FETCH_LOCKS_GUARD = threading.Lock()

def _data_fetch_lock(key):
    with _DATA_FETCH_LOCKS_GUARD:
        return _DATA_FETCH_LOCKS[key]
...
def get_bars(interval, limit=300):
    ...
    # Double-check after acquiring the per-key lock so concurrent callers share
    # the same provider response instead of multiplying API traffic.
    with _data_fetch_lock(key):
        now = time.time()
        cached = DATA_CACHE.get(key)
        if cached and now - cached[0] < CACHE_SECONDS:
            return cached[1].copy()
        ...
        DATA_CACHE[key] = (time.time(), df.copy())
        return df.copy()
...
# ============================================================
# Concurrent analysis coalescing — performance-only boundary
# ============================================================
_ANALYSIS_CACHE_LOCK = threading.RLock()
_ANALYSIS_SINGLEFLIGHT_LOCK = threading.Lock()
_ANALYSIS_CACHE = {"ts": 0.0, "result": None}
ANALYSIS_COALESCE_SECONDS = max(0.5, float(os.getenv("ANALYSIS_COALESCE_SECONDS", "3")))

def evaluate_signal(force_fresh=False):
    """Run one shared analysis for concurrent read-only callers.

    Default callers (menus/reports) may reuse an analysis produced within a very
    short coalescing window. Trade execution and the automatic scanner explicitly
    request force_fresh=True, so trading decisions are never served from this
    cache. Strategy, thresholds, scoring and S/R rules are untouched.
    """
    now = time.monotonic()
    if not force_fresh:
        with _ANALYSIS_CACHE_LOCK:
            cached = _ANALYSIS_CACHE.get("result")
            if cached is not None and now - _ANALYSIS_CACHE.get("ts", 0.0) <= ANALYSIS_COALESCE_SECONDS:
                return copy.deepcopy(cached)
    with _ANALYSIS_SINGLEFLIGHT_LOCK:
        if not force_fresh:
            with _ANALYSIS_CACHE_LOCK:
                cached = _ANALYSIS_CACHE.get("result")
                if cached is not None and time.monotonic() - _ANALYSIS_CACHE.get("ts", 0.0) <= ANALYSIS_COALESCE_SECONDS:
                    return copy.deepcopy(cached)
        started = time.perf_counter()
        result = _evaluate_signal_impl()
        elapsed_ms = (time.perf_counter() - started) * 1000.0
        with _ANALYSIS_CACHE_LOCK:
            _ANALYSIS_CACHE["ts"] = time.monotonic()
            _ANALYSIS_CACHE["result"] = copy.deepcopy(result)
        logger.info("ANALYSIS_LATENCY | force_fresh=%s ms=%.1f", force_fresh, elapsed_ms)
        return result
...
async def trade_now(update, context):
    ...
    try:
        result = await asyncio.to_thread(evaluate_signal, True)
...
async def audit_report(update, context):
    ...
    try:
        ...
        lines = [
            ...
            "",
            "🖥️ صحة النظام",
            f"• CPU: {system.get('cpu_pct', 0):.1f}% | RAM: {system.get('rss_mb', 0):.1f} MB",
            f"• Webhook P95: {system.get('request_p95_ms', 0):.1f} ms",
            f"• Handler P95: {system.get('handler_p95_ms', 0):.1f} ms",
            f"• Event Loop P95: {system.get('event_loop_lag_ms', 0):.1f} ms",
            f"• Throughput: {system.get('throughput_per_min', 0):.1f}/min",
            f"• Queue: {system.get('queue_depth', 0)}/{system.get('queue_capacity', 0)} | Drops: {system.get('queue_drops', 0)}",
            f"• Errors: {system.get('error_count', 0)} | Alerts: {system.get('alert_count', 0)} | Self-Heal: {system.get('self_heal_count', 0)}",
            "",
...
async def router(update, context):
    ...
    try:
        ...
        if PERFORMANCE_AUDITOR is not None:
            try:
                PERFORMANCE_AUDITOR.metrics.record_handler((time.perf_counter() - _started) * 1000.0)
            except Exception:
                pass
...
def _auditor_alert_callback(level, reason, snapshot):
    ...
    try:
        ...
        text = (f"🛡️ تنبيه مراقب الأداء\n\nالمستوى: {level}\nالمشكلة: {reason}\n"
                ...
                f"Webhook P95: {snapshot.get('request_p95_ms', 0):.1f} ms | Handler P95: {snapshot.get('handler_p95_ms', 0):.1f} ms\n"
