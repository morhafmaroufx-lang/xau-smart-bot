...
VERSION = "v18.37-PRE-SMC-AUDITED-FIXED"
...
NEWS_MAX_CACHE_AGE_MIN = int(os.environ.get("NEWS_MAX_CACHE_AGE_MIN", "30"))
NEWS_CACHE_MIN_COVERAGE_MIN = int(os.environ.get("NEWS_CACHE_MIN_COVERAGE_MIN", "180"))
NEWS_HTTP_TIMEOUT = float(os.environ.get("NEWS_HTTP_TIMEOUT", "12"))
NEWS_TE_API_KEY = os.environ.get("TE_API_KEY", "").strip()
NEWS_FINNHUB_API_KEY = os.environ.get("FINNHUB_API_KEY", "").strip()
NEWS_TE_ENABLED = os.environ.get("NEWS_TE_ENABLED", "1").strip().lower() not in {"0", "false", "no"}
NEWS_FINNHUB_ENABLED = os.environ.get("NEWS_FINNHUB_ENABLED", "1").strip().lower() not in {"0", "false", "no"}
...
def institutional_analysis(df):
    ...
    work = df.iloc[:-1].copy() if df is not None and len(df) >= 31 else df.copy()
    if work is None or len(work) < 30:
        raise ValueError("بيانات غير كافية للتحليل المؤسسي")
    price = sf(work["close"].iloc[-1])
    atr = max(sf(ATR(work).iloc[-1], 1), 1e-9)
    vol_state, vol_ratio = volume_analysis(work)
    struct = structure(work)
    recent = work.tail(min(20, len(work)))
...
def _load_persisted_news_cache():
    """تحميل آخر تقويم صحيح محفوظ محلياً بعد إعادة تشغيل Render/الخادم."""
    try:
        ...
        for item in payload.get("events", []):
            dt = _parse_news_datetime(item.get("time"), None)
            if dt is None:
                continue
            events.append({
                "time": dt,
                ...
                "impact": "HIGH",
                "event": str(item.get("event", "خبر اقتصادي مرتفع التأثير")),
                "source": str(item.get("source", "cache")),
            })
        if not events:
            return None
        return {"saved_at": saved_at, "events": events, "source": "cache"}
    except Exception:
        logger.exception("NEWS_CACHE_LOAD_ERROR")
        return None


def _save_persisted_news_cache(events, source):
    """حفظ آخر نتيجة موثوقة فقط؛ لا يحفظ UNKNOWN أو نتائج فارغة."""
    try:
        path = _news_cache_path()
        if not path:
            return
            ...
            "events": [
                {
                    ...
                    "event": e.get("event", "خبر اقتصادي مرتفع التأثير"),
        ...
        logger.exception("NEWS_CACHE_SAVE_ERROR")


def _normalize_news_impact(value):
    raw = str(value or "").strip().lower()
    if raw in {"high", "high impact", "3", "red", "high impact expected", "critical", "major"}:
        return "HIGH"
    # TradingEconomics may return numeric importance.
    try:
        if float(raw) >= 3:
            return "HIGH"
    except Exception:
...
def _normalize_news_event(dt_value, currency, impact, event_name, source):
    impact_norm = _normalize_news_impact(impact)
    if impact_norm != "HIGH":
        return None
    dt = _parse_news_datetime(dt_value, None)
    if dt is None:
        return None
    return {
        "time": dt,
        ...
        "impact": "HIGH",
        "event": str(event_name or "خبر اقتصادي مرتفع التأثير").strip(),
        "source": source,
    }


def _news_is_gold_relevant(event):
    """XAU/USD execution filter: USD/US macro events only by default."""
    currency = str(event.get("currency", "")).upper()
    name = str(event.get("event", "")).lower()
    if currency in {"USD", "US", "UNITED STATES", "USA"}:
        return True
    keywords = (
        "federal reserve", "fed ", "fomc", "interest rate", "inflation", "cpi",
        "core cpi", "pce", "nonfarm", "payroll", "unemployment", "employment",
        "gdp", "ism", "retail sales", "powell", "treasury", "jobless claims",
        "consumer confidence", "ppi", "producer price"
    )
    return any(k in name for k in keywords)


def _fetch_tradingeconomics_news():
    if not NEWS_TE_ENABLED:
        raise RuntimeError("Trading Economics provider disabled")
    if not NEWS_TE_API_KEY:
    ...
    response = requests.get(url, params={"c": key}, timeout=NEWS_HTTP_TIMEOUT,
                            headers={"User-Agent": "XAU-Smart-Trader/18.37"})
    response.raise_for_status()
    data = response.json()
    if not isinstance(data, list):
        raise RuntimeError("Trading Economics returned an invalid payload")
    events = []
    for row in data:
        event = _normalize_news_event(
            row.get("Date") or row.get("date"),
            row.get("Currency") or row.get("currency") or row.get("Country") or row.get("country"),
            row.get("Importance") if row.get("Importance") is not None else row.get("importance"),
            row.get("Event") or row.get("event") or row.get("Category") or row.get("category"),
            "tradingeconomics",
        )
        if event and _news_is_gold_relevant(event):
            events.append(event)
    if not events:
        raise RuntimeError("Trading Economics returned no relevant HIGH-impact US events")
    return events


def _fetch_finnhub_news():
    if not NEWS_FINNHUB_ENABLED:
        raise RuntimeError("Finnhub provider disabled")
    if not NEWS_FINNHUB_API_KEY:
        raise RuntimeError("FINNHUB_API_KEY is not configured")
    now = now_damascus()
    start = (now - timedelta(days=1)).strftime("%Y-%m-%d")
    end = (now + timedelta(days=2)).strftime("%Y-%m-%d")
    url = "https://finnhub.io/api/v1/calendar/economic"
    response = requests.get(url, params={"from": start, "to": end, "token": NEWS_FINNHUB_API_KEY},
    ...
    if not isinstance(rows, list):
        raise RuntimeError("Finnhub returned an invalid payload")
    events = []
    for row in rows:
        dt_value = row.get("time") or row.get("date")
        if isinstance(dt_value, (int, float)):
            dt_value = datetime.fromtimestamp(float(dt_value), tz=ZoneInfo("UTC")).isoformat()
        event = _normalize_news_event(
            dt_value,
            row.get("country") or row.get("currency"),
            row.get("impact") or row.get("importance"),
            row.get("event") or row.get("name"),
            "finnhub",
        )
        if event and _news_is_gold_relevant(event):
            events.append(event)
    if not events:
        raise RuntimeError("Finnhub returned no relevant HIGH-impact US events")
    return events


def _fetch_configured_news_url():
    """Legacy/custom API hook; accepted only after strict normalization."""
    configured = os.environ.get("NEWS_CALENDAR_URL", "").strip()
    if not configured:
        raise RuntimeError("No custom NEWS_CALENDAR_URL configured")
    response = requests.get(configured, timeout=NEWS_HTTP_TIMEOUT,
                            headers={"User-Agent": "XAU-Smart-Trader/18.37"})
    response.raise_for_status()
    text = response.text.strip()
    if not text:
        raise RuntimeError("Configured news source returned an empty response")
    try:
        payload = response.json()
        rows = payload if isinstance(payload, list) else payload.get("economicCalendar", payload.get("data", []))
        if not isinstance(rows, list):
            raise RuntimeError("Configured JSON source returned an invalid payload")
        events = []
        for row in rows:
            event = _normalize_news_event(
                row.get("time") or row.get("date") or row.get("datetime"),
                row.get("currency") or row.get("country"),
                row.get("impact") or row.get("importance"),
                row.get("event") or row.get("title") or row.get("name"),
                "custom",
            )
            if event and _news_is_gold_relevant(event):
                events.append(event)
        ...
        pass
    raise RuntimeError("Configured news source is not a supported JSON economic-calendar format")


def _news_cache_has_coverage(events, now=None):
    """Require cached calendar to cover the present and a meaningful future window."""
    if not events:
        return False
    now = now or now_damascus()
    times = [e.get("time") for e in events if isinstance(e.get("time"), datetime)]
    if not times:
        return False
    future = max(times)
    return future >= now + timedelta(minutes=NEWS_CACHE_MIN_COVERAGE_MIN)


def _news_provider_status(errors):
    if not errors:
        return "ONLINE"
    return "PARTIAL" if len(errors) < 2 else "DEGRADED"


def get_news_events(force=False):
    """Production-grade economic calendar: provider failover + freshness + coverage validation."""
    global NEWS_CACHE
    now_ts = time.time()
    now = now_damascus()
    cached_memory = NEWS_CACHE.get("events") or []
    if (not force and cached_memory and
            now_ts - NEWS_CACHE.get("time", 0) < NEWS_CACHE_SECONDS and
            _news_cache_has_coverage(cached_memory, now)):
        return cached_memory

    providers = []
    if os.environ.get("NEWS_CALENDAR_URL", "").strip():
        providers.append(("custom", _fetch_configured_news_url))
    providers.extend([
        ("tradingeconomics", _fetch_tradingeconomics_news),
    ...
    for source, fetcher in providers:
        try:
            events = fetcher()
            if not events or not _news_cache_has_coverage(events, now):
                raise RuntimeError("calendar payload lacks sufficient future coverage")
            events.sort(key=lambda e: e["time"])
            NEWS_CACHE = {"time": now_ts, "events": events, "source": source,
                          "status": _news_provider_status(errors)}
            _save_persisted_news_cache(events, source)
            logger.info("NEWS_PROVIDER_OK source=%s events=%s", source, len(events))
            return events
        except Exception as exc:
            errors.append(f"{source}: {exc}")
            logger.warning("NEWS_PROVIDER_FAILED source=%s error=%s", source, exc)

    persisted = _load_persisted_news_cache()
    if persisted:
        age_min = (now_ts - persisted["saved_at"]) / 60.0
        events = persisted.get("events") or []
        if age_min <= NEWS_MAX_CACHE_AGE_MIN and _news_cache_has_coverage(events, now):
            NEWS_CACHE = {"time": now_ts, "events": events, "source": "cache",
                          "status": "STALE_VALID"}
            logger.warning("NEWS_CACHE_FALLBACK age=%.1fmin source=%s", age_min, persisted.get("source"))
            return events

    raise RuntimeError("تعذر الحصول على تقويم اقتصادي موثوق؛ " + " | ".join(errors))

def news_filter():
    """حالة الأخبار التنفيذية: HIGH أو UNKNOWN يمنع فتح صفقة جديدة."""
    try:
        ...
        if upcoming:
            ...
            return True, "🚫 التداول محجوب مؤقتاً بسبب خبر عالي التأثير.\n" + "\n".join(details)

        source = NEWS_CACHE.get("source", "غير معروف")
        status = NEWS_CACHE.get("status", "ONLINE")
        return False, f"🟢 لا يوجد حالياً خبر عالي التأثير ضمن نافذة الحجب. | المصدر: {source} | الحالة: {status}"
    except Exception as exc:
        logger.warning("News calendar unavailable; execution blocked safely: %s", exc)
        return True, f"⚠️ حالة الأخبار UNKNOWN — إيقاف الإشارة التنفيذية احترازياً.\nالسبب: {exc}"
...
def build_analysis(include_trade=True):
    ...
    lines = [
        ...
        f"🎯 التوجيه: {direction}",
        f"💪 درجة السيناريو: {result.get('scenario_score', result['score'])} نقطة", f"🎯 درجة الإعداد التنفيذي: {result.get('setup_score', result['score'])} نقطة", f"🚦 حالة التنفيذ: {result.get('execution_state', 'WATCH')}", "",
        "🧠 الأطر الزمنية", "",
    ...
    if include_trade and result["signal"] and result["trade"]:
        ...
        lines += [
            "", "━━━━━━━━━━━━━━━━━━", "🚨 إشارة تداول", "",
            f"📈 الصفقة: {d}", f"💪 الجودة التنفيذية: {result['setup_score']} نقطة", quality,
        ...
        # الأخبار جزء من بوابة التنفيذ: HIGH/UNKNOWN يمنعان إنشاء/عرض صفقة جديدة.
...
async def reply(update, text):
    try:
        ...
        if q:
            data = q.data or ""
            section = "reports" if data in {"menu_daily_report","menu_weekly_report","menu_trade_history"} else \
                      "trades" if data in {"menu_trade_now","menu_trade_alerts"} else \
...
def _ux_main_keyboard():
    return [
        ["🏠 الرئيسية", "⚡ التحليل السريع"],
        ["📊 التحليلات", "📑 التقارير"],
        ["🎯 الصفقات", "🌍 السوق والأخبار"],
        ["🏦 التحليل المؤسسي", "🎯 مناطق السيولة"],
        ["🔔 التنبيهات", "💰 سعر الذهب"],
        ["💳 الباقات", "👤 اشتراكي"],
        ["ℹ️ المساعدة", "🟢 حالة النظام"],
    ]
...
async def start(update, context):
    ...
    text = (
        ...
        "⚡ التحليل السريع يبقى وصولاً مباشراً من الرئيسية."
    )
...
async def quick_analysis(update, context):
    ...
    try:
        ...
        text = (
            ...
            f"🚦 حالة التنفيذ: {result.get('execution_state', 'WATCH')}\n"
            f"📰 الأخبار: {result.get('news', 'غير متاحة')}\n"
            f"💪 قوة السيناريو: {score} نقطة — {scenario_quality}\n"
            f"🛡️ جودة التنفيذ: {execution_quality}\n"
...
async def callback_router(update, context):
    ...
    if fn:
        ...
        section = "reports" if data in {"menu_daily_report","menu_weekly_report","menu_trade_history"} else                   "trades" if data in {"menu_trade_now","menu_trade_alerts"} else                   "market" if data in {"menu_news","menu_markets","menu_gold_price"} else "analyses"
...
async def institutional_menu(update, context):
    ...
    try:
        ...
        body = "🧠 السياق المؤسسي\\n" + str(context_text) + "\\n\\n📡 البيانات المؤسسية\\nالبيانات المباشرة تعتمد على توفر مصدر مباشر؛ لا يتم اختلاق بيانات غير متاحة."
...
async def liquidity_menu(update, context):
    ...
    try:
        ...
        body = (f"الانحياز: {liq.get('bias','غير متوفر')}\\n"
                f"🧲 Buy-side الأقرب: {fmt(liq.get('nearest_buy'))}\\n"
                f"🧲 Sell-side الأقرب: {fmt(liq.get('nearest_sell'))}\\n"
                f"🔄 السحب: {liq.get('sweep_text','غير متوفر')}\\n"
                f"🔁 إعادة الاختبار: {liquidity_retest_summary(liq)}")
        await _ux_render(update, "🎯 مناطق السيولة", body, "liquidity")
    except Exception as e:
        await _ux_render(update, "🎯 مناطق السيولة", f"❌ تعذر تحديث السيولة: {e}", "liquidity")


async def help_menu(update, context):
    body = ("ابدأ من الرئيسية ثم اختر القسم المطلوب.\\n\\n"
            "⚡ التحليل السريع — قراءة لحظية مباشرة.\\n"
            "📊 التحليلات — التحليل الكامل.\\n"
            "📑 التقارير — يومي، أسبوعي، وسجل الصفقات.\\n"
            "🎯 الصفقات — فرصة الآن والتنبيهات والسجل.\\n"
            "🌍 السوق والأخبار — السعر والجلسات والأخبار.\\n"
            "🏦 المؤسسي / 🎯 السيولة — سياق السوق المتقدم.")
...
async def router(update, context):
    ...
    routes = {
        ...
        "📑 التقارير": lambda u, c: _ux_render(u, "📑 التقارير", "اختر التقرير أو السجل المطلوب:", "reports"),
        "🎯 الصفقات": trades_menu,
