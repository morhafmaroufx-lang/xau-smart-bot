...
async def _account_notification_scheduler():
    """مراقب خفيف لحالة الاشتراكات: لا يستدعي تحليلًا أو بيانات سوق."""
    ...
    try:
        ...
        while True:
            try:
                ...
                try:
                    rows = conn.execute(
                        "SELECT chat_id,plan,status,expiry_date FROM users WHERE chat_id NOT IN (%s)" %
                        ",".join("?" for _ in ADMIN_IDS) if ADMIN_IDS else "SELECT chat_id,plan,status,expiry_date FROM users",
                        tuple(int(x) for x in ADMIN_IDS),
                    ).fetchall() if ADMIN_IDS else conn.execute(
                        "SELECT chat_id,plan,status,expiry_date FROM users"
                    ).fetchall()
                    users = [dict(r) for r in rows]
                finally:
                    conn.close()

                # انتهاء المنح المجانية: active=0 + تاريخ منتهٍ يميّز الانتهاء عن الإلغاء اليدوي.
                _ensure_admin_free_grant_table()
                conn = _db()
                try:
                    grants = conn.execute(
...
async def _free_quota_guard(update, feature):
    ...
    if is_admin_chat(user_id):
        return True
    canonical = _canonical_feature(feature)
    if not has_feature(chat_id, canonical):
        await reply(update, "🔒 هذه الميزة غير متاحة ضمن باقتك الحالية.\n\n💳 استخدم /plans لمعرفة الباقات.")
        return False
    row = get_member(chat_id)
    plan = row['plan'] if row else 'FREE'
