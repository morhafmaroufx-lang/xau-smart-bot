...
def sr_analyze():
    """Independent S/R analysis path. It deliberately bypasses main signal/trade evaluators."""
    df = get_bars("M15", limit=300)
    if df is None or len(df) < 30:
        return {"ok": False, "reason": "بيانات M15 غير كافية."}
    sr = support_resistance(df, lookback=min(120, len(df)))
    candidates = [_sr_candidate_setup(df, sr, "BUY"), _sr_candidate_setup(df, sr, "SELL")]
    candidates = [x for x in candidates if x]
    if not candidates:
        return {"ok": True, "signal": None, "sr": sr, "reason": "لا توجد إشارة S/R مستقلة مكتملة."}
    candidates.sort(key=lambda x: x["score"], reverse=True)
    trade = _sr_build_trade(candidates[0])
    if not trade:
        return {"ok": True, "signal": None, "sr": sr, "candidate": candidates[0], "reason": "لا يوجد TP من S/R المقابل يحقق R:R المطلوب."}
    return {"ok": True, "signal": trade, "sr": sr, "candidate": candidates[0]}
...
async def sr_button(update, context):
    try:
        ...
        if result.get("signal"):
            setup_id, is_new = await asyncio.to_thread(_sr_insert_trade, result["signal"], _sr_snapshot(get_bars("M15", limit=120), result.get("sr",{}), result["signal"]))
            if setup_id and is_new:
                text += f"\n\n🆔 {setup_id}\n🔒 هذه الصفقة مستقلة عن نظام الصفقات الرئيسي."
        await reply(update, text)
    except Exception:
        logger.exception("S/R button error")
        await reply(update, "❌ تعذر تنفيذ استراتيجية S/R حالياً.")
