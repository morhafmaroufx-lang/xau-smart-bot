...
VERSION = "v18.60-INSTITUTIONAL-IMF-IRFCL-CROSS-VALIDATED-WGC-ARABIC-PRE-SMC-INDEPENDENT-NEWS-EMBEDDED"
...
def _imf_irfcl_monthly(indicator):
    """Fetch IMF IRFCL monthly gold-reserve series robustly via SDMX 3.0.

    The IMF Data API publishes SDMX 3.0 data and the verified IRFCL key is
    COUNTRY.INDICATOR.SECTOR.FREQUENCY.  We request compact JSON explicitly
    and support the two common SDMX-JSON observation layouts so a harmless
    upstream serialization change does not turn valid IMF data into
    ``UNAVAILABLE``.
    """
    # Keep the request narrow enough for Render while retaining a long enough
    # history to absorb country reporting lags and to calculate a same-period
    # month-over-month change.  The latest two common months are selected
    # after parsing rather than assuming the API's final row is current.
    end_period=pd.Timestamp.utcnow().strftime('%Y-%m')
    start_period=(pd.Timestamp.utcnow()-pd.DateOffset(months=18)).strftime('%Y-%m')
    key=f'*.{indicator}.*.M'
    urls=[
        f"{IMF_IRFCL_DATAFLOW}/{key}?startPeriod={start_period}&endPeriod={end_period}&format=jsondata",
        f"{IMF_IRFCL_DATAFLOW}/{key}?startPeriod={start_period}&endPeriod={end_period}&dimensionAtObservation=TIME_PERIOD&format=jsondata",
        f"{IMF_IRFCL_DATAFLOW}/{key}?startPeriod={start_period}&endPeriod={end_period}&format=sdmx-json",
    ]
    last=None
    ...
    def _dim_values(dim):
        ...
        out=[]
        for v in vals or []:
            if isinstance(v,dict): out.append(v.get('id') or v.get('value') or v.get('name'))
            else: out.append(v)
        return out

    def _parse_payload(payload):
        # SDMX JSON 2.0/3.0 compact layout.
        ...
        if not sdim:
            # Some SDMX 3 responses expose dimensions under a structure
            # reference.  Accept a flattened dimensions object as well.
            sdim=dims.get('seriesDimensions',[]) or []
        country_dim=next((d for d in sdim if str(d.get('id','')).upper() in ('COUNTRY','REF_AREA')),None)
        if country_dim is None:
            # Fallback to a dimension list carrying explicit position metadata.
            all_dims=struct.get('dimensions') if isinstance(struct.get('dimensions'),list) else []
            country_dim=next((d for d in all_dims if str(d.get('id','')).upper() in ('COUNTRY','REF_AREA')),None)
            if country_dim is not None:
                sdim=all_dims
        if country_dim is None:
            raise RuntimeError('IMF country dimension unavailable')
        country_pos=next((i for i,d in enumerate(sdim) if d is country_dim),None)
        if country_pos is None: raise RuntimeError('IMF country dimension position unavailable')
        countries=_dim_values(country_dim)

        time_dim=next((d for d in odim if str(d.get('id','')).upper()=='TIME_PERIOD'),None)
        if time_dim is None:
            # In some responses TIME_PERIOD is supplied as an observation
            # dimension while the series carries no explicit observation dims.
            time_dim=next((d for d in (dims.get('observationDimensions',[]) or []) if str(d.get('id','')).upper()=='TIME_PERIOD'),None)
        times=_dim_values(time_dim) if time_dim else []

        out={}
        for key_,series in series_obj.items():
            ...
            for obs_key,obs_val in obs.items():
                try:
                    if isinstance(obs_val,(list,tuple)): raw_val=obs_val[0]
                    elif isinstance(obs_val,dict): raw_val=obs_val.get('value')
                    else: raw_val=obs_val
                    period=times[int(obs_key)] if times else str(obs_key)
                    val=float(raw_val)
                    if period and np.isfinite(val): rec['byMonth'][str(period)]=val
                except Exception: continue

        # A few SDMX responses can arrive as a flat observation table.
        if not out:
            rows=data.get('data') or payload.get('data')
            if isinstance(rows,list):
                for row in rows:
                    if not isinstance(row,dict): continue
                    cid=row.get('COUNTRY') or row.get('REF_AREA') or row.get('country')
                    period=row.get('TIME_PERIOD') or row.get('time') or row.get('period')
                    val=row.get('OBS_VALUE') if row.get('OBS_VALUE') is not None else row.get('value')
                    try:
                        if cid and period and val is not None:
                            out.setdefault(str(cid),{'name':str(cid),'byMonth':{}})['byMonth'][str(period)]=float(val)
                    except Exception: pass
        return out
        ...
        for attempt in range(3):
            try:
                r=_inst_http(url,timeout=90,headers={
                    'User-Agent':'Mozilla/5.0 (XAU Smart Trader IMF Gateway)',
                    'Accept':'application/vnd.sdmx.data+json;version=2.0.0,application/json;q=0.9,*/*;q=0.1'
                })
                ctype=(r.headers.get('content-type') or '').lower()
                payload=r.json()
                out=_parse_payload(payload)
                # Do not accept a structurally valid but effectively empty
                # response. The live IRFCL gold series has >100 country series.
                populated={cid:rec for cid,rec in out.items() if rec.get('byMonth')}
                if len(populated)>=20:
                    return populated
                raise RuntimeError(f'IMF IRFCL parsed only {len(populated)} country series; content-type={ctype}')
            except Exception as exc:
                last=exc
...
def _cb_real():
    ...
    try:
        ...
        if len(months)<2: raise RuntimeError('سلسلة IMF تحتوي على أقل من شهرين')
        ...
        imf={'period':str(latest),'net_change_tonnes':float(delta),'source':'صندوق النقد الدولي IMF — IRFCL','reported_only':True,'countries_covered':len(raw)}
...
def run_institutional_self_tests():
    ...
    check('version', VERSION.startswith('v18.60-'), VERSION)
