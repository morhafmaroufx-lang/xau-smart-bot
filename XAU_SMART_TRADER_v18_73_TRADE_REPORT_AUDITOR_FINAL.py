...
# ============================================================
# PERFORMANCE AUDITOR — TRADE/REPORT ONLY v18.73
...
AUDITOR_WEEKLY_REPORT_HOUR = int(os.getenv("AUDITOR_WEEKLY_REPORT_HOUR", "0"))
...
class Database:
    ...
    def initialize(self) -> None:
        with self._lock, self.connect() as conn:
            ...
            # Lightweight forward migrations for databases created by older v18.x builds.
            ...
            migrations = {
                ...
                "sr_zone_distance": "ALTER TABLE Live_Trades ADD COLUMN sr_zone_distance REAL",
            }
            for col, ddl in migrations.items():
                if col not in cols:
                    conn.execute(ddl)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_live_trades_type_time ON Live_Trades(trade_type, signal_time)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_live_trades_close_time ON Live_Trades(close_time)")
            conn.commit()
...
class TradeAuditor:
    ...
    def register(self, trade: TradeInput, trade_type: str = "MAIN", sr_meta: Optional[dict[str, Any]] = None) -> bool:
        ...
        trade_type = str(trade_type or "MAIN").upper()
        if direction not in {"BUY", "SELL"}:
            raise ValueError("direction must be BUY or SELL")
        if trade_type not in {"MAIN", "SR"}:
            raise ValueError("trade_type must be MAIN or SR")
        sr_meta = sr_meta or {}
        ...
        with self.db.transaction() as conn:
            ...
            conn.execute("""INSERT INTO Live_Trades(
                trade_id,signal_time,direction,entry,sl,tp1,tp2,tp3,tp_final,score,quality,risk_reward,timeframe,
                trade_type,sr_side,sr_level,sr_trigger,sr_strength,sr_zone_distance,expiry_time,
                ...
                trade.tp_final, trade.score, trade.quality, trade.risk_reward, trade.timeframe,
                trade_type, sr_meta.get("side"), sr_meta.get("level"), sr_meta.get("trigger"),
                sr_meta.get("strength"), sr_meta.get("zone_distance"), trade.expiry_time,
        ...
        with self.db.transaction() as conn:
            ...
            for row in rows:
                ...
                if sl_hit:
                    ...
                    had_target = conn.execute("SELECT 1 FROM Trade_Events WHERE trade_id=? AND event_type IN ('TP1','TP2','TP3','TP_FINAL') LIMIT 1", (trade["trade_id"],)).fetchone()
                    final = "PARTIAL_SUCCESS" if had_target else "FAILED"
                    conn.execute("UPDATE Live_Trades SET status='CLOSED',final_result=?,close_time=?,updated_at=? WHERE trade_id=?", (final,bar.timestamp,bar.timestamp,trade["trade_id"]))
                    changed.append(trade["trade_id"]); continue
...
class Metrics:
    """High-precision trade-only metrics. No CPU/RAM/event-loop/system health metrics."""
    def __init__(self, db: Database): self.db = db

    @staticmethod
    def _parse_local(value: Optional[str]) -> Optional[datetime]:
        if not value: return None
        try: return parse_dt(value).astimezone(ZoneInfo(AUDITOR_WEEKLY_TZ))
        except Exception: return None

    @staticmethod
    def _trade_stats(items: list[dict]) -> dict:
        total=len(items)
        full=sum(x.get('final_result')=='FULL_SUCCESS' for x in items)
        failed=sum(x.get('final_result')=='FAILED' for x in items)
        partial=sum(x.get('final_result')=='PARTIAL_SUCCESS' for x in items)
        expired=sum(x.get('final_result')=='EXPIRED' for x in items)
        ambiguous=sum(x.get('final_result')=='AMBIGUOUS' for x in items)
        closed=full+failed+partial+expired+ambiguous
        decisive=full+failed+partial
        avg=lambda k: (sum(float(x[k]) for x in items if x.get(k) is not None)/sum(x.get(k) is not None for x in items)) if any(x.get(k) is not None for x in items) else 0.0
        return {
            'total':total,'closed':closed,'open':max(0,total-closed),'full_success':full,'partial_success':partial,
            'failed':failed,'expired':expired,'ambiguous':ambiguous,
            'win_rate_pct':100.0*(full+partial)/decisive if decisive else 0.0,
            'full_target_rate_pct':100.0*full/closed if closed else 0.0,
            'avg_rr':avg('risk_reward'),'avg_drawdown':avg('max_drawdown'),'avg_mae':avg('max_adverse_excursion'),'avg_mfe':avg('max_favorable_excursion'),
            'avg_score':avg('score')
        }

    def _load(self) -> list[dict]:
        with self.db.connect() as c:
            return [dict(r) for r in c.execute("SELECT * FROM Live_Trades ORDER BY signal_time").fetchall()]

    def report(self) -> dict:
        trades=self._load(); main=[t for t in trades if (t.get('trade_type') or 'MAIN')!='SR']; sr=[t for t in trades if (t.get('trade_type') or 'MAIN')=='SR']
        return {'trades':self._trade_stats(trades),'main_trades':self._trade_stats(main),'sr_trades':self._trade_stats(sr),'score_performance':self._score(trades)}

    def _score(self, trades):
        out=defaultdict(lambda:{'total':0,'full':0,'partial':0,'failed':0,'expired':0,'ambiguous':0})
        for t in trades:
            s=t.get('score'); bucket='UNKNOWN' if s is None else ('50-59' if s<60 else '60-69' if s<70 else '70-79' if s<80 else '80-89' if s<90 else '90-100' if s<=100 else '100+')
            out[bucket]['total']+=1; out[bucket]['full']+=t.get('final_result')=='FULL_SUCCESS'; out[bucket]['partial']+=t.get('final_result')=='PARTIAL_SUCCESS'; out[bucket]['failed']+=t.get('final_result')=='FAILED'; out[bucket]['expired']+=t.get('final_result')=='EXPIRED'; out[bucket]['ambiguous']+=t.get('final_result')=='AMBIGUOUS'
        return dict(out)

    def period_report(self, start: datetime, end: datetime) -> dict:
        trades=[]
        for t in self._load():
            dt=self._parse_local(t.get('signal_time'))
            if dt and start <= dt < end: trades.append(t)
        sr=[t for t in trades if (t.get('trade_type') or 'MAIN')=='SR']
        main=[t for t in trades if (t.get('trade_type') or 'MAIN')!='SR']
        by_day={}
        for t in trades:
            dt=self._parse_local(t.get('signal_time')); key=dt.strftime('%A') if dt else 'UNKNOWN'
            by_day.setdefault(key,[]).append(t)
        def sr_breakdown(items):
            return {
                'total':len(items),
                'support':sum((x.get('sr_side') or '').upper()=='SUPPORT' for x in items),
                'resistance':sum((x.get('sr_side') or '').upper()=='RESISTANCE' for x in items),
                'triggers':dict((k,sum((x.get('sr_trigger') or '')==k for x in items)) for k in ('RETEST','BREAKOUT','REJECTION','FALSE_BREAKOUT','FLIP')),
                'avg_strength':(sum(float(x['sr_strength']) for x in items if x.get('sr_strength') is not None)/sum(x.get('sr_strength') is not None for x in items)) if any(x.get('sr_strength') is not None for x in items) else 0.0,
                'win_rate_pct':self._trade_stats(items)['win_rate_pct']
            }
        stats=self._trade_stats(trades)
        return {'period':{'start':start.isoformat(),'end':end.isoformat()},'trades':stats,'main_trades':self._trade_stats(main),'sr_trades':self._trade_stats(sr),'sr_breakdown':sr_breakdown(sr),'score_performance':self._score(trades),'by_day':{k:self._trade_stats(v) for k,v in by_day.items()},'best_trade':max(trades,key=lambda x: (1 if x.get('final_result')=='FULL_SUCCESS' else 0, float(x.get('score') or 0), float(x.get('risk_reward') or 0)), default=None),'worst_trade':min(trades,key=lambda x: (1 if x.get('final_result')=='FAILED' else 0, float(x.get('score') or 999), -float(x.get('risk_reward') or 0)), default=None)}


class Reporter:
    def __init__(self, metrics: Metrics): self.metrics=metrics
    def _render(self, r: dict, title: str) -> str:
        t=r['trades']; m=r['main_trades']; s=r['sr_trades']; sb=r.get('sr_breakdown',{})
        lines=[f'🛡️ XAU SMART TRADER — {title}','═'*42,'','📊 إجمالي الصفقات',f"• الإجمالي: {t['total']} | المغلقة: {t['closed']} | المفتوحة: {t['open']}",f"• نجاح كامل: {t['full_success']} | جزئي: {t['partial_success']} | فشل: {t['failed']}",f"• منتهية: {t['expired']} | غامضة: {t['ambiguous']}",f"• Win Rate (الناجحة + الجزئية): {t['win_rate_pct']:.2f}%",f"• Full Target Rate: {t['full_target_rate_pct']:.2f}%",f"• متوسط الجودة: {t['avg_score']:.2f} نقطة | متوسط R:R: {t['avg_rr']:.2f}",f"• متوسط MAE: {t['avg_mae']:.4f} | متوسط MFE: {t['avg_mfe']:.4f}",'','📈 الصفقات الرئيسية',f"• الإجمالي: {m['total']} | نجاح كامل: {m['full_success']} | جزئي: {m['partial_success']} | فشل: {m['failed']}",f"• Win Rate: {m['win_rate_pct']:.2f}% | متوسط الجودة: {m['avg_score']:.2f}",'','📐 صفقات S/R',f"• الإجمالي: {s['total']} | نجاح كامل: {s['full_success']} | جزئي: {s['partial_success']} | فشل: {s['failed']}",f"• Win Rate: {s['win_rate_pct']:.2f}% | متوسط الجودة: {s['avg_score']:.2f} | متوسط R:R: {s['avg_rr']:.2f}"]
        if sb:
            lines += [f"• دعم: {sb.get('support',0)} | مقاومة: {sb.get('resistance',0)} | متوسط قوة المستوى: {sb.get('avg_strength',0):.2f}","• السيناريوهات: "+" | ".join(f"{k}={v}" for k,v in sb.get('triggers',{}).items())]
        lines += ['','🎯 أداء درجات الجودة']
        for bucket,v in r.get('score_performance',{}).items(): lines.append(f"• {bucket}: {v['total']} | كامل={v['full']} | جزئي={v['partial']} | فشل={v['failed']}")
        if r.get('by_day'):
            lines += ['','📅 حسب أيام الأسبوع']
            for day,v in r['by_day'].items(): lines.append(f"• {day}: {v['total']} صفقة | Win Rate {v['win_rate_pct']:.2f}%")
        bt=r.get('best_trade'); wt=r.get('worst_trade')
        if bt: lines += ['','🏆 أفضل صفقة',f"• {bt.get('trade_id')} | {bt.get('trade_type','MAIN')} | {bt.get('direction')} | {bt.get('final_result')} | {float(bt.get('score') or 0):.0f} نقطة"]
        if wt: lines += ['','⚠️ أضعف صفقة',f"• {wt.get('trade_id')} | {wt.get('trade_type','MAIN')} | {wt.get('direction')} | {wt.get('final_result')} | {float(wt.get('score') or 0):.0f} نقطة"]
        lines += ['','🔒 المراقب مخصص للصفقات والتقارير فقط؛ لا يراقب CPU/RAM/Event Loop ولا يغيّر قرارات التداول.']
        return '\n'.join(lines)
    def text(self) -> str: return self._render(self.metrics.report(), 'تقرير أداء الصفقات')
    def daily_text(self, when: Optional[datetime]=None) -> str:
        tz=ZoneInfo(AUDITOR_WEEKLY_TZ); now=(when or datetime.now(tz)).astimezone(tz); start=now.replace(hour=0,minute=0,second=0,microsecond=0); end=start+timedelta(days=1); return self._render(self.metrics.period_report(start,end), f'التقرير اليومي — {start.date()}')
    def weekly_text(self, when: Optional[datetime]=None) -> tuple[str,dict]:
        tz=ZoneInfo(AUDITOR_WEEKLY_TZ); now=(when or datetime.now(tz)).astimezone(tz); start=(now-timedelta(days=now.weekday())).replace(hour=0,minute=0,second=0,microsecond=0); end=start+timedelta(days=5); r=self.metrics.period_report(start,end); return self._render(r, f'التقرير الأسبوعي — {start.date()} إلى {(end-timedelta(seconds=1)).date()}'), r


class PerformanceAuditor:
    """Trade Performance Auditor: asynchronous price observation + precise trade/report auditing."""
    def __init__(self, db_path: str = AUDITOR_DB_PATH, price_engine: Optional[LivePriceEngine] = None):
        self.db=Database(db_path); self.price_engine=price_engine or LivePriceEngine(); self.trade_auditor=TradeAuditor(self.db); self.metrics=Metrics(self.db); self.reporter=Reporter(self.metrics); self._stop=threading.Event(); self._thread=None; self._last_weekly_key=None
    @staticmethod
    def _now() -> str: return datetime.now(timezone.utc).isoformat()
    @staticmethod
    def _id(prefix: str) -> str: return f"{prefix}_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}_{uuid.uuid4().hex[:8]}"
    def register_trade(self, *, trade_id: Optional[str]=None, signal_time: Optional[str]=None, direction: str, entry: float, sl: float, tp1: Optional[float]=None, tp2: Optional[float]=None, tp3: Optional[float]=None, tp_final: Optional[float]=None, score: Optional[float]=None, quality: Optional[str]=None, risk_reward: Optional[float]=None, timeframe: Optional[str]=None, expiry_time: Optional[str]=None, analysis_snapshot: Optional[dict[str,Any]]=None, trade_type: str='MAIN', sr_meta: Optional[dict[str,Any]]=None) -> str:
        ...
        expiry_time=expiry_time or (parse_dt(signal_time)+timedelta(hours=AUDITOR_DEFAULT_TRADE_EXPIRY_HOURS)).isoformat()
        ...
        self.trade_auditor.register(obj, trade_type=trade_type, sr_meta=sr_meta); return trade_id
    def observe_price(self, price: float, timestamp: Optional[str]=None, source: str='manual') -> list[str]: return self.trade_auditor.observe_tick(PricePoint(timestamp or self._now(),float(price),source))
    def metrics_report(self) -> dict: return self.metrics.report()
    def daily_report(self, when: Optional[datetime]=None) -> str: return self.reporter.daily_text(when)
    def weekly_report(self, when: Optional[datetime]=None, persist: bool=True) -> tuple[str,dict]:
        text,data=self.reporter.weekly_text(when)
        if persist:
            start=data['period']['start']; end=data['period']['end']; key=start[:10]
            with self.db.transaction() as conn:
                conn.execute("INSERT OR REPLACE INTO Weekly_Trade_Reports(report_key,week_start,week_end,generated_at,trades_total,trades_closed,win_rate,report_json,report_text) VALUES(?,?,?,?,?,?,?,?,?)",(key,start,end,self._now(),data['trades']['total'],data['trades']['closed'],data['trades']['win_rate_pct'],self.db.jdump(data),text))
        return text,data
    def latest_weekly_report(self) -> Optional[str]:
        with self.db.connect() as conn:
            row=conn.execute("SELECT report_text FROM Weekly_Trade_Reports ORDER BY week_end DESC LIMIT 1").fetchone()
            return row['report_text'] if row else None
    def _weekly_key_to_finalize(self, now_local: datetime) -> Optional[datetime]:
        # Trading week = Monday 00:00 through Friday 23:59:59. Generate once on Saturday.
        if now_local.weekday()==5 and now_local.hour >= AUDITOR_WEEKLY_REPORT_HOUR:
            return now_local
        return None
    def _maybe_weekly_report(self, point_time: str) -> None:
        try:
            local=parse_dt(point_time).astimezone(ZoneInfo(AUDITOR_WEEKLY_TZ)); marker=self._weekly_key_to_finalize(local)
            if marker is None: return
            week_start=(local-timedelta(days=5)).replace(hour=0,minute=0,second=0,microsecond=0)
            key=week_start.strftime('%Y-%m-%d')
            with self.db.connect() as c:
                exists=c.execute("SELECT 1 FROM Weekly_Trade_Reports WHERE report_key=?",(key,)).fetchone()
            if exists: return
            text,_=self.weekly_report(week_start, persist=True)
            # Best-effort admin delivery; report remains persisted if Telegram is unavailable.
            if 'BOT_LOOP' in globals() and BOT_LOOP and BOT_LOOP.is_running() and 'APPLICATION' in globals() and APPLICATION:
                coro=_send_admin_weekly_report(text)
                asyncio.run_coroutine_threadsafe(coro, BOT_LOOP)
        except Exception:
            AUDITOR_LOGGER.exception('WEEKLY_REPORT_GENERATION_ERROR')
    def run_forever(self) -> None:
        AUDITOR_LOGGER.info('Trade Performance Auditor started')
        cleanup_counter=0
        while not self._stop.is_set():
            try:
                with self.db.connect() as conn: open_count=conn.execute("SELECT COUNT(*) FROM Live_Trades WHERE status='ACTIVE'").fetchone()[0]
                if open_count:
                    ...
                    self._maybe_weekly_report(point.timestamp)
                else:
                    self._maybe_weekly_report(self._now())
            except Exception as exc:
                AUDITOR_LOGGER.error('Trade audit cycle failure: %s',exc,exc_info=True)
            cleanup_counter += 1
            if cleanup_counter >= max(1,int(3600/max(1,AUDITOR_POLL_SECONDS))):
                ...
                try:
                    ...
                    with self.db.connect() as conn: conn.execute('DELETE FROM Price_Samples WHERE sample_time < ?', (cutoff,)); conn.commit()
                except Exception: AUDITOR_LOGGER.exception('PRICE_SAMPLE_CLEANUP_ERROR')
            self._stop.wait(AUDITOR_POLL_SECONDS)
    def start_background(self) -> None:
        if self._thread and self._thread.is_alive(): return
        self._stop.clear(); self._thread=threading.Thread(target=self.run_forever,name='xau-trade-auditor',daemon=True); self._thread.start()
    def stop(self) -> None:
        self._stop.set()
        if self._thread and self._thread.is_alive() and self._thread is not threading.current_thread(): self._thread.join(timeout=max(2.0,AUDITOR_POLL_SECONDS+1.0))
        self._thread=None
...
def _auditor_register_sr_trade_direct(trade, snapshot, setup_id):
    if PERFORMANCE_AUDITOR is None or not trade: return
    try:
        sr=snapshot.get("levels",{}) if isinstance(snapshot,dict) else {}
        side = "SUPPORT" if trade.get("direction")=="BUY" else "RESISTANCE"
        level = snapshot.get("trade",{}).get("zone") if isinstance(snapshot,dict) else None
        if level is None:
            level = trade.get("levels",{}).get("support1" if side=="SUPPORT" else "resistance1") if isinstance(trade.get("levels"),dict) else None
        sr_meta={"side":side,"level":level,"trigger":trade.get("trigger"),"strength":trade.get("score"),"zone_distance":None}
        PERFORMANCE_AUDITOR.register_trade(trade_id=setup_id, signal_time=(snapshot.get("time") if isinstance(snapshot,dict) else None), direction=trade.get("direction"), entry=trade.get("entry"), sl=trade.get("sl"), tp1=trade.get("tp1"), tp2=trade.get("tp2"), tp3=trade.get("tp3"), tp_final=trade.get("tp3"), score=trade.get("score"), quality="S/R", risk_reward=trade.get("rr"), timeframe="M15", analysis_snapshot=snapshot if isinstance(snapshot,dict) else {}, trade_type="SR", sr_meta=sr_meta)
    except Exception:
        logging.getLogger(__name__).exception("AUDITOR_SR_TRADE_BRIDGE_ERROR")


def _auditor_register_sr_trade(trade, snapshot, setup_id):
    if PERFORMANCE_AUDITOR is None or not trade or not setup_id: return
    _auditor_enqueue(("sr_trade", dict(trade), dict(snapshot or {}), setup_id))
...
def _auditor_register_analysis(horizon, *, issue_time, direction, expected_min, expected_max, target, score, snapshot):
    """Disabled by design in v18.73: this monitor is trade/report-only."""
    return False
    ...
    while not AUDITOR_WORKER_STOP.is_set():
        ...
        try:
            ...
            if kind == "trade":
                ...
                _auditor_register_sr_trade_direct(task[1], task[2], task[3])
            elif kind == "analysis":
...
async def audit_report(update, context):
    """تقرير مراقب الصفقات فقط — للإدارة، قراءة فقط."""
    ...
    if not (is_admin_chat(user_id) or is_admin_chat(update.effective_chat.id)):
        ...
        return
    if PERFORMANCE_AUDITOR is None:
        await reply(update, "🛡️ مراقب الصفقات غير متاح حالياً.")
        return
    try:
        report = await asyncio.to_thread(PERFORMANCE_AUDITOR.metrics_report)
        t=report['trades']; s=report['sr_trades']
        lines=["🛡️ XAU SMART TRADER — مراقب الصفقات","",f"📊 الصفقات: {t['total']} | المغلقة: {t['closed']} | المفتوحة: {t['open']}",f"✅ كامل: {t['full_success']} | 🟡 جزئي: {t['partial_success']} | ❌ فشل: {t['failed']}",f"📈 Win Rate: {t['win_rate_pct']:.2f}% | متوسط الجودة: {t['avg_score']:.2f} | متوسط R:R: {t['avg_rr']:.2f}","",f"📐 S/R: {s['total']} | كامل: {s['full_success']} | جزئي: {s['partial_success']} | فشل: {s['failed']}",f"📐 S/R Win Rate: {s['win_rate_pct']:.2f}% | متوسط الجودة: {s['avg_score']:.2f}","","📅 التقرير اليومي: /audit_daily","📆 التقرير الأسبوعي الكامل: /audit_weekly","","🔒 المراقب يراقب الصفقات والتقارير فقط."]
        buttons=[[InlineKeyboardButton("📅 التقرير اليومي",callback_data="audit_daily"),InlineKeyboardButton("📆 التقرير الأسبوعي",callback_data="audit_weekly")],[InlineKeyboardButton("🔄 تحديث",callback_data="menu_audit")]]
...
async def audit_daily_report(update, context):
    user_id=update.effective_user.id if update.effective_user else update.effective_chat.id
    if not (is_admin_chat(user_id) or is_admin_chat(update.effective_chat.id)): return await reply(update,"⛔ للإدارة فقط.")
    if PERFORMANCE_AUDITOR is None: return await reply(update,"🛡️ المراقب غير متاح.")
...
async def audit_weekly_report(update, context):
    ...
    if not (is_admin_chat(user_id) or is_admin_chat(update.effective_chat.id)): return await reply(update,"⛔ للإدارة فقط.")
    if PERFORMANCE_AUDITOR is None: return await reply(update,"🛡️ المراقب غير متاح.")
    text=await asyncio.to_thread(lambda: PERFORMANCE_AUDITOR.weekly_report(persist=True)[0])
    await reply(update, text)


async def _send_admin_weekly_report(text):
    if not APPLICATION: return
    for chat_id in list(ADMIN_IDS):
        try: await APPLICATION.bot.send_message(chat_id=chat_id,text=with_market_header(text))
        except Exception: logger.exception("WEEKLY_ADMIN_DELIVERY_ERROR chat=%s",chat_id)
