...
async def admin_user_details(update, context, chat_id):
    if not await _admin_guard(update):
        return
    row = await asyncio.to_thread(_admin_user_lookup, chat_id)
    if not row:
        await reply(update, "🔎 لم يتم العثور على هذا الحساب.")
        return
    context.user_data["admin_selected_user"] = int(chat_id)
    context.user_data.pop("admin_mode", None)
    grant = await asyncio.to_thread(_active_admin_free_grant, chat_id)
    lines = [
        "👥 بيانات المستخدم", "",
        f"• Chat ID: {row['chat_id']}",
    ...
    if grant:
        lines += ["", "🎁 منحة ELITE مجانية نشطة", f"• تنتهي: {grant['expiry_date']}" ]
    rows = [
        [("🎁 منح اشتراك مجاني", "admin_grant_free_menu")],
...
# ============================================================
# v19.05 — ADMIN ACCOUNT STATUS CONTROL
# - تمييز المنح المجانية: FREE+1 = BASIC، FREE+2 = ELITE
# - إلغاء المنحة المجانية واستعادة الحالة السابقة
# - إلغاء اشتراك الحساب وإعادته إلى FREE
# - حظر الحساب / فك الحظر
# - لا يغيّر منطق التداول أو شروط الإشارة
# ============================================================
V19_05_VERSION = "v19.05_ADMIN_ACCOUNT_STATUS_CONTROL"


def _account_is_blocked(chat_id):
    try:
        row = get_member(int(chat_id))
        return bool(row and str(row["status"]).lower() == "blocked")
    except Exception:
        return False


def _free_grant_label(grant):
    if not grant:
        return None
    gp = str(grant.get("plan") or "").upper()
    if gp == "BASIC":
        return "FREE+1"
    if gp == "ELITE":
        return "FREE+2"
...
def _display_plan_for_admin(row, grant=None):
    grant = grant or {}
    label = _free_grant_label(grant)
    if label:
        return label
    return str(row.get("plan") or "FREE")


def _cancel_free_grant(chat_id, admin_id):
    cid = int(chat_id)
    aid = int(admin_id)
    if not is_admin_chat(aid):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    _ensure_admin_free_grant_table()
    now = now_damascus()
    conn = _db()
    try:
        ...
        # استعادة الحالة التي كانت قبل المنحة، ثم تعطيل المنحة.
        previous_plan = grant["previous_plan"] if grant["previous_plan"] in PLANS else "FREE"
        previous_status = grant["previous_status"] or "active"
        conn.execute(
            f"UPDATE {ADMIN_FREE_GRANT_TABLE} SET active=0 WHERE id=?",
            (grant["id"],),
        )
        conn.execute(
            ...
            (previous_plan, previous_status, grant["previous_start_date"], grant["previous_expiry_date"], now.isoformat(), cid),
        )
        conn.commit()
        return {
            "ok": True,
            "restored_plan": previous_plan,
...
def _cancel_subscription(chat_id, admin_id):
    """إلغاء الاشتراك التجاري/الوصول المجاني وإعادة الحساب إلى FREE مع حفظ السجل."""
    cid = int(chat_id)
    aid = int(admin_id)
    if not is_admin_chat(aid):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    ...
    try:
        active_grants = conn.execute(
            f"SELECT id FROM {ADMIN_FREE_GRANT_TABLE} WHERE chat_id=? AND active=1",
            (cid,),
        ).fetchall()
        if active_grants:
            conn.execute(
                f"UPDATE {ADMIN_FREE_GRANT_TABLE} SET active=0 WHERE chat_id=? AND active=1",
                (cid,),
            )
        row = conn.execute("SELECT status FROM users WHERE chat_id=?", (cid,)).fetchone()
        if not row:
            return {"ok": False, "reason": "NOT_FOUND"}
        # لا نحذف السجل ولا الصفقات؛ فقط نعيد خطة الوصول إلى FREE.
        conn.execute(
            "UPDATE users SET plan='FREE',status='active',start_date=?,expiry_date=NULL,updated_at=? WHERE chat_id=?",
            (now.isoformat(), now.isoformat(), cid),
        )
        conn.commit()
        return {"ok": True, "status_before": row["status"], "grants_cancelled": len(active_grants)}
    finally:
        conn.close()


def _set_account_blocked(chat_id, admin_id, blocked=True):
    cid = int(chat_id)
    aid = int(admin_id)
    if not is_admin_chat(aid):
        raise PermissionError("هذه العملية مخصصة للإدارة فقط")
    ...
    try:
        ...
        conn.execute("UPDATE users SET status=?,updated_at=? WHERE chat_id=?", (status, now.isoformat(), cid))
        conn.commit()
        return True
    finally:
        conn.close()
...
# Effective access: blocked accounts have no paid/free feature access.
_LEGACY_HAS_FEATURE_V19_05 = has_feature

def has_feature(chat_id, feature):
    if is_admin_chat(chat_id):
        return True
    if _account_is_blocked(chat_id):
        return False
    return _LEGACY_HAS_FEATURE_V19_05(chat_id, feature)


# User-facing status explicitly identifies temporary administrative free access.
_LEGACY_PLAN_STATUS_TEXT_V19_05 = plan_status_text

def plan_status_text(chat_id):
    if is_admin_chat(chat_id):
        return _LEGACY_PLAN_STATUS_TEXT_V19_05(chat_id)
    if _account_is_blocked(chat_id):
        return "⛔ الحساب محظور من الإدارة\n📌 لا يمكن استخدام خدمات البوت أثناء الحظر."
    _reconcile_admin_free_grant(chat_id)
    grant = _active_admin_free_grant(chat_id)
    if grant and grant.get("plan") in ("BASIC", "ELITE"):
        gp = grant["plan"]
        marker = "FREE+1" if gp == "BASIC" else "FREE+2"
        expiry = datetime.fromisoformat(str(grant["expiry_date"]))
        remaining = max(0, int((expiry - now_damascus()).total_seconds()))
        days = remaining // 86400
        hours = (remaining % 86400) // 3600
        return (
            f"🆓 {marker} — 🎁 منحة {PLANS[gp]['name']} مجانية\n"
            f"⏳ المدة المتبقية: {days} يوم و{hours} ساعة\n"
            f"📅 ينتهي: {expiry.strftime('%Y-%m-%d %H:%M')} دمشق\n"
            f"🎯 الصفقات: {TRADE_LIMIT_TEXT[gp]}"
        )
    return _LEGACY_PLAN_STATUS_TEXT_V19_05(chat_id)
...
async def admin_user_details(update, context, chat_id):
    ...
    context.user_data.pop("admin_mode", None)
    grant = await asyncio.to_thread(_active_admin_free_grant, chat_id)
    marker = _free_grant_label(grant)
    plan_display = marker or str(row["plan"] or "FREE")
    lines = [
        "👥 بيانات المستخدم", "",
        f"• Chat ID: {row['chat_id']}",
        f"• الاسم: {row['first_name'] or '—'}",
        f"• Username: @{row['username']}" if row['username'] else "• Username: —",
        f"• الباقة: {plan_display}",
        f"• الحالة: {'⛔ محظور' if row['status'] == 'blocked' else row['status']}",
        f"• البداية: {row['start_date'] or '—'}",
        f"• الانتهاء: {row['expiry_date'] or '—'}",
    ]
    if grant:
        gp = grant.get("plan")
        lines += ["", f"🎁 منحة {gp} مجانية نشطة ({_free_grant_label(grant)})", f"• تنتهي: {grant['expiry_date']}"]
    rows = []
    if grant:
        rows.append([("❌ إلغاء الباقة المجانية", "admin_cancel_free_grant")])
    rows.append([("🚫 إلغاء اشتراك الحساب", "admin_cancel_subscription")])
    if row["status"] == "blocked":
        rows.append([("🟢 فك حظر الحساب", "admin_unblock_user")])
    else:
        rows.append([("⛔ حظر الحساب", "admin_block_user")])
    rows += [
        [("🎁 منح اشتراك مجاني", "admin_grant_free_menu")],
        [("🔙 إدارة المستخدمين", "admin_users"), ("👑 لوحة الإدارة", "admin_home")],
    ]
...
async def admin_users(update, context):
    if not await _admin_guard(update):
        return
    # v19.09: the free-grant table is an optional administration table.
    # It must exist before the dashboard performs its LEFT/INNER JOIN query.
    # Creating it here is idempotent and isolated from trading/subscriptions.
    await asyncio.to_thread(_ensure_admin_free_grant_table)
    total, base_rows = await asyncio.to_thread(_admin_users_snapshot)
    conn = _db()
    try:
        ...
        conn.close()
    grant_counts = {(str(r["plan"]), str(r["status"])): int(r["n"]) for r in grant_rows}
    lines = ["👥 إدارة المستخدمين", "", f"• إجمالي الحسابات: {total}"]
    # نعرض المنحة بصفتها FREE+1/FREE+2 بدل أن تختلط مع الاشتراك التجاري الحقيقي.
    shown = set()
    for key, label in (("BASIC", "FREE+1"), ("ELITE", "FREE+2")):
        for status in ("active", "blocked"):
            n = grant_counts.get((key, status), 0)
            if n:
                lines.append(f"• {label} / {status}: {n}")
                shown.add((key, status))
    for r in base_rows:
        key = (str(r["plan"]), str(r["status"]))
        if key in shown:
            # هذا الصف يتضمن الحسابات ذات المنح أيضاً؛ نطرحها حتى لا يتكرر العد.
            remaining = int(r["n"]) - grant_counts.get(key, 0)
            if remaining > 0:
                lines.append(f"• {r['plan']} / {r['status']}: {remaining}")
        else:
            lines.append(f"• {r['plan']} / {r['status']}: {r['n']}")
    context.user_data["admin_mode"] = "search_user"
    lines += ["", "🔎 أرسل Chat ID للبحث عن حساب محدد.", "📌 FREE+1 = منحة BASIC مجانية | FREE+2 = منحة ELITE مجانية."]
    await _admin_render(update, "\n".join(lines), "users")
...
async def admin_user_grant_menu(update, context):
    ...
    marker = _free_grant_label(grant)
    text = (
        "🎁 إدارة الباقة المجانية\n\n"
        f"• Chat ID: {row['chat_id']}\n"
        f"• الباقة الحالية: {marker or row['plan']}\n\n"
        "اختر الباقة المجانية التي تريد منحها:\n"
        "🥉 BASIC → تظهر للمستخدم FREE+1\n"
        "💎 ELITE → تظهر للمستخدم FREE+2\n\n"
        "بعد الاختيار أرسل عدد الأيام المطلوب."
    )
    rows = [
        [("🥉 منح BASIC مجانًا", "admin_grant_free_basic"), ("💎 منح ELITE مجانًا", "admin_grant_free_elite")],
        [("❌ إلغاء الباقة المجانية", "admin_cancel_free_grant")],
        [("🔙 بيانات المستخدم", "admin_user_back")],
    ]
    await _admin_render(update, text, rows)
...
async def callback_router(update, context):
    ...
    data = q.data if q else ""
    admin_callbacks = {
        "admin_cancel_free_grant", "admin_cancel_subscription", "admin_block_user", "admin_unblock_user"
    }
    if data in admin_callbacks:
        ...
        if not cid:
            ...
            return
        try:
            if data == "admin_cancel_free_grant":
                result = await asyncio.to_thread(_cancel_free_grant, cid, update.effective_user.id)
                if not result.get("ok"):
                    await reply(update, "ℹ️ لا توجد باقة مجانية إدارية نشطة لهذا الحساب.")
                else:
                    await reply(update, with_market_header(
                        f"✅ تم إلغاء الباقة المجانية للحساب {cid}.\n"
                        f"• تمت استعادة الحالة السابقة: {result['restored_plan']} / {result['restored_status']}"
                    ))
                await admin_user_details(update, context, cid)
                return
            if data == "admin_cancel_subscription":
                result = await asyncio.to_thread(_cancel_subscription, cid, update.effective_user.id)
                if not result.get("ok"):
                    await reply(update, "🔎 لم يتم العثور على الحساب.")
                else:
                    await reply(update, with_market_header(
                        f"✅ تم إلغاء اشتراك الحساب {cid}.\n"
                        "• الباقة الحالية: FREE\n"
                        "• تم حفظ سجل الحساب والصفقات ولم يتم حذف البيانات."
                    ))
                await admin_user_details(update, context, cid)
                return
            if data == "admin_block_user":
                ok = await asyncio.to_thread(_set_account_blocked, cid, update.effective_user.id, True)
                await reply(update, "⛔ تم حظر الحساب." if ok else "🔎 لم يتم العثور على الحساب.")
                await admin_user_details(update, context, cid)
                return
            if data == "admin_unblock_user":
                ok = await asyncio.to_thread(_set_account_blocked, cid, update.effective_user.id, False)
                await reply(update, "🟢 تم فك حظر الحساب وإعادته إلى الحالة active." if ok else "🔎 لم يتم العثور على الحساب.")
                await admin_user_details(update, context, cid)
                return
        except Exception as exc:
            await reply(update, f"❌ تعذر تنفيذ العملية: {exc}")
        return

    # منع الحسابات المحظورة من استخدام واجهات الأزرار، مع إبقاء الإدارة مستقلة.
    if not is_admin_chat(update.effective_chat.id) and _account_is_blocked(update.effective_chat.id):
        await q.answer("الحساب محظور من الإدارة.", show_alert=True)
        return
...
# v19.07 — Surgical fix: Admin Users callback path only
# This layer intentionally changes only the 👥 إدارة المستخدمين callback path.
# No trading, S/R, scheduler, alerts, webhook, database schema, or plan logic is changed.
V19_07_VERSION = "v19.07_ADMIN_USERS_ROUTING_FIXED"

# Preserve the immediately preceding callback router as the fallback for every
# callback_data value unrelated to Admin Users.
_LEGACY_CALLBACK_ROUTER_V19_06 = callback_router
...
async def callback_router(update, context):
    ...
    # The only surgical change in v19.07: deterministic direct dispatch for
    # 👥 إدارة المستخدمين. It never falls through to an older router.
    if data == "admin_users":
        await q.answer()
        if not await _admin_guard(update):
            return
    ...
    # Every other callback keeps the exact v19.06 behavior.
    await _LEGACY_CALLBACK_ROUTER_V19_06(update, context)


def run_v19_07_admin_users_tests():
    """Static, side-effect-free tests for the Admin Users callback path only."""
    import ast
    source = Path(__file__).read_text(encoding="utf-8")
    ...
    callbacks = [n for n in tree.body if isinstance(n, ast.AsyncFunctionDef) and n.name == "callback_router"]
    admin_users_defs = [n for n in tree.body if isinstance(n, ast.AsyncFunctionDef) and n.name == "admin_users"]

    # v19.06 contains historical router layers; v19.07 must make the final
    # runtime definition deterministic without rewriting unrelated legacy code.
    check("callback_router_definitions_present", len(callbacks) >= 1)
    check("admin_users_definition_present", len(admin_users_defs) >= 1)

    # Final callback router must contain an explicit admin_users branch.
    final_router = callbacks[-1] if callbacks else None
    router_source = ast.get_source_segment(source, final_router) if final_router else ""
    check("explicit_admin_users_route", 'data == "admin_users"' in router_source)
    check("direct_admin_users_call", "await admin_users(update, context)" in router_source)
    check("admin_guard_present", "await _admin_guard(update)" in router_source)
    check("callback_answer_present", "await q.answer()" in router_source)
    check("legacy_fallback_only_after_admin_route", "_LEGACY_CALLBACK_ROUTER_V19_06(update, context)" in router_source)

    # The production handler registration must use the final callback_router.
    check("callback_handler_registration", "CallbackQueryHandler(_trace_handler(callback_router" in source)
    check("admin_callback_data_exists", 'callback_data="admin_users"' in source)

    # Guard against accidental broad changes in the surgical layer.
    check("no_v19_07_trading_calls", "evaluate_signal(" not in router_source and "build_trade(" not in router_source)
    check("version_marker", V19_07_VERSION == "v19.07_ADMIN_USERS_ROUTING_FIXED")

    return {"ok": not failures, "passed": 10 - len(failures), "total": 10, "failures": failures}


# v19.08 — Render startup fix. The previous v19.07 entry point referenced
# main_v19_06(), but that function is not present in the deployed file.
# Reuse the actual production startup sequence already defined in this file.
V19_08_VERSION = "v19.08_ADMIN_USERS_RENDER_STARTUP_FIXED"

def main_v19_08():
    if "--v19-07-admin-users-self-test" in sys.argv or "--v19-08-self-test" in sys.argv:
        result = run_v19_07_admin_users_tests()
...
# v19.08 premature __main__ block intentionally removed in v19.09.
# The canonical bootstrap is defined at the end of this file so every final
# handler/router definition is bound before Telegram Application startup.

# Block enforcement at the message-router boundary.
_LEGACY_ROUTER_V19_05 = router
async def router(update, context):
    if not _admin_ok(update) and _account_is_blocked(update.effective_chat.id):
        await reply(update, with_market_header("⛔ هذا الحساب محظور من الإدارة.\n📌 تواصل مع الإدارة لطلب فك الحظر."))
        return
    text = (update.effective_message.text or "").strip() if getattr(update, "effective_message", None) else ""
    mode = context.user_data.get("admin_mode") if getattr(context, "user_data", None) is not None else None
    if mode == "grant_free_plan":
        if not _admin_ok(update):
            context.user_data.pop("admin_mode", None); context.user_data.pop("admin_grant_plan", None)
            await reply(update, "⛔ هذا القسم مخصص للإدارة فقط."); return
        if not text.isdigit() or int(text) <= 0:
            await reply(update, "⚠️ أرسل عدد أيام صحيحًا أكبر من صفر، مثل: 7 أو 15 أو 30"); return
        cid = context.user_data.get("admin_selected_user")
        grant_plan = context.user_data.get("admin_grant_plan", "BASIC")
        if not cid:
            context.user_data.pop("admin_mode", None); await reply(update, "⛔ لم يتم تحديد حساب."); return
        context.user_data.pop("admin_mode", None); context.user_data.pop("admin_grant_plan", None)
        try:
            result = await asyncio.to_thread(_grant_free_plan, cid, int(text), update.effective_user.id, grant_plan)
            suffix = " وتم تمديد المنحة الحالية." if result.get("extended") else ""
            marker = "FREE+1" if grant_plan == "BASIC" else "FREE+2"
            await reply(update, with_market_header(
                f"✅ تم منح {grant_plan} مجانًا ({marker}) بنجاح{suffix}\n\n"
                f"• Chat ID: {cid}\n• المدة: {int(text)} يوم\n• تنتهي: {result['expiry'].strftime('%Y-%m-%d %H:%M')} دمشق"
            ))
        except Exception as exc:
            await reply(update, f"❌ تعذر تنفيذ المنحة: {exc}")
        return
    await _LEGACY_ROUTER_V19_05(update, context)
...
async def admin_subscriptions(update, context):
    ...
    text = (
        "💳 إدارة الاشتراكات\n\n"
        "🎁 المنح المجانية الإدارية\n"
        "• BASIC المجانية تظهر كـ FREE+1.\n"
        "• ELITE المجانية تظهر كـ FREE+2.\n"
        "• يمكن إلغاء المنحة واستعادة حالة الحساب السابقة.\n\n"
        "🚫 إلغاء اشتراك الحساب يعيد الوصول إلى FREE مع حفظ سجل الحساب والصفقات.\n"
        "⛔ الحظر يمنع الحساب من استخدام البوت، والإدارة تستطيع فك الحظر لاحقًا."
    )
    await _admin_render(update, text, "subscriptions")


def run_v19_05_self_tests():
    failures = []
    def check(name, cond):
        if not cond: failures.append(name)
    check("version", V19_05_VERSION == "v19.05_ADMIN_ACCOUNT_STATUS_CONTROL")
    check("free_plus_basic", _free_grant_label({"plan":"BASIC"}) == "FREE+1")
    check("free_plus_elite", _free_grant_label({"plan":"ELITE"}) == "FREE+2")
    check("cancel_free_fn", callable(_cancel_free_grant))
    check("cancel_subscription_fn", callable(_cancel_subscription))
    check("block_fn", callable(_set_account_blocked))
    check("blocked_feature_guard", callable(has_feature))
    return {"ok": not failures, "passed": 7 - len(failures), "total": 7, "failures": failures}


def main_v19_06():
    if "--v19-05-self-test" in sys.argv:
        print(json.dumps(run_v19_05_self_tests(), ensure_ascii=False, indent=2)); return
    global BOT_LOOP
    server = threading.Thread(target=run_flask, daemon=True); server.start()
    loop = asyncio.new_event_loop(); BOT_LOOP = loop; asyncio.set_event_loop(loop)
    try: loop.run_until_complete(integrated_start_bot())
...
# v19.09: historical v19.06 entrypoint removed; canonical bootstrap is at EOF.

# ============================================================
# v19.09 — ADMIN USERS CALLBACK ROOT FIX
# Scope: Admin Users callback path + startup binding only.
# No trading, S/R, Scheduler, Alert Dispatcher, Webhook, or strategy logic changed.
# ============================================================
V19_09_VERSION = "v19.09_ADMIN_USERS_CALLBACK_ROOT_FIX"

# Capture the fully built callback router that existed before this final binding.
_LEGACY_CALLBACK_ROUTER_V19_09 = callback_router
...
async def callback_router(update, context):
    ...
    # Deterministic Admin Users route. This executes before every legacy router.
    if data == "admin_users":
        await q.answer()
        if not await _admin_guard(update):
            return
    ...
    # Preserve all unrelated callback behavior.
    await _LEGACY_CALLBACK_ROUTER_V19_09(update, context)


def run_v19_09_admin_users_root_tests():
    """Static/runtime-binding tests for the Admin Users root path."""
    source = Path(__file__).read_text(encoding="utf-8")
    tree = ast.parse(source)
    failures = []
    def check(name, condition):
    ...
    callbacks = [n for n in tree.body if isinstance(n, ast.AsyncFunctionDef) and n.name == "callback_router"]
    admins = [n for n in tree.body if isinstance(n, ast.AsyncFunctionDef) and n.name == "admin_users"]
    mains = [n for n in tree.body if isinstance(n, ast.FunctionDef) and n.name == "main"]

    final_router = callbacks[-1] if callbacks else None
    router_source = ast.get_source_segment(source, final_router) if final_router else ""

    check("version_marker", V19_09_VERSION == "v19.09_ADMIN_USERS_CALLBACK_ROOT_FIX")
    check("final_callback_router", bool(final_router))
    check("final_admin_users_definition", bool(admins))
    check("explicit_admin_users_route", 'data == "admin_users"' in router_source)
    check("callback_answer", "await q.answer()" in router_source)
    check("admin_guard", "await _admin_guard(update)" in router_source)
    check("direct_admin_users", "await admin_users(update, context)" in router_source)
    check("legacy_fallback", "_LEGACY_CALLBACK_ROUTER_V19_09(update, context)" in router_source)
    check("grant_table_guard", "_ensure_admin_free_grant_table" in source)
    check("canonical_main", bool(mains))
    check("no_main_v19_08_entrypoint", 'if __name__ == "__main__":\n    main_v19_08()' not in source)
    check("final_entrypoint", source.rstrip().endswith("main()"))
    check("callback_registration_dynamic", "CallbackQueryHandler(_trace_handler(callback_router" in source)
    return {"ok": not failures, "passed": 13 - len(failures), "total": 13, "failures": failures}


def main():
    if "--v19-09-admin-users-self-test" in sys.argv:
        print(json.dumps(run_v19_09_admin_users_root_tests(), ensure_ascii=False, indent=2))
        return
    global BOT_LOOP
    server = threading.Thread(target=run_flask, daemon=True, name="flask-server")
    server.start()
    loop = asyncio.new_event_loop()
    BOT_LOOP = loop
    asyncio.set_event_loop(loop)
